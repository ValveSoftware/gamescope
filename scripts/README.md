...
Scripts ending in `.lua` are executed recursively in alphabetical order from the following directories:
 - `/usr/share/gamescope/scripts`
 - `/etc/gamescope/scripts`
 - `$XDG_CONFIG_HOME/gamescope/scripts` (`$HOME/.config/gamescope/scripts` when `XDG_CONFIG_HOME` is unset)
...
If you wish to make modifications that will persist as a user, simply make a new `.lua` file in `$XDG_CONFIG_HOME/gamescope/scripts` which is usually `$HOME/.config/gamescope/scripts` with what you want to change.

For example, to make the Steam Deck LCD use spec colorimetry instead of the measured colorimetry you could create the following file `~/.config/gamescope/scripts/my_deck_lcd_colorimetry.lua` with the following contents:
...
You could also place this in `/etc/gamescope/scripts` if you really want it to apply to all users/system-wide, but that would need root privelages.
