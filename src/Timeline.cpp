...
namespace gamescope
{
    ...
    static uint32_t SyncobjFdToHandle( int32_t nFd )
    {
        int32_t  nRet;
        uint32_t uHandle = 0;
        if ( ( nRet = drmSyncobjFDToHandle(
                   g_device.drmRenderFd( ), nFd, &uHandle ) ) < 0 )
            return 0;

        return uHandle;
    }

    /*static*/ std::shared_ptr<CTimeline>
    CTimeline::Create( const TimelineCreateDesc_t &desc )
    {
        std::shared_ptr<VulkanTimelineSemaphore_t> pSemaphore =
            g_device.CreateTimelineSemaphore( desc.ulStartingPoint, true );
        if ( !pSemaphore ) return nullptr;

        return std::make_shared<CTimeline>(
            pSemaphore->GetFd( ), std::move( pSemaphore ) );
    }
    ...
    CTimeline::CTimeline(
        ...
        CTimeline(
            nSyncobjFd,
            SyncobjFdToHandle( nSyncobjFd ),
            std::move( pSemaphore ) )
    {}

    CTimeline::CTimeline(
        int32_t                                    nSyncobjFd,
        uint32_t                                   uSyncobjHandle,
        std::shared_ptr<VulkanTimelineSemaphore_t> pSemaphore ) :
    ...
    CTimeline::~CTimeline( )
    {
        ...
        if ( m_nSyncobjFd >= 0 ) close( m_nSyncobjFd );
    }

    int32_t CTimeline::GetDrmRenderFD( ) { return g_device.drmRenderFd( ); }
    ...
    CTimelinePoint<Type>::CTimelinePoint(
        std::shared_ptr<CTimeline> pTimeline, uint64_t ulPoint ) :
        m_pTimeline{ std::move( pTimeline ) }, m_ulPoint{ ulPoint }
    {}

    template<TimelinePointType Type> CTimelinePoint<Type>::~CTimelinePoint( )
    {
        if ( ShouldSignalOnDestruction( ) )
        {
            ...
            drmSyncobjTimelineSignal(
                m_pTimeline->GetDrmRenderFD( ), &uHandle, &m_ulPoint, 1 );
        }
    }
    ...
    //
    // Fence flags tl;dr
    // 0                                      -> Wait for signal on a
    // materialized fence, -ENOENT if not materialized
    // DRM_SYNCOBJ_WAIT_FLAGS_WAIT_AVAILABLE  -> Wait only for materialization
    // DRM_SYNCOBJ_WAIT_FLAGS_WAIT_FOR_SUBMIT -> Wait for materialization +
    // signal
    //
    template<TimelinePointType Type>
    std::pair<int32_t, bool> CTimelinePoint<Type>::CreateEventFd( )
    {
        ...
        int      nRet             = drmSyncobjQuery(
            m_pTimeline->GetDrmRenderFD( ), &uHandle, &ulSignalledPoint, 1u );
        if ( nRet != 0 )
        {
        ...
        if ( ulSignalledPoint >= m_ulPoint ) { return k_AlreadySignalledEvent; }
        else
        {
            const int32_t nExplicitSyncEventFd = eventfd( 0, EFD_CLOEXEC );
            if ( nExplicitSyncEventFd < 0 )
            {
                s_TimelineLog.errorf_errno(
                    "Failed to create eventfd (%d)", nExplicitSyncEventFd );
                return k_InvalidEvent;
            }
            ...
            {
                s_TimelineLog.errorf_errno(
                    "DRM_IOCTL_SYNCOBJ_EVENTFD failed" );
