...
namespace gamescope
{
    ...
    static ConVar<bool> cv_script_use_local_scripts{
        "script_use_local_scripts",
        false,
        "Whether or not to use the local scripts (../config) as opposed to the "
        "ones in /etc/gamescope.d"
    };
    static ConVar<bool> cv_script_use_user_scripts{
        "script_use_user_scripts",
        true,
        "Whether or not to use user config scripts ($XDG_CONFIG_DIR/gamescope) "
        "at all."
    };
    ...
    static inline int ExceptionFunction(
        lua_State                            *pState,
        sol::optional<const std::exception &> oException,
        sol::string_view                      psvDescription )
    {
        // L is the lua state, which you can wrap in a state_view if necessary
        // maybe_exception will contain exception, if it exists
        // description will either be the what() of the exception or a
        // description saying that we hit the general-case catch(...)

        s_ScriptLog.errorf(
            "An exception occurred:\n    %.*s",
            ( int )psvDescription.length( ),
            psvDescription.data( ) );

        // you must push 1 element onto the stack to be
        // transported through as the error object in Lua
        // note that Lua -- and 99.5% of all Lua users and libraries -- expects
        // a string so we push a single string (in our case, the description of
        // the error)
    ...
    CScriptManager::CScriptManager( )
    {
        ...
        if ( !s_bSetDefaultHandler )
        {
            ...
            sol::protected_function::set_default_handler(
                m_State[ "_gamescope_error_handler" ] );
            s_bSetDefaultHandler = true;
        }

        m_State.set_panic(
            sol::c_call<decltype( &PanicFunction ), &PanicFunction> );
        m_State.set_exception_handler( &ExceptionFunction );

        m_Gamescope.Base = m_State.create_named_table( "gamescope" );
        m_Gamescope.Base[ "hook" ] =
            [ this ]( std::string_view svName, sol::function fnFunc )
        {
            m_Hooks.emplace(
                std::make_pair(
                    svName,
                    Hook_t{ std::move( fnFunc ), m_nCurrentScriptId } ) );
        };
        m_Gamescope.Base.new_enum<EOTF>(
            "eotf",
            {
        ...
        m_Gamescope.Base.new_enum<LogPriority>(
            "log_priority",
            {
                { "silent", LOG_SILENT },
                { "error", LOG_ERROR },
        ...
        m_Gamescope.Base[ "log" ] =
            []( LogPriority ePriority, std::string_view svText )
        { s_ScriptLog.log( ePriority, svText ); };
        ...
        m_Gamescope.Config.Base.set(
            "known_displays", m_Gamescope.Config.KnownDisplays );
    }
    ...
    void CScriptManager::RunDefaultScripts( )
    {
        ...
        if ( cv_script_use_local_scripts ) { RunFolder( "../scripts", true ); }
        else if ( sScriptPathEnv )
        {
            std::vector<std::string_view> sScriptPaths =
                gamescope::Split( sScriptPathEnv, ":" );
        ...
        {
            std::string sUserConfigs =
                std::string{ GetConfigDir( ) } + "/gamescope/scripts";
    ...
    bool
    CScriptManager::RunFolder( std::string_view svDirectory, bool bRecursive )
    {
        s_ScriptMgrLog.infof(
            "Loading scripts from: '%.*s'",
            int( svDirectory.size( ) ),
            svDirectory.data( ) );

        std::filesystem::path dirConfig = std::filesystem::path{ svDirectory };
        if ( !std::filesystem::is_directory( dirConfig ) )
        {
            s_ScriptMgrLog.warnf(
    ...
    void CScriptManager::InvalidateAllHooks( ) { m_Hooks.clear( ); }
    ...
    void CScriptManager::InvalidateHooksForScript( int32_t nScriptId )
    {
        ...
        std::erase_if(
            m_Hooks,
            [ nScriptId ]( const auto &iter ) -> bool
    ...
    GamescopeScript_t::Config_t::LookupDisplay(
        ...
        std::string_view   psvDataString )
    {
        ...
        {
            sol::optional<sol::table> otTable =
                iter.second.as<sol::optional<sol::table>>( );
            if ( !otTable ) continue;
            sol::table tTable = *otTable;

            sol::optional<sol::function> ofnMatches = tTable[ "matches" ];
            if ( !ofnMatches ) continue;

            sol::function fnMatches = *ofnMatches;
            if ( !fnMatches ) continue;

            int nPriority = fnMatches( tDisplay );
            if ( nPriority <= nMaxPrority ) continue;
