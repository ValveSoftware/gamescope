...
public:
    ...
    static void
    xwayland_ready_callback( struct wl_listener *listener, void *data );
    ...
    void handle_override_window_content(
        struct wl_client   *client,
        struct wl_resource *gamescope_swapchain_resource,
        struct wlr_surface *surface,
        uint32_t            x11_window );
    void destroy_content_override(
        struct wlserver_x11_surface_info *x11_surface,
        struct wlr_surface               *surf );
...
private:
    ...
    struct wl_listener          xwayland_ready_listener = {
        .notify = xwayland_ready_callback
    };
...
struct wlserver_t
{
    ...
    struct
    {
        ...
        std::vector<std::unique_ptr<gamescope_xwayland_server_t>>
            xwayland_servers;
    } wlr;

    struct wlr_surface *mouse_focus_surface;
    struct wlr_surface *kb_focus_surface;
    std::unordered_map<struct wlr_surface *, std::pair<int, int>>
                      current_dropdown_surfaces;
    ...
    std::atomic<struct wlr_pointer_constraint_v1 *> mouse_constraint = {
        nullptr
    };
    ...
    bool HasMouseConstraint( ) const
    {
        ...
        // Used by the steamcompmgr thread to check if there is currently a
        // mouse constraint.
    ...
    // Share one single keymap and state between all connected physical
    // keyboards
...
void wlserver_keyboardfocus(
    struct wlr_surface *surface, bool bConstrain = true );
