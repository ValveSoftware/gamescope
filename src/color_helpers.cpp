...
glm::vec3 xyY_to_XYZ( const glm::vec2 &xy, float Y )
{
    ...
    {
        return glm::vec3(
            Y * xy.x / xy.y, Y, ( 1.f - xy.x - xy.y ) * Y / xy.y );
    }
}
...
glm::mat3 normalised_primary_matrix(
    const primaries_t &rgbPrimaries,
    const glm::vec2   &whitePrimary,
    float              whiteLuminance )
{
    glm::mat3 matPrimaries(
        xy_to_xyz( rgbPrimaries.r ),
        xy_to_xyz( rgbPrimaries.g ),
        xy_to_xyz( rgbPrimaries.b ) );
    glm::vec3 whiteXYZ = xyY_to_XYZ( whitePrimary, whiteLuminance );
    glm::mat3 whiteScale =
        glm::diagonal3x3( glm::inverse( matPrimaries ) * whiteXYZ );
    return matPrimaries * whiteScale;
}

glm::mat3 chromatic_adaptation_matrix(
    const glm::vec3           &sourceWhiteXYZ,
    const glm::vec3           &destWhiteXYZ,
    EChromaticAdaptationMethod eMethod )
{
    static const glm::mat3 k_matBradford(
        0.8951f,
        -0.7502f,
        0.0389f,
        0.2664f,
        ...
        0.0367f,
        1.0296f );
    glm::mat3 matAdaptation    = eMethod == k_EChromaticAdapatationMethod_XYZ
                                     ? glm::diagonal3x3( glm::vec3( 1, 1, 1 ) )
                                     : k_matBradford;
    glm::vec3 coneResponseDest = matAdaptation * destWhiteXYZ;
    glm::vec3 coneResponseSource = matAdaptation * sourceWhiteXYZ;
    glm::vec3 scale              = glm::vec3(
        coneResponseDest.x / coneResponseSource.x,
        coneResponseDest.y / coneResponseSource.y,
        coneResponseDest.z / coneResponseSource.z );
    return glm::inverse( matAdaptation ) * glm::diagonal3x3( scale ) *
           matAdaptation;
}

displaycolorimetry_t
lerp( const displaycolorimetry_t &a, const displaycolorimetry_t &b, float t )
{
...
colormapping_t lerp( const colormapping_t &a, const colormapping_t &b, float t )
{
    ...
    result.blendEnableMinSat =
        flerp( a.blendEnableMinSat, b.blendEnableMinSat, t );
    result.blendEnableMaxSat =
        flerp( a.blendEnableMaxSat, b.blendEnableMaxSat, t );
...
std::shared_ptr<lut3d_t>
LoadCubeLut( FILE *pFile, bool &bRaisesBlackLevelFloor )
{
    // R changes fastest
    // ...
    ...
    while ( fgets( line, sizeof( line ), pFile ) )
    {
        if ( lut3d->lutEdgeSize )
        {
            ...
            {
                if ( lut3d->data.empty( ) ) { blackFloor = val; }
                lut3d->data.push_back( val );
            }
        }
        else if ( sscanf( line, "LUT_3D_SIZE %d", &lut3d->lutEdgeSize ) == 1 )
        {
            if ( lut3d->lutEdgeSize < 2 ||
                 lut3d->lutEdgeSize > 128 ) // sanity check
            {
                return nullptr;
            }
            lut3d->data.reserve(
                lut3d->lutEdgeSize * lut3d->lutEdgeSize * lut3d->lutEdgeSize );
        }
    }
    ...
    bool bValid =
        ( nExpectedElements > 0 &&
          ( nExpectedElements == ( int )lut3d->data.size( ) ) );
    if ( !bValid ) { return nullptr; }

    bRaisesBlackLevelFloor = !close_enough( blackFloor.x, 0.0f ) ||
                             !close_enough( blackFloor.y, 0.0f ) ||
                             !close_enough( blackFloor.z, 0.0f );
    return lut3d;
}

std::shared_ptr<lut3d_t>
LoadCubeLut( const char *pchFileName, bool &bRaisesBlackLevelFloor )
{
    bRaisesBlackLevelFloor = false;
...
// Linear
inline void
lerp_rgb( float *out, const float *a, const float *b, const float *z )
{
...
inline glm::vec3
ApplyLut3D_Trilinear( const lut3d_t &lut3d, const glm::vec3 &input )
{
    ...
    const int n000 = GetLut3DIndexRedFastRGB(
        indexLow[ 0 ], indexLow[ 1 ], indexLow[ 2 ], lut3d.lutEdgeSize );
    const int n100 = GetLut3DIndexRedFastRGB(
        indexHigh[ 0 ], indexLow[ 1 ], indexLow[ 2 ], lut3d.lutEdgeSize );
    const int n010 = GetLut3DIndexRedFastRGB(
        indexLow[ 0 ], indexHigh[ 1 ], indexLow[ 2 ], lut3d.lutEdgeSize );
    const int n001 = GetLut3DIndexRedFastRGB(
        indexLow[ 0 ], indexLow[ 1 ], indexHigh[ 2 ], lut3d.lutEdgeSize );
    const int n110 = GetLut3DIndexRedFastRGB(
        indexHigh[ 0 ], indexHigh[ 1 ], indexLow[ 2 ], lut3d.lutEdgeSize );
    const int n101 = GetLut3DIndexRedFastRGB(
        indexHigh[ 0 ], indexLow[ 1 ], indexHigh[ 2 ], lut3d.lutEdgeSize );
    const int n011 = GetLut3DIndexRedFastRGB(
        indexLow[ 0 ], indexHigh[ 1 ], indexHigh[ 2 ], lut3d.lutEdgeSize );
    const int n111 = GetLut3DIndexRedFastRGB(
        indexHigh[ 0 ], indexHigh[ 1 ], indexHigh[ 2 ], lut3d.lutEdgeSize );
    ...
    lerp_rgb(
        ( float * )&out,
        ( float * )&lut3d.data[ n000 ].r,
        ( float * )&lut3d.data[ n001 ].r,
        ( float * )&lut3d.data[ n010 ].r,
        ( float * )&lut3d.data[ n011 ].r,
        ( float * )&lut3d.data[ n100 ].r,
        ( float * )&lut3d.data[ n101 ].r,
        ( float * )&lut3d.data[ n110 ].r,
        ( float * )&lut3d.data[ n111 ].r,
        x,
        y,
        z );
...
inline glm::vec3
ApplyLut3D_Tetrahedral( const lut3d_t &lut3d, const glm::vec3 &input )
{
    const float dimMinusOne = float( lut3d.lutEdgeSize ) - 1.f;
    ...
    if ( fx > fy )
    {
        if ( fy > fz )
        {
            out = ( 1 - fx ) * lut3d.data[ n000 ] +
                  ( fx - fy ) * lut3d.data[ n100 ] +
                  ( fy - fz ) * lut3d.data[ n110 ] + ( fz )*lut3d.data[ n111 ];
        }
        else if ( fx > fz )
        {
            out = ( 1 - fx ) * lut3d.data[ n000 ] +
                  ( fx - fz ) * lut3d.data[ n100 ] +
                  ( fz - fy ) * lut3d.data[ n101 ] + ( fy )*lut3d.data[ n111 ];
        }
        else
        {
            out = ( 1 - fz ) * lut3d.data[ n000 ] +
                  ( fz - fx ) * lut3d.data[ n001 ] +
                  ( fx - fy ) * lut3d.data[ n101 ] + ( fy )*lut3d.data[ n111 ];
        }
    }
    else
    {
        if ( fz > fy )
        {
            out = ( 1 - fz ) * lut3d.data[ n000 ] +
                  ...
                  ( fy - fx ) * lut3d.data[ n011 ] + ( fx )*lut3d.data[ n111 ];
        }
        else if ( fz > fx )
        {
            out = ( 1 - fy ) * lut3d.data[ n000 ] +
                  ( fy - fz ) * lut3d.data[ n010 ] +
                  ( fz - fx ) * lut3d.data[ n011 ] + ( fx )*lut3d.data[ n111 ];
        }
        else
        {
            out = ( 1 - fy ) * lut3d.data[ n000 ] +
                  ( fy - fx ) * lut3d.data[ n010 ] +
                  ( fx - fz ) * lut3d.data[ n110 ] + ( fz )*lut3d.data[ n111 ];
        }
    }
...
inline glm::vec3 ApplyLut1D_Linear( const lut1d_t &lut, const glm::vec3 &input )
{
    ...
    float vLow[ 3 ]  = { lut.dataR[ indexLow[ 0 ] ],
                         lut.dataG[ indexLow[ 1 ] ],
                         lut.dataB[ indexLow[ 2 ] ] };
    float vHigh[ 3 ] = { lut.dataR[ indexHigh[ 0 ] ],
                         lut.dataG[ indexHigh[ 1 ] ],
                         lut.dataB[ indexHigh[ 2 ] ] };
...
inline float FindLutInv(
    ...
    const float  val )
{
    // Note that the LUT data pointed to by start/end must be in increasing
    // order, regardless of whether the original LUT was increasing or
    // decreasing because this function uses std::lower_bound().
    ...
    // [first,last) which does not compare less than val (but could be equal)."
    // (NB: This is correct using either end or end+1 since lower_bound will
    // return a
    //  value one greater than the second argument if no values in the array are
    //  >= cv.)
    // http://www.sgi.com/tech/stl/lower_bound.html
    const float *lowbound = std::lower_bound( start, end, cv );

    // lower_bound() returns first entry >= val so decrement it unless val ==
    // *start.
    if ( lowbound > start ) { --lowbound; }

    const float *highbound = lowbound;
    if ( highbound < end ) { ++highbound; }
...
int FindNonFlatStartIndex( const std::vector<float> &data )
{
    if ( !data.empty( ) )
    {
        ...
        {
            if ( data[ nIndex ] != data[ 0 ] ) { return nIndex - 1; }
        }
    }
...
inline glm::vec3
ApplyLut1D_Inverse_Linear( const lut1d_t &lut, const glm::vec3 &input )
{
    // Disallow inverse if not finalized
    if ( lut.startIndexR < 0 ) { return glm::vec3( -1.f ); }

    return glm::vec3(
        FindLutInv(
            lut.dataR.data( ) + lut.startIndexR,
            lut.startIndexR,
            lut.dataR.data( ) + lut.dataR.size( ) - 1,
            1.f / ( lut.dataR.size( ) - 1.f ),
            input.r ),
        FindLutInv(
            lut.dataG.data( ) + lut.startIndexG,
            lut.startIndexG,
            lut.dataG.data( ) + lut.dataG.size( ) - 1,
            1.f / ( lut.dataG.size( ) - 1.f ),
            input.g ),
        FindLutInv(
            lut.dataB.data( ) + lut.startIndexB,
            lut.startIndexB,
            lut.dataB.data( ) + lut.dataB.size( ) - 1,
            1.f / ( lut.dataB.size( ) - 1.f ),
            input.b ) );
}
...
inline glm::vec3 rgb_to_hsv( const glm::vec3 &rgb )
{
    ...
    glm::vec3 hsv;
    hsv.y = ( fabsf( flMax ) < std::numeric_limits<float>::min( ) )
                ? 0.f
                : flDelta / flMax;
    hsv.z = flMax;

    if ( hsv.y == 0.f ) { hsv.x = -1.0f; }
    else
    {
        if ( rgb.x == flMax ) { hsv.x = ( rgb.y - rgb.z ) / flDelta; }
        else if ( rgb.y == flMax )
        {
...
{
    return (
        color.x < 0.f || color.x > 1.f || color.y < 0.f || color.y > 1.f ||
        color.z < 0.f || color.z > 1.f );
}

template<typename T>
inline T
calcEOTFToLinear( const T &input, EOTF eotf, const tonemapping_t &tonemapping )
{
    if ( eotf == EOTF_Gamma22 )
    {
        return glm::pow( input, T( 2.2f ) ) * tonemapping.g22_luminance;
    }
    else if ( eotf == EOTF_PQ ) { return pq_to_nits( input ); }
...
template<typename T>
inline T
calcLinearToEOTF( const T &input, EOTF eotf, const tonemapping_t &tonemapping )
{
    if ( eotf == EOTF_Gamma22 )
    {
        T val = input;
        ...
        {
            val = glm::clamp(
                input / tonemapping.g22_luminance, T( 0.f ), T( 1.f ) );
        }
        return glm::pow( val, T( 1.f / 2.2f ) );
    }
    else if ( eotf == EOTF_PQ ) { return T( nits_to_pq( input ) ); }
...
template<typename T>
inline T applyShaper(
    const T             &input,
    EOTF                 source,
    EOTF                 dest,
    const tonemapping_t &tonemapping,
    float                flGain )
{
    ...
    return calcLinearToEOTF(
        flLinear, g_bUseSourceEOTFForShaper ? source : dest, tonemapping );
}

bool g_bHuePreservationWhenClipping = false;

template<uint32_t lutEdgeSize3d>
void calcColorTransform(
    lut1d_t                    *pShaper,
    int                         nLutSize1d,
    lut3d_t                    *pLut3d,
    ...
    EOTF                        destEOTF,
    const glm::vec2            &destVirtualWhite,
    EChromaticAdaptationMethod  eMethod,
    const colormapping_t       &mapping,
...
{
    // Generate shaper lut
    // Note: while this is typically a 1D approximation of our end to end
    // transform, it need not be! Conceptually this is just to determine the
    // interpolation properties... The 3d lut should be considered a 'matched'
    // pair where the transform is only complete when applying both.  I.e., you
    // can put ANY transform in here, and it should work.

    static constexpr int32_t nLutEdgeSize3d =
        static_cast<int32_t>( lutEdgeSize3d );
    if ( pShaper )
    {
        ...
        {
            glm::vec3 sourceColorEOTFEncoded = { nVal * flScale,
                                                 nVal * flScale,
                                                 nVal * flScale };
            glm::vec3 shapedSourceColor      = applyShaper(
                sourceColorEOTFEncoded,
                sourceEOTF,
                destEOTF,
                tonemapping,
                flGain );
    ...
    if ( pLut3d )
    {
        glm::mat3 xyz_from_dest =
            normalised_primary_matrix( dest.primaries, dest.white, 1.f );
        glm::mat3 dest_from_xyz = glm::inverse( xyz_from_dest );

        glm::mat3 xyz_from_source =
            normalised_primary_matrix( source.primaries, source.white, 1.f );
        glm::mat3 dest_from_source =
            dest_from_xyz *
            xyz_from_source; // XYZ scaling for white point adjustment

        // Precalc night mode scalars & digital gain
        // amount and saturation are overdetermined but we separate the two as
        // they conceptually represent different quantities, and this preserves
        // forwards algorithmic compatibility
        glm::vec3 nightModeMultHSV(
            nightmode.hue,
            clamp01( nightmode.saturation * nightmode.amount ),
            1.f );
        glm::vec3 vMultLinear =
            glm::pow( hsv_to_rgb( nightModeMultHSV ), glm::vec3( 2.2f ) );
        ...
        {
            // if source white is within tiny tolerance of
            // sourceWhitePointOverride don't do the override? (aka two
            // quantizations of d65)
            glm::mat3x3 virtualWhiteXYZFromPhysicalWhiteXYZ =
                chromatic_adaptation_matrix(
                    xy_to_xyz( dest.white ),
                    xy_to_xyz( destVirtualWhite ),
                    eMethod );
            whitePointDestAdaptation = dest_from_xyz *
                                       virtualWhiteXYZFromPhysicalWhiteXYZ *
                                       xyz_from_dest;

            // Consider lerp-ing the gain limiting between 0-1? That would allow
            // partial clipping so that contrast ratios wouldnt be sacrified too
            // bad with alternate white points
            static const bool k_bLimitGain = true;
            if ( k_bLimitGain )
            {
                glm::vec3 white =
                    whitePointDestAdaptation * glm::vec3( 1.f, 1.f, 1.f );
                float whiteMax =
                    std::max( white.r, std::max( white.g, white.b ) );
                float normScale = 1.f / whiteMax;
                whitePointDestAdaptation =
                    whitePointDestAdaptation *
                    glm::diagonal3x3( glm::vec3( normScale ) );
            }
        }
        ...
        {
            vSourceColorEOTFEncodedEdge[ nIndex ] =
                glm::vec3( nIndex * flEdgeScale );
            if ( pShaper )
            {
                vSourceColorEOTFEncodedEdge[ nIndex ] =
                    ApplyLut1D_Inverse_Linear(
                        *pShaper, vSourceColorEOTFEncodedEdge[ nIndex ] );
            }
        }
        ...
        for ( int nBlue = 0; nBlue < nLutEdgeSize3d; ++nBlue )
        {
            for ( int nGreen = 0; nGreen < nLutEdgeSize3d; ++nGreen )
            {
                ...
                {
                    glm::vec3 sourceColorEOTFEncoded = glm::vec3(
                        vSourceColorEOTFEncodedEdge[ nRed ].r,
                        vSourceColorEOTFEncodedEdge[ nGreen ].g,
                        vSourceColorEOTFEncodedEdge[ nBlue ].b );

                    if ( pLook && !pLook->data.empty( ) )
                    {
                        sourceColorEOTFEncoded = ApplyLut3D_Tetrahedral(
                            *pLook, sourceColorEOTFEncoded );
                    }

                    // Convert to linearized display referred for source
                    // colorimetry
                    glm::vec3 sourceColorLinear = calcEOTFToLinear(
                        sourceColorEOTFEncoded, sourceEOTF, tonemapping );

                    // Convert to dest colorimetry (linearized display referred)
                    glm::vec3 destColorLinear =
                        dest_from_source * sourceColorLinear;
                    ...
                    float amount          = cfit(
                        colorSaturation,
                        mapping.blendEnableMinSat,
                        mapping.blendEnableMaxSat,
                        mapping.blendAmountMin,
                        mapping.blendAmountMax );
                    destColorLinear =
                        glm::mix( destColorLinear, sourceColorLinear, amount );
                    ...
                    destColorLinear =
                        whitePointDestAdaptation * destColorLinear;
                    ...
                    {
                        float flMax = std::max(
                            std::max( destColorLinear.r, destColorLinear.g ),
                            destColorLinear.b );
                        // TODO: Don't use g22_luminance here or in tonemapping,
                        // use whatever maxContentLightLevel is for the
                        // connector.
                    ...
                    // Apply dest EOTF
                    glm::vec3 destColorEOTFEncoded = calcLinearToEOTF(
                        destColorLinear, destEOTF, tonemapping );

                    // Write LUT
                    pLut3d->data[ GetLut3DIndexRedFastRGB(
                        nRed, nGreen, nBlue, nLutEdgeSize3d ) ] =
                        destColorEOTFEncoded;
                }
            }
        }
    }
...
bool BIsWideGamut( const displaycolorimetry_t &nativeDisplayOutput )
{
    ...
    return (
        nativeDisplayOutput.primaries.r.x > 0.650f &&
        nativeDisplayOutput.primaries.r.y < 0.320f );
}

void buildSDRColorimetry(
    displaycolorimetry_t       *pColorimetry,
    colormapping_t             *pMapping,
    float                       flSDRGamutWideness,
    const displaycolorimetry_t &nativeDisplayOutput )
{
    if ( BIsWideGamut( nativeDisplayOutput ) )
    {
        // If not set, make it native.
        if ( flSDRGamutWideness < 0 ) flSDRGamutWideness = 1.0f;
        ...
        *pMapping                 = noRemap;
        *pColorimetry =
            lerp( r709NativeWhite, nativeDisplayOutput, flSDRGamutWideness );
    }
    else
    {
        // If not set, make it native.
        if ( flSDRGamutWideness < 0 ) flSDRGamutWideness = 0.0f;
        ...
        displaycolorimetry_t wideGamutNativeWhite =
            displaycolorimetry_widegamutgeneric;
        ...
        if ( flSDRGamutWideness < 0.5f )
        {
            ...
            *pColorimetry =
                lerp( nativeDisplayOutput, wideGamutNativeWhite, t );
...
bool approxEqual(
    const glm::vec3 &a, const glm::vec3 &b, float flTolerance = 1e-5f )
{
...
const glm::mat3 k_xyz_from_709 = normalised_primary_matrix(
    displaycolorimetry_709.primaries, displaycolorimetry_709.white, 1.f );
const glm::mat3 k_709_from_xyz = glm::inverse( k_xyz_from_709 );

const glm::mat3 k_xyz_from_2020 = normalised_primary_matrix(
    displaycolorimetry_2020.primaries, displaycolorimetry_2020.white, 1.f );
