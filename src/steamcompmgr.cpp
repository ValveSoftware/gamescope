...
gamescope::ConVar<bool> cv_overlay_unmultiplied_alpha{
    "overlay_unmultiplied_alpha", false
};

gamescope::ConVar<bool> cv_vr_show_forwarded_overlays{
    "vr_show_forwarded_overlays", false
};
...
static const char *cgroupPathFromPID( pid_t pid )
{
    ...
    while ( getline( &line, &lineSize, procfsFile ) > 0 )
    {
        ...
        if ( strncmp( line, pathHeader, strlen( pathHeader ) ) ) continue;
        char *path = line;
...
static bool                              pick_primary_focus_and_override(
    focus_t                                 *out,
...
gamescope::ConVar<bool> cv_adaptive_sync(
    "adaptive_sync",
    false,
    "Whether or not adaptive sync is enabled if available." );
gamescope::ConVar<bool> cv_adaptive_sync_ignore_overlay(
    "adaptive_sync_ignore_overlay",
    false,
    "Whether or not to ignore overlay planes for pushing commits with adaptive "
    "sync." );
gamescope::ConVar<int> cv_adaptive_sync_overlay_cycles(
    "adaptive_sync_overlay_cycles",
    1,
    "Number of vblank cycles to ignore overlay repaints before forcing a "
    "commit with adaptive sync." );

gamescope::ConVar<bool> cv_upscale_preemptive(
    "upscale_preemptive", true, "Allow pre-emptive upscaling" );
gamescope::ConVar<bool> cv_upscale_preemptive_debug_force_sync(
    "upscale_preemptive_debug_force_sync",
    false,
    "Force synchronize pre-emptive upscaling" );
...
static void create_color_mgmt_luts(
    const gamescope_color_mgmt_t &newColorMgmt,
    gamescope_color_mgmt_luts     outColorMgmtLuts[ EOTF_Count ] )
{
    const displaycolorimetry_t &displayColorimetry =
        newColorMgmt.displayColorimetry;
    const displaycolorimetry_t &outputEncodingColorimetry =
        newColorMgmt.outputEncodingColorimetry;

    for ( uint32_t nInputEOTF = 0; nInputEOTF < EOTF_Count; nInputEOTF++ )
    {
        if ( !outColorMgmtLuts[ nInputEOTF ].vk_lut1d )
            outColorMgmtLuts[ nInputEOTF ].vk_lut1d =
                vulkan_create_1d_lut( s_nLutSize1d );

        if ( !outColorMgmtLuts[ nInputEOTF ].vk_lut3d )
            outColorMgmtLuts[ nInputEOTF ].vk_lut3d = vulkan_create_3d_lut(
                s_nLutEdgeSize3d, s_nLutEdgeSize3d, s_nLutEdgeSize3d );

        if ( g_ColorMgmtLutsOverride[ nInputEOTF ].HasLuts( ) )
        {
            memcpy(
                g_ColorMgmtLuts[ nInputEOTF ].lut1d,
                g_ColorMgmtLutsOverride[ nInputEOTF ].lut1d,
                sizeof( g_ColorMgmtLutsOverride[ nInputEOTF ].lut1d ) );
            memcpy(
                g_ColorMgmtLuts[ nInputEOTF ].lut3d,
                g_ColorMgmtLutsOverride[ nInputEOTF ].lut3d,
                sizeof( g_ColorMgmtLutsOverride[ nInputEOTF ].lut3d ) );
        }
        else
        {
            ...
            std::shared_ptr<lut3d_t> pSharedLook =
                g_ColorMgmtLooks[ nInputEOTF ];
            lut3d_t *pLook = pSharedLook && pSharedLook->lutEdgeSize > 0
                                 ? pSharedLook.get( )
                                 : nullptr;
            ...
            if ( inputEOTF == EOTF_Gamma22 )
            {
                ...
                // The final display colorimetry is used to build the output
                // mapping, as we want a gamut-aware handling for
                // sdrGamutWideness indepdendent of the output encoding (for SDR
                // data), and when mapping SDR -> PQ output we only want to
                // utilize a portion of the gamut the actual display can
                // reproduce
                buildSDRColorimetry(
                    &inputColorimetry,
                    &colorMapping,
                    newColorMgmt.sdrGamutWideness,
                    displayColorimetry );
            }
            else if ( inputEOTF == EOTF_PQ )
            {
                ...
                if ( newColorMgmt.outputEncodingEOTF == EOTF_Gamma22 )
                {
                    ...
                    tonemap_info_t source =
                        newColorMgmt.hdrTonemapSourceMetadata;
                    tonemap_info_t dest =
                        newColorMgmt.hdrTonemapDisplayMetadata;
                    // Otherwise, rely on the Vulkan source info and the EDID
                    // TODO: If source is invalid, use the provided metadata.
                    // TODO: If hdrTonemapDisplayMetadata is invalid, use the
                    // one provided by the display

                    // Adjust the source brightness range by the requested HDR
                    // input gain
                    ...
                    if ( source.BIsValid( ) && dest.BIsValid( ) )
                    {
                        ...
                        tonemapping.eetf2390.init(
                            source, newColorMgmt.hdrTonemapDisplayMetadata );
                    }
                    else
                    {
                    ...
                    /*
                    xwm_log.infof("PQ -> 2.2  -   g22_luminance %f gain %f",
                    tonemapping.g22_luminance, flGain ); xwm_log.infof("source
                    %f %f", source.flBlackPointNits, source.flWhitePointNits );
                    xwm_log.infof("dest %f %f", dest.flBlackPointNits,
                    dest.flWhitePointNits ); xwm_log.infof("operator %d", (int)
                    tonemapping.eOperator );*/
                }
                else if ( newColorMgmt.outputEncodingEOTF == EOTF_PQ )
                {
                    // PQ -> PQ passthrough (though this does apply gain)
                    // TODO: should we manipulate the output static metadata to
                    // reflect the gain factor?
                    tonemapping.g22_luminance = 1.f;
                    // xwm_log.infof("PQ -> PQ");
                }

                buildPQColorimetry(
                    &inputColorimetry, &colorMapping, displayColorimetry );
            }

            calcColorTransform<s_nLutEdgeSize3d>(
                &g_tmpLut1d,
                s_nLutSize1d,
                &g_tmpLut3d,
                inputColorimetry,
                ...
                newColorMgmt.outputVirtualWhite,
                newColorMgmt.chromaticAdaptationMode,
                colorMapping,
                newColorMgmt.nightmode,
                tonemapping,
                pLook,
                flGain );
            ...
            {
                outColorMgmtLuts[ nInputEOTF ].lut1d[ 4 * i + 0 ] =
                    quantize_lut_value_16bit( g_tmpLut1d.dataR[ i ] );
                outColorMgmtLuts[ nInputEOTF ].lut1d[ 4 * i + 1 ] =
                    quantize_lut_value_16bit( g_tmpLut1d.dataG[ i ] );
                outColorMgmtLuts[ nInputEOTF ].lut1d[ 4 * i + 2 ] =
                    quantize_lut_value_16bit( g_tmpLut1d.dataB[ i ] );
                outColorMgmtLuts[ nInputEOTF ].lut1d[ 4 * i + 3 ] = 0;
            }

            for ( size_t i = 0, end = g_tmpLut3d.data.size( ); i < end; ++i )
            {
                outColorMgmtLuts[ nInputEOTF ].lut3d[ 4 * i + 0 ] =
                    quantize_lut_value_16bit( g_tmpLut3d.data[ i ].r );
                outColorMgmtLuts[ nInputEOTF ].lut3d[ 4 * i + 1 ] =
                    quantize_lut_value_16bit( g_tmpLut3d.data[ i ].g );
                outColorMgmtLuts[ nInputEOTF ].lut3d[ 4 * i + 2 ] =
                    quantize_lut_value_16bit( g_tmpLut3d.data[ i ].b );
        ...
        vulkan_update_luts(
            outColorMgmtLuts[ nInputEOTF ].vk_lut1d,
            outColorMgmtLuts[ nInputEOTF ].vk_lut3d,
            outColorMgmtLuts[ nInputEOTF ].lut1d,
            outColorMgmtLuts[ nInputEOTF ].lut3d );
    }
}

gamescope::ConVar<bool> cv_tearing_enabled{
    "tearing_enabled", false, "Whether or not tearing is enabled."
};
...
gamescope::ConVar<bool> cv_hdr_enabled{
    "hdr_enabled", false, "Whether or not HDR is enabled if it is available."
};
bool g_bHDRItmEnable                   = false;
int  g_nCurrentRefreshRate_CachedValue = 0;

static void update_color_mgmt( )
{
    if ( !GetBackend( )->GetCurrentConnector( ) ) return;

    GetBackend( )->GetCurrentConnector( )->GetNativeColorimetry(
        g_bOutputHDREnabled,
        &g_ColorMgmt.pending.displayColorimetry,
        &g_ColorMgmt.pending.displayEOTF,
        &g_ColorMgmt.pending.outputEncodingColorimetry,
        &g_ColorMgmt.pending.outputEncodingEOTF );
...
#ifdef COLOR_MGMT_MICROBENCH
    double delta =
        ( timespec_to_nanos( t1 ) - timespec_to_nanos( t0 ) ) / 1000000.0;
...
static void update_screenshot_color_mgmt( )
{
    create_color_mgmt_luts( k_ScreenshotColorMgmt, g_ScreenshotColorMgmtLuts );
    create_color_mgmt_luts(
        k_ScreenshotColorMgmtHDR, g_ScreenshotColorMgmtLutsHDR );
}

bool set_color_sdr_gamut_wideness( float flVal )
{
    if ( g_ColorMgmt.pending.sdrGamutWideness == flVal ) return false;
...
{
    if ( flVal < 1.f ) { flVal = 203.f; }

    if ( g_ColorMgmt.pending.flSDROnHDRBrightness == flVal ) return false;
...
{
    if ( flVal < 0.f ) { flVal = 1.f; }

    if ( g_ColorMgmt.pending.flHDRInputGain == flVal ) return false;
...
bool set_sdr_input_gain( float flVal )
{
    ...
    if ( g_ColorMgmt.pending.flSDRInputGain == flVal ) return false;
...
{
    if ( g_ColorMgmt.pending.nightmode == nightmode ) return false;
...
{
    if ( g_ColorMgmt.pending.enabled == bEnabled ) return false;
...
static gamescope::OwningRc<CVulkanTexture>
    s_MuraCorrectionImage[ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ];
static std::shared_ptr<gamescope::BackendBlob>
             s_MuraCTMBlob[ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ];
static float g_flMuraScale               = 1.0f;
static bool  g_bMuraCompensationDisabled = false;

bool is_mura_correction_enabled( )
{
    if ( !GetBackend( )->GetCurrentConnector( ) ) return false;

    return s_MuraCorrectionImage[ GetBackend( )
                                      ->GetCurrentConnector( )
                                      ->GetScreenType( ) ] != nullptr &&
           !g_bMuraCompensationDisabled;
}

void update_mura_ctm( )
{
    s_MuraCTMBlob[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] = nullptr;
    if ( s_MuraCorrectionImage[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] ==
         nullptr )
        return;
    ...
    // Mura's influence scales non-linearly with brightness, so we have an
    // additional scale on top of the scale factor for the underlying mura map.
    ...
    s_MuraCTMBlob[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        GetBackend( )->CreateBackendBlob( mura_scale_offset );
}
...
{
    xwm_log.infof(
        "[josh mura correction] Setting mura correction image to: %s", path );
    s_MuraCorrectionImage[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        nullptr;
    update_mura_ctm( );
    ...
    unsigned char *red_data =
        stbi_load( red_path.c_str( ), &red_w, &red_h, &red_comp, 1 );
    int            green_w, green_h, green_comp;
    unsigned char *green_data =
        stbi_load( green_path.c_str( ), &green_w, &green_h, &green_comp, 1 );
    if ( !red_data || !green_data || red_w != green_w || red_h != green_h ||
         red_comp != green_comp || red_comp != 1 || green_comp != 1 )
    {
        xwm_log.infof(
            "[josh mura correction] Couldn't load mura correction image, "
            "disabling mura correction." );
        return true;
    }
    ...
    for ( int y = 0; y < h; y++ )
    {
        ...
        {
            data[ ( y * w * 4 ) + ( x * 4 ) + 0 ] =
                g_bMuraDebugFullColor ? 255 : red_data[ y * w + x ];
            data[ ( y * w * 4 ) + ( x * 4 ) + 1 ] =
                g_bMuraDebugFullColor ? 255 : green_data[ y * w + x ];
            data[ ( y * w * 4 ) + ( x * 4 ) + 2 ] = 127; // offset of 0.
            data[ ( y * w * 4 ) + ( x * 4 ) + 3 ] =
                0; // Make alpha = 0 so we act as addtive.
        }
    }
    free( red_data );
    free( green_data );
    ...
    s_MuraCorrectionImage[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        vulkan_create_texture_from_bits(
            w, h, w, h, DRM_FORMAT_ABGR8888, texCreateFlags, ( void * )data );
    free( data );
...
bool set_color_3dlut_override( const char *path )
{
    ...
    fread(
        g_ColorMgmtLutsOverride[ nLutIndex ].lut3d,
        elems,
        sizeof( uint16_t ),
        f );
...
bool set_color_shaperlut_override( const char *path )
{
    ...
    fread(
        g_ColorMgmtLutsOverride[ nLutIndex ].lut1d,
        elems,
        sizeof( uint16_t ),
        f );
...
{
    bool bRaisesBlackLevelFloor = false;
    g_ColorMgmtLooks[ EOTF_Gamma22 ] =
        LoadCubeLut( path, bRaisesBlackLevelFloor );
    cv_overlay_unmultiplied_alpha = bRaisesBlackLevelFloor;
    g_ColorMgmt.pending.externalDirtyCtr++;
    return true;
}
...
gamescope::CAsyncWaiter<gamescope::Rc<commit_t>> g_ImageWaiter{
    "gamescope_img"
};
...
Window x11_win( steamcompmgr_win_t *w )
{
    ...
    if ( w->type != steamcompmgr_win_type_t::XWAYLAND ) return None;
    return w->xwayland( ).id;
}

static std::atomic<uint64_t> s_ulFocusSerial = 0ul;
void                         MakeFocusDirty( ) { s_ulFocusSerial++; }

static inline uint64_t GetFocusSerial( ) { return s_ulFocusSerial; }

bool focus_t::IsDirty( ) { return ulCurrentFocusSerial != GetFocusSerial( ); }
...
struct global_focus_t : public focus_t
{
    ...
    {
        gamescope::IBackendConnector *pConnector =
            this->pVirtualConnector.get( );
        if ( !pConnector ) pConnector = GetBackend( )->GetCurrentConnector( );

        if ( pConnector ) { return pConnector->GetNestedHints( ); }

        return nullptr;
    }
};

std::unordered_map<gamescope::VirtualConnectorKey_t, global_focus_t>
                g_VirtualConnectorFocuses;
global_focus_t *GetCurrentFocus( )
{
    uint64_t ulKey =
        GetBackend( )->GetCurrentConnector( )
            ? GetBackend( )->GetCurrentConnector( )->GetVirtualConnectorKey( )
            : 0;

    auto iter = g_VirtualConnectorFocuses.find( ulKey );
    if ( iter != g_VirtualConnectorFocuses.end( ) ) return &iter->second;
...
{
    uint64_t ulKey = GetBackend( )->GetCurrentMouseConnector( )
                         ? GetBackend( )
                               ->GetCurrentMouseConnector( )
...
static gamescope::ConCommand cc_debug_force_repaint(
    "debug_force_repaint",
    "Force a repaint",
    []( std::span<std::string_view> args ) { hasRepaint = true; } );
...
static int g_nDynamicRefreshRate[ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ] = {
    0, 0
};
...
static int g_nCombinedAppRefreshCycleOverride
    [ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ] = { 0, 0 };
bool g_nCombinedAppRefreshCycleChangeRefresh
    [ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ] = { true, true };
bool g_nCombinedAppRefreshCycleChangeFPS
    [ gamescope::GAMESCOPE_SCREEN_TYPE_COUNT ] = { true, true };
...
static void _update_app_target_refresh_cycle( )
{
    ...
    gamescope::GamescopeScreenType type =
        GetBackend( )->GetCurrentConnector( )->GetScreenType( );
    ...
    {
        auto rates = GetBackend( )
                         ->GetCurrentConnector( )
                         ->GetValidDynamicRefreshRates( );
...
static void update_app_target_refresh_cycle( )
{
    ...
    if ( !!g_nSteamCompMgrTargetFPS != !!nPrevFPSLimit ) update_runtime_info( );
}

void steamcompmgr_set_app_refresh_cycle_override(
    gamescope::GamescopeScreenType type,
    int                            override_fps,
    bool                           change_refresh,
    bool                           change_fps_cap )
...
gamescope::ConCommand cc_debug_set_fps_limit(
    "debug_set_fps_limit",
    "Set refresh cycle (debug)",
    []( std::span<std::string_view> svArgs )
    {
        if ( svArgs.size( ) < 2 ) return;
        ...
        steamcompmgr_set_app_refresh_cycle_override(
            GetBackend( )->GetScreenType( ), nFps, true, true );
    } );
...
static bool window_is_steam( steamcompmgr_win_t *w )
{ return w && ( w->isSteamLegacyBigPicture || w->appID == 769 ); }

static bool window_is_vr_scene_app( steamcompmgr_win_t *w )
{
    return w && w->appID &&
           w->appID ==
               g_unCurrentVRSceneAppId.load( std::memory_order_relaxed );
}
...
{
    return w && !window_is_steam( w ) && !window_is_vr_scene_app( w ) &&
           !w->isOverlay && !w->isExternalOverlay;
}

static bool steamcompmgr_user_has_any_game_open( )
{
    gamescope_xwayland_server_t *server = NULL;
    for ( size_t i = 0; ( server = wlserver_get_xwayland_server( i ) ); i++ )
    {
        if ( !server->ctx ) continue;

        if ( steamcompmgr_window_should_limit_fps(
                 server->ctx->focus.focusWindow ) )
            return true;
    }
...
gamescope::ConVar<bool> cv_composite_force{
    "composite_force", false, "Force composition always, never use scanout"
};
...
namespace gamescope
{
    ...
    static ConCommand cc_screenshot(
        "screenshot",
        "Take a screenshot to a given path.",
        []( std::span<std::string_view> args )
        {
            std::string_view szPath = "/tmp/gamescope.png";
            if ( args.size( ) > 1 ) szPath = args[ 1 ];

            gamescope_control_screenshot_type eScreenshotType =
                GAMESCOPE_CONTROL_SCREENSHOT_TYPE_BASE_PLANE_ONLY;
            if ( args.size( ) > 2 )
            {
                std::optional<uint32_t> oType = Parse<uint32_t>( args[ 2 ] );
                if ( oType )
                    eScreenshotType =
                        static_cast<gamescope_control_screenshot_type>(
                            *oType );
            }
...
void statsThreadMain( void )
{
    ...
    while ( statsPipeFD == -1 )
    {
        ...
        if ( statsPipeFD == -1 ) { sleep( 10 ); }
    }

wait:
    statsThreadSem.wait( );

    if ( statsThreadRun == false ) { return; }
...
retry:
{
    ...
    if ( statsEventQueue.empty( ) ) { goto wait; }
...
find_win( xwayland_ctx_t *ctx, Window id, bool find_children = true )
{
    ...
    {
        if ( w->xwayland( ).id == id ) { return w; }
    }

    if ( !find_children ) return nullptr;
    ...
    if ( root == parent || parent == None ) { return NULL; }

    return find_win( ctx, parent );
}

static steamcompmgr_win_t *
find_win( xwayland_ctx_t *ctx, struct wlr_surface *surf )
{
    steamcompmgr_win_t *w = nullptr;

    for ( w = ctx->list; w; w = w->xwayland( ).next )
    {
        if ( w->xwayland( ).surface.main_surface == surf ) return w;

        if ( w->xwayland( ).surface.override_surface == surf ) return w;
    }
...
static gamescope::Rc<commit_t> import_commit(
    steamcompmgr_win_t                             *w,
    ...
    bool                                            fifo )
{
    ...
    if ( gamescope::OwningRc<CVulkanTexture> pTexture =
             s_BufferMemos.LookupVulkanTexture( buf ) )
    {
    ...
    gamescope::OwningRc<CVulkanTexture> pOwnedTexture =
        vulkan_create_texture_from_wlr_buffer( buf, std::move( pBackendFb ) );
...
static int32_t window_last_done_commit_index( steamcompmgr_win_t *w )
{
    int32_t lastCommit = -1;
    for ( uint32_t i = 0; i < w->commit_queue.size( ); i++ )
    {
        if ( w->commit_queue[ i ]->done ) { lastCommit = i; }
    }

    return lastCommit;
}

static bool window_has_commits( steamcompmgr_win_t *w )
{ return window_last_done_commit_index( w ) != -1; }

static void get_window_last_done_commit(
    steamcompmgr_win_t *w, gamescope::Rc<commit_t> &commit )
{
...
static commit_t *get_window_last_done_commit_peek( steamcompmgr_win_t *w )
{
    int32_t lastCommit = window_last_done_commit_index( w );

    if ( lastCommit == -1 ) { return nullptr; }

    return w->commit_queue[ lastCommit ].get( );
}

static int64_t window_last_done_commit_id( steamcompmgr_win_t *w )
{
    if ( !w ) return 0;
...
// For Steam, etc.
static bool window_wants_no_focus_when_mouse_hidden( steamcompmgr_win_t *w )
{ return window_is_steam( w ); }

static bool window_is_fullscreen( steamcompmgr_win_t *w )
{ return w && ( window_is_steam( w ) || w->isFullscreen ); }

void calc_scale_factor_scaler(
    float &out_scale_x,
    float &out_scale_y,
    float  sourceWidth,
    float  sourceHeight )
...
void calc_scale_factor(
    ...
    float  sourceHeight )
{
    calc_scale_factor_scaler(
        out_scale_x, out_scale_y, sourceWidth, sourceHeight );
...
/**
 * Constructor for a cursor. It is hidden in the beginning (normally until moved
 * by user).
 */
MouseCursor::MouseCursor( xwayland_ctx_t *ctx ) :
    m_texture( nullptr ), m_dirty( true ), m_imageEmpty( false ), m_ctx( ctx )
{ updateCursorFeedback( true ); }

void MouseCursor::UpdatePosition( )
{
    wlserver_lock( );
    struct wlr_pointer_constraint_v1 *pConstraint =
        wlserver.GetCursorConstraint( );
...
void MouseCursor::checkSuspension( )
{
    ...
    if ( ShouldDrawCursor( ) )
    {
        const bool suspended = int64_t( get_time_in_nanos( ) ) -
                                   int64_t( wlserver.ulLastMovedCursorTime ) >
        ...
        if ( !wlserver.bCursorHidden && suspended )
        {
            ...
            if ( window )
            {
                // Move the cursor to the bottom right corner, just off screen
                // if we can if the window (ie. Steam) doesn't want hover/focus
                // events.
                if ( window_wants_no_focus_when_mouse_hidden( window ) )
                {
                    wlserver_lock( );
                    wlserver_fake_mouse_pos(
                        window->GetGeometry( ).nWidth - 1,
                        window->GetGeometry( ).nHeight - 1 );
                    wlserver_mousehide( );
                    wlserver_unlock( );
                }
            }
...
bool MouseCursor::setCursorImage( char *data, int w, int h, int hx, int hy )
{
    ...
    if ( !( pixmap = XCreatePixmap(
                m_ctx->dpy, DefaultRootWindow( m_ctx->dpy ), w, h, 32 ) ) )
    {
    ...
    if ( !( pictformat =
                XRenderFindStandardFormat( m_ctx->dpy, PictStandardARGB32 ) ) )
    {
        xwm_log.errorf( "Failed to create pictformat for cursor" );
        goto error_pictformat;
    }

    if ( !( picture = XRenderCreatePicture(
                m_ctx->dpy, pixmap, pictformat, 0, NULL ) ) )
    {
...
int MouseCursor::x( ) const { return m_x; }

int MouseCursor::y( ) const { return m_y; }
...
bool MouseCursor::getTexture( )
{
    ...
    if ( GetBackend( )->GetCurrentConnector( ) )
        ulConnectorId =
            GetBackend( )->GetCurrentConnector( )->GetConnectorID( );
    ...
    if ( !m_dirty ) { return !m_imageEmpty; }
    ...
    glm::uvec2 surfaceSize = GetBackend( )->CursorSurfaceSize(
        glm::uvec2{ ( uint32_t )nDesiredWidth, ( uint32_t )nDesiredHeight } );
    ...
    if ( image->width && image->height )
    {
        if ( nDesiredWidth < image->width || nDesiredHeight < image->height )
        {
            ...
            for ( int i = 0; i < image->height; i++ )
            {
                ...
                {
                    pixels[ i * image->width + j ] =
                        image->pixels[ i * image->width + j ];
                }
            }
            std::vector<uint32_t> resizeBuffer(
            ...
            stbir_resize_uint8_srgb(
                ...
                image->height,
                0,
                ( unsigned char * )resizeBuffer.data( ),
                nDesiredWidth,
                nDesiredHeight,
                0,
                4,
            ...
            cursorBuffer =
                std::vector<uint32_t>( surfaceWidth * surfaceHeight );
            for ( int i = 0; i < nDesiredHeight; i++ )
            {
                for ( int j = 0; j < nDesiredWidth; j++ )
                {
                    cursorBuffer[ i * surfaceWidth + j ] =
                        resizeBuffer[ i * nDesiredWidth + j ];
                }
            }
    ...
    if ( bNoCursor ) cursorBuffer.clear( );
    ...
    {
        if ( GetBackend( )->GetCurrentConnector( ) &&
             GetBackend( )->GetCurrentConnector( )->GetNestedHints( ) )
            GetBackend( )
                ->GetCurrentConnector( )
                ->GetNestedHints( )
    ...
    m_texture = vulkan_create_texture_from_bits(
        surfaceWidth,
        surfaceHeight,
        nContentWidth,
        nContentHeight,
...
void MouseCursor::GetDesiredSize( int &nWidth, int &nHeight )
{
    ...
    {
        nSize =
            nSize * floor( g_nOutputHeight / ( float )g_nCursorScaleHeight );
    ...
    nSize = std::min<int>(
        nSize,
        glm::compMin(
            GetBackend( )->CursorSurfaceSize(
                glm::uvec2{ ( uint32_t )nSize, ( uint32_t )nSize } ) ) );
...
void MouseCursor::paint(
    steamcompmgr_win_t *window,
    steamcompmgr_win_t *fit,
    struct FrameInfo_t *frameInfo )
{
    if ( m_imageEmpty || wlserver.bCursorHidden ) return;
    ...
    if ( fit )
    {
        // If we have an override window, try to fit it in as long as it won't
        // make our scale go below 1.0.
        sourceWidth = std::max<int32_t>(
            sourceWidth,
            clamp<int>(
                fit->GetGeometry( ).nX + fit->GetGeometry( ).nWidth,
                0,
                currentOutputWidth ) );
        sourceHeight = std::max<int32_t>(
            sourceHeight,
            clamp<int>(
    ...
    calc_scale_factor(
        currentScaleRatio_x, currentScaleRatio_y, sourceWidth, sourceHeight );

    cursorOffsetX =
        ( currentOutputWidth - sourceWidth * currentScaleRatio_x ) / 2.0f;
    cursorOffsetY =
        ( currentOutputHeight - sourceHeight * currentScaleRatio_y ) / 2.0f;

    // Actual point on scaled screen where the cursor hotspot should be
    scaledX = ( winX - window->GetGeometry( ).nX ) * currentScaleRatio_x +
              cursorOffsetX;
    scaledY = ( winY - window->GetGeometry( ).nY ) * currentScaleRatio_y +
              cursorOffsetY;
    ...
    layer->filter      = cursor_scale != 1.0f ? GamescopeUpscaleFilter::LINEAR
                                              : GamescopeUpscaleFilter::NEAREST;
    layer->blackBorder = false;
    layer->ctm         = nullptr;
    layer->hdr_metadata_blob = nullptr;
    layer->colorspace        = GAMESCOPE_APP_TEXTURE_COLORSPACE_SRGB;

    layer->eAlphaBlendingMode = cv_overlay_unmultiplied_alpha
...
void MouseCursor::updateCursorFeedback( bool bForce )
{
    ...
    if ( m_dirty && !bForce ) return;

    bool bVisible = !isHidden( );

    if ( m_bCursorVisibleFeedback == bVisible && !bForce ) return;
    ...
    XChangeProperty(
        m_ctx->dpy,
        m_ctx->root,
        m_ctx->atoms.gamescopeCursorVisibleFeedback,
        XA_CARDINAL,
        32,
        PropModeReplace,
        ( unsigned char * )&value,
...
static void paint_cached_base_layer(
    const gamescope::Rc<commit_t> &commit,
    const BaseLayerInfo_t         &base,
    struct FrameInfo_t            *frameInfo,
    float                          flOpacityScale,
    bool                           bOverrideOpacity )
{
    ...
    layer->opacity =
        bOverrideOpacity ? flOpacityScale : base.opacity * flOpacityScale;
...
{
    if ( g_HeldCommits[ HELD_COMMIT_BASE ] == nullptr ) return nullptr;
...
gamescope::ConVar<bool> cv_paint_debug_pause_base_plane(
    "paint_debug_pause_base_plane", false, "Pause updates to the base plane." );

static FrameInfo_t::Layer_t *paint_window_commit(
    const gamescope::Rc<commit_t> &lastCommit,
    steamcompmgr_win_t            *w,
    steamcompmgr_win_t            *scaleW,
    struct FrameInfo_t            *frameInfo,
    MouseCursor                   *cursor,
    PaintWindowFlags               flags          = 0,
    float                          flOpacityScale = 1.0f,
    steamcompmgr_win_t            *fit            = nullptr )

{
    ...
    if ( !GetBackend( )->ShouldFitWindows( ) ) fit = nullptr;
    ...
    // instance either.
    if ( !w || lastCommit == nullptr ) return nullptr;
    ...
    layer->filter = ( flags & PaintWindowFlag::NoFilter )
                        ? GamescopeUpscaleFilter::LINEAR
                        : g_upscaleFilter;

    layer->tex = lastCommit->GetTexture(
        layer->filter, g_upscaleScaler, layer->colorspace );
    ...
    else
    {
        ...
        if ( fit )
        {
            // If we have an override window, try to fit it in as long as it
            // won't make our scale go below 1.0.
            sourceWidth = std::max<uint32_t>(
                sourceWidth,
                clamp<int>(
                    fit->GetGeometry( ).nX + fit->GetGeometry( ).nWidth,
                    0,
                    currentOutputWidth ) );
            sourceHeight = std::max<uint32_t>(
                sourceHeight,
                clamp<int>(
                    fit->GetGeometry( ).nY + fit->GetGeometry( ).nHeight,
                    0,
            ...
            baseWidth = std::max<uint32_t>(
                baseWidth,
                clamp<int>(
                    fit->GetGeometry( ).nX + fit->GetGeometry( ).nWidth,
                    0,
                    currentOutputWidth ) );
            baseHeight = std::max<uint32_t>(
                baseHeight,
                clamp<int>(
                    fit->GetGeometry( ).nY + fit->GetGeometry( ).nHeight,
                    0,
    ...
    bool offset =
        ( ( w->GetGeometry( ).nX || w->GetGeometry( ).nY ) && w != scaleW );

    if ( sourceWidth != ( int32_t )currentOutputWidth ||
         sourceHeight != ( int32_t )currentOutputHeight || offset ||
         globalScaleRatio != 1.0f )
    {
        ...
        drawXOffset = ( ( int )currentOutputWidth -
                        ( int )sourceWidth * currentScaleRatio_x ) /
                      2.0f;
        drawYOffset = ( ( int )currentOutputHeight -
                        ( int )sourceHeight * currentScaleRatio_y ) /
                      2.0f;
        ...
        calc_scale_factor(
            baseScaleRatio_x, baseScaleRatio_y, baseWidth, baseHeight );
        if ( zoomScaleRatio != 1.0 )
        {
            drawXOffset +=
                ( ( ( int )baseWidth / 2 ) - ( cursor ? cursor->x( ) : 0 ) ) *
                baseScaleRatio_x;
            drawYOffset +=
                ( ( ( int )baseHeight / 2 ) - ( cursor ? cursor->y( ) : 0 ) ) *
                baseScaleRatio_y;
        }
    }

    layer->opacity = ( ( w->isOverlay || w->isExternalOverlay )
                           ? w->opacity / ( float )OPAQUE
                           : 1.0f ) *
                     flOpacityScale;
    ...
    if ( w != scaleW ) { layer->zpos = g_zposOverride; }

    if ( w->isOverlay || w->isSteamStreamingClient )
    {
        layer->zpos = g_zposOverlay;
    }
    if ( w->isExternalOverlay ) { layer->zpos = g_zposExternalOverlay; }
    ...
    {
        // Don't bother doing more expensive filtering if we are sharp +
        // integer.
        if ( float_is_integer( currentScaleRatio_x ) &&
             float_is_integer( currentScaleRatio_y ) )
            layer->filter = GamescopeUpscaleFilter::NEAREST;
    }

    layer->eAlphaBlendingMode = ( flags & PaintWindowFlag::CoverageMode )
                                    ? ALPHA_BLENDING_MODE_COVERAGE
                                    : ALPHA_BLENDING_MODE_PREMULTIPLIED;
    ...
    steamcompmgr_win_t *fit            = nullptr )
{
    ...
    if ( w ) get_window_last_done_commit( w, lastCommit );
    ...
    if ( flags & PaintWindowFlag::BasePlane )
    {
        if ( lastCommit == nullptr || cv_paint_debug_pause_base_plane )
        {
            ...
            {
                paint_cached_base_layer(
                    g_HeldCommits[ HELD_COMMIT_BASE ],
                    g_CachedPlanes[ HELD_COMMIT_BASE ],
                    frameInfo,
                    flOpacityScale,
                    true );
                return;
    ...
    FrameInfo_t::Layer_t *layer = paint_window_commit(
        lastCommit, w, scaleW, frameInfo, cursor, flags, flOpacityScale, fit );
...
static bool is_fading_out( ) { return fadeOutStartTime || g_bPendingFade; }

static void update_touch_scaling( const struct FrameInfo_t *frameInfo )
{
    if ( !frameInfo->layerCount ) return;

    focusedWindowScaleX =
        frameInfo->layers[ frameInfo->layerCount - 1 ].scale.x;
    focusedWindowScaleY =
        frameInfo->layers[ frameInfo->layerCount - 1 ].scale.y;
    focusedWindowOffsetX =
        frameInfo->layers[ frameInfo->layerCount - 1 ].offset.x;
    focusedWindowOffsetY =
        frameInfo->layers[ frameInfo->layerCount - 1 ].offset.y;
}
...
static void paint_pipewire( )
{
    ...
    if ( !s_pPipewireBuffer ) s_pPipewireBuffer = dequeue_pipewire_buffer( );

    if ( !s_pPipewireBuffer || !s_pPipewireBuffer->texture ) return;
    ...
    for ( uint32_t nInputEOTF = 0; nInputEOTF < EOTF_Count; nInputEOTF++ )
    {
        frameInfo.lut3D[ nInputEOTF ] =
            g_ScreenshotColorMgmtLuts[ nInputEOTF ].vk_lut3d;
        frameInfo.shaperLut[ nInputEOTF ] =
            g_ScreenshotColorMgmtLuts[ nInputEOTF ].vk_lut1d;
    }
    ...
    if ( ulFocusAppId )
    {
        ...
        {
            xwm_log.infof(
                "Exposing appid %lu (%u 32-bit) focus-wise on pipewire stream.",
                ulFocusAppId,
                uint32_t( ulFocusAppId ) );
    ...
    if ( !pFocus->focusWindow ) return;

    const bool bAppIdMatches =
        !ulFocusAppId || pFocus->focusWindow->appID == ulFocusAppId;
    if ( !bAppIdMatches ) return;

    // If the commits are the same as they were last time, don't repaint and
    // don't push a new buffer on the stream.
    static uint64_t s_ulLastFocusCommitId    = 0;
    static uint64_t s_ulLastOverrideCommitId = 0;

    uint64_t ulFocusCommitId =
        window_last_done_commit_id( pFocus->focusWindow );
    uint64_t ulOverrideCommitId =
        window_last_done_commit_id( pFocus->overrideWindow );
    ...
    if ( pFocus->overrideWindow &&
        ...
        paint_window(
            ...
            &frameInfo,
            nullptr,
            PaintWindowFlag::NoFilter,
            1.0f,
            pFocus->overrideWindow );

    if ( !ulFocusAppId && pFocus->overlayWindow &&
         pFocus->overlayWindow->opacity )
    {
        paint_window(
            pFocus->overlayWindow,
            pFocus->overlayWindow,
            &frameInfo,
            nullptr,
            PaintWindowFlag::DrawBorders | PaintWindowFlag::NoFilter |
                ( cv_overlay_unmultiplied_alpha ? PaintWindowFlag::CoverageMode
                                                : 0 ) );
    }

    gamescope::Rc<CVulkanTexture> pRGBTexture =
        s_pPipewireBuffer->texture->isYcbcr( )
            ? vulkan_acquire_screenshot_texture(
                  uWidth, uHeight, false, DRM_FORMAT_XRGB2101010 )
            : gamescope::Rc<CVulkanTexture>{ s_pPipewireBuffer->texture };

    gamescope::Rc<CVulkanTexture> pYUVTexture =
        s_pPipewireBuffer->texture->isYcbcr( ) ? s_pPipewireBuffer->texture
                                               : nullptr;

    std::optional<uint64_t> oPipewireSequence =
        vulkan_screenshot( &frameInfo, pRGBTexture, pYUVTexture );
    // If we ever want the fat compositing path, use this.
    // std::optional<uint64_t> oPipewireSequence = vulkan_composite( &frameInfo,
    // s_pPipewireBuffer->texture, false, pRGBTexture, false );
...
gamescope::ConVar<int> cv_cursor_composite{
    "cursor_composite",
    1,
    "0 = Never composite a cursor. 1 = Composite cursor when not nested. 2 = "
    "Always composite a cursor manually"
};
bool ShouldDrawCursor( )
{
    if ( cv_cursor_composite == 2 ) return true;

    if ( cv_cursor_composite == 0 ) return false;

    if ( g_bForceRelativeMouse ) return true;

    global_focus_t *pFocus = GetCurrentFocus( );
    if ( !pFocus ) return false;

    if ( !pFocus->GetNestedHints( ) ) return true;
...
static void ForwardVROverlayTargets( )
{
    ...
    for ( size_t i = 0; ( server = wlserver_get_xwayland_server( i ) ); i++ )
    {
        for ( steamcompmgr_win_t *w = server->ctx->list; w;
              w                     = w->xwayland( ).next )
        {
            if ( w->oulTargetVROverlay && w->bNeedsForwarding )
            {
                ...
                gamescope::IBackendFb *pFb =
                    lastCommit->vulkanTex->GetBackendFb( );
                if ( !pFb ) continue;

                const uint64_t ulOverlayHandle = *w->oulTargetVROverlay;
                GetBackend( )->ForwardFramebuffer(
                    w->pForwarderPlane, pFb, &ulOverlayHandle );
...
gamescope::ConVar<bool> cv_paint_override_redirect_plane{
    "paint_override_redirect_plane", true
};
gamescope::ConVar<bool> cv_paint_steam_overlay_plane{
    "paint_steam_overlay_plane", true
};
gamescope::ConVar<bool> cv_paint_external_overlay_plane{
    "paint_external_overlay_plane", true
};
...
static void paint_all( global_focus_t *pFocus, bool async )
{
    if ( !pFocus ) return;
    ...
    gamescope_xwayland_server_t *root_server =
        wlserver_get_xwayland_server( 0 );
    ...
    bool fadingOut = ( currentTime - fadeOutStartTime < g_FadeOutDuration ||
                       g_bPendingFade ) &&
    ...
    {
        currentFrameRate =
            300 * 1000.0f / ( currentTime - lastSampledFrameTime );
        lastSampledFrameTime = currentTime;
        frameCounter         = 0;

        stats_printf( "fps=%f\n", currentFrameRate );

        if ( window_is_steam( w ) ) { stats_printf( "focus=steam\n" ); }
        else
        {
    ...
    if ( cv_paint_primary_plane )
    {
        if ( w )
        {
            if ( w->isSteamStreamingClient == true )
            {
                ...
                gamescope_xwayland_server_t *server = NULL;
                for ( size_t i = 0;
                      ( server = wlserver_get_xwayland_server( i ) );
                      i++ )
                {
                    for ( videow = server->ctx->list; videow;
                          videow = videow->xwayland( ).next )
                    {
                        if ( videow->isSteamStreamingClientVideo == true )
                        {
                            // TODO: also check matching AppID so we can have
                            // several pairs
                            paint_window(
                                videow,
                                videow,
                ...
                uint32_t flags = 0;
                if ( !bHasVideoUnderlay ) flags |= PaintWindowFlag::BasePlane;
                paint_window( w, w, &frameInfo, pFocus->cursor, flags );
                if ( pFocus == GetCurrentFocus( ) )
                    update_touch_scaling( &frameInfo );

                // paint UI unless it's fully hidden, which it communicates to
                // us through opacity=0 we paint it to extract scaling
                // coefficients above, then remove the layer if one was added
                if ( w->opacity == TRANSLUCENT && bHasVideoUnderlay &&
                     nOldLayerCount < frameInfo.layerCount )
            ...
            else
            {
                    ...
                    float opacityScale =
                        g_bPendingFade ? 0.0f
                                       : ( ( currentTime - fadeOutStartTime ) /
                                           ( float )g_FadeOutDuration );

                    paint_cached_base_layer(
                        g_HeldCommits[ HELD_COMMIT_FADE ],
                        g_CachedPlanes[ HELD_COMMIT_FADE ],
                        &frameInfo,
                        1.0f - opacityScale,
                        false );
                    paint_window(
                ...
                else
                {
                    ...
                    // Just draw focused window as normal, be it Steam or the
                    // game
                    paint_window(
                        w,
                        w,
                    ...
                    bool needsScaling =
                        frameInfo.layers[ 0 ].scale.x < 0.999f &&
                        frameInfo.layers[ 0 ].scale.y < 0.999f;
                    frameInfo.useFSRLayer0 =
                        g_upscaleFilter == GamescopeUpscaleFilter::FSR &&
                        needsScaling;
                    frameInfo.useNISLayer0 =
    ...
    // TODO: We want to paint this at the same scale as the normal window and
    // probably with an offset. Josh: No override if we're streaming video as we
    // will have too many layers. Better to be safe than sorry.
    if ( override && w && !w->isSteamStreamingClient &&
         cv_paint_override_redirect_plane )
    {
        paint_window(
            override,
            w,
    ...
    // If we have any layers that aren't a cursor or overlay, then we have valid
            ...
            paint_window(
                ...
                PaintWindowFlag::NoScale | PaintWindowFlag::NoFilter |
                    ( cv_overlay_unmultiplied_alpha
                          ? PaintWindowFlag::CoverageMode
                          : 0 ) );

            if ( externalOverlay == pFocus->inputFocusWindow &&
                 pFocus == GetCurrentFocus( ) )
                update_touch_scaling( &frameInfo );
        }
    }
    ...
    if ( cv_paint_steam_overlay_plane )
    {
        if ( overlay && overlay->opacity )
        {
            ...
            if ( overlay == pFocus->inputFocusWindow &&
                 pFocus == GetCurrentFocus( ) )
                update_touch_scaling( &frameInfo );
        }
        else if (
            !GetBackend( )->UsesVulkanSwapchain( ) &&
            GetBackend( )->IsSessionBased( ) )
        {
            ...
            if ( tex != nullptr )
            {
                ...
                layer->scale.x = g_nOutputWidth == tex->width( )
                                     ? 1.0f
                                     : tex->width( ) / ( float )g_nOutputWidth;
                layer->scale.y =
                    g_nOutputHeight == tex->height( )
                        ? 1.0f
                        : tex->height( ) / ( float )g_nOutputHeight;
                layer->offset.x       = 0.0f;
                layer->offset.y       = 0.0f;
                layer->opacity        = 1.0f; // BLAH
                layer->zpos           = g_zposOverlay;
                layer->applyColorMgmt = g_ColorMgmt.pending.enabled;
                layer->eAlphaBlendingMode =
                    cv_overlay_unmultiplied_alpha
                        ? ALPHA_BLENDING_MODE_COVERAGE
                        : ALPHA_BLENDING_MODE_PREMULTIPLIED;
    ...
    {
        pFocus->cursor->paint(
            input, w == input ? override : nullptr, &frameInfo );
    }

    if ( !bValidContents || GetBackend( )->IsPaused( ) ) { return; }

    unsigned int blurFadeTime =
        get_time_in_milliseconds( ) - g_BlurFadeStartTime;
    bool     blurFading = blurFadeTime < g_BlurFadeDuration;
    BlurMode currentBlurMode =
        blurFading ? std::max( g_BlurMode, g_BlurModeOld ) : g_BlurMode;

    if ( currentBlurMode &&
         !( frameInfo.layerCount <= 1 && currentBlurMode == BLUR_MODE_COND ) )
    {
    ...
    if ( const auto &heldCommit = g_HeldCommits[ HELD_COMMIT_BASE ];
         heldCommit && heldCommit->upscaledTexture )
    {
        g_bFSRActive =
            ( heldCommit->upscaledTexture->eFilter ==
    ...
    const bool bSupportsDynamicRefresh =
        pConnector && !pConnector->GetValidDynamicRefreshRates( ).empty( );
    if ( bSupportsDynamicRefresh )
    {
        auto rates = pConnector->GetValidDynamicRefreshRates( );

        int nDynamicRefreshHz =
            g_nDynamicRefreshRate[ GetBackend( )->GetScreenType( ) ];

        int nTargetRefreshHz =
            nDynamicRefreshHz && steamcompmgr_window_should_refresh_switch(
                                     pFocus->focusWindow )
                ? nDynamicRefreshHz
        ...
        // The actual output hz generated by the mode, could be off by one
        // either side, due to rounding.
        //
        // Check what we cached for the current dynamic refresh Hz we put in --
        // otherwise, convert g_nOutputRefresh -> Hz rounded.
        int32_t nCurrentDynamicOutputHz =
            g_nDynamicRefreshHz ? g_nDynamicRefreshHz
    ...
    bool bDoMuraCompensation = is_mura_correction_enabled( ) &&
                               frameInfo.layerCount && cv_paint_mura_plane;
    if ( bDoMuraCompensation )
    {
        auto &MuraCorrectionImage =
            s_MuraCorrectionImage[ GetBackend( )->GetScreenType( ) ];
        int curLayer = frameInfo.layerCount++;
        ...
        // Blending needs to be done in Gamma 2.2 space for mura correction to
        // work.
    ...
    if ( oScreenshotInfo )
    {
        std::filesystem::path path =
            std::filesystem::path{ oScreenshotInfo->szScreenshotPath };
        ...
        if ( drmCaptureFormat != DRM_FORMAT_INVALID )
            pScreenshotTexture = vulkan_acquire_screenshot_texture(
                g_nOutputWidth, g_nOutputHeight, false, drmCaptureFormat );

        if ( pScreenshotTexture )
        {
            bool bHDRScreenshot =
                path.extension( ) == ".avif" && frameInfo.layerCount > 0 &&
                ColorspaceIsHDR( frameInfo.layers[ 0 ].colorspace ) &&
                oScreenshotInfo->eScreenshotType !=
            ...
            if ( drmCaptureFormat == DRM_FORMAT_NV12 ||
                 oScreenshotInfo->eScreenshotType !=
                     GAMESCOPE_CONTROL_SCREENSHOT_TYPE_SCREEN_BUFFER )
            {
                // Basically no color mgmt applied for screenshots. (aside from
                // being able to handle HDR content with LUTs)
                for ( uint32_t nInputEOTF = 0; nInputEOTF < EOTF_Count;
                      nInputEOTF++ )
                {
                    auto &luts = bHDRScreenshot ? g_ScreenshotColorMgmtLutsHDR
                                                : g_ScreenshotColorMgmtLuts;
                    frameInfo.lut3D[ nInputEOTF ] = luts[ nInputEOTF ].vk_lut3d;
                    frameInfo.shaperLut[ nInputEOTF ] =
                        luts[ nInputEOTF ].vk_lut1d;
                }

                if ( oScreenshotInfo->eScreenshotType ==
                     GAMESCOPE_CONTROL_SCREENSHOT_TYPE_BASE_PLANE_ONLY )
                {
                    // Remove everything but base planes from the screenshot.
                    for ( int i = 0; i < frameInfo.layerCount; i++ )
                    {
                        if ( frameInfo.layers[ i ].zpos >=
                             ( int )g_zposExternalOverlay )
                        {
                ...
                // Re-enable output color management (blending) if it was
                // disabled by mura.
                frameInfo.applyOutputColorMgmt = true;
            }

            frameInfo.outputEncodingEOTF =
                bHDRScreenshot ? EOTF_PQ : EOTF_Gamma22;
            ...
            if ( drmCaptureFormat == DRM_FORMAT_NV12 )
                oScreenshotSeq = vulkan_composite(
                    &frameInfo, pScreenshotTexture, false, nullptr );
            else if (
                oScreenshotInfo->eScreenshotType ==
                    GAMESCOPE_CONTROL_SCREENSHOT_TYPE_FULL_COMPOSITION ||
                oScreenshotInfo->eScreenshotType ==
                    GAMESCOPE_CONTROL_SCREENSHOT_TYPE_SCREEN_BUFFER )
                oScreenshotSeq = vulkan_composite(
                    &frameInfo, nullptr, false, pScreenshotTexture );
            else
                oScreenshotSeq = vulkan_screenshot(
                    &frameInfo, pScreenshotTexture, nullptr );
            ...
            if ( bHDRScreenshot )
            {
                ...
                if ( !maxCLLNits && !maxFALLNits )
                {
                    ...
                    {
                        maxCLLNits =
                            pConnector->GetHDRInfo( ).uMaxContentLightLevel;
                        maxFALLNits =
                            pConnector->GetHDRInfo( ).uMaxFrameAverageLuminance;
                    }
                }
                ...
                if ( !maxCLLNits && !maxFALLNits )
                {
                    maxCLLNits =
                        g_ColorMgmt.pending.flInternalDisplayBrightness;
                    maxFALLNits =
                        g_ColorMgmt.pending.flInternalDisplayBrightness * 0.8f;
                }
            }
            ...
            std::thread screenshotThread = std::thread(
                [ = ]
                {
                    ...
                    const uint8_t *mappedData =
                        pScreenshotTexture->mappedData( );

                    bool bScreenshotSuccess = false;

                    if ( pScreenshotTexture->format( ) ==
                         VK_FORMAT_A2R10G10B10_UNORM_PACK32 )
                    {
                        // Make our own copy of the image to remove the alpha
                        // channel.
                        constexpr uint32_t kCompCnt  = 3;
                        auto               imageData = std::vector<uint16_t>(
                            g_nOutputWidth * g_nOutputHeight * kCompCnt );

                        for ( uint32_t y = 0; y < g_nOutputHeight; y++ )
                        {
                            for ( uint32_t x = 0; x < g_nOutputWidth; x++ )
                            {
                                uint32_t *pInPixel = ( uint32_t * )&mappedData
                                    [ ( y * pScreenshotTexture->rowPitch( ) ) +
                                      x * ( 32 / 8 ) ];
                                uint32_t uInPixel = *pInPixel;

                                imageData
                                    [ y * g_nOutputWidth * kCompCnt +
                                      x * kCompCnt + 0 ] =
                                        ( uInPixel & ( 0b1111111111 << 20 ) ) >>
                                        20;
                                imageData
                                    [ y * g_nOutputWidth * kCompCnt +
                                      x * kCompCnt + 1 ] =
                                        ( uInPixel & ( 0b1111111111 << 10 ) ) >>
                                        10;
                                imageData
                                    [ y * g_nOutputWidth * kCompCnt +
                                      x * kCompCnt + 2 ] =
                                        ( uInPixel & ( 0b1111111111 << 0 ) ) >>
                                        0;
                            }
                        }
                        ...
                        avifImage *pAvifImage = avifImageCreate(
                            g_nOutputWidth,
                            g_nOutputHeight,
                            10,
                        ...
                        pAvifImage->colorPrimaries =
                            bHDRScreenshot ? AVIF_COLOR_PRIMARIES_BT2020
                                           : AVIF_COLOR_PRIMARIES_BT709;
                        pAvifImage->transferCharacteristics =
                            bHDRScreenshot
                                ? AVIF_TRANSFER_CHARACTERISTICS_SMPTE2084
                                : AVIF_TRANSFER_CHARACTERISTICS_SRGB;
                        // We are not actually using YUV, but storing raw GBR
                        // (yes not RGB) data This does not compress as well,
                        // but is always lossless!
                        pAvifImage->matrixCoefficients =
                            AVIF_MATRIX_COEFFICIENTS_IDENTITY;

                        if ( oScreenshotInfo->eScreenshotType ==
                             GAMESCOPE_CONTROL_SCREENSHOT_TYPE_SCREEN_BUFFER )
                        {
                            // When dumping the screen output buffer for
                            // debugging, mark the primaries as UNKNOWN as stuff
                            // has likely been transformed to native if HDR on
                            // Deck OLED etc. We want everything to be seen
                            // unadulterated by a viewer/image editor.
                            pAvifImage->colorPrimaries =
                        ...
                        rgbAvifImage.rowBytes =
                            g_nOutputWidth * kCompCnt * sizeof( uint16_t );

                        if ( ( avifResult = avifImageRGBToYUV(
                                   pAvifImage, &rgbAvifImage ) ) !=
                             AVIF_RESULT_OK ) // Not really! See Matrix
                                              // Coefficients IDENTITY above.
                        {
                            xwm_log.errorf(
                                "Failed to convert RGB to YUV: %u",
                                avifResult );
                            return;
                        }
                        ...
                        if ( ( avifResult = avifEncoderAddImage(
                                   pEncoder,
                                   pAvifImage,
                                   1,
                        ...
                        {
                            xwm_log.errorf(
                                "Failed to add image to avif encoder: %u",
                                avifResult );
                            return;
                        }
                        ...
                        if ( ( avifResult = avifEncoderFinish(
                                   pEncoder, &avifOutput ) ) != AVIF_RESULT_OK )
                        {
                            xwm_log.errorf(
                                "Failed to finish encoder: %u", avifResult );
                            return;
                        }

                        FILE *pScreenshotFile = nullptr;
                        if ( ( pScreenshotFile = fopen(
                                   oScreenshotInfo->szScreenshotPath.c_str( ),
                                   "wb" ) ) == nullptr )
                        {
                            xwm_log.errorf(
                        ...
                        pScreenshotTexture->format( ) ==
                        VK_FORMAT_B8G8R8A8_UNORM )
                    {
                        // Make our own copy of the image to remove the alpha
                        // channel.
                        auto imageData = std::vector<uint8_t>(
                            currentOutputWidth * currentOutputHeight * 4 );
                        ...
                        for ( uint32_t y = 0; y < currentOutputHeight; y++ )
                        {
                            ...
                            {
                                // BGR...
                                imageData[ y * pitch + x * comp + 0 ] =
                                    mappedData
                                        [ y * pScreenshotTexture->rowPitch( ) +
                                          x * comp + 2 ];
                                imageData[ y * pitch + x * comp + 1 ] =
                                    mappedData
                                        [ y * pScreenshotTexture->rowPitch( ) +
                                          x * comp + 1 ];
                                imageData[ y * pitch + x * comp + 2 ] =
                                    mappedData
                                        [ y * pScreenshotTexture->rowPitch( ) +
                                          x * comp + 0 ];
                    ...
                    {
                        FILE *file = fopen(
                            oScreenshotInfo->szScreenshotPath.c_str( ), "wb" );
                        if ( file )
                        {
                            fwrite(
                                mappedData,
                                1,
                                pScreenshotTexture->totalSize( ),
                                file );
                            fclose( file );
                    ...
                    if ( oScreenshotInfo->bX11PropertyRequested )
                    {
                        XDeleteProperty(
                            root_ctx->dpy,
                            root_ctx->root,
                            root_ctx->atoms.gamescopeScreenShotAtom );
                        XDeleteProperty(
                            root_ctx->dpy,
                            root_ctx->root,
                            root_ctx->atoms.gamescopeDebugScreenShotAtom );
                    }

                    if ( bScreenshotSuccess &&
                         oScreenshotInfo->bWaylandRequested )
                    {
                        wlserver_lock( );
                        for ( const auto &control :
                              wlserver.gamescope_controls )
                        {
                            gamescope_control_send_screenshot_taken(
                                control,
                                oScreenshotInfo->szScreenshotPath.c_str( ) );
                        }
                        wlserver_unlock( );
        ...
        {
            xwm_log.errorf(
                "Oh no, we ran out of screenshot images. Not actually writing "
                "a screenshot." );
            if ( oScreenshotInfo->bX11PropertyRequested )
            {
                XDeleteProperty(
...
get_u64_prop( xwayland_ctx_t *ctx, Window win, Atom prop )
{
    ...
    return ( ( uint64_t )( values[ 0 ] ) ) |
           ( ( ( uint64_t )values[ 1 ] ) << 32ul );
}

std::string get_string_prop( xwayland_ctx_t *ctx, Window win, Atom prop )
{
    XTextProperty tp;
    if ( !XGetTextProperty( ctx->dpy, win, &tp, prop ) ) return "";
...
{
    XTextProperty text_property = { .value = ( unsigned char * )value.c_str( ),
...
static bool win_has_game_id( steamcompmgr_win_t *w ) { return w->appID != 0; }

static bool win_is_useless( steamcompmgr_win_t *w )
{
    if ( w->isSysTrayIcon ) return true;
...
static bool win_is_override_redirect( steamcompmgr_win_t *w )
{
    if ( w->type != steamcompmgr_win_type_t::XWAYLAND ) return false;

    return w->xwayland( ).a.override_redirect && !w->ignoreOverrideRedirect &&
           !win_is_useless( w );
}

static bool win_skip_taskbar_and_pager( steamcompmgr_win_t *w )
{ return w->skipTaskbar && w->skipPager; }

static bool win_skip_and_not_fullscreen( steamcompmgr_win_t *w )
{ return win_skip_taskbar_and_pager( w ) && !w->isFullscreen; }

static bool win_maybe_a_dropdown( steamcompmgr_win_t *w )
{
    if ( w->type != steamcompmgr_win_type_t::XWAYLAND ) return false;
    ...
    if ( w->appID == 230410 && w->maybe_a_dropdown &&
         w->xwayland( ).transientFor && ( w->skipPager || w->skipTaskbar ) )
        return !win_is_useless( w );

    // Work around Antichamber splash screen until we hook up
    // the Proton window style deduction.
    if ( w->appID == 219890 ) return false;

    // The Launcher in Witcher 2 (20920) has a clear window with WS_EX_LAYERED
    // on top of it.
    //
    // The Age of Empires 2 Launcher also has a WS_EX_LAYERED window to separate
    // controls from its backing, which this seems to handle, although we
    // seemingly don't handle it's transparency yet, which I do not understand.
    //
    ...
    //
    // Only do this if we have CONTROLPARENT right now. Some other apps, such as
    // the Street Fighter V (310950) Splash Screen also use LAYERED and
    // TOOLWINDOW, and we don't want that to be overlayed. Ignore LAYERED if
    // it's marked as top-level with WS_EX_APPWINDOW.
    ...
    // Forza Horizon 4 & 5 create a black background window that might
    // incorrectly be considered a valid dropdown and steal focus.
    if ( ( w->appID == 1293830 || w->appID == 1551360 ) &&
         w->maybe_a_dropdown && w->requestedWidth == 0 &&
         w->requestedHeight == 0 )
        return false;

    // Josh:
    // The logic here is as follows. The window will be treated as a dropdown
    // if:
    //
    ...
    // 		- If the window has transient for, disregard it, as it is trying to
    // redirecting us elsewhere
    //        ie. a settings menu dialog popup or something.
    //      - If the window has both skip taskbar and pager, treat it as a
    //      dialog.
    bool valid_maybe_a_dropdown =
        w->maybe_a_dropdown &&
        ( ( !w->is_dialog || ( !w->xwayland( ).transientFor &&
                               win_skip_and_not_fullscreen( w ) ) ) &&
          ( w->skipPager || w->skipTaskbar ) );
    return ( valid_maybe_a_dropdown || win_is_override_redirect( w ) ) &&
           !win_is_useless( w );
}

static bool win_is_disabled( steamcompmgr_win_t *w )
{
    if ( !w->hasHwndStyle ) return false;
...
is_focus_priority_greater( steamcompmgr_win_t *a, steamcompmgr_win_t *b )
{
    ...
    if ( a->type != steamcompmgr_win_type_t::XWAYLAND ) { return true; }
    ...
    if ( win_has_game_id( a ) &&
         a->xwayland( ).damage_sequence != b->xwayland( ).damage_sequence )
        return a->xwayland( ).damage_sequence > b->xwayland( ).damage_sequence;

    return false;
}

static bool is_good_override_candidate(
    steamcompmgr_win_t *override, steamcompmgr_win_t *focus )
{
    // Some Chrome/Edge dropdowns (ie. FH5 xbox login) will automatically close
    // themselves if you focus them while they are meant to be offscreen (-1,-1
    // and 1x1) so check that the override's position is on-screen.
    if ( !focus ) return false;

    // The pids should probably match for a dropdown to be a good candidate for
    // this window.
    if ( override->pid != focus->pid ) return false;

    auto rect = override->GetGeometry( );
    return override != focus && ( rect.nX + rect.nWidth ) > 0 &&
           ( rect.nY + rect.nHeight ) > 0;
}

static void handle_desktop_window( steamcompmgr_win_t *w );
    ...
    gamescope::VirtualConnectorStrategy      eStrategy )
{
    ...
    if ( controlledFocus )
    {
             ...
             gamescope::VirtualConnectorStrategies::SteamControlled )
        {
            ...
            {
                for ( steamcompmgr_win_t *focusable_window :
                      vecPossibleFocusWindows )
                {
                    if ( focusable_window->type !=
                         steamcompmgr_win_type_t::XWAYLAND )
                        continue;

                    if ( focusable_window->xwayland( ).id ==
                         focusControlWindow )
                    {
    ...
    auto resolveTransientOverrides = [ & ]( bool maybe )
    {
        ...
        while ( true )
        {
            ...
            for ( steamcompmgr_win_t *candidate : vecPossibleFocusWindows )
            {
                ...
                bool is_dropdown = maybe
                                       ? win_maybe_a_dropdown( candidate )
                                       : win_is_override_redirect( candidate );
                if ( ( !override_focus || candidate != override_focus ) &&
                     candidate != focus &&
                     ( ( !override_focus &&
                         candidate->xwayland( ).transientFor ==
                             focus->xwayland( ).id ) ||
                       ( override_focus &&
                         candidate->xwayland( ).transientFor ==
                             override_focus->xwayland( ).id ) ) &&
                     is_dropdown )
                {
            ...
            // Hopefully we can't have transient cycles or we'll have to
            // maintain a list of visited windows here
            if ( bFoundTransient == false ) break;
        }
    };
    ...
    if ( focus && focus->type == steamcompmgr_win_type_t::XWAYLAND )
    {
        ...
        {
            // Do some searches through game windows to follow transient links
            // if needed
            while ( true )
            {
                bool bFoundTransient = false;
        ...
        if ( !override_focus )
        {
            if ( !ctxFocusControlAppIDs.empty( ) )
            {
                ...
                {
                    if ( win_is_override_redirect( override ) &&
                         is_good_override_candidate( override, focus ) &&
                         override->appID == focus->appID )
                    {
    ...
    if ( focus )
    {
        if ( window_has_commits( focus ) ) out->focusWindow = focus;
        else
        ...
        if ( !globalFocus ) out->focusWindow = focus;
    }
    ...
    if ( !override_focus && focus )
    {
        if ( controlledFocus )
        {
            for ( auto focusable_appid : ctxFocusControlAppIDs )
            {
                for ( steamcompmgr_win_t *fake_override :
                      vecPossibleFocusWindows )
                {
                    if ( fake_override->appID == focusable_appid )
                    {
                        if ( win_maybe_a_dropdown( fake_override ) &&
                             is_good_override_candidate(
                                 fake_override, focus ) &&
                             fake_override->appID == focus->appID )
                        {
        ...
        else
        {
            ...
            {
                if ( win_maybe_a_dropdown( fake_override ) &&
                     is_good_override_candidate( fake_override, focus ) )
                {
                    override_focus = fake_override;
                    goto found2;
                }
...
std::vector<steamcompmgr_win_t *> xwayland_ctx_t::GetPossibleFocusWindows( )
{
    ...
    for ( steamcompmgr_win_t *w = this->list; w; w = w->xwayland( ).next )
    {
        ...
        if ( w->isSteamStreamingClientVideo ) { continue; }

        if ( gamescope::cv_backend_virtual_connector_strategy ==
             gamescope::VirtualConnectorStrategies::SteamControlled )
        {
            if ( !( win_has_game_id( w ) || window_is_steam( w ) ||
                    w->isSteamStreamingClient ) )
                continue;
        }

        if ( w->xwayland( ).a.map_state == IsViewable &&
             w->xwayland( ).a.c_class == InputOutput &&
...
void xwayland_ctx_t::DetermineAndApplyFocus(
    const std::vector<steamcompmgr_win_t *> &vecPossibleFocusWindows )
{
    ...
    pick_primary_focus_and_override(
        &ctx->focus,
        ctx->focusControlWindow,
        vecPossibleFocusWindows,
        false,
        vecFocuscontrolAppIDs,
        ulKey,
        eStrategy );
    ...
    if ( inputFocus == NULL ) { inputFocus = ctx->focus.focusWindow; }

    if ( ctx->focus.focusWindow )
    {
        if ( prevFocusWindow != ctx->focus.focusWindow )
        {
            /* Some games (e.g. DOOM Eternal) don't react well to being put back
             * as iconic, so never do that. Only take them out of iconic. */
            set_wm_state(
                ctx,
    ...
    else
    {
        ...
        {
            gamescope::VirtualConnectorKey_t ulMouseKey =
                GetBackend( )
                    ->GetCurrentMouseConnector( )
                    ->GetVirtualConnectorKey( );
            ...
            {
                inputFocus = mouse_focus.overrideWindow
                                 ? mouse_focus.overrideWindow
                                 : mouse_focus.focusWindow;
        ...
        {
            for ( steamcompmgr_win_t *queryWindow = ctx->list; queryWindow;
                  queryWindow = queryWindow->xwayland( ).next )
            {
                if ( queryWindow->oulTargetVROverlay &&
                     *queryWindow->oulTargetVROverlay ==
                         ulFocusedKeyboardOverlayVR )
                {
                    focus_log.debugf(
                        "[XWL] Overriding keyboard focus window with VR "
                        "forwarder overlay! Overlay: 0x%lx XWindow: 0x%x "
                        "Title: %s",
                        ulFocusedKeyboardOverlayVR,
                        queryWindow->id( ),
                        queryWindow->debug_name( ) );
                ...
                {
                    // We don't want to do any mouse input for target VR
                    // overlays right now. SteamWebHelper is handling this.

                    if ( !inputFocus ) inputFocus = queryWindow;
    ...
    if ( !ctx->focus.focusWindow ) { return; }

    if ( !inputFocus ) { inputFocus = ctx->focus.focusWindow; }

    Window keyboardFocusWindow =
        keyboardFocusWin ? keyboardFocusWin->xwayland( ).id : None;

    // If the top level parent of our current keyboard window is the same as our
    // target (top level) input focus window then keep focus on that and don't
    // yank it away to the top level input focus window. Fixes dropdowns in
    // Steam CEF.
         ...
         ctx->currentKeyboardFocusWindow != keyboardFocusWindow )
    {
        ...
        {
            xwm_log.debugf(
                "determine_and_apply_focus inputFocus %lu",
                inputFocus->xwayland( ).id );
        }

        if ( !ctx->focus.overrideWindow ||
             ctx->focus.overrideWindow != keyboardFocusWin )
            XSetInputFocus(
                ctx->dpy,
                keyboardFocusWin->xwayland( ).id,
                RevertToNone,
                CurrentTime );
             ...
             ctx->focus.inputFocusMode != inputFocus->inputFocusMode )
        {
            ...
            if ( window_wants_no_focus_when_mouse_hidden( inputFocus ) )
                ...
                window_wants_no_focus_when_mouse_hidden( inputFocus ) !=
                window_wants_no_focus_when_mouse_hidden(
                    ctx->focus.inputFocusWindow ) )
                ctx->focus.bResetToCenter = true;

            // cursor is likely not interactable anymore in its original
            // context, hide don't care if we change kb focus window due to that
            // happening when going from override -> focus and we don't want to
            // hide then as it's probably a dropdown.
            ctx->cursor->hide( );
        }
    ...
    {
        if ( ctx->list[ 0 ].xwayland( ).id !=
             ctx->focus.overrideWindowMouse->xwayland( ).id )
        {
            XRaiseWindow(
                ctx->dpy, ctx->focus.overrideWindowMouse->xwayland( ).id );
        }
    }
    else
    {
    ...
    if ( win_has_game_id( w ) )
    {
        if ( window_is_fullscreen( ctx->focus.focusWindow ) ||
             ctx->force_windows_fullscreen )
        {
            bool bIsSteam  = window_is_steam( ctx->focus.focusWindow );
            int  fs_width  = ctx->root_width;
            int  fs_height = ctx->root_height;
            if ( bIsSteam && g_nSteamMaxHeight &&
                 ctx->root_height > g_nSteamMaxHeight )
            {
                float steam_height_scale =
                    g_nSteamMaxHeight / ( float )ctx->root_height;
                fs_height = g_nSteamMaxHeight;
                fs_width  = ctx->root_width * steam_height_scale;
            }

            if ( w->GetGeometry( ).nWidth != fs_width ||
                 w->GetGeometry( ).nHeight != fs_height ||
                 globalScaleRatio != 1.0f )
                XResizeWindow(
                    ctx->dpy,
                    ctx->focus.focusWindow->xwayland( ).id,
        ...
        else
        {
            if ( ctx->focus.focusWindow->sizeHintsSpecified &&
                 ( ( unsigned )ctx->focus.focusWindow->GetGeometry( ).nWidth !=
                       ctx->focus.focusWindow->requestedWidth ||
                   ( unsigned )ctx->focus.focusWindow->GetGeometry( ).nHeight !=
                       ctx->focus.focusWindow->requestedHeight ) )
            {
                XResizeWindow(
                    ctx->dpy,
                    ctx->focus.focusWindow->xwayland( ).id,
...
const char *get_win_display_name( steamcompmgr_win_t *window )
{
    if ( window->type == steamcompmgr_win_type_t::XWAYLAND )
        return window->xwayland( )
            .ctx->xwayland_server->get_nested_display_name( );
...
static std::vector<steamcompmgr_win_t *> GetGlobalPossibleFocusWindows( )
{
    ...
    {
        gamescope_xwayland_server_t *server = NULL;
        for ( size_t i = 0; ( server = wlserver_get_xwayland_server( i ) );
              i++ )
        {
            std::vector<steamcompmgr_win_t *> vecLocalPossibleFocusWindows =
                server->ctx->GetPossibleFocusWindows( );
            vecPossibleFocusWindows.insert(
                vecPossibleFocusWindows.end( ),
                vecLocalPossibleFocusWindows.begin( ),
                vecLocalPossibleFocusWindows.end( ) );
        }
    }
...
static void steamcompmgr_xdg_determine_and_apply_focus(
    const std::vector<steamcompmgr_win_t *> &vecPossibleFocusWindows )
{
...
static void determine_and_apply_focus( global_focus_t *pFocus )
{
    gamescope_xwayland_server_t *root_server =
        wlserver_get_xwayland_server( 0 );
    xwayland_ctx_t *root_ctx           = root_server->ctx.get( );
    ...
    *pFocus                            = global_focus_t{};
    ...
    // Apply focus to the XWayland contexts.
    {
              ...
              i++ )
        {
            ...
            if ( server->ctx->focus.IsDirty( ) )
                server->ctx->DetermineAndApplyFocus(
                    vecLocalPossibleFocusWindows );
        }
    }

    // Apply focus to XDG contexts (TODO merge me with some nice abstraction of
    // "environments")
    {
        std::vector<steamcompmgr_win_t *> vecLocalPossibleFocusWindows =
            steamcompmgr_xdg_get_possible_focus_windows( );
        if ( g_steamcompmgr_xdg_focus.IsDirty( ) )
            steamcompmgr_xdg_determine_and_apply_focus(
                vecLocalPossibleFocusWindows );
    }
    ...
    for ( steamcompmgr_win_t *focusable_window : vecPossibleFocusWindows )
    {
        ...
        // Exclude windows that are useless (1x1), skip taskbar + pager or
        // override redirect windows from the reported focusable windows to
        // Steam.
        ...
        if ( unAppID != 0 )
        {
            ...
            {
                if ( focusable_appids[ j ] == unAppID ) { break; }
            }
    ...
    XChangeProperty(
        root_ctx->dpy,
        root_ctx->root,
        root_ctx->atoms.gamescopeFocusableAppsAtom,
        XA_CARDINAL,
        32,
        PropModeReplace,
        ( unsigned char * )focusable_appids.data( ),
        focusable_appids.size( ) );

    XChangeProperty(
        root_ctx->dpy,
        root_ctx->root,
        root_ctx->atoms.gamescopeFocusableWindowsAtom,
        XA_CARDINAL,
        32,
        PropModeReplace,
        ( unsigned char * )focusable_windows.data( ),
        focusable_windows.size( ) );

    gameFocused = pick_primary_focus_and_override(
        pFocus,
        root_ctx->focusControlWindow,
        vecPossibleFocusWindows,
        true,
        vecFocuscontrolAppIDs,
    ...
    {
        pFocus->externalOverlayWindow =
            g_steamcompmgr_xdg_focus.externalOverlayWindow;
    }

    bool bUseOverlay =
        gamescope::VirtualConnectorIsSingleOutput( ) ||
        gamescope::VirtualConnectorKeyIsSteam( pFocus->ulVirtualFocusKey );
    if ( !bUseOverlay )
    {
    ...
    if ( !gamescope::VirtualConnectorIsSingleOutput( ) )
    {
        ...
        focus_log.debugf(
            "Current focus VR overlays: keyboard 0x%lx | mouse 0x%lx",
            ulFocusedKeyboardOverlayVR,
            ulFocusedMouseOverlayVR );

        if ( ulFocusedKeyboardOverlayVR || ulFocusedMouseOverlayVR )
        {
            for ( steamcompmgr_win_t *queryWindow = root_ctx->list; queryWindow;
                  queryWindow = queryWindow->xwayland( ).next )
            {
                if ( queryWindow->oulTargetVROverlay &&
                     *queryWindow->oulTargetVROverlay ==
                ...
                {
                    focus_log.debugf(
                        "[WL GLOBAL] Overriding keyboard focus window with VR "
                        "forwarder overlay! Overlay: 0x%lx XWindow: 0x%x "
                        "Title: %s",
                        ulFocusedKeyboardOverlayVR,
                        queryWindow->id( ),
    ...
    {
        if ( pFocus->inputFocusWindow->type ==
             steamcompmgr_win_type_t::XWAYLAND )
            pFocus->cursor =
                pFocus->inputFocusWindow->xwayland( ).ctx->cursor.get( );
        else
        {
            // TODO XDG:
    ...
    if ( pFocus == GetCurrentFocus( ) )
    {
        ...
        if ( pFocus->keyboardFocusWindow !=
                 previousLocalFocus.keyboardFocusWindow ||
             pFocus->ulVirtualFocusKey != s_ulPreviousGlobalFocusKey )
        {
                 ...
                 win_surface( pFocus->keyboardFocusWindow ) != nullptr )
            {
                ...
                {
                    wlserver_keyboardfocus(
                        pFocus->keyboardFocusWindow->main_surface( ) );
                    focus_log.debugf(
                        "Setting keyboard focus to: %s (%x)",
                        pFocus->keyboardFocusWindow->debug_name( ),
                        pFocus->keyboardFocusWindow->id( ) );
                }
        ...
        if ( pFocus->inputFocusWindow )
        {
            ...
            // do wlserver_mousefocus and need to update m_x and m_y of the
            // cursor.
            if ( pFocus->inputFocusWindow->GetFocus( )->bResetToCorner )
            {
                wlserver_lock( );
                wlserver_mousewarp(
                    pFocus->inputFocusWindow->GetGeometry( ).nWidth / 2,
                    pFocus->inputFocusWindow->GetGeometry( ).nHeight / 2,
                    0,
                    true );
                wlserver_fake_mouse_pos(
                    pFocus->inputFocusWindow->GetGeometry( ).nWidth - 1,
                    pFocus->inputFocusWindow->GetGeometry( ).nHeight - 1 );
                wlserver_unlock( );
            }
             ...
             pFocus->ulVirtualFocusKey != s_ulPreviousGlobalFocusKey )
        {
            if ( win_surface( pFocus->inputFocusWindow ) != nullptr )
            {
                ...
                if ( win_surface( pFocus->overrideWindow ) != nullptr )
                    wlserver_notify_dropdown(
                        pFocus->overrideWindow->main_surface( ),
                        pFocus->overrideWindow->xwayland( ).a.x,
                        pFocus->overrideWindow->xwayland( ).a.y );

                if ( gamescope::VirtualConnectorInSteamPerAppState( ) &&
                     pFocus->inputFocusWindow )
                {
                    ...
                    {
                        xwayland_ctx_t *ctx =
                            pFocus->inputFocusWindow->xwayland( ).ctx;
                        bool bTouchPointerEmulation =
                            gamescope::VirtualConnectorKeyIsNonSteamWindow(
                                pFocus->ulVirtualFocusKey );

                        if ( ctx->bTouchPointerEmulation !=
                             bTouchPointerEmulation )
                        {
                            xwm_log.infof(
                                "Changing touch pointer emulation for display "
                                "%u to %s\n",
                                ctx->xwayland_server->get_index( ),
                                bTouchPointerEmulation ? "true" : "false" );

                            uint32_t uValue = bTouchPointerEmulation ? 1 : 0;
                            XChangeProperty(
                                ctx->dpy,
                                ctx->root,
                                ctx->atoms.steamosTouchPointerEmulation,
                                XA_CARDINAL,
                                32,
                                PropModeReplace,
                                ( unsigned char * )&uValue,
                                1 );
                            ctx->bTouchPointerEmulation =
                                bTouchPointerEmulation;
                        }
                    }
                }
                ...
                {
                    wlserver_mousefocus(
                        pFocus->inputFocusWindow->main_surface( ),
                        pFocus->cursor->x( ),
                        pFocus->cursor->y( ) );
                    focus_log.debugf(
                        "Setting mouse focus to: %s (%x)",
                        pFocus->inputFocusWindow->debug_name( ),
                        pFocus->inputFocusWindow->id( ) );
                }
            ...
            // xwayland ctx.
            // don't care if we change kb focus window due to that happening
            // when going from override -> focus and we don't want to hide then
            // as it's probably a dropdown.
            if ( pFocus->cursor && pFocus->inputFocusWindow !=
                                       previousLocalFocus.inputFocusWindow )
                pFocus->cursor->hide( );
        }
    ...
    pid_t newFocusedWindowPID =
        pFocus->focusWindow ? pFocus->focusWindow->pid : 0;
    if ( sdFocusWindow_pid != newFocusedWindowPID )
    {
        const char *unfocusedWindowCGroup =
            cgroupPathFromPID( sdFocusWindow_pid );
        const char *focusedWindowCGroup =
            cgroupPathFromPID( newFocusedWindowPID );
        bool sameUnit = unfocusedWindowCGroup && focusedWindowCGroup &&
                        !strcmp( unfocusedWindowCGroup, focusedWindowCGroup );
    ...
    {
        pFocus->pVirtualConnector->SetProperty(
            gamescope::ConnectorProperty::IsFileBrowser,
            pFocus->inputFocusWindow->bIsDolphin );
    }
    ...
    const char   *focused_display =
        root_ctx->xwayland_server->get_nested_display_name( );
    const char *focused_keyboard_display =
        root_ctx->xwayland_server->get_nested_display_name( );
    const char *focused_mouse_display =
        root_ctx->xwayland_server->get_nested_display_name( );
    ...
    if ( pFocus->inputFocusWindow )
    {
        focused_mouse_display =
            get_win_display_name( pFocus->inputFocusWindow );
    }

    if ( pFocus->keyboardFocusWindow )
    {
        focused_keyboard_display =
            get_win_display_name( pFocus->keyboardFocusWindow );
    }
    ...
    if ( pFocus == GetCurrentFocus( ) )
    {
        if ( steamMode )
        {
            XChangeProperty(
                root_ctx->dpy,
                root_ctx->root,
                root_ctx->atoms.gamescopeFocusedAppAtom,
                XA_CARDINAL,
                32,
                PropModeReplace,
                ( unsigned char * )&focusedAppId,
                focusedAppId != 0 ? 1 : 0 );

            XChangeProperty(
                root_ctx->dpy,
                root_ctx->root,
                root_ctx->atoms.gamescopeFocusedAppGfxAtom,
                XA_CARDINAL,
                32,
                PropModeReplace,
                ( unsigned char * )&focusedBaseAppId,
                focusedBaseAppId != 0 ? 1 : 0 );
        }

        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeFocusedWindowAtom,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&focusedWindow,
            focusedWindow != 0 ? 1 : 0 );

        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeFocusDisplay,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )focused_display,
            strlen( focused_display ) + 1 );

        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeMouseFocusDisplay,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )focused_mouse_display,
            strlen( focused_mouse_display ) + 1 );

        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeKeyboardFocusDisplay,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )focused_keyboard_display,
            strlen( focused_keyboard_display ) + 1 );
         ...
         previousLocalFocus.focusWindow != pFocus->focusWindow )
    {
        ...
        if ( g_FadeOutDuration != 0 && !g_bFirstFrame && bDoFade )
        {
            ...
            {
                // If we end up fading back to what we were going to fade to,
                // cancel the fade.
                if ( pFocus->fadeWindow != nullptr &&
                     pFocus->focusWindow == pFocus->fadeWindow )
                {
                    g_HeldCommits[ HELD_COMMIT_FADE ] = nullptr;
                    g_bPendingFade                    = false;
                    fadeOutStartTime                  = 0;
    ...
    if ( !cv_paint_debug_pause_base_plane )
    {
        ...
        if ( pFocus->focusWindow &&
             ...
             !pFocus->focusWindow->isSteamStreamingClient )
        {
            get_window_last_done_commit(
                pFocus->focusWindow, g_HeldCommits[ HELD_COMMIT_BASE ] );
        }
    }
    ...
    if ( pFocus->GetNestedHints( ) )
    {
        if ( pFocus->focusWindow )
        {
            ...
            {
                pFocus->GetNestedHints( )->SetTitle(
                    pFocus->focusWindow->title );
...
static void get_win_type( xwayland_ctx_t *ctx, steamcompmgr_win_t *w )
{
    w->is_dialog = !!w->xwayland( ).transientFor;
    ...
    if ( get_prop( ctx, w->xwayland( ).id, ctx->atoms.winTypeAtom, atoms ) )
    {
        ...
        {
            if ( atom == ctx->atoms.winDialogAtom ) { w->is_dialog = true; }
            if ( atom == ctx->atoms.winNormalAtom ) { w->is_dialog = false; }
        }
    }
}

static void get_size_hints( xwayland_ctx_t *ctx, steamcompmgr_win_t *w )
{
    XSizeHints hints;
    ...
    const bool bHasPositionAndGravityHints =
        ( hintsSpecified & ( PPosition | PWinGravity ) ) ==
        ( PPosition | PWinGravity );
    if ( bHasPositionAndGravityHints && hints.x >= 0 && hints.y >= 0 &&
    ...
    if ( hintsSpecified & ( PMaxSize | PMinSize ) && hints.max_width &&
         hints.max_height && hints.min_width && hints.min_height &&
    ...
    else
    {
        ...
        // Below block checks for a pattern that matches old SDL fullscreen
        // applications; SDL creates a fullscreen overrride-redirect window and
        // reparents the game window under it, centered. We get rid of the
        // modeswitch and also want that black border gone.
        if ( w->xwayland( ).a.override_redirect )
        {
            ...
            if ( nchildren == 1 )
            {
                ...
                // If we have a unique children that isn't override-reidrect
                // that is contained inside this fullscreen window, it's
                // probably it.
...
get_win_title( xwayland_ctx_t *ctx, steamcompmgr_win_t *w, Atom atom )
{
    ...
    bool is_utf8;
    if ( tp.encoding == ctx->atoms.utf8StringAtom ) { is_utf8 = true; }
    else if ( tp.encoding == XA_STRING ) { is_utf8 = false; }
    else
    {
        return;
    }
...
static void get_net_wm_state( xwayland_ctx_t *ctx, steamcompmgr_win_t *w )
{
    Atom           type;
    int            format;
    ...
    for ( size_t i = 0; i < nitems; i++ )
    {
        ...
        {
            xwm_log.debugf(
                "Unhandled initial NET_WM_STATE property: %s",
                XGetAtomName( ctx->dpy, props[ i ] ) );
        }
    }
...
static void get_win_icon( xwayland_ctx_t *ctx, steamcompmgr_win_t *w )
{
...
static void handle_desktop_window( steamcompmgr_win_t *w )
{
    ...
    if ( w->type != steamcompmgr_win_type_t::XWAYLAND ) return;

    if ( w->xwayland( ).a.override_redirect ||
         ( w->oulTargetVROverlay && !cv_vr_show_forwarded_overlays ) )
        return;

    if ( win_maybe_a_dropdown( w ) || win_is_useless( w ) ) return;

    xwayland_ctx_t *ctx = w->xwayland( ).ctx;

    if ( w->sizeHintsSpecified &&
         !( window_is_fullscreen( w ) || ctx->force_windows_fullscreen ) )
    {
        if ( ( unsigned )w->GetGeometry( ).nWidth != w->requestedWidth ||
...
static void map_win( xwayland_ctx_t *ctx, Window id, unsigned long sequence )
{
    ...
    XSelectInput(
        ctx->dpy,
        id,
        PropertyChangeMask | SubstructureNotifyMask | LeaveWindowMask |
            FocusChangeMask );
    ...
    w->opacity =
        get_prop( ctx, w->xwayland( ).id, ctx->atoms.opacityAtom, OPAQUE );

    w->isSteamLegacyBigPicture =
        get_prop( ctx, w->xwayland( ).id, ctx->atoms.steamAtom, 0 );
    ...
    w->inputFocusMode =
        get_prop( ctx, w->xwayland( ).id, ctx->atoms.steamInputFocusAtom, 0 );

    w->isSteamStreamingClient = get_prop(
        ctx, w->xwayland( ).id, ctx->atoms.steamStreamingClientAtom, 0 );
    w->isSteamStreamingClientVideo = get_prop(
        ctx, w->xwayland( ).id, ctx->atoms.steamStreamingClientVideoAtom, 0 );

    if ( steamMode == true )
    {
        uint32_t appID =
            get_prop( ctx, w->xwayland( ).id, ctx->atoms.gameAtom, 0 );
    ...
    if ( w->isSteamLegacyBigPicture ) w->appID = 769;

    w->isOverlay =
        get_prop( ctx, w->xwayland( ).id, ctx->atoms.overlayAtom, 0 );
    w->isExternalOverlay =
        get_prop( ctx, w->xwayland( ).id, ctx->atoms.externalOverlayAtom, 0 );

    // misyl: Disable appID for overlay types, as parts of the code don't expect
    // that focus-wise. Fixes mangoapp usage when nested, and not in SteamOS.
    if ( w->isExternalOverlay ) w->appID = 0;

    w->oulTargetVROverlay = get_u64_prop(
        ctx, w->xwayland( ).id, ctx->atoms.steamGamescopeVROverlayTarget );
    ...
    if ( wmHints != nullptr )
    {
        if ( wmHints->flags & ( InputHint | StateHint ) &&
             wmHints->input == true && wmHints->initial_state == NormalState )
        {
    ...
    if ( w == ctx->focus.inputFocusWindow ||
         w->xwayland( ).id == ctx->currentKeyboardFocusWindow )
    {
        XSetInputFocus(
            ctx->dpy, w->xwayland( ).id, RevertToNone, CurrentTime );
    }
...
static void finish_unmap_win( xwayland_ctx_t *ctx, steamcompmgr_win_t *w )
{
...
static void unmap_win( xwayland_ctx_t *ctx, Window id, bool fade )
{
    steamcompmgr_win_t *w = find_win( ctx, id );
    if ( !w ) return;
...
std::string get_name_from_pid( pid_t pid )
{
    ...
    if ( !proc_stat_file.is_open( ) || proc_stat_file.bad( ) ) return "";
    ...
    for ( uint32_t i = 0; i < proc_stat.length( ); i++ )
    {
        ...
        if ( proc_stat[ i ] == ')' ) { lastParens = &proc_stat[ i ]; }
    }

    if ( !lastParens ) return "";

    *lastParens = '\0';

    if ( procName ) procNameStr = procName;
...
uint32_t get_appid_from_pid( pid_t pid )
{
    ...
    while ( 1 )
    {
        ...
        if ( !proc_stat_file.is_open( ) || proc_stat_file.bad( ) ) break;
        ...
        *lastParens = '\0';
        ...
        {
            snprintf(
                filename, sizeof( filename ), "/proc/%i/cmdline", next_pid );
            ...
            for ( uint32_t j = 0; j < proc_cmdline.length( ); j++ )
            {
                     ...
                     j + 1 < proc_cmdline.length( ) )
                {
                    if ( strcmp( "SteamLaunch", &proc_cmdline[ j + 1 ] ) == 0 )
                    {
                        ...
                        sscanf(
                            &proc_cmdline[ j + 1 ], "AppId=%u", &unAppId ) ==
                            1 &&
                        unAppId != 0 )
                    {
                        if ( bSteamLaunch == true ) { unFoundAppId = unAppId; }
                    }
        ...
        if ( parent_pid == -1 || parent_pid == 0 ) { break; }
        else
        {
            next_pid = parent_pid;
        }
...
static pid_t get_win_pid( xwayland_ctx_t *ctx, Window id )
{
    ...
    if ( pid <= 0 ) xwm_log.errorf( "Failed to find PID for window 0x%lx", id );
    return pid;
}
...
add_win( xwayland_ctx_t *ctx, Window id, Window prev, unsigned long sequence )
{
    ...
    {
        new_win->xwayland( ).damage =
            XDamageCreate( ctx->dpy, id, XDamageReportRawRectangles );
    }
    new_win->opacity = OPAQUE;

    if ( useXRes == true ) { new_win->pid = get_win_pid( ctx, id ); }
    else
    {
        new_win->pid = -1;
    }
    ...
    if ( new_win->isExternalOverlay ) new_win->appID = 0;

    std::string pid_name = get_name_from_pid( new_win->pid );
    new_win->pid_name    = pid_name;
    if ( pid_name == "steam" ) new_win->bIsSteamPid = true;
    if ( pid_name == "steamwebhelper" ) new_win->bIsSteamWebHelperPid = true;
    if ( pid_name == "vrwebhelper" ) new_win->bIsVRWebHelperPid = true;
    if ( pid_name == "dolphin" ) new_win->bIsDolphin = true;
    ...
    wlserver_x11_surface_info_init(
        &new_win->xwayland( ).surface, ctx->xwayland_server, id );
...
restack_win( xwayland_ctx_t *ctx, steamcompmgr_win_t *w, Window new_above )
{
    ...
    if ( w->xwayland( ).next ) old_above = w->xwayland( ).next->xwayland( ).id;
    else
        old_above = None;
    if ( old_above != new_above )
    {
        ...
        *prev = w->xwayland( ).next;
        ...
        for ( prev = &ctx->list; *prev; prev = &( *prev )->xwayland( ).next )
        {
            if ( ( *prev )->xwayland( ).id == new_above ) break;
        }
...
static void configure_win( xwayland_ctx_t *ctx, XConfigureEvent *ce )
{
    ...
    if ( !w || w->xwayland( ).id != ce->window )
    {
        if ( ce->window == ctx->root )
        {
            ...
            XDeleteProperty(
                root_ctx->dpy,
                root_ctx->root,
                root_ctx->atoms.gamescopeXWaylandModeControl );
            XFlush( root_ctx->dpy );
        }
        return;
    }
...
static void circulate_win( xwayland_ctx_t *ctx, XCirculateEvent *ce )
{
    steamcompmgr_win_t *w = find_win( ctx, ce->window );
    Window              new_above;

    if ( !w || w->xwayland( ).id != ce->window ) return;

    if ( ce->place == PlaceOnTop ) new_above = ctx->list->xwayland( ).id;
    else
        new_above = None;
...
static void configure_request(
    xwayland_ctx_t *ctx, XConfigureRequestEvent *configureRequest )
{
    XWindowChanges changes = { .x            = configureRequest->x,
...
static void circulate_request(
    xwayland_ctx_t *ctx, XCirculateRequestEvent *circulateRequest )
{
    XCirculateSubwindows(
        ctx->dpy, circulateRequest->window, circulateRequest->place );
}
...
static void finish_destroy_win( xwayland_ctx_t *ctx, Window id, bool gone )
{
    ...
    for ( prev = &ctx->list; ( w = *prev ); prev = &w->xwayland( ).next )
        if ( w->xwayland( ).id == id )
        {
            if ( gone ) finish_unmap_win( ctx, w );
...
static void destroy_win( xwayland_ctx_t *ctx, Window id, bool gone, bool fade )
{
    // Context
...
static void damage_win( xwayland_ctx_t *ctx, XDamageNotifyEvent *de )
{
    ...
    if ( w->IsAnyOverlay( ) && !w->opacity ) return;
    ...
    if ( win_is_useless( w ) || w->IsAnyOverlay( ) ||
         ( w->oulTargetVROverlay && !cv_vr_show_forwarded_overlays ) ||
         w->isSysTrayIcon || w->xwayland( ).a.map_state != IsViewable )
    {
    ...
    gpuvis_trace_printf(
        "damage_win win %lx appID %u", w->xwayland( ).id, w->appID );
}

static void handle_wl_surface_id(
    xwayland_ctx_t *ctx, steamcompmgr_win_t *w, uint32_t surfaceID )
{
...
static void update_net_wm_state( uint32_t action, bool *value )
{
    switch ( action )
    {
...
static void handle_net_wm_state(
    xwayland_ctx_t *ctx, steamcompmgr_win_t *w, XClientMessageEvent *ev )
{
    ...
    for ( size_t i = 0; i < 2; i++ )
    {
        ...
        {
            xwm_log.debugf(
                "Unhandled NET_WM_STATE property change: %s",
                XGetAtomName( ctx->dpy, props[ i ] ) );
        }
    }
}
...
static void handle_wm_change_state(
    xwayland_ctx_t *ctx, steamcompmgr_win_t *w, XClientMessageEvent *ev )
{
    ...
    {
        xwm_log.debugf(
            "Faking WM_CHANGE_STATE to ICONIC for window 0x%lx",
            w->xwayland( ).id );
    ...
    {
        xwm_log.debugf(
            "Unhandled WM_CHANGE_STATE to %ld for window 0x%lx",
            state,
            w->xwayland( ).id );
    }
}
...
{
    if ( ev->window == ctx->ourWindow &&
         ev->message_type == ctx->atoms.netSystemTrayOpcodeAtom )
    {
    ...
    if ( w )
    {
        ...
        {
            xwm_log.debugf(
                "Unhandled client message: %s",
                XGetAtomName( ctx->dpy, ev->message_type ) );
        }
    }
}

static void x11_set_selection_owner(
    xwayland_ctx_t    *ctx,
    std::string        contents,
    GamescopeSelection eSelectionTarget )
{
    Atom target;
...
void gamescope_set_selection(
    std::string contents, GamescopeSelection eSelection )
{
    if ( eSelection == GAMESCOPE_SELECTION_CLIPBOARD ) { clipboard = contents; }
    ...
    for ( int i = 0; ( server = wlserver_get_xwayland_server( i ) ); i++ )
    {
        ...
        if ( ctx ) x11_set_selection_owner( ctx, contents, eSelection );
    }
}
...
void gamescope_set_reshade_effect( std::string effect_path )
{
    ...
    set_string_prop(
        server->ctx.get( ),
        server->ctx->atoms.gamescopeReshadeEffect,
        effect_path );
}
...
{
    std::string *selection = ev->selection == ctx->atoms.primarySelection
                                 ? &primarySelection
                                 : &clipboard;
    ...
    if ( ev->requestor == ctx->ourWindow ) { return; }
    ...
    if ( ev->target == ctx->atoms.targets )
    {
        ...
        XChangeProperty(
            ...
            sizeof( targetList ) / sizeof( targetList[ 0 ] ) );
        response.xselection.property = ev->property;
        response.xselection.target   = ev->target;
    }
    else if (
        !strcmp( targetString, "text/plain;charset=utf-8" ) ||
    ...
    {
        xwm_log.debugf(
            "Unsupported clipboard type: %s.  Ignoring", targetString );
    }
...
static void handle_selection_notify( xwayland_ctx_t *ctx, XSelectionEvent *ev )
{
    Atom           actual_type;
    int            actual_format;
    unsigned long  nitems;
    ...
    if ( actual_type == ctx->atoms.utf8StringAtom && actual_format == 8 )
    {
        ...
        if ( data )
        {
            ...
            if ( ev->selection == ctx->atoms.clipboard )
            {
                ...
                {
                    hints->SetSelection(
                        szContents, GAMESCOPE_SELECTION_CLIPBOARD );
                }
                else
                {
                    gamescope_set_selection(
                        contents, GAMESCOPE_SELECTION_CLIPBOARD );
                }
            }
            else if ( ev->selection == ctx->atoms.primarySelection )
            {
                ...
                {
                    hints->SetSelection(
                        szContents, GAMESCOPE_SELECTION_PRIMARY );
                }
                else
                {
                    gamescope_set_selection(
                        contents, GAMESCOPE_SELECTION_PRIMARY );
                }
            }
            else
            {
                xwm_log.errorf(
                    "Selection '%s' not supported.  Ignoring",
                    XGetAtomName( ctx->dpy, ev->selection ) );
            }
        }
    }
}

template<typename T, typename J> T bit_cast( const J &src )
{
    T dst;
...
static void steamcompmgr_flush_frame_done( steamcompmgr_win_t *w )
{
    wlr_surface *current_surface = w->current_surface( );
    if ( current_surface && w->unlockedForFrameCallback &&
         w->receivedDoneCommit )
    {
...
static bool steamcompmgr_should_vblank_window(
    bool                bShouldLimitFPS,
    uint64_t            vblank_idx,
    steamcompmgr_win_t *w   = nullptr,
    uint64_t            now = 0 )
{
    ...
    int nRefreshHz = gamescope::ConvertmHzToHz(
        ...
        if ( g_nSteamCompMgrTargetFPS && bShouldLimitFPS && w && !bCloseEnough )
        {
            ...
            if ( now + k_ulVRRScheduleFudge < schedule )
            {
                ...
                if ( !s_oLowestFPSLimitScheduleVRR )
                    ...
                    s_oLowestFPSLimitScheduleVRR =
                        std::min( *s_oLowestFPSLimitScheduleVRR, schedule );
            }
        }
    }
    else
    {
        if ( g_nSteamCompMgrTargetFPS && bShouldLimitFPS &&
             nRefreshHz > nTargetFPS )
        {
            int nVblankDivisor = nRefreshHz / nTargetFPS;

            if ( vblank_idx % nVblankDivisor != 0 ) bSendCallback = false;
        }
    }
...
static bool steamcompmgr_should_vblank_window(
    steamcompmgr_win_t *w, uint64_t vblank_idx, uint64_t now )
{
    return steamcompmgr_should_vblank_window(
        steamcompmgr_window_should_limit_fps( w ), vblank_idx, w, now );
}

static void steamcompmgr_latch_frame_done(
    steamcompmgr_win_t *w, uint64_t vblank_idx, uint64_t now )
{
...
static void handle_property_notify( xwayland_ctx_t *ctx, XPropertyEvent *ev )
{
    ...
    if ( ev->atom == ctx->atoms.opacityAtom )
    {
        ...
        if ( w != nullptr )
        {
            unsigned int newOpacity = get_prop(
                ctx, w->xwayland( ).id, ctx->atoms.opacityAtom, OPAQUE );

            if ( newOpacity != w->opacity )
            {
                w->opacity = newOpacity;

                if ( gameFocused && ( w == ctx->focus.overlayWindow ||
                                      w == ctx->focus.notificationWindow ) )
                {
                    hasRepaintNonBasePlane = true;
                }
            ...
            for ( w = ctx->list; w; w = w->xwayland( ).next )
            {
                if ( w->isOverlay )
                {
                    if ( w->GetGeometry( ).nWidth > 1200 &&
                         w->opacity >= maxOpacity )
                    {
                        ctx->focus.overlayWindow = w;
                        maxOpacity               = w->opacity;
                    }
    ...
    if ( ev->atom == ctx->atoms.steamInputFocusAtom )
    {
        ...
        if ( w )
        {
            w->inputFocusMode = get_prop(
                ctx, w->xwayland( ).id, ctx->atoms.steamInputFocusAtom, 0 );
            MakeFocusDirty( );
        }
    }
    if ( ev->atom == ctx->atoms.steamTouchClickModeAtom )
    {
        gamescope::cv_touch_click_mode = ( gamescope::TouchClickMode )get_prop(
            ctx, ctx->root, ctx->atoms.steamTouchClickModeAtom, 0u );
    }
    if ( ev->atom == ctx->atoms.steamStreamingClientAtom )
    {
        ...
        if ( w )
        {
            w->isSteamStreamingClient = get_prop(
                ctx,
                w->xwayland( ).id,
                ctx->atoms.steamStreamingClientAtom,
                0 );
            MakeFocusDirty( );
        }
    }
    if ( ev->atom == ctx->atoms.steamStreamingClientVideoAtom )
    {
        ...
        if ( w )
        {
            w->isSteamStreamingClientVideo = get_prop(
                ctx,
                w->xwayland( ).id,
                ctx->atoms.steamStreamingClientVideoAtom,
                0 );
            MakeFocusDirty( );
        }
    }
    if ( ev->atom == ctx->atoms.steamGamescopeVROverlayTarget )
    {
        ...
        if ( w )
        {
            w->oulTargetVROverlay = get_u64_prop(
                ctx,
                w->xwayland( ).id,
                ctx->atoms.steamGamescopeVROverlayTarget );
            w->pForwarderPlane = nullptr;
            MakeFocusDirty( );
            hasRepaint                   = true;
            g_bUpdateForwardedVROverlays = true;
    ...
    if ( ev->atom == ctx->atoms.gamescopeCtrlAppIDAtom )
    {
        get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeCtrlAppIDAtom,
            vecFocuscontrolAppIDs );
        MakeFocusDirty( );
    }
    if ( ev->atom == ctx->atoms.gamescopeCtrlWindowAtom )
    {
        ctx->focusControlWindow = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeCtrlWindowAtom, None );
        MakeFocusDirty( );
    }
                ...
                gamescope::GamescopeScreenshotInfo{
                    ...
                    .eScreenshotType =
                        ( gamescope_control_screenshot_type )get_prop(
                            ctx,
                            ctx->root,
                            ctx->atoms.gamescopeScreenShotAtom,
                            None ),
                    .uScreenshotFlags      = 0,
                ...
                gamescope::GamescopeScreenshotInfo{
                    ...
                    .eScreenshotType =
                        ( gamescope_control_screenshot_type )get_prop(
                            ctx,
                            ctx->root,
                            ctx->atoms.gamescopeDebugScreenShotAtom,
                            None ),
                    .uScreenshotFlags      = 0,
                    .bX11PropertyRequested = true,
                } );
    ...
    if ( ev->atom == ctx->atoms.externalOverlayAtom )
    {
        ...
        if ( w )
        {
            w->isExternalOverlay = get_prop(
                ctx, w->xwayland( ).id, ctx->atoms.externalOverlayAtom, 0 );
            if ( w->isExternalOverlay ) w->appID = 0;
            MakeFocusDirty( );
        }
    }
    ...
    {
        gamesRunningCount =
            get_prop( ctx, ctx->root, ctx->atoms.gamesRunningAtom, 0 );
    ...
    {
        overscanScaleRatio =
            get_prop( ctx, ctx->root, ctx->atoms.screenScaleAtom, 0xFFFFFFFF ) /
            ( double )0xFFFFFFFF;
        ...
        {
            global_focus_t *pFocus = &iter.second;
            if ( pFocus->focusWindow ) { hasRepaint = true; }
        }
    ...
    {
        zoomScaleRatio =
            get_prop( ctx, ctx->root, ctx->atoms.screenZoomAtom, 0xFFFF ) /
            ( double )0xFFFF;
    ...
    {
        int nScalingMode =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeScalingFilter, 0 );
        switch ( nScalingMode )
        {
            default:
            case 0:
    ...
    if ( ev->atom == ctx->atoms.gamescopeFSRSharpness ||
         ev->atom == ctx->atoms.gamescopeSharpness )
    {
        g_upscaleFilterSharpness =
            ( int )clamp( get_prop( ctx, ctx->root, ev->atom, 2 ), 0u, 20u );
        if ( g_upscaleFilter == GamescopeUpscaleFilter::FSR ||
             g_upscaleFilter == GamescopeUpscaleFilter::NIS )
            hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeXWaylandModeControl )
    {
        ...
        bool                  hasModeCtrl = get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeXWaylandModeControl,
            xwayland_mode_ctl );
        if ( hasModeCtrl && xwayland_mode_ctl.size( ) == 4 )
        {
            ...
            gamescope_xwayland_server_t *server =
                wlserver_get_xwayland_server( server_idx );
            if ( server )
            {
                bool root_size_identical = server->ctx->root_width == width &&
                                           server->ctx->root_height == height;

                wlserver_lock( );
                wlserver_set_xwayland_server_mode(
                    server_idx, width, height, g_nOutputRefresh );
                wlserver_unlock( );
    ...
    for ( int i = 0; i < gamescope::GAMESCOPE_SCREEN_TYPE_COUNT; i++ )
    {
        ...
        {
            g_nDynamicRefreshRate[ i ] = get_prop(
                ctx, ctx->root, ctx->atoms.gamescopeDynamicRefresh[ i ], 0 );
        }
    }
    if ( ev->atom == ctx->atoms.gamescopeLowLatency )
    {
        g_bLowLatency =
            !!get_prop( ctx, ctx->root, ctx->atoms.gamescopeLowLatency, 0 );
    }
    if ( ev->atom == ctx->atoms.gamescopeBlurMode )
    {
        BlurMode newBlur = ( BlurMode )get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeBlurMode, 0 );
    ...
    {
        unsigned int pixel =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeBlurRadius, 0 );
        g_BlurRadius =
            ( int )clamp( ( pixel / 2 ) + 1, 1u, kMaxBlurRadius - 1 );
        if ( g_BlurMode ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeBlurFadeDuration )
    {
        g_BlurFadeDuration =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeBlurFadeDuration, 0 );
    }
    if ( ev->atom == ctx->atoms.gamescopeCompositeForce )
    {
        cv_composite_force =
            !!get_prop( ctx, ctx->root, ctx->atoms.gamescopeCompositeForce, 0 );
        hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeCompositeDebug )
    {
        cv_composite_debug =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeCompositeDebug, 0 );
    ...
    {
        cv_tearing_enabled =
            !!get_prop( ctx, ctx->root, ctx->atoms.gamescopeAllowTearing, 0 );
    }
    if ( ev->atom == ctx->atoms.gamescopeSteamMaxHeight )
    {
        g_nSteamMaxHeight =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeSteamMaxHeight, 0 );
        MakeFocusDirty( );
    }
    if ( ev->atom == ctx->atoms.gamescopeVRREnabled )
    {
        bool enabled =
            !!get_prop( ctx, ctx->root, ctx->atoms.gamescopeVRREnabled, 0 );
        cv_adaptive_sync = enabled;
    }
    if ( ev->atom == ctx->atoms.gamescopeDisplayForceInternal )
    {
        g_bForceInternal = !!get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeDisplayForceInternal, 0 );
    ...
    if ( ev->atom == ctx->atoms.gamescopeDisplayModeNudge )
    {
        ...
        XDeleteProperty(
            ctx->dpy, ctx->root, ctx->atoms.gamescopeDisplayModeNudge );
    }
    if ( ev->atom == ctx->atoms.gamescopeNewScalingFilter )
    {
        GamescopeUpscaleFilter nScalingFilter =
            ( GamescopeUpscaleFilter )get_prop(
                ctx, ctx->root, ctx->atoms.gamescopeNewScalingFilter, 0 );
    ...
    {
        GamescopeUpscaleScaler nScalingScaler =
            ( GamescopeUpscaleScaler )get_prop(
                ctx, ctx->root, ctx->atoms.gamescopeNewScalingScaler, 0 );
    ...
    {
        cv_hdr_enabled = !!get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeDisplayHDREnabled, 0 );
        hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeDebugForceHDR10Output )
    {
        g_bForceHDR10OutputDebug = !!get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeDebugForceHDR10Output, 0 );
        hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeDebugForceHDRSupport )
    {
        g_bForceHDRSupportDebug = !!get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeDebugForceHDRSupport, 0 );
    ...
    {
        uint32_t heatmap =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeDebugHDRHeatmap, 0 );
        ...
        if ( heatmap != 0 ) cv_composite_debug |= CompositeDebugFlag::Heatmap;
        if ( heatmap == 2 )
            cv_composite_debug |= CompositeDebugFlag::Heatmap_MSWCG;
        if ( heatmap == 3 )
            cv_composite_debug |= CompositeDebugFlag::Heatmap_Hard;
    ...
    {
        g_ColorMgmt.pending.hdrTonemapOperator = ( ETonemapOperator )get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeHDRTonemapOperator, 0 );
        hasRepaint = true;
    }
    ...
    if ( ev->atom == ctx->atoms.gamescopeHDRTonemapDisplayMetadata )
    {
        ...
        if ( get_prop(
                 ctx,
                 ctx->root,
                 ctx->atoms.gamescopeHDRTonemapDisplayMetadata,
                 user_vec ) &&
             user_vec.size( ) >= 2 )
        {
            g_ColorMgmt.pending.hdrTonemapDisplayMetadata.flBlackPointNits =
                bit_cast<float>( user_vec[ 0 ] );
            g_ColorMgmt.pending.hdrTonemapDisplayMetadata.flWhitePointNits =
                bit_cast<float>( user_vec[ 1 ] );
        }
        else
        {
    ...
    if ( ev->atom == ctx->atoms.gamescopeHDRTonemapSourceMetadata )
    {
        ...
        if ( get_prop(
                 ctx,
                 ctx->root,
                 ctx->atoms.gamescopeHDRTonemapSourceMetadata,
                 user_vec ) &&
             user_vec.size( ) >= 2 )
        {
            g_ColorMgmt.pending.hdrTonemapSourceMetadata.flBlackPointNits =
                bit_cast<float>( user_vec[ 0 ] );
            g_ColorMgmt.pending.hdrTonemapSourceMetadata.flWhitePointNits =
                bit_cast<float>( user_vec[ 1 ] );
        }
        else
        {
    ...
    {
        uint32_t val = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeSDROnHDRContentBrightness, 0 );
    ...
    {
        g_bHDRItmEnable =
            !!get_prop( ctx, ctx->root, ctx->atoms.gamescopeHDRItmEnable, 0 );
        hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeHDRItmSDRNits )
    {
        g_flHDRItmSdrNits =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeHDRItmSDRNits, 0 );
        if ( g_flHDRItmSdrNits < 1.f ) g_flHDRItmSdrNits = 100.f;
    ...
    if ( ev->atom == ctx->atoms.gamescopeHDRItmTargetNits )
    {
        g_flHDRItmTargetNits =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeHDRItmTargetNits, 0 );
        if ( g_flHDRItmTargetNits < 1.f ) g_flHDRItmTargetNits = 1000.f;
    ...
    {
        std::string path =
            get_string_prop( ctx, ctx->root, ctx->atoms.gamescopeColorLookPQ );
        if ( set_color_look_pq( path.c_str( ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorLookG22 )
    {
        std::string path =
            get_string_prop( ctx, ctx->root, ctx->atoms.gamescopeColorLookG22 );
        if ( set_color_look_g22( path.c_str( ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorOutputVirtualWhite )
    {
        ...
        if ( get_prop(
                 ctx,
                 ctx->root,
                 ctx->atoms.gamescopeColorOutputVirtualWhite,
                 user_vec ) &&
             user_vec.size( ) >= 2 )
        {
            g_ColorMgmt.pending.outputVirtualWhite.x =
                santitize_float( bit_cast<float>( user_vec[ 0 ] ) );
            g_ColorMgmt.pending.outputVirtualWhite.y =
                santitize_float( bit_cast<float>( user_vec[ 1 ] ) );
        }
        else
        {
    ...
    {
        uint32_t val =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeHDRInputGain, 0 );
        if ( set_hdr_input_gain( bit_cast<float>( val ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeSDRInputGain )
    {
        uint32_t val =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeSDRInputGain, 0 );
        if ( set_sdr_input_gain( bit_cast<float>( val ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeForceWindowsFullscreen )
    {
        ctx->force_windows_fullscreen = !!get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeForceWindowsFullscreen, 0 );
        MakeFocusDirty( );
    }
    if ( ev->atom == ctx->atoms.gamescopeColorLut3DOverride )
    {
        std::string path = get_string_prop(
            ctx, ctx->root, ctx->atoms.gamescopeColorLut3DOverride );
        if ( set_color_3dlut_override( path.c_str( ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorShaperLutOverride )
    {
        std::string path = get_string_prop(
            ctx, ctx->root, ctx->atoms.gamescopeColorShaperLutOverride );
        if ( set_color_shaperlut_override( path.c_str( ) ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorSDRGamutWideness )
    {
        uint32_t val = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeColorSDRGamutWideness, 0 );
    ...
    {
        std::vector<uint32_t> user_vec;
        bool                  bHasVec = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeColorNightMode, user_vec );
        ...
        if ( set_color_nightmode( nightmode ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorManagementDisable )
    {
        uint32_t val = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeColorManagementDisable, 0 );
        if ( set_color_mgmt_enabled( !val ) ) hasRepaint = true;
    }
    if ( ev->atom == ctx->atoms.gamescopeColorSliderInUse )
    {
        uint32_t val =
            get_prop( ctx, ctx->root, ctx->atoms.gamescopeColorSliderInUse, 0 );
    ...
    if ( ev->atom == ctx->atoms.gamescopeColorChromaticAdaptationMode )
    {
        uint32_t val = get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeColorChromaticAdaptationMode,
            0 );
        g_ColorMgmt.pending.chromaticAdaptationMode =
            ( EChromaticAdaptationMethod )val;
    }
    // TODO: Hook up gamescopeColorMuraCorrectionImage for external.
    if ( ev->atom == ctx->atoms.gamescopeColorMuraCorrectionImage
                         [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] )
    {
        std::string path = get_string_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeColorMuraCorrectionImage
                [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] );
        if ( set_mura_overlay( path.c_str( ) ) ) hasRepaint = true;
    }
    // TODO: Hook up gamescopeColorMuraScale for external.
    if ( ev->atom == ctx->atoms.gamescopeColorMuraScale
                         [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] )
    {
        uint32_t val = get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeColorMuraScale
                [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ],
            0 );
        float new_scale = bit_cast<float>( val );
        if ( set_mura_scale( new_scale ) ) hasRepaint = true;
    }
    // TODO: Hook up gamescopeColorMuraCorrectionDisabled for external.
    if ( ev->atom == ctx->atoms.gamescopeColorMuraCorrectionDisabled
                         [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] )
    {
        bool disabled = !!get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeColorMuraCorrectionDisabled
                [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ],
            0 );
    ...
    {
        uint32_t identifier = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeCreateXWaylandServer, 0 );
        if ( identifier )
        {
            wlserver_lock( );
            uint32_t server_id =
            ...
            assert( server_id != ~0u );
            gamescope_xwayland_server_t *server =
                wlserver_get_xwayland_server( server_id );
            ...
            snprintf(
                ...
                identifier,
                server_id,
                server->get_nested_display_name( ) );
            ...
            XSetTextProperty(
                ...
                ctx->root,
                &text_property,
                ctx->atoms.gamescopeCreateXWaylandServerFeedback );
            wlserver_unlock( );
        }
    }
    if ( ev->atom == ctx->atoms.gamescopeDestroyXWaylandServer )
    {
        uint32_t server_id = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeDestroyXWaylandServer, 0 );
        ...
        if ( server )
        {
            for ( auto &iter : g_VirtualConnectorFocuses )
            {
                global_focus_t *pFocus = &iter.second;
                if ( pFocus->focusWindow &&
                     pFocus->focusWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->focusWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->focusWindow = nullptr;

                if ( pFocus->inputFocusWindow &&
                     pFocus->inputFocusWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->inputFocusWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->inputFocusWindow = nullptr;
                ...
                if ( pFocus->overlayWindow &&
                     pFocus->overlayWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->overlayWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->overlayWindow = nullptr;

                if ( pFocus->externalOverlayWindow &&
                     pFocus->externalOverlayWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->externalOverlayWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->externalOverlayWindow = nullptr;

                if ( pFocus->notificationWindow &&
                     pFocus->notificationWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->notificationWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->notificationWindow = nullptr;

                if ( pFocus->overrideWindow &&
                     pFocus->overrideWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->overrideWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->overrideWindow = nullptr;

                if ( pFocus->keyboardFocusWindow &&
                     pFocus->keyboardFocusWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
                     pFocus->keyboardFocusWindow->xwayland( ).ctx ==
                         server->ctx.get( ) )
                    pFocus->keyboardFocusWindow = nullptr;

                if ( pFocus->fadeWindow &&
                     pFocus->fadeWindow->type ==
                         steamcompmgr_win_type_t::XWAYLAND &&
    ...
    {
        uint32_t technique_idx = get_prop(
            ctx, ctx->root, ctx->atoms.gamescopeReshadeTechniqueIdx, 0 );
        g_reshade_technique_idx = technique_idx;
    }
    if ( ev->atom == ctx->atoms.gamescopeReshadeEffect )
    {
        std::string path = get_string_prop(
            ctx, ctx->root, ctx->atoms.gamescopeReshadeEffect );
        g_reshade_effect = path;
    }
    if ( ev->atom ==
         ctx->atoms.gamescopeDisplayDynamicRefreshBasedOnGamePresence )
    {
        g_bChangeDynamicRefreshBasedOnGameOpenRatherThanActive = !!get_prop(
            ctx,
            ctx->root,
            ctx->atoms.gamescopeDisplayDynamicRefreshBasedOnGamePresence,
            0 );
    ...
    if ( ev->atom == ctx->atoms.wineHwndStyle )
    {
        ...
        if ( w )
        {
            w->hasHwndStyle = true;
            w->hwndStyle =
                get_prop( ctx, w->xwayland( ).id, ctx->atoms.wineHwndStyle, 0 );
            MakeFocusDirty( );
        }
    }
    if ( ev->atom == ctx->atoms.wineHwndStyleEx )
    {
        ...
        if ( w )
        {
            ...
            w->hwndStyleEx    = get_prop(
                ctx, w->xwayland( ).id, ctx->atoms.wineHwndStyleEx, 0 );
            MakeFocusDirty( );
        }
    }
}
...
static void steamcompmgr_exit( void )
{
    ...
    for ( auto &lut : g_ColorMgmtLuts )
        lut.shutdown( );
    for ( auto &lut : g_ColorMgmtLutsOverride )
        lut.shutdown( );
    for ( auto &lut : g_ScreenshotColorMgmtLuts )
        lut.shutdown( );
    for ( auto &lut : g_ScreenshotColorMgmtLutsHDR )
        lut.shutdown( );
...
[[noreturn]] static int handle_io_error( Display *dpy )
{
    xwm_log.errorf( "X11 I/O error! This is fatal. Aborting..." );
    abort( );
}

static bool register_cm( xwayland_ctx_t *ctx )
{
    Window      w;
    Atom        a;
    ...
    if ( w != None )
    {
        ...
        if ( XmbTextPropertyToTextList( ctx->dpy, &tp, &strs, &count ) ==
             Success )
        {
            xwm_log.errorf(
    ...
    w = XCreateSimpleWindow(
        ctx->dpy, RootWindow( ctx->dpy, ctx->scr ), 0, 0, 1, 1, 0, None, None );
    ...
    Atom atomWmCheck =
        XInternAtom( ctx->dpy, "_NET_SUPPORTING_WM_CHECK", false );
    XChangeProperty(
        ctx->dpy,
        ctx->root,
        atomWmCheck,
    ...
    XChangeProperty(
        ctx->dpy,
        ctx->root,
        XInternAtom( ctx->dpy, "_NET_SUPPORTED", false ),
        XA_ATOM,
        32,
        PropModeAppend,
...
static void register_systray( xwayland_ctx_t *ctx )
{
    static char net_system_tray_name[] = "_NET_SYSTEM_TRAY_Sxx";

    snprintf(
        net_system_tray_name,
        sizeof( net_system_tray_name ),
    ...
    uint64_t            earliestLatchTime )
{
    ...
    for ( j = 0; j < w->commit_queue.size( ); j++ )
    {
        ...
        {
            gpuvis_trace_printf(
                "commit %lu done", w->commit_queue[ j ]->commitID );
            w->commit_queue[ j ]->done                  = true;
            w->commit_queue[ j ]->earliest_present_time = earliestPresentTime;
            w->commit_queue[ j ]->present_margin =
                earliestPresentTime - earliestLatchTime;
            bFoundWindow = true;

            // Window just got a new available commit, determine if that's worth
            // a repaint
            ...
            for ( auto &iter : g_VirtualConnectorFocuses )
            {
                ...
                if ( w == pFocus->notificationWindow &&
                     w->opacity != TRANSLUCENT )
                {
                    hasRepaintNonBasePlane = true;
                }

                // matt: the performance overlay in Steam will interfere with
                // VRR if we let this repaint. This has been broken since the
                // logic for external overlay repaints was moved out of
                // outdatedInteractiveFocus. It can cause displays to jump
                // between the focused app's refresh rate and the maximum panel
                // refresh rate when presenting any type of overlay, creating
                // noticeable VRR flicker in the process.
                // TODO: fix this properly for all overlays, including Steam
                // notifications and QAM HACK: If VRR is active, prevent
                // external overlays, i.e. mangoapp, from repainting the base
                // plane
                if ( ( w == pFocus->externalOverlayWindow &&
                       w->opacity != TRANSLUCENT ) &&
                     ( GetBackend( )->GetCurrentConnector( ) &&
                       !GetBackend( )
                            ->GetCurrentConnector( )
                ...
                if ( w->isSteamStreamingClientVideo && pFocus->focusWindow &&
                     pFocus->focusWindow->isSteamStreamingClient )
                {
                    if ( !cv_paint_debug_pause_base_plane )
                        g_HeldCommits[ HELD_COMMIT_BASE ] =
    ...
    {
        if ( j > 0 )
            w->commit_queue.erase(
                w->commit_queue.begin( ), w->commit_queue.begin( ) + j );
...
void handle_done_commits_xwayland(
    xwayland_ctx_t *ctx, bool vblank, uint64_t vblank_idx )
{
    std::lock_guard<std::mutex> lock( ctx->doneCommits.listCommitsDoneLock );

    uint64_t next_refresh_time =
        g_SteamCompMgrVBlankTime.schedule.ulTargetVBlank;

    // commits that were not ready to be presented based on their display
    // timing.
    ...
    for ( auto &entry : ctx->doneCommits.listCommitsDone )
    {
        ...
        if ( GetBackend( )->GetCurrentConnector( ) &&
             GetBackend( )->GetCurrentConnector( )->IsVRRActive( ) )
        {
            for ( steamcompmgr_win_t *w = ctx->list; w;
                  w                     = w->xwayland( ).next )
            {
                if ( w->seq != entry.winSeq ) continue;

                entry_vblank =
                    entry_vblank && steamcompmgr_should_vblank_window(
                                        true, vblank_idx, w, now );
            }
        }
        else
        {
            entry_vblank = entry_vblank && steamcompmgr_should_vblank_window(
                                               true, vblank_idx );
        }

        if ( entry.fifo &&
             ( !entry_vblank || fifo_win_seqs.count( entry.winSeq ) > 0 ) )
        {
        ...
        {
            if ( w->seq != entry.winSeq ) continue;
            if ( handle_done_commit(
                     w,
                     ctx,
                     entry.commitID,
                     entry.earliestPresentTime,
                     entry.earliestLatchTime ) )
            {
                if ( entry.fifo ) fifo_win_seqs.insert( entry.winSeq );
                break;
            }
        }
    }
...
void handle_done_commits_xdg( bool vblank, uint64_t vblank_idx )
{
    std::lock_guard<std::mutex> lock(
        g_steamcompmgr_xdg_done_commits.listCommitsDoneLock );
    ...
    {
        if ( entry.fifo &&
             ( !vblank || fifo_win_seqs.count( entry.winSeq ) > 0 ) )
        {
            commits_before_their_time.push_back( entry );
            continue;
        }
        ...
        {
            if ( xdg_win->seq != entry.winSeq ) continue;
            if ( handle_done_commit(
                     xdg_win.get( ),
                     nullptr,
                     entry.commitID,
    ...
    g_steamcompmgr_xdg_done_commits.listCommitsDone.swap(
        commits_before_their_time );
}

gamescope::ConVar<bool> cv_mangoapp_use_output_timing{
    "mangoapp_use_output_timing", true
};
...
void handle_presented_for_window( steamcompmgr_win_t *w )
{
    ...
    if ( lastCommit )
    {
        if ( !cv_mangoapp_use_output_timing )
        {
            ...
            // there will be no frametime delta as the last frame was just
            // re-used.
            uint64_t frametime_ns =
                lastCommit->present_time - w->last_commit_present_time;
            if ( frametime_ns > 0 )
            {
                if ( w->appID > 0 )
...
void nudge_steamcompmgr( void ) { g_SteamCompMgrWaiter.Nudge( ); }
...
void ClearUpscaleImages( ) { g_pUpscaleImages.clear( ); }

static TempUpscaleImage_t *
GetTempUpscaleImage( uint32_t uWidth, uint32_t uHeight, uint32_t uDrmFormat )
{
    ...
    {
        if ( !image.pTexture->IsInUse( ) ) return &image;
    }
    ...
    std::shared_ptr<gamescope::CTimeline> pTimeline =
        gamescope::CTimeline::Create( );
    if ( !pTimeline ) return nullptr;
    ...
    pTexture->BInit(
        g_nOutputWidth, g_nOutputHeight, 1, uDrmFormat, imageFlags );
    TempUpscaleImage_t &image = g_pUpscaleImages.emplace_back(
        std::move( pTexture ), std::move( pTimeline ) );

    return &image;
}

gamescope::ConVar<bool> cv_surface_update_force_only_current_surface(
    "surface_update_force_only_current_surface",
    false,
    "Force updates to apply only to the current surface, ignoring commits for "
    "other surfaces." );

void update_wayland_res(
    CommitDoneList_t   *doneCommits,
    steamcompmgr_win_t *w,
    ResListEntry_t     &reslistentry )
{
    ...
    if ( w == nullptr )
    {
        ...
        {
            wlserver_presentation_feedback_discard(
                reslistentry.surf, reslistentry.presentation_feedbacks );
            // presentation_feedbacks cleared by
            // wlserver_presentation_feedback_discard
        }
    ...
    // If we ever use HDR on the surface, only ever accept flip commits from the
    // WSI layer.
    if ( reslistentry.feedback && reslistentry.feedback->vk_colorspace !=
                                      VK_COLOR_SPACE_SRGB_NONLINEAR_KHR )
    {
        w->bHasHadNonSRGBColorSpace = true;
    }

    // If there are random commits that are really thin/small when we have the
    // WSI layer active ever, let's just ignore these as they are probably bogus
    // commits from glamor.
    bool bPossiblyBogus =
        reslistentry.buf->width <= 2 || reslistentry.buf->height <= 2;

    // If the buffer has no damage, always prefer our override surface.
    bool bHasDamage = ( reslistentry.surf->buffer_damage.extents.x2 -
                        reslistentry.surf->buffer_damage.extents.x1 ) > 2 &&
                      ( reslistentry.surf->buffer_damage.extents.y2 -
                        reslistentry.surf->buffer_damage.extents.y1 ) > 2;

    // If we have an override surface, make sure this commit is for the current
    // surface or if the commit is probably bogus.
    bool bOnlyCurrentSurface = w->bHasHadNonSRGBColorSpace || bPossiblyBogus ||
                               !bHasDamage ||
                               cv_surface_update_force_only_current_surface;

    bool for_current_surface =
        !w->override_surface( ) || w->current_surface( ) == reslistentry.surf;
    ...
    {
        if ( existing_commit->buf == buf ) already_exists = true;
    }

    if ( already_exists && !reslistentry.feedback &&
         reslistentry.presentation_feedbacks.empty( ) )
    {
        wlserver_lock( );
        wlr_buffer_unlock( buf );
        wlserver_unlock( );
        ...
        // If we have a duplicated commit + frame callback, ensure that is
        // signalled. This matches Mutter and Weston behavior, so it's plausible
        // that some application relies on forward progress. We're essentially
        // discarding the commit here, so consider it complete right away.
        w->receivedDoneCommit = true;
        return;
    }
    ...
    if ( newCommit != nullptr )
    {
        ...
        static bool bMangoappSocketDisable =
            env_to_bool( getenv( "GAMESCOPE_MANGOAPP_SOCKET_DISABLE" ) );

        // Whether or not to nudge mango app when this commit is done.
        const bool mango_nudge =
            pCurrentFocus &&
            ( ( w == pCurrentFocus->focusWindow &&
                !w->isSteamStreamingClient ) ||
              ( pCurrentFocus->focusWindow &&
                pCurrentFocus->focusWindow->isSteamStreamingClient &&
                w->isSteamStreamingClientVideo ) ) &&
            !bMangoappSocketDisable;

        bool bValidPreemptiveScale =
            reslistentry.pAcquirePoint && pCurrentFocus &&
            w == pCurrentFocus->focusWindow && cv_upscale_preemptive;
        bool bPreemptiveUpscale =
            bValidPreemptiveScale && newCommit->ShouldPreemptivelyUpscale( );

        bool bKnownReady = false;

        std::pair<int32_t, bool> eventFd =
            gamescope::CAcquireTimelinePoint::k_InvalidEvent;

        if ( bPreemptiveUpscale )
        {
            FrameInfo_t upscaledFrameInfo{};
            upscaledFrameInfo.applyOutputColorMgmt = true;
            upscaledFrameInfo.outputEncodingEOTF =
                ( newCommit->colorspace( ) ==
            ...
            upscaledFrameInfo.useFSRLayer0 =
                g_upscaleFilter == GamescopeUpscaleFilter::FSR;
            upscaledFrameInfo.useNISLayer0 =
                g_upscaleFilter == GamescopeUpscaleFilter::NIS;
            globalScaleRatio   = flOldGlobalScale;
            zoomScaleRatio     = flOldZoomScale;
            overscanScaleRatio = flOldOverscanScale;

            TempUpscaleImage_t *pTempImage = GetTempUpscaleImage(
                g_nOutputWidth, g_nOutputHeight, g_output.uOutputFormat );
            if ( pTempImage )
            {
                const uint64_t ulNextReleasePoint = ++pTempImage->ulLastPoint;

                std::unique_ptr<CVulkanCmdBuffer> pCommandBuffer =
                    g_device.commandBuffer( );

                pCommandBuffer->AddDependency(
                    reslistentry.pAcquirePoint->GetTimeline( )
                        ->ToVkSemaphore( ),
                    reslistentry.pAcquirePoint->GetPoint( ) );
                pCommandBuffer->AddSignal(
                    pTempImage->pReleaseTimeline->ToVkSemaphore( ),
                    ulNextReleasePoint );
                ...
                std::optional<uint64_t> seqNo = vulkan_composite(
                    &upscaledFrameInfo,
                    nullptr,
                    false,
                    pTempImage->pTexture,
                    false,
                    std::move( pCommandBuffer ) );
                ...
                newCommit->upscaledTexture = std::optional<UpscaledTexture_t>{
                    ...
                    g_nOutputHeight,
                    pTempImage->pTexture,
                    upscaledFrameInfo.outputEncodingEOTF == EOTF_Gamma22
                        ? VK_COLOR_SPACE_SRGB_NONLINEAR_KHR
                        : VK_COLOR_SPACE_HDR10_ST2084_EXT,
                };

                // Manifest a new acquire timeline point with this inline work.
                eventFd = gamescope::CAcquireTimelinePoint(
                              pTempImage->pReleaseTimeline, ulNextReleasePoint )
                              .CreateEventFd( );
        ...
        if ( !bPreemptiveUpscale )
        {
            if ( bValidPreemptiveScale ) { ClearUpscaleImages( ); }
        ...
        if ( gamescope::IBackendFb *pBackendFb =
                 newCommit->vulkanTex->GetBackendFb( ) )
        {
        ...
        gpuvis_trace_printf(
            ...
            newCommit->commitID,
            w->type == steamcompmgr_win_type_t::XWAYLAND ? w->xwayland( ).id
                                                         : 0 );
        {
            newCommit->SetFence( fence, mango_nudge, doneCommits );
            if ( bKnownReady ) newCommit->Signal( );
            else
...
void check_new_xwayland_res( xwayland_ctx_t *ctx )
{
    ...
    std::vector<ResListEntry_t> &tmp_queue =
        ctx->xwayland_server->retrieve_commits( );
...
void check_new_xdg_res( )
{
    ...
    for ( uint32_t i = 0; i < tmp_queue.size( ); i++ )
    {
        for ( const auto &xdg_win : g_steamcompmgr_xdg_wins )
        {
            ...
            {
                update_wayland_res(
                    &g_steamcompmgr_xdg_done_commits,
                    xdg_win.get( ),
                    tmp_queue[ i ] );
                break;
            }
...
static void handle_xfixes_selection_notify(
    xwayland_ctx_t *ctx, XFixesSelectionNotifyEvent *event )
{
    if ( event->owner == ctx->ourWindow ) { return; }
...
void xwayland_ctx_t::Dispatch( )
{
    ...
    while ( XPending( ctx->dpy ) )
    {
        ...
        switch ( ev.type )
        {
            ...
            {
                steamcompmgr_win_t *w =
                    find_win( ctx, ev.xdestroywindow.window );
            ...
            case FocusOut:
            {
                ...
                // If focus escaped the current desired keyboard focus window,
                // check where it went
                if ( w && w->xwayland( ).id == ctx->currentKeyboardFocusWindow )
                {
                    ...
                    {
                        if ( kbw->xwayland( ).id ==
                             ctx->currentKeyboardFocusWindow )
                        {
                            // focus went to a child, this is fine, make note of
                            // it in case we need to fix it
            ...
            case ReparentNotify:
                ...
                {
                    steamcompmgr_win_t *w =
                        find_win( ctx, ev.xreparent.window );
            ...
            default:
                ...
                else if ( ev.type == ctx->xfixes_event + XFixesSelectionNotify )
                {
                    handle_xfixes_selection_notify(
                        ctx, ( XFixesSelectionNotifyEvent * )&ev );
                }
                break;
        }
        XFlush( ctx->dpy );
...
load_mouse_cursor( MouseCursor *cursor, const char *path, int hx, int hy )
{
    int     w, h, channels;
    rgba_t *data =
        ( rgba_t * )stbi_load( path, &w, &h, &channels, STBI_rgb_alpha );
    if ( !data )
    {
    ...
    std::transform(
        ...
        []( rgba_t x )
        {
            if ( x.a == 0 ) return rgba_t{};
            return rgba_t{ uint8_t( ( x.b * x.a ) / 255 ),
                           uint8_t( ( x.g * x.a ) / 255 ),
                           uint8_t( ( x.r * x.a ) / 255 ),
                           x.a };
        } );
...
void init_xwayland_ctx(
    uint32_t serverId, gamescope_xwayland_server_t *xwayland_server )
{
    ...
    if ( !XDamageQueryExtension(
             ctx->dpy, &ctx->damage_event, &ctx->damage_error ) )
    {
        xwm_log.errorf( "No damage extension" );
        exit( 1 );
    }
    if ( !XFixesQueryExtension(
             ctx->dpy, &ctx->xfixes_event, &ctx->xfixes_error ) )
    {
        xwm_log.errorf( "No XFixes extension" );
        exit( 1 );
    }
    if ( !XShapeQueryExtension(
             ctx->dpy, &ctx->xshape_event, &ctx->xshape_error ) )
    {
    ...
    {
        xwm_log.errorf(
            "Unsupported XRes version: have %d.%d, want 1.2",
            xres_major,
            xres_minor );
        exit( 1 );
    }
    ...
    if ( !register_cm( ctx ) ) { exit( 1 ); }

    register_systray( ctx );

    /* get atoms */
    ctx->atoms.steamAtom = XInternAtom( ctx->dpy, STEAM_PROP, false );
    ctx->atoms.steamInputFocusAtom =
        XInternAtom( ctx->dpy, "STEAM_INPUT_FOCUS", false );
    ctx->atoms.steamTouchClickModeAtom =
        XInternAtom( ctx->dpy, "STEAM_TOUCH_CLICK_MODE", false );
    ctx->atoms.gameAtom    = XInternAtom( ctx->dpy, GAME_PROP, false );
    ctx->atoms.overlayAtom = XInternAtom( ctx->dpy, OVERLAY_PROP, false );
    ctx->atoms.externalOverlayAtom =
        XInternAtom( ctx->dpy, EXTERNAL_OVERLAY_PROP, false );
    ctx->atoms.opacityAtom = XInternAtom( ctx->dpy, OPACITY_PROP, false );
    ctx->atoms.gamesRunningAtom =
        XInternAtom( ctx->dpy, GAMES_RUNNING_PROP, false );
    ctx->atoms.screenScaleAtom =
        XInternAtom( ctx->dpy, SCREEN_SCALE_PROP, false );
    ctx->atoms.screenZoomAtom =
        XInternAtom( ctx->dpy, SCREEN_MAGNIFICATION_PROP, false );
    ctx->atoms.winTypeAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE", false );
    ctx->atoms.winDesktopAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_DESKTOP", false );
    ctx->atoms.winDockAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_DOCK", false );
    ctx->atoms.winToolbarAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_TOOLBAR", false );
    ctx->atoms.winMenuAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_MENU", false );
    ctx->atoms.winUtilAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_UTILITY", false );
    ctx->atoms.winSplashAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_SPLASH", false );
    ctx->atoms.winDialogAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_DIALOG", false );
    ctx->atoms.winNormalAtom =
        XInternAtom( ctx->dpy, "_NET_WM_WINDOW_TYPE_NORMAL", false );
    ctx->atoms.sizeHintsAtom =
        XInternAtom( ctx->dpy, "WM_NORMAL_HINTS", false );
    ctx->atoms.netWMStateFullscreenAtom =
        XInternAtom( ctx->dpy, "_NET_WM_STATE_FULLSCREEN", false );
    ctx->atoms.activeWindowAtom =
        XInternAtom( ctx->dpy, "_NET_ACTIVE_WINDOW", false );
    ctx->atoms.netWMStateAtom = XInternAtom( ctx->dpy, "_NET_WM_STATE", false );
    ctx->atoms.WMTransientForAtom =
        XInternAtom( ctx->dpy, "WM_TRANSIENT_FOR", false );
    ctx->atoms.netWMStateHiddenAtom =
        XInternAtom( ctx->dpy, "_NET_WM_STATE_HIDDEN", false );
    ctx->atoms.netWMStateFocusedAtom =
        XInternAtom( ctx->dpy, "_NET_WM_STATE_FOCUSED", false );
    ctx->atoms.netWMStateSkipTaskbarAtom =
        XInternAtom( ctx->dpy, "_NET_WM_STATE_SKIP_TASKBAR", false );
    ctx->atoms.netWMStateSkipPagerAtom =
        XInternAtom( ctx->dpy, "_NET_WM_STATE_SKIP_PAGER", false );
    ctx->atoms.WLSurfaceIDAtom =
        XInternAtom( ctx->dpy, "WL_SURFACE_ID", false );
    ...
    ctx->atoms.netSystemTrayOpcodeAtom =
        XInternAtom( ctx->dpy, "_NET_SYSTEM_TRAY_OPCODE", false );
    ctx->atoms.steamStreamingClientAtom =
        XInternAtom( ctx->dpy, "STEAM_STREAMING_CLIENT", false );
    ctx->atoms.steamStreamingClientVideoAtom =
        XInternAtom( ctx->dpy, "STEAM_STREAMING_CLIENT_VIDEO", false );
    ctx->atoms.steamGamescopeVROverlayTarget =
        XInternAtom( ctx->dpy, "STEAM_GAMESCOPE_VROVERLAY_TARGET", false );
    ctx->atoms.gamescopePid = XInternAtom( ctx->dpy, "GAMESCOPE_PID", false );
    ctx->atoms.gamescopeVROverlayForwarding =
        XInternAtom( ctx->dpy, "GAMESCOPE_VROVERLAY_FORWARDING", false );
    ctx->atoms.gamescopeFocusableAppsAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUSABLE_APPS", false );
    ctx->atoms.gamescopeFocusableWindowsAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUSABLE_WINDOWS", false );
    ctx->atoms.gamescopeFocusedAppAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUSED_APP", false );
    ctx->atoms.gamescopeFocusedAppGfxAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUSED_APP_GFX", false );
    ctx->atoms.gamescopeFocusedWindowAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUSED_WINDOW", false );
    ctx->atoms.gamescopeCtrlAppIDAtom =
        XInternAtom( ctx->dpy, "GAMESCOPECTRL_BASELAYER_APPID", false );
    ctx->atoms.gamescopeCtrlWindowAtom =
        XInternAtom( ctx->dpy, "GAMESCOPECTRL_BASELAYER_WINDOW", false );
    ctx->atoms.WMChangeStateAtom =
        XInternAtom( ctx->dpy, "WM_CHANGE_STATE", false );
    ctx->atoms.gamescopeInputCounterAtom =
        XInternAtom( ctx->dpy, "GAMESCOPE_INPUT_COUNTER", false );
    ctx->atoms.gamescopeScreenShotAtom =
        XInternAtom( ctx->dpy, "GAMESCOPECTRL_REQUEST_SCREENSHOT", false );
    ctx->atoms.gamescopeDebugScreenShotAtom = XInternAtom(
        ctx->dpy, "GAMESCOPECTRL_DEBUG_REQUEST_SCREENSHOT", false );

    ctx->atoms.gamescopeFocusDisplay =
        XInternAtom( ctx->dpy, "GAMESCOPE_FOCUS_DISPLAY", false );
    ctx->atoms.gamescopeMouseFocusDisplay =
        XInternAtom( ctx->dpy, "GAMESCOPE_MOUSE_FOCUS_DISPLAY", false );
    ctx->atoms.gamescopeKeyboardFocusDisplay =
        XInternAtom( ctx->dpy, "GAMESCOPE_KEYBOARD_FOCUS_DISPLAY", false );

    // In nanoseconds...
    ctx->atoms.gamescopeTuneableVBlankRedZone =
        XInternAtom( ctx->dpy, "GAMESCOPE_TUNEABLE_VBLANK_REDZONE", false );
    ctx->atoms.gamescopeTuneableRateOfDecay = XInternAtom(
        ctx->dpy, "GAMESCOPE_TUNEABLE_VBLANK_RATE_OF_DECAY_PERCENTAGE", false );

    ctx->atoms.gamescopeScalingFilter =
        XInternAtom( ctx->dpy, "GAMESCOPE_SCALING_FILTER", false );
    ctx->atoms.gamescopeFSRSharpness =
        XInternAtom( ctx->dpy, "GAMESCOPE_FSR_SHARPNESS", false );
    ctx->atoms.gamescopeSharpness =
        XInternAtom( ctx->dpy, "GAMESCOPE_SHARPNESS", false );

    ctx->atoms.gamescopeXWaylandModeControl =
        XInternAtom( ctx->dpy, "GAMESCOPE_XWAYLAND_MODE_CONTROL", false );
    ctx->atoms.gamescopeFPSLimit =
        XInternAtom( ctx->dpy, "GAMESCOPE_FPS_LIMIT", false );
    ctx->atoms
        .gamescopeDynamicRefresh[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        XInternAtom( ctx->dpy, "GAMESCOPE_DYNAMIC_REFRESH", false );
    ctx->atoms
        .gamescopeDynamicRefresh[ gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL ] =
        XInternAtom( ctx->dpy, "GAMESCOPE_DYNAMIC_REFRESH_EXTERNAL", false );
    ctx->atoms.gamescopeLowLatency =
        XInternAtom( ctx->dpy, "GAMESCOPE_LOW_LATENCY", false );

    ctx->atoms.gamescopeFSRFeedback =
        XInternAtom( ctx->dpy, "GAMESCOPE_FSR_FEEDBACK", false );

    ctx->atoms.gamescopeBlurMode =
        XInternAtom( ctx->dpy, "GAMESCOPE_BLUR_MODE", false );
    ctx->atoms.gamescopeBlurRadius =
        XInternAtom( ctx->dpy, "GAMESCOPE_BLUR_RADIUS", false );
    ctx->atoms.gamescopeBlurFadeDuration =
        XInternAtom( ctx->dpy, "GAMESCOPE_BLUR_FADE_DURATION", false );

    ctx->atoms.gamescopeCompositeForce =
        XInternAtom( ctx->dpy, "GAMESCOPE_COMPOSITE_FORCE", false );
    ctx->atoms.gamescopeCompositeDebug =
        XInternAtom( ctx->dpy, "GAMESCOPE_COMPOSITE_DEBUG", false );

    ctx->atoms.gamescopeAllowTearing =
        XInternAtom( ctx->dpy, "GAMESCOPE_ALLOW_TEARING", false );

    ctx->atoms.gamescopeDisplayForceInternal =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_FORCE_INTERNAL", false );
    ctx->atoms.gamescopeDisplayModeNudge =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_MODE_NUDGE", false );

    ctx->atoms.gamescopeDisplayIsExternal =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_IS_EXTERNAL", false );
    ctx->atoms.gamescopeDisplayModeListExternal =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_MODE_LIST_EXTERNAL", false );

    ctx->atoms.gamescopeCursorVisibleFeedback =
        XInternAtom( ctx->dpy, "GAMESCOPE_CURSOR_VISIBLE_FEEDBACK", false );

    ctx->atoms.gamescopeSteamMaxHeight =
        XInternAtom( ctx->dpy, "GAMESCOPE_STEAM_MAX_HEIGHT", false );
    ctx->atoms.gamescopeVRREnabled =
        XInternAtom( ctx->dpy, "GAMESCOPE_VRR_ENABLED", false );
    ctx->atoms.gamescopeVRRCapable =
        XInternAtom( ctx->dpy, "GAMESCOPE_VRR_CAPABLE", false );
    ctx->atoms.gamescopeVRRInUse =
        XInternAtom( ctx->dpy, "GAMESCOPE_VRR_FEEDBACK", false );

    ctx->atoms.gamescopeNewScalingFilter =
        XInternAtom( ctx->dpy, "GAMESCOPE_NEW_SCALING_FILTER", false );
    ctx->atoms.gamescopeNewScalingScaler =
        XInternAtom( ctx->dpy, "GAMESCOPE_NEW_SCALING_SCALER", false );

    ctx->atoms.gamescopeDisplayEdidPath =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_EDID_PATH", false );
    ctx->atoms.gamescopeXwaylandServerId =
        XInternAtom( ctx->dpy, "GAMESCOPE_XWAYLAND_SERVER_ID", false );

    ctx->atoms.gamescopeDisplaySupportsHDR =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_SUPPORTS_HDR", false );
    ctx->atoms.gamescopeDisplayHDREnabled =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_HDR_ENABLED", false );
    ctx->atoms.gamescopeDebugForceHDR10Output =
        XInternAtom( ctx->dpy, "GAMESCOPE_DEBUG_FORCE_HDR10_PQ_OUTPUT", false );
    ctx->atoms.gamescopeDebugForceHDRSupport =
        XInternAtom( ctx->dpy, "GAMESCOPE_DEBUG_FORCE_HDR_SUPPORT", false );
    ctx->atoms.gamescopeDebugHDRHeatmap =
        XInternAtom( ctx->dpy, "GAMESCOPE_DEBUG_HDR_HEATMAP", false );
    ctx->atoms.gamescopeHDROutputFeedback =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_OUTPUT_FEEDBACK", false );
    ctx->atoms.gamescopeSDROnHDRContentBrightness = XInternAtom(
        ctx->dpy, "GAMESCOPE_SDR_ON_HDR_CONTENT_BRIGHTNESS", false );
    ctx->atoms.gamescopeHDRInputGain =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_INPUT_GAIN", false );
    ctx->atoms.gamescopeSDRInputGain =
        XInternAtom( ctx->dpy, "GAMESCOPE_SDR_INPUT_GAIN", false );
    ctx->atoms.gamescopeHDRItmEnable =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_ITM_ENABLE", false );
    ctx->atoms.gamescopeHDRItmSDRNits =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_ITM_SDR_NITS", false );
    ctx->atoms.gamescopeHDRItmTargetNits =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_ITM_TARGET_NITS", false );
    ctx->atoms.gamescopeColorLookPQ =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_LOOK_PQ", false );
    ctx->atoms.gamescopeColorLookG22 =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_LOOK_G22", false );
    ctx->atoms.gamescopeColorOutputVirtualWhite =
        XInternAtom( ctx->dpy, "GAMESCOPE_DISPLAY_VIRTUAL_WHITE", false );
    ctx->atoms.gamescopeHDRTonemapDisplayMetadata = XInternAtom(
        ctx->dpy, "GAMESCOPE_HDR_TONEMAP_DISPLAY_METADATA", false );
    ctx->atoms.gamescopeHDRTonemapSourceMetadata =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_TONEMAP_SOURCE_METADATA", false );
    ctx->atoms.gamescopeHDRTonemapOperator =
        XInternAtom( ctx->dpy, "GAMESCOPE_HDR_TONEMAP_OPERATOR", false );

    ctx->atoms.gamescopeForceWindowsFullscreen =
        XInternAtom( ctx->dpy, "GAMESCOPE_FORCE_WINDOWS_FULLSCREEN", false );

    ctx->atoms.gamescopeColorLut3DOverride =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_3DLUT_OVERRIDE", false );
    ctx->atoms.gamescopeColorShaperLutOverride =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_SHAPERLUT_OVERRIDE", false );
    ctx->atoms.gamescopeColorSDRGamutWideness =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_SDR_GAMUT_WIDENESS", false );
    ctx->atoms.gamescopeColorNightMode =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_NIGHT_MODE", false );
    ctx->atoms.gamescopeColorManagementDisable =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_MANAGEMENT_DISABLE", false );
    ctx->atoms.gamescopeColorAppWantsHDRFeedback = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_APP_WANTS_HDR_FEEDBACK", false );
    ctx->atoms.gamescopeColorAppHDRMetadataFeedback = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_APP_HDR_METADATA_FEEDBACK", false );
    ctx->atoms.gamescopeColorSliderInUse = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_MANAGEMENT_CHANGING_HINT", false );
    ctx->atoms.gamescopeColorChromaticAdaptationMode = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_CHROMATIC_ADAPTATION_MODE", false );
    ctx->atoms.gamescopeColorMuraCorrectionImage
        [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_MURA_CORRECTION_IMAGE", false );
    ctx->atoms.gamescopeColorMuraCorrectionImage
        [ gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL ] = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_MURA_CORRECTION_IMAGE_EXTERNAL", false );
    ctx->atoms
        .gamescopeColorMuraScale[ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_MURA_SCALE", false );
    ctx->atoms
        .gamescopeColorMuraScale[ gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL ] =
        XInternAtom( ctx->dpy, "GAMESCOPE_COLOR_MURA_SCALE_EXTERNAL", false );
    ctx->atoms.gamescopeColorMuraCorrectionDisabled
        [ gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL ] = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_MURA_CORRECTION_DISABLED", false );
    ctx->atoms.gamescopeColorMuraCorrectionDisabled
        [ gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL ] = XInternAtom(
        ctx->dpy, "GAMESCOPE_COLOR_MURA_CORRECTION_DISABLED_EXTERNAL", false );

    ctx->atoms.gamescopeCreateXWaylandServer =
        XInternAtom( ctx->dpy, "GAMESCOPE_CREATE_XWAYLAND_SERVER", false );
    ctx->atoms.gamescopeCreateXWaylandServerFeedback = XInternAtom(
        ctx->dpy, "GAMESCOPE_CREATE_XWAYLAND_SERVER_FEEDBACK", false );
    ctx->atoms.gamescopeDestroyXWaylandServer =
        XInternAtom( ctx->dpy, "GAMESCOPE_DESTROY_XWAYLAND_SERVER", false );

    ctx->atoms.gamescopeReshadeEffect =
        XInternAtom( ctx->dpy, "GAMESCOPE_RESHADE_EFFECT", false );
    ctx->atoms.gamescopeReshadeTechniqueIdx =
        XInternAtom( ctx->dpy, "GAMESCOPE_RESHADE_TECHNIQUE_IDX", false );

    ctx->atoms.gamescopeDisplayRefreshRateFeedback = XInternAtom(
        ctx->dpy, "GAMESCOPE_DISPLAY_REFRESH_RATE_FEEDBACK", false );
    ctx->atoms.gamescopeDisplayDynamicRefreshBasedOnGamePresence = XInternAtom(
        ctx->dpy,
        "GAMESCOPE_DISPLAY_DYNAMIC_REFRESH_BASED_ON_GAME_PRESENCE",
        false );

    ctx->atoms.gamescopeMainSteamVROverlay =
        XInternAtom( ctx->dpy, "GAMESCOPE_MAIN_STEAMVR_OVERLAY", false );
    ctx->atoms.steamosTouchPointerEmulation =
        XInternAtom( ctx->dpy, "_STEAMOS_TOUCH_POINTER_EMULATION", false );

    ctx->atoms.wineHwndStyle =
        XInternAtom( ctx->dpy, "_WINE_HWND_STYLE", false );
    ctx->atoms.wineHwndStyleEx =
        XInternAtom( ctx->dpy, "_WINE_HWND_EXSTYLE", false );
    ...
    ctx->atoms.wm_delete_window =
        XInternAtom( ctx->dpy, "WM_DELETE_WINDOW", false );
    ...
    uint32_t unVROverlayForwardingSupported =
        GetBackend( )->SupportsVROverlayForwarding( ) ? 2 : 0;
    XChangeProperty(
        ctx->dpy,
        ctx->root,
        ctx->atoms.gamescopeVROverlayForwarding,
        ...
        32,
        PropModeReplace,
        ( unsigned char * )&unVROverlayForwardingSupported,
        1 );

    XGrabServer( ctx->dpy );

    XCompositeRedirectSubwindows(
        ctx->dpy, ctx->root, CompositeRedirectManual );
    ...
    {
        if ( !load_mouse_cursor(
                 ctx->cursor.get( ),
                 g_customCursorPath,
                 g_customCursorHotspotX,
                 g_customCursorHotspotY ) )
            xwm_log.errorf(
                "Failed to load mouse cursor: %s", g_customCursorPath );
    }
    else
    {
        if ( std::shared_ptr<gamescope::INestedHints::CursorInfo> pHostCursor =
                 gamescope::GetX11HostCursor( ) )
        {
...
void update_vrr_atoms(
    xwayland_ctx_t *root_ctx, bool force, bool *needs_flush = nullptr )
{
    bool capable = GetBackend( )->GetCurrentConnector( ) &&
                   GetBackend( )->GetCurrentConnector( )->SupportsVRR( );
    if ( capable != g_bVRRCapable_CachedValue || force )
    {
        ...
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeVRRCapable,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&capable_value,
            1 );
        g_bVRRCapable_CachedValue = capable;
        if ( needs_flush ) *needs_flush = true;
    }

    bool HDR = GetBackend( )->GetCurrentConnector( ) &&
               GetBackend( )->GetCurrentConnector( )->SupportsHDR( );
    if ( HDR != g_bSupportsHDR_CachedValue || force )
    {
        ...
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeDisplaySupportsHDR,
            XA_CARDINAL,
            32,
            PropModeReplace,
    ...
    bool in_use = GetBackend( )->GetCurrentConnector( ) &&
                  GetBackend( )->GetCurrentConnector( )->IsVRRActive( );
    if ( in_use != g_bVRRInUse_CachedValue || force )
    {
        ...
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeVRRInUse,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&in_use_value,
            1 );
    ...
    if ( g_nOutputRefresh != g_nCurrentRefreshRate_CachedValue || force )
    {
        ...
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeDisplayRefreshRateFeedback,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&nRefresh,
            1 );
    ...
    if ( force )
    {
        ...
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeVRREnabled,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&enabled_value,
            1 );
        if ( needs_flush ) *needs_flush = true;
    }
}
...
void update_mode_atoms( xwayland_ctx_t *root_ctx, bool *needs_flush = nullptr )
{
             ...
             gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL )
    {
        XDeleteProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeDisplayModeListExternal );

        uint32_t zero = 0;
        XChangeProperty(
            root_ctx->dpy,
            root_ctx->root,
            root_ctx->atoms.gamescopeDisplayIsExternal,
            XA_CARDINAL,
            32,
            PropModeReplace,
            ( unsigned char * )&zero,
    ...
    int  len            = 0;
    for ( int i = 0; remaining_size > 0 && i < ( int )connectorModes.size( );
          i++ )
    {
...
void steamcompmgr_check_xdg( bool vblank, uint64_t vblank_idx )
{
    if ( wlserver_xdg_dirty( ) )
    {
        for ( auto &iter : g_VirtualConnectorFocuses )
        {
            global_focus_t *pFocus = &iter.second;
            if ( pFocus->focusWindow &&
                 pFocus->focusWindow->type == steamcompmgr_win_type_t::XDG )
                pFocus->focusWindow = nullptr;
            if ( pFocus->inputFocusWindow && pFocus->inputFocusWindow->type ==
                                                 steamcompmgr_win_type_t::XDG )
                pFocus->inputFocusWindow = nullptr;
            if ( pFocus->overlayWindow &&
                 pFocus->overlayWindow->type == steamcompmgr_win_type_t::XDG )
                pFocus->overlayWindow = nullptr;
            if ( pFocus->notificationWindow &&
                 pFocus->notificationWindow->type ==
                     steamcompmgr_win_type_t::XDG )
                pFocus->notificationWindow = nullptr;
            if ( pFocus->overrideWindow &&
                 pFocus->overrideWindow->type == steamcompmgr_win_type_t::XDG )
                pFocus->overrideWindow = nullptr;
            if ( pFocus->fadeWindow &&
                 pFocus->fadeWindow->type == steamcompmgr_win_type_t::XDG )
                pFocus->fadeWindow = nullptr;
        }
    ...
    // When we have observed both a complete commit and a VBlank, we should
    // request a new frame.
    if ( vblank )
    {
        for ( const auto &xdg_win : g_steamcompmgr_xdg_wins )
...
void update_edid_prop( )
{
    ...
    for ( size_t i = 0; ( server = wlserver_get_xwayland_server( i ) ); i++ )
    {
        ...
        XSetTextProperty(
            ...
            server->ctx->root,
            &text_property,
            server->ctx->atoms.gamescopeDisplayEdidPath );
    }
}
...
gamescope::ConVar<bool> cv_shutdown_on_primary_child_death(
    "shutdown_on_primary_child_death",
    true,
    "Should gamescope shutdown when the primary application launched in it was "
    "shut down?" );
static LogScope s_LaunchLogScope( "launch" );

static std::vector<uint32_t>          s_uRelativeMouseFilteredAppids;
static gamescope::ConVar<std::string> cv_mouse_relative_filter_appids(
    "mouse_relative_filter_appids",
    ...
    {
        std::vector<std::string_view> sFilterAppids =
            gamescope::Split( cvar, "," );
        std::vector<uint32_t> uFilterAppids;
        uFilterAppids.reserve( sFilterAppids.size( ) );
        for ( auto &sFilterAppid : sFilterAppids )
        {
            std::optional<uint32_t> ouFilterAppid =
                gamescope::Parse<uint32_t>( sFilterAppid );
...
void LaunchNestedChildren( char **ppPrimaryChildArgv )
{
    std::string sNewPreload;
    {
        ...
        if ( pszCurrentPreload && *pszCurrentPreload )
        {
            ...
            {
                std::vector<std::string_view> svLibraries =
                    gamescope::Split( pszCurrentPreload, " :" );
                std::erase_if(
                    svLibraries,
                    []( std::string_view svPreload )
                    {
                        return svPreload.find( "gameoverlayrenderer.so" ) !=
    ...
    // when Gamescope dies.
    // This allows us to launch stuff alongside Gamescope if we ever wanted --
    // rather than being under it. (eg. if we wanted a drm janitor or
    // something.)

    if ( ppPrimaryChildArgv && *ppPrimaryChildArgv )
    {
        pid_t nPrimaryChildPid = gamescope::Process::SpawnProcessInWatchdog(
            ppPrimaryChildArgv, false );
        ...
        std::thread waitThread(
            [ nPrimaryChildPid ]( )
            {
                ...
                if ( cv_shutdown_on_primary_child_death ) ShutdownGamescope( );
            } );
        waitThread.detach( );
    }
...
void steamcompmgr_main( int argc, char **argv )
{
        ...
        switch ( o )
        {
            ...
            case 0: // long options without a short option
                ...
                else if ( strcmp( opt_name, "force-windows-fullscreen" ) == 0 )
                {
                    ...
                    strcmp( opt_name, "hdr-enabled" ) == 0 ||
                    strcmp( opt_name, "hdr-enable" ) == 0 )
                {
                    cv_hdr_enabled = true;
                }
                ...
                else if ( strcmp( opt_name, "hdr-debug-force-output" ) == 0 )
                {
                    ...
                    strcmp( opt_name, "hdr-itm-enabled" ) == 0 ||
                    strcmp( opt_name, "hdr-itm-enable" ) == 0 )
                {
    ...
    int subCommandArg = -1;
    if ( optind < argc ) { subCommandArg = optind; }
    ...
    std::unique_lock<std::mutex> xwayland_server_guard(
    ...
    gamesRunningCount = get_prop(
        root_ctx, root_ctx->root, root_ctx->atoms.gamesRunningAtom, 0 );
    overscanScaleRatio = get_prop(
                             root_ctx,
                             root_ctx->root,
                             root_ctx->atoms.screenScaleAtom,
                             0xFFFFFFFF ) /
                         ( double )0xFFFFFFFF;
    zoomScaleRatio =
        get_prop(
            root_ctx, root_ctx->root, root_ctx->atoms.screenZoomAtom, 0xFFFF ) /
        ( double )0xFFFF;

    globalScaleRatio = overscanScaleRatio * zoomScaleRatio;

    static constexpr uint64_t k_unSingleOutputVirtualConnectorKey = 0;
    g_VirtualConnectorFocuses[ k_unSingleOutputVirtualConnectorKey ] =
        global_focus_t{
            .ulVirtualFocusKey = k_unSingleOutputVirtualConnectorKey,
            .pVirtualConnector = GetBackend( )->UsesVirtualConnectors( )
                                     ? GetBackend( )->CreateVirtualConnector(
                                           k_unSingleOutputVirtualConnectorKey )
                                     : nullptr,
        };
    ...
    {
        global_focus_t *pFocus = &iter.second;
        if ( pFocus->IsDirty( ) ) determine_and_apply_focus( pFocus );
    }
...

    {
        ...
        for ( size_t i = 0; ( pServer = wlserver_get_xwayland_server( i ) );
              i++ )
        {
    ...
    if ( !GetBackend( )->PostInit( ) ) return;

    if ( g_pVROverlayKey )
    {
        set_string_prop(
            root_ctx,
            root_ctx->atoms.gamescopeMainSteamVROverlay,
            *g_pVROverlayKey );
    }
    ...
    LaunchNestedChildren(
        subCommandArg >= 0 ? &argv[ subCommandArg ] : nullptr );

    // Transpose to get this 3x3 matrix into the right state for applying as a
    // 3x4 on DRM + the Vulkan side. ie. color.rgb = color.rgba *
    // u_ctm[offsetLayerIdx];
    s_scRGB709To2020Matrix = GetBackend( )->CreateBackendBlob(
        glm::mat3x4( glm::transpose( k_2020_from_709 ) ) );
    ...
    {
        {
            ...
            {
                assert( server->ctx );
                if ( server->ctx->HasQueuedEvents( ) ) server->ctx->Dispatch( );
            }
        }
        ...
        bool vblank = false;
        if ( std::optional<gamescope::VBlankTime> pendingVBlank =
                 GetVBlankTimer( ).ProcessVBlank( ) )
        {
            g_SteamCompMgrVBlankTime = *pendingVBlank;
            vblank                   = true;
        }

        if ( g_bRun == false ) { break; }

        // If this is from the timer or not.
        // Consider this to also be "is this vblank, the fastest refresh cycle
        // after our last commit?" as a question.
        const bool bIsVBlankFromTimer = vblank;

        // We can always vblank if VRR.
        const bool bVRR = GetBackend( )->GetCurrentConnector( ) &&
                          GetBackend( )->GetCurrentConnector( )->IsVRRActive( );
        if ( bVRR ) vblank = true;
        ...
        if ( inputCounter != lastPublishedInputCounter )
        {
            XChangeProperty(
                root_ctx->dpy,
                root_ctx->root,
                root_ctx->atoms.gamescopeInputCounterAtom,
                XA_CARDINAL,
                32,
                PropModeReplace,
        ...
        if ( g_bFSRActive != g_bWasFSRActive )
        {
            ...
            XChangeProperty(
                root_ctx->dpy,
                root_ctx->root,
                root_ctx->atoms.gamescopeFSRFeedback,
                XA_CARDINAL,
                32,
                PropModeReplace,
        ...
        static gamescope::VirtualConnectorStrategy
                                            s_eLastVirtualConnectorStrategy =
                                                gamescope::cv_backend_virtual_connector_strategy;
        gamescope::VirtualConnectorStrategy eVirtualConnectorStrategy =
            gamescope::cv_backend_virtual_connector_strategy;

        if ( eVirtualConnectorStrategy != s_eLastVirtualConnectorStrategy ||
             bBackendJustInitted )
        {
            // If our virtual connector strategy changes, clear out our virtual
            // connector global focuses.
            g_VirtualConnectorFocuses.clear( );
            s_eLastVirtualConnectorStrategy = eVirtualConnectorStrategy;

            xwm_log.infof( "Late init of virtual connector stuff." );

            // misyl: Make the virtual connector up-front if we are in a
            // single-output mode. So we don't delay in getting display/output
            // info to the game
            static constexpr uint64_t k_unSingleOutputVirtualConnectorKey = 0;
            ...
            g_VirtualConnectorFocuses[ k_unSingleOutputVirtualConnectorKey ] =
                global_focus_t{
                    .ulVirtualFocusKey = k_unSingleOutputVirtualConnectorKey,
                    .pVirtualConnector =
                        GetBackend( )->UsesVirtualConnectors( )
                            ? GetBackend( )->CreateVirtualConnector(
                                  k_unSingleOutputVirtualConnectorKey )
                            : nullptr,
                };
        ...
        //	if ( bDirtyFocuses )
        {
            ...
            if ( !gamescope::VirtualConnectorIsSingleOutput( ) )
            {
                ...
                {
                    // Exclude windows that are useless (1x1), or override
                    // redirect windows
                    if ( win_is_useless( pWindow ) ||
                         ( pWindow->type == steamcompmgr_win_type_t::XWAYLAND &&
                           pWindow->xwayland( ).a.override_redirect &&
                           !pWindow->isSteamLegacyBigPicture ) ) // bootstrapper
                                                                 // is override
                                                                 // redirect :(
                    {
                        continue;
                    ...
                    gamescope::VirtualConnectorKey_t ulKey =
                        pWindow->GetVirtualConnectorKey(
                            eVirtualConnectorStrategy );
                    ...
                    if ( gamescope::Algorithm::Contains( keysToClose, ulKey ) )
                    {
                             ...
                             steamcompmgr_win_type_t::XWAYLAND )
                        {
                            ...
                            event.xclient.message_type =
                                pWindow->xwayland( ).ctx->atoms.wm_protocols;
                        ...
                        {
                            xwm_log.errorf(
                                "Closing Wayland windows not supported yet." );
                        }
                    }
                ...
                {
                    bool bIsSteam =
                        gamescope::VirtualConnectorKeyIsSteam( ulKey );
                    ...
                    else if ( !bIsSteam && !bIsBaseKey ) // Never remove Steam's
                                                         // virtual connector or
                                                         // the 0th connector.
                    {
        ...
        g_bOutputHDREnabled =
            ( g_bSupportsHDR_CachedValue || g_bForceHDR10OutputDebug ) &&
            cv_hdr_enabled;

        // Pick our width/height for this potential frame, regardless of how it
        // might change later At some point we might even add proper locking so
        // we get real updates atomically instead of whatever jumble of races
        // the below might cause over a couple of frames
             ...
             currentHDRForce != g_bForceHDRSupportDebug )
        {
            ...
            {
                g_nNestedHeight =
                    ( g_nNestedWidth * g_nOutputHeight ) / g_nOutputWidth;
                wlserver_lock( );
                // Update only Steam, the root ctx, with the new output size for
                // now
                wlserver_set_xwayland_server_mode(
                    0, g_nOutputWidth, g_nOutputHeight, g_nOutputRefresh );
                wlserver_unlock( );
            }
...

            {
                ...
                {
                    uint32_t hdr_value =
                        ( g_bOutputHDREnabled || g_bForceHDRSupportDebug ) ? 1
                                                                           : 0;
                    XChangeProperty(
                        server->ctx->dpy,
                        server->ctx->root,
                        server->ctx->atoms.gamescopeHDROutputFeedback,
                        XA_CARDINAL,
                        32,
                        PropModeReplace,
                        ( unsigned char * )&hdr_value,
                    ...
                    if ( server->ctx.get( ) == root_ctx ) { flush_root = true; }
                    else
                    {
        ...
        // When we observe a new commit being complete for a surface, we ask for
        // a new frame. This ensures that FIFO works properly, since otherwise
        // we might ask for a new frame application can commit a new frame that
        // completes before we ever displayed the current pending commit.
        ...
        {
            {
                ...
                for ( const auto &xdg_win : g_steamcompmgr_xdg_wins )
                {
                    steamcompmgr_latch_frame_done(
                        xdg_win.get( ), vblank_idx, now );
                }
            }
        }

        {
                  ...
                  i++ )
            {
                handle_done_commits_xwayland(
                    server->ctx.get( ), vblank, vblank_idx );

                // When we have observed both a complete commit and a VBlank, we
                // should request a new frame.
                if ( vblank )
                {
                    for ( steamcompmgr_win_t *w = server->ctx->list; w;
                          w                     = w->xwayland( ).next )
        ...
        if ( vblank )
        {
            ...
            int nRealRefreshmHz =
                g_nNestedRefresh ? g_nNestedRefresh : g_nOutputRefresh;
            g_SteamCompMgrAppRefreshCycle =
                gamescope::mHzToRefreshCycle( nRealRefreshmHz );
            g_SteamCompMgrLimitedAppRefreshCycle =
                g_SteamCompMgrAppRefreshCycle;
            if ( g_nSteamCompMgrTargetFPS )
            {
                int nRealRefreshHz =
                    gamescope::ConvertmHzToHz( nRealRefreshmHz );
                int nTargetFPS = g_nSteamCompMgrTargetFPS;
                     ...
                     GetBackend( )->GetCurrentConnector( )->IsVRRActive( ) )
                {
                    g_SteamCompMgrLimitedAppRefreshCycle =
                        gamescope::mHzToRefreshCycle(
                            gamescope::ConvertHztomHz( nTargetFPS ) );
                }
                else
                {
                    ...
                    g_SteamCompMgrLimitedAppRefreshCycle =
                        g_SteamCompMgrAppRefreshCycle * nVblankDivisor;
                }
            }
        }
        ...
        // Notes:
        //
        // We send the presented event just after the latest latch time possible
        // so PresentWait in Vulkan still returns pretty optimally. The extra
        // 2ms or so can be "display latency" We still provide the predicted TTL
        // refresh time in the presented event though.
        //
        // We ignore or lie most of the flags because they aren't particularly
        // useful for a client to know anyway and it would delay us sending this
        // at an optimal time. (particularly for DXGI frame latency handles
        // under Proton.)
        //
        // The boat is still out as to whether we should do latest latch or
        // pageflip/ttl for the event. For now, going to keep this, and if we
        // change our minds later, it's no big deal.
        //
        // It's a little strange, but we return `presented` for any window not
        // visible and `presented` for anything visible. It's a little
        // disingenuous because we didn't actually show a window if it wasn't
        // visible, but we could! And that is the first opportunity it had. It's
        // confusing but we need this for forward progress.
        ...
        {
            GamescopeAppTextureColorspace current_app_colorspace =
                GAMESCOPE_APP_TEXTURE_COLORSPACE_SRGB;
            std::shared_ptr<gamescope::BackendBlob> app_hdr_metadata = nullptr;
            if ( g_HeldCommits[ HELD_COMMIT_BASE ] != nullptr )
            {
                current_app_colorspace =
            ...
            if ( app_wants_hdr != g_bAppWantsHDRCached )
            {
                ...
                XChangeProperty(
                    root_ctx->dpy,
                    root_ctx->root,
                    root_ctx->atoms.gamescopeColorAppWantsHDRFeedback,
                    XA_CARDINAL,
                    32,
                    PropModeReplace,
                    ( unsigned char * )&app_wants_hdr_prop,
                    1 );
            ...
            if ( app_hdr_metadata != g_ColorMgmt.pending.appHDRMetadata )
            {
                if ( app_hdr_metadata )
                {
                    ...
                    app_hdr_metadata_blob.resize(
                        ( sizeof( hdr_metadata_infoframe ) +
                          ( sizeof( uint32_t ) - 1 ) ) /
                        sizeof( uint32_t ) );
                    memset(
                        app_hdr_metadata_blob.data( ),
                        0,
                        sizeof( uint32_t ) * app_hdr_metadata_blob.size( ) );
                    memcpy(
                        app_hdr_metadata_blob.data( ),
                        &app_hdr_metadata->View<hdr_output_metadata>( )
                             .hdmi_metadata_type1,
                        sizeof( hdr_metadata_infoframe ) );
                    ...
                    XChangeProperty(
                        root_ctx->dpy,
                        root_ctx->root,
                        root_ctx->atoms.gamescopeColorAppHDRMetadataFeedback,
                        XA_CARDINAL,
                        32,
                        PropModeReplace,
                        ( unsigned char * )app_hdr_metadata_blob.data( ),
                        ( int )app_hdr_metadata_blob.size( ) );
                }
                else
                {
                    XDeleteProperty(
        ...
        // Doesn't realllly matter but avoids an extra frame of being on the
        // wrong window.
        for ( auto &iter : g_VirtualConnectorFocuses )
        {
            global_focus_t *pFocus = &iter.second;
        ...
        // XXX(misyl): This is bad! We shouldnt change the upscaler like this at
        // all!!! We should move this to business logic in paint_window or
        // something!
        if ( GetCurrentFocus( ) &&
             window_is_steam( GetCurrentFocus( )->focusWindow ) )
        {
        ...
        if ( is_fading_out( ) ) hasRepaint = true;

        bool bPainted = false;

        static int nIgnoredOverlayRepaints = 0;

        if ( !hasRepaintNonBasePlane ) nIgnoredOverlayRepaints = 0;

        if ( cv_adaptive_sync_ignore_overlay ) nIgnoredOverlayRepaints = 0;
        ...
        for ( auto &iter : g_VirtualConnectorFocuses )
        {
            ...
            if ( pPaintFocus->GetNestedHints( ) && !g_bForceRelativeMouse )
            {
                const bool bImageEmpty =
                    ( pPaintFocus->cursor &&
                      pPaintFocus->cursor->imageEmpty( ) ) &&
                    ( !window_is_steam( pPaintFocus->inputFocusWindow ) );

                const bool bHasPointerConstraint =
                    pPaintFocus->cursor &&
                    pPaintFocus->cursor->IsConstrained( );

                uint32_t uAppId = pPaintFocus->inputFocusWindow
                                      ? pPaintFocus->inputFocusWindow->appID
                                      : 0;

                const bool bExcludedAppId =
                    uAppId && gamescope::Algorithm::Contains(
                                  s_uRelativeMouseFilteredAppids, uAppId );

                const bool bRelativeMouseMode =
                    bImageEmpty && bHasPointerConstraint && !bExcludedAppId;

                pPaintFocus->GetNestedHints( )->SetRelativeMouseMode(
                    bRelativeMouseMode );
            }

            // HACK: Disable tearing if we have an overlay to avoid stutters
            // right now
            // TODO: Fix properly.
            const bool bHasOverlay =
                ( pPaintFocus->overlayWindow &&
                  pPaintFocus->overlayWindow->opacity ) ||
                ( pPaintFocus->externalOverlayWindow &&
                  pPaintFocus->externalOverlayWindow->opacity ) ||
                ( pPaintFocus->overrideWindow && pPaintFocus->focusWindow &&
                  !pPaintFocus->focusWindow->isSteamStreamingClient &&
                  pPaintFocus->overrideWindow->opacity );

            // If we are running behind, allow tearing.

            // A false vblank value means bShouldPaint will resolve to false
            // below, effectively ignoring this flag and losing any request to
            // force a repaint. Don't clear g_bForceRepaint unless vblank is
            // true.
            const bool bForceRepaint =
                vblank && g_bForceRepaint.exchange( false );
            const bool bForceSyncFlip = bForceRepaint || is_fading_out( );

            // If we are compositing, always force sync flips because we
            // currently wait for composition to finish before submitting. If we
            // want to do async + composite, we should set up syncfile stuff and
            // have DRM wait on it.
            const bool bSurfaceWantsAsync =
                ( g_HeldCommits[ HELD_COMMIT_BASE ] != nullptr &&
                  g_HeldCommits[ HELD_COMMIT_BASE ]->async );
            const bool bTearing = cv_tearing_enabled &&
                                  GetBackend( )->SupportsTearing( ) &&
                                  bSurfaceWantsAsync;
            ...
            if ( bForceSyncFlip ) eFlipType = FlipType::Normal;
            else if ( bVRR )
            ...
            else if ( bTearing )
            {
                ...
                if ( nIgnoredOverlayRepaints ) eFlipType = FlipType::Normal;
                if ( bHasOverlay ) // Don't tear if the Steam or perf overlay is
                                   // up atm.
            ...
            if ( true )
            {
                switch ( eFlipType )
                {
                    ...
                    {
                        bShouldPaint =
                            vblank && ( hasRepaint || hasRepaintNonBasePlane ||
                                        bForceSyncFlip );
                        break;
                    }
                    ...
                    case FlipType::VRR:
                    {
                        ...
                        if ( bIsVBlankFromTimer )
                        {
                            if ( hasRepaintNonBasePlane )
                            {
                                if ( nIgnoredOverlayRepaints >=
                                     cv_adaptive_sync_overlay_cycles )
                                {
                                    // If we hit vblank and we previously punted
                                    // on drawing an overlay we should go ahead
                                    // and draw now.
                                    bShouldPaint = true;
                                }
                                else if ( !bShouldPaint )
                                {
                                    // If we hit vblank (ie. fastest refresh
                                    // cycle since last commit), and we aren't
                                    // painting and we have a pending overlay,
                                    // then: defer it until the next game update
                                    // or next true vblank.
                        ...
                        // If we have a pending page flip and doing VRR, lets
                        // not do another...
                        if ( GetBackend( )->GetCurrentConnector( ) &&
                             GetBackend( )
                                     ->GetCurrentConnector( )
                                     ->PresentationFeedback( )
                                     .CurrentPresentsInFlight( ) != 0 )
                            bShouldPaint = false;
...
#if HAVE_PIPEWIRE
            if ( pipewire_is_streaming( ) ) paint_pipewire( );
#endif
        }
        ...
        if ( flush_root ) { XFlush( root_ctx->dpy ); }
...
struct wlr_surface *steamcompmgr_get_server_input_surface( size_t idx )
{
    ...
    if ( server && server->ctx && server->ctx->focus.inputFocusWindow &&
         server->ctx->focus.inputFocusWindow->xwayland( ).surface.main_surface )
        return server->ctx->focus.inputFocusWindow->xwayland( )
            .surface.main_surface;
    return NULL;
}

struct wlserver_x11_surface_info *lookup_x11_surface_info_from_xid(
    gamescope_xwayland_server_t *xwayland_server, uint32_t xid )
{
    if ( !xwayland_server ) return nullptr;

    if ( !xwayland_server->ctx ) return nullptr;
...
{
    gamescope_xwayland_server_t *server = wlserver_get_xwayland_server( idx );
    if ( server && server->ctx ) return server->ctx->cursor.get( );
    return nullptr;
}
