...
inline GamescopeAppTextureColorspace
VkColorSpaceToGamescopeAppTextureColorSpace(
...
public:
    ...
    bool BInit(
        ...
        CVulkanTexture        *pExistingImageToReuseMemory    = nullptr,
        gamescope::OwningRc<gamescope::IBackendFb> pBackendFb = nullptr );
    bool BInitFromSwapchain(
    ...
    inline gamescope::IBackendFb *GetBackendFb( )
    { return m_pBackendFb.get( ); }
    ...
    inline EStreamColorspace streamColorspace( ) const
    { return m_streamColorspace; }
    inline void setStreamColorspace( EStreamColorspace colorspace )
    { m_streamColorspace = colorspace; }
...
struct FrameInfo_t
{
    ...
    struct Layer_t
    {
        ...
        bool isScreenSize( ) const
        {
            return close_enough( scale.x, 1.0f ) &&
                   ...
                   float_is_integer( offset.x ) && float_is_integer( offset.y );
        }

        bool viewConvertsToLinearAutomatically( ) const
        {
            if ( isYcbcr( ) ) return true;
    ...
    uint32_t borderMask( ) const
    {
        ...
        {
            if ( layers[ i ].blackBorder ) result |= 1 << i;
        }
        return result;
    }
    uint32_t ycbcrMask( ) const
    {
        ...
        for ( int i = 0; i < layerCount; i++ )
        {
            if ( layers[ i ].isYcbcr( ) ) result |= 1 << i;
        }
        return result;
    }
    uint32_t colorspaceMask( ) const
    {
        ...
        for ( int i = 0; i < layerCount; i++ )
        {
            result |= layers[ i ].colorspace
                      << ( i * GamescopeAppTextureColorspace_Bits );
        }
        return result;
    }
};
...
std::optional<uint64_t> vulkan_composite(
    ...
    bool                              partial,
    gamescope::Rc<CVulkanTexture>     pOutputOverride  = nullptr,
    bool                              increment        = true,
    std::unique_ptr<CVulkanCmdBuffer> pInCommandBuffer = nullptr );
...
struct gamescope_color_mgmt_t
{
    ...
    // If non-zero, use this as the emulated "virtual" white point for the
    // output
    glm::vec2                  outputVirtualWhite = { 0.f, 0.f };
    EChromaticAdaptationMethod chromaticAdaptationMode =
...
struct gamescope_color_mgmt_luts
{
    ...
    uint16_t
        lut3d[ s_nLutEdgeSize3d * s_nLutEdgeSize3d * s_nLutEdgeSize3d * 4 ];
    uint16_t lut1d[ s_nLutSize1d * 4 ];

    gamescope::Rc<CVulkanTexture> vk_lut3d;
    gamescope::Rc<CVulkanTexture> vk_lut1d;

    bool HasLuts( ) const { return bHasLut3D && bHasLut1D; }
...
struct VulkanOutput_t
{
    ...
    uint32_t nOutImage; // swapchain index in nested mode, or ping/pong between
                        // two RTs
...
struct PipelineInfo_t
{
    ...
    {
        return shaderType == o.shaderType && layerCount == o.layerCount &&
               ycbcrMask == o.ycbcrMask && blurLayerCount == o.blurLayerCount &&
               compositeDebug == o.compositeDebug &&
               colorspaceMask == o.colorspaceMask &&
               outputEOTF == o.outputEOTF && itmEnable == o.itmEnable;
    }
};
...
{
    return old_hash ^
           ( new_hash + 0x9e3779b9 + ( old_hash << 6 ) + ( old_hash >> 2 ) );
}

namespace std
{
    template<> struct hash<PipelineInfo_t>
    {
...
public:
    ...
    VkPipeline pipeline(
        ...
        uint32_t   ycbcrMask       = 0,
        uint32_t   blur_layers     = 0,
        uint32_t   colorspace_mask = 0,
        uint32_t   output_eotf     = EOTF_Gamma22,
        bool       itm_enable      = false );
    ...
    inline VkDescriptorSet descriptorSet( )
    {
        ...
        m_currentDescriptorSet =
            ( m_currentDescriptorSet + 1 ) % m_descriptorSets.size( );
        return ret;
    }

    std::shared_ptr<VulkanTimelineSemaphore_t>
    CreateTimelineSemaphore( uint64_t ulStartingPoint, bool bShared = false );
    std::shared_ptr<VulkanTimelineSemaphore_t>
    ImportTimelineSemaphore( gamescope::CTimeline *pTimeline );
    ...
    inline std::vector<VkExtensionProperties> &supportedExtensions( )
...
public:
    ...
    const std::vector<VulkanTimelinePoint_t> &GetExternalDependencies( ) const
    { return m_ExternalDependencies; }
    const std::vector<VulkanTimelinePoint_t> &GetExternalSignals( ) const
    { return m_ExternalSignals; }
...
uint32_t VulkanFormatToDRM(
    VkFormat vkFormat, std::optional<bool> obHasAlphaOverride = std::nullopt );
