...
namespace gamescope
{
    ...
    {
        if ( !g_pOriginalDisplay ) return nullptr;
        ...
        std::vector<uint32_t> cursorData =
            std::vector<uint32_t>( image->width * image->height );
        for ( uint32_t y = 0; y < image->height; y++ )
        {
            for ( uint32_t x = 0; x < image->width; x++ )
            {
                cursorData[ y * image->width + x ] = static_cast<uint32_t>(
                    image->pixels[ image->height * y + x ] );
            }
        }

        return std::make_shared<INestedHints::CursorInfo>(
            INestedHints::CursorInfo{
