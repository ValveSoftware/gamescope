...
namespace gamescope
{
    ...
    public:
        ...
        std::span<GamescopeFeature> GetFeatures( )
        { return std::span<GamescopeFeature>{ m_Features }; }
        const std::optional<GamescopeActiveDisplayInfo> &GetActiveDisplayInfo( )
        { return m_ActiveDisplayInfo; }
    ...
    private:
        ...
        void Wayland_GamescopeControl_FeatureSupport(
            ...
            uint32_t           uVersion,
            uint32_t           uFlags );
        void Wayland_GamescopeControl_ActiveDisplayInfo(
            gamescope_control *pGamescopeControl,
            const char        *pConnectorName,
            const char        *pDisplayMake,
            const char        *pDisplayModel,
            uint32_t           uDisplayFlags,
            wl_array          *pValidRefreshRatesArray );
        void Wayland_GamescopeControl_ScreenshotTaken(
            gamescope_control *pGamescopeControl, const char *pPath );
        static const gamescope_control_listener s_GamescopeControlListener;

        void Wayland_GamescopePrivate_Log(
            gamescope_private *pGamescopePrivate, const char *pText );
        void Wayland_GamescopePrivate_CommandExecuted(
            gamescope_private *pGamescopePrivate );
    ...
    bool GamescopeCtl::Init( bool bInitControl, bool bInitPrivate )
    {
...

        {
            ...
            wl_registry_add_listener(
                pRegistry, &s_RegistryListener, ( void * )this );
            wl_display_roundtrip( m_pDisplay );
            wl_display_roundtrip( m_pDisplay );

            if ( !( !m_bInitControl || m_pGamescopeControl ) ||
                 !( !m_bInitPrivate || m_pGamescopePrivate ) )
            {
    ...
    bool GamescopeCtl::Execute( std::span<std::string_view> args )
    {
        ...
        gamescope_private_execute(
            m_pGamescopePrivate, szArg1.c_str( ), szArg2.c_str( ) );
        wl_display_roundtrip( m_pDisplay );

        return true;
    }

    void GamescopeCtl::Wayland_Registry_Global(
        wl_registry *pRegistry,
        uint32_t     uName,
        const char  *pInterface,
        uint32_t     uVersion )
    {
        if ( m_bInitControl &&
             !strcmp( pInterface, gamescope_control_interface.name ) )
        {
            m_pGamescopeControl =
                ( decltype( m_pGamescopeControl ) )wl_registry_bind(
                    pRegistry, uName, &gamescope_control_interface, uVersion );
            gamescope_control_add_listener(
                m_pGamescopeControl, &s_GamescopeControlListener, this );
        }
        else if (
            m_bInitPrivate &&
            !strcmp( pInterface, gamescope_private_interface.name ) )
        {
            m_pGamescopePrivate =
                ( decltype( m_pGamescopePrivate ) )wl_registry_bind(
                    pRegistry, uName, &gamescope_private_interface, uVersion );
            gamescope_private_add_listener(
                m_pGamescopePrivate, &s_GamescopePrivateListener, this );
        }
    }

    const wl_registry_listener GamescopeCtl::s_RegistryListener = {
        .global =
            WAYLAND_USERDATA_TO_THIS( GamescopeCtl, Wayland_Registry_Global ),
        .global_remove = WAYLAND_NULL( ),
    };

    void GamescopeCtl::Wayland_GamescopeControl_FeatureSupport(
        gamescope_control *pGamescopeControl,
        uint32_t           uFeature,
        uint32_t           uVersion,
        uint32_t           uFlags )
    {
        gamescope_control_feature eFeature =
            static_cast<gamescope_control_feature>( uFeature );
        if ( eFeature == GAMESCOPE_CONTROL_FEATURE_DONE ) return;
        m_Features.emplace_back(
            GamescopeFeature{ .eFeature = eFeature,
                              .uVersion = uVersion,
                              .uFlags   = uFlags } );
    }
    void GamescopeCtl::Wayland_GamescopeControl_ActiveDisplayInfo(
        gamescope_control *pGamescopeControl,
        const char        *pConnectorName,
        const char        *pDisplayMake,
        const char        *pDisplayModel,
        ...
        wl_array          *pValidRefreshRatesArray )
    {
        const uint32_t *pValidRefreshRates =
            reinterpret_cast<const uint32_t *>( pValidRefreshRatesArray->data );
        std::vector<uint32_t> validRefreshRates;
        for ( size_t i = 0;
              i < pValidRefreshRatesArray->size / sizeof( uint32_t );
              i++ )
    ...
    const gamescope_control_listener
        GamescopeCtl::s_GamescopeControlListener = {
            .feature_support = WAYLAND_USERDATA_TO_THIS(
                GamescopeCtl, Wayland_GamescopeControl_FeatureSupport ),
            .active_display_info = WAYLAND_USERDATA_TO_THIS(
                GamescopeCtl, Wayland_GamescopeControl_ActiveDisplayInfo ),
            .screenshot_taken = WAYLAND_USERDATA_TO_THIS(
                GamescopeCtl, Wayland_GamescopeControl_ScreenshotTaken ),
        };

    void GamescopeCtl::Wayland_GamescopePrivate_Log(
        gamescope_private *pGamescopePrivate, const char *pText )
    { fprintf( stderr, "%s\n", pText ); }

    void GamescopeCtl::Wayland_GamescopePrivate_CommandExecuted(
        gamescope_private *pGamescopePrivate )
    { m_uCommandCount++; }

    const gamescope_private_listener
        GamescopeCtl::s_GamescopePrivateListener = {
            .log = WAYLAND_USERDATA_TO_THIS(
                GamescopeCtl, Wayland_GamescopePrivate_Log ),
            .command_executed = WAYLAND_USERDATA_TO_THIS(
                GamescopeCtl, Wayland_GamescopePrivate_CommandExecuted ),
        };
    ...
    static int RunGamescopeCtl( int argc, char *argv[] )
    {
        ...
        if ( !gamescopeCtl.Init( bInfoOnly, !bInfoOnly ) ) return 1;

        if ( bInfoOnly )
        {
            PrintVersion( );
            fprintf( stdout, "gamescope_control info:\n" );
            const auto &oActiveDisplayInfo =
                gamescopeCtl.GetActiveDisplayInfo( );
            if ( oActiveDisplayInfo )
            {
                fprintf(
                    stdout,
                    "  - Connector Name: %.*s\n",
                    ( int )oActiveDisplayInfo->szConnectorName.length( ),
                    oActiveDisplayInfo->szConnectorName.data( ) );
                fprintf(
                    stdout,
                    "  - Display Make: %.*s\n",
                    ( int )oActiveDisplayInfo->szDisplayMake.length( ),
                    oActiveDisplayInfo->szDisplayMake.data( ) );
                fprintf(
                    stdout,
                    "  - Display Model: %.*s\n",
                    ( int )oActiveDisplayInfo->szDisplayModel.length( ),
                    oActiveDisplayInfo->szDisplayModel.data( ) );
                fprintf(
                    stdout,
                    "  - Display Flags: 0x%x\n",
                ...
                for ( size_t i = 0;
                      i < oActiveDisplayInfo->ValidRefreshRates.size( );
                      i++ )
                {
                    bool bLast =
                        i == oActiveDisplayInfo->ValidRefreshRates.size( ) - 1;
            ...
            for ( const GamescopeFeature &feature :
                  gamescopeCtl.GetFeatures( ) )
            {
                std::string_view szFeatureName =
                    GetFeatureName( feature.eFeature );
                fprintf(
                    stdout,
                    "  - %.*s (%u) - Version: %u - Flags: 0x%x\n",
                    ( int )szFeatureName.size( ),
                    szFeatureName.data( ),
                    uint32_t{ feature.eFeature },
                    feature.uVersion,
                    feature.uFlags );
            }
            fprintf(
                stdout,
                "You can execute any debug command in Gamescope using this "
                "tool.\n" );
            fprintf(
                stdout,
                "For a list of commands and convars, use 'gamescopectl "
                "help'\n" );
            return 0;
        }
        ...
        if ( !gamescopeCtl.Execute( std::span{ args } ) ) return 1;
