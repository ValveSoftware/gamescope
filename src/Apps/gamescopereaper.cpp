...
namespace gamescope
{
    ...
    //
    // Gamescope can have a lot of bad things happen to it, crashes, segfaults,
    // whatever but we always want to make sure that we cleanly kill all of our
    // children when we die. This child process attempts to stay alive as long
    // as it can in order to fulfil Gamescope's dying wish -- to kill all of
    // it's children.
    int GamescopeReaperProcess( int argc, char **argv )
    {
        ...
        int nOptIndex = -1;
        int nOption   = -1;
        while ( ( nOption = getopt_long(
                      argc, argv, "", k_ReaperOptions, &nOptIndex ) ) != -1 )
        {
            if ( nOption == '?' ) { s_ReaperLog.errorf( "Unknown option." ); }
            assert( nOption == 0 );
            ...
            else if ( !strcmp( pszOptionName, "respawn" ) ) { bRespawn = true; }
        ...
        // We typically don't make a new sid, as we want to keep the same
        // stdin/stdout Don't really care about it for pgroup reasons, as
        // processes can leave those.
        if ( bNewSession ) setsid( );
        ...
        reaperSignalHandler.sa_handler = []( int nSignal )
        {
            switch ( nSignal )
            {
                ...
                case SIGTERM:
                    ...
                    {
                        s_ReaperLog.infof(
                            "Parent of gamescopereaper was killed. Killing "
                            "children." );
        ...
        if ( nPrimaryChild > 0 )
        {
            // Wait for the primary child to die, then forward the death signal
            // to all of the other children, if we aren't in a PID namespace.
            ...
            if ( bRespawn )
            {
                ...
                {
                    s_ReaperLog.infof(
                        "\"%s\" process shut down. Restarting.",
                        argv[ nSubCommandArgc ] );

                    nPrimaryChild =
                        Process::SpawnProcess( &argv[ nSubCommandArgc ] );
                    Process::WaitForAllChildren( nPrimaryChild );
                }
            }
        ...
        {
            s_ReaperLog.errorf_errno(
                "Failed to create child process \"%s\" in reaper.",
                argv[ nSubCommandArgc ] );
