...
#include "gamescope-input-method-client-protocol.h"
...
static void ime_handle_done(
    void *data, struct gamescope_input_method *ime, uint32_t serial )
{
    ( void )data;
    ( void )ime;
    ime_serial = serial;
...
static void
ime_handle_unavailable( void *data, struct gamescope_input_method *ime )
{
    ( void )data;
    ( void )ime;
    ...
    uint32_t            version )
{
    ...
    {
        ime_manager = wl_registry_bind(
            registry, name, &gamescope_input_method_manager_interface, 2 );
    }
...
int main( )
{
    ...
    if ( !display_name ) display_name = "gamescope-0";
    ...
    {
        fprintf(
            stderr, "Compositor doesn't support input-method-unstable-v2\n" );
        return 1;
    }

    struct gamescope_input_method *ime =
        gamescope_input_method_manager_create_input_method( ime_manager, seat );
    ...
    for ( ;; )
    {
            ...
            switch ( ch )
            {
                case 'A':
                    gamescope_input_method_set_action(
                        ime, GAMESCOPE_INPUT_METHOD_ACTION_MOVE_UP );
                    gamescope_input_method_commit( ime, ime_serial );
                    break;
                case 'B':
                    gamescope_input_method_set_action(
                        ime, GAMESCOPE_INPUT_METHOD_ACTION_MOVE_DOWN );
                    gamescope_input_method_commit( ime, ime_serial );
                    break;
                case 'C':
                    gamescope_input_method_set_action(
                        ime, GAMESCOPE_INPUT_METHOD_ACTION_MOVE_RIGHT );
                    gamescope_input_method_commit( ime, ime_serial );
                    break;
                case 'D':
                    gamescope_input_method_set_action(
                        ime, GAMESCOPE_INPUT_METHOD_ACTION_MOVE_LEFT );
                    gamescope_input_method_commit( ime, ime_serial );
                    break;
                case '3':
                    gamescope_input_method_set_action(
                        ime, GAMESCOPE_INPUT_METHOD_ACTION_DELETE_RIGHT );
                    gamescope_input_method_commit( ime, ime_serial );
                    getch( );
                    break;
                case '5':
        ...
        if ( ch == '\b' || ch == 0x7f ) // delete
            gamescope_input_method_set_action(
                ime, GAMESCOPE_INPUT_METHOD_ACTION_DELETE_LEFT );
        else if ( ch == '\r' )
            continue;
        else if ( ch == '\n' )
            gamescope_input_method_set_action(
