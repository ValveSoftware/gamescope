...
namespace gamescope
{
    ...
    public:
        bool Init(
            gamescope_action_binding_manager *pManager,
            std::span<uint32_t>               pKeySyms )
        {
            Shutdown( );

            m_pBinding = gamescope_action_binding_manager_create_action_binding(
                pManager );
            if ( !m_pBinding ) return false;
            ...
            gamescope_action_binding_add_listener(
                m_pBinding, &s_BindingListener, ( void * )this );
            gamescope_action_binding_add_keyboard_trigger( m_pBinding, &array );
            gamescope_action_binding_set_description(
                m_pBinding, "My Example Hotkey :)" );
        ...
        void Wayland_Triggered(
            gamescope_action_binding *pBinding,
            uint32_t                  uSequence,
    ...
    const gamescope_action_binding_listener
        CActionBinding::s_BindingListener = {
            .triggered =
                WAYLAND_USERDATA_TO_THIS( CActionBinding, Wayland_Triggered ),
        };
    ...
    void GamescopeHotkeyExample::Run( )
    {
        ...
        std::vector<uint32_t> uKeySyms = {
            0xffe1, 0x0070
        }; // XKB_KEY_Shift_L + XKB_KEY_p

        CActionBinding binding;
        if ( !binding.Init( m_pActionBindingManager, uKeySyms ) ) return;
    ...
    void GamescopeHotkeyExample::Wayland_Registry_Global(
        wl_registry *pRegistry,
        uint32_t     uName,
        const char  *pInterface,
        uint32_t     uVersion )
    {
        if ( !strcmp(
                 pInterface, gamescope_action_binding_manager_interface.name ) )
        {
            m_pActionBindingManager =
                ( decltype( m_pActionBindingManager ) )wl_registry_bind(
                    pRegistry,
                    uName,
                    &gamescope_action_binding_manager_interface,
                    uVersion );
        }
    }
    ...
    const wl_registry_listener GamescopeHotkeyExample::s_RegistryListener = {
        .global = WAYLAND_USERDATA_TO_THIS(
            GamescopeHotkeyExample, Wayland_Registry_Global ),
    ...
    static int RunHotkeyExample( int argc, char *argv[] )
    {
        ...
        if ( !hotkeyExample.Init( ) ) return 1;
