///////////////////////////////////////////////////////////////////////////////////////////////////////
// Gracefully butchered from
// https://docs.pipewire.org/spa_2examples_2adapter-control_8c-example.html by
// Wim Taymans under MIT
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
void spa_gamescopestream_log(
    struct spa_debug_context *ctx, const char *fmt, ... )
{
    va_list args;
    va_start( args, fmt );
...
static void handle_events( struct data *pData )
{
    ...
    if ( wl_display_prepare_read( pData->pDisplay ) == 0 )
    {
        ...
        if ( nRet > 0 ) wl_display_read_events( pData->pDisplay );
        else
...
static struct spa_pod *build_format(
    struct data            *data,
    ...
    int                     modifier_count )
{
    ...
    spa_pod_builder_add(
        b, SPA_FORMAT_mediaType, SPA_POD_Id( SPA_MEDIA_TYPE_video ), 0 );
    spa_pod_builder_add(
        b, SPA_FORMAT_mediaSubtype, SPA_POD_Id( SPA_MEDIA_SUBTYPE_raw ), 0 );
    /* format */
    spa_pod_builder_add( b, SPA_FORMAT_VIDEO_format, SPA_POD_Id( format ), 0 );
    /* modifiers */
    if ( modifier_count == 1 && modifiers[ 0 ] == DRM_FORMAT_MOD_INVALID )
    {
        // we only support implicit modifiers, use shortpath to skip fixation
        // phase
    ...
    else if ( modifier_count > 0 )
    {
        ...
        for ( i = 0, c = 0; i < modifier_count; i++ )
        {
            ...
            if ( c++ == 0 ) spa_pod_builder_long( b, modifiers[ i ] );
        }
    ...
    spa_pod_builder_add(
        ...
        SPA_POD_CHOICE_RANGE_Fraction( &frac1, &frac2, &frac3 ),
        0 );
    ...
    if ( data->appid )
        spa_pod_builder_add(
            ...
            SPA_POD_Long( uint64_t( data->appid ) ),
            0 );

    return ( spa_pod * )spa_pod_builder_pop( b, &f[ 0 ] );
}

void commit_libdecor(
    struct data *data, libdecor_configuration *pConfiguration )
{
...
static void on_process( void *_data )
{
    ...
    {
        struct pw_buffer *t;
        if ( ( t = pw_stream_dequeue_buffer( stream ) ) == nullptr ) break;
        if ( b ) pw_stream_queue_buffer( stream, b );
        b = t;
    }
    if ( b == nullptr )
    {
    ...
    zwp_linux_buffer_params_v1 *pBufferParams =
        zwp_linux_dmabuf_v1_create_params( data->pLinuxDmabuf );
    if ( !pBufferParams )
    {
        pw_stream_queue_buffer( stream, b );
        return;
    ...
    static constexpr wl_buffer_listener s_BufferListener = {
        .release =
            []( void *pData, wl_buffer *pBuffer )
        {
            StreamBuffer *pStreamBuffer = ( StreamBuffer * )pData;
            pw_stream_queue_buffer(
                pStreamBuffer->pData->stream, pStreamBuffer->pPipewireBuffer );
    ...
    if ( data->needs_decor_commit ) commit_libdecor( data, nullptr );
    ...
    const char          *error )
{
    struct data *data = ( struct data * )_data;
    s_StreamLog.debugf(
        "stream state: \"%s\"", pw_stream_state_as_string( state ) );
    if ( error ) s_StreamLog.errorf( "error: \"%s\"", error );
    switch ( state )
    {
...
on_stream_param_changed( void *_data, uint32_t id, const struct spa_pod *param )
{
    ...
    if ( spa_format_parse(
             param, &data->format.media_type, &data->format.media_subtype ) <
         0 )
        return;
    ...
    const struct spa_pod  **params )
{
    ...
    if ( data->m_FormatModifiers.contains( DRM_FORMAT_XRGB8888 ) )
        params[ n_params++ ] = build_format(
            ...
            data->m_FormatModifiers[ DRM_FORMAT_XRGB8888 ].data( ),
            uint32_t(
                data->m_FormatModifiers[ DRM_FORMAT_XRGB8888 ].size( ) ) );
    params[ n_params++ ] =
        build_format( data, b, SPA_VIDEO_FORMAT_BGRx, nullptr, 0 );
...
static void reneg_format( void *_data, uint64_t expiration )
{
    ...
    if ( data->format.info.raw.format == 0 ) return;
...
int main( int argc, char *argv[] )
{
    ...
    pw_loop_add_signal(
        pw_main_loop_get_loop( data.loop ), SIGINT, do_quit, &data );
    pw_loop_add_signal(
        pw_main_loop_get_loop( data.loop ), SIGTERM, do_quit, &data );
    ...
    if ( data.path ) /* Set stream target if given on command line */
    ...
    if ( !( data.pDisplay = wl_display_connect( nullptr ) ) ) return -1;

    wl_registry *pRegistry;
    if ( !( pRegistry = wl_display_get_registry( data.pDisplay ) ) ) return -1;
    ...
    static constexpr wl_registry_listener s_RegistryListener = {
                ...
                uint32_t     uVersion )
        {
            ...
            if ( !strcmp( pInterface, wl_compositor_interface.name ) &&
                 uVersion >= 4u )
            {
                pData->pCompositor = ( wl_compositor * )wl_registry_bind(
                ...
                uVersion >= 3 )
            {
                ...
                static constexpr zwp_linux_dmabuf_v1_listener s_Listener = {
                    ...
                    {
                        uint64_t ulModifier =
                            ( uint64_t( uModifierHi ) << 32 ) | uModifierLo;

                        struct data *pData = ( struct data * )pUserData;
                        if ( ulModifier != DRM_FORMAT_MOD_INVALID )
                            pData->m_FormatModifiers[ uFormat ].emplace_back(
                                ulModifier );
                    },
                };
                zwp_linux_dmabuf_v1_add_listener(
                    pData->pLinuxDmabuf, &s_Listener, pData );
            }
        },
    ...
    if ( !data.pCompositor || !data.pLinuxDmabuf ) return -1;
    ...
    if ( !data.pDecor ) return -1;

    static libdecor_frame_interface s_LibDecorFrameInterface{
        .configure =
            []( libdecor_frame         *pFrame,
                libdecor_configuration *pConfiguration,
                void                   *pUserData )
        {
            struct data *pData = ( struct data * )pUserData;
        ...
        { raise( SIGTERM ); },
        .commit =
            []( libdecor_frame *pFrame, void *pUserData )
        {
            struct data *pData        = ( struct data * )pUserData;
            pData->needs_decor_commit = true;
        },
        .dismiss_popup = []( libdecor_frame *pFrame,
                             const char     *pSeatName,
                             void           *pUserData ) {},
    };
    data.pSurface = wl_compositor_create_surface( data.pCompositor );
    data.pFrame   = libdecor_decorate(
        data.pDecor, data.pSurface, &s_LibDecorFrameInterface, &data );
    ...
    if ( ( res = pw_stream_connect(
               ...
               PW_ID_ANY,
               pw_stream_flags(
                   PW_STREAM_FLAG_AUTOCONNECT |  /* try to automatically connect
                                                    this stream */
                   PW_STREAM_FLAG_MAP_BUFFERS ), /* mmap the buffer data for us
                                                  */
               params,
               n_params ) ) /* extra parameters, see above */
         < 0 )
    {
    ...
    data.reneg = pw_loop_add_event(
        pw_main_loop_get_loop( data.loop ), reneg_format, &data );
