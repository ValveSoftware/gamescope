...
namespace messagey
{
    ...
    {
        int fd_pipe[ 2 ]; /* fd_pipe[0]: read end of pipe, fd_pipe[1]: write end
                             of pipe */
        pid_t pid1;

        const char *zenityDisableEnv = getenv( "GAMESCOPE_ZENITY_DISABLE" );
        if ( zenityDisableEnv && *zenityDisableEnv &&
             atoi( zenityDisableEnv ) != 0 )
            return 0;
        ...
        { /* child process */
            int         argc = 5, i;
            const char *argv
                [ 5 + 2 /* icon name */ + 2 /* title */ + 2 /* message */ +
                  2 * MaxButtons + 1 /* nullptr */ ] = { "zenity",
                                                         "--question",
                                                         "--switch",
                                                         "--no-wrap",
                                                         "--no-markup" };
            ...
            if ( dup2( fd_pipe[ 1 ], STDOUT_FILENO ) == -1 ) { _exit( 128 ); }
            ...
            for ( i = 0; i < messageboxdata->numbuttons; ++i )
            {
                if ( messageboxdata->buttons[ i ].text &&
                     messageboxdata->buttons[ i ].text[ 0 ] )
                {
        ...
        else
        {
            ...
            if ( waitpid( pid1, &status, 0 ) == pid1 )
            {
                if ( WIFEXITED( status ) )
                {
                    if ( WEXITSTATUS( status ) < 128 )
                    {
                        ...
                        /* At this point, if no button ID is needed, we can just
                         * bail as soon as the process has completed.
                         */
                        if ( buttonid == NULL )
                        {
                            close( fd_pipe[ 0 ] );
                        ...
                        *buttonid = -1;
                        ...
                        for ( i = 0; i < messageboxdata->numbuttons; ++i )
                        {
                            ...
                            {
                                const size_t button_len =
                                    strlen( messageboxdata->buttons[ i ].text );
                        ...
                        if ( !stdout )
                        {
                            ...
                            close( fd_pipe[ 0 ] );
                            return SetError(
                                "Couldn't open pipe for reading: %s",
                                strerror( errno ) );
                        }
                    ...
                    {
                        return SetError(
                            "zenity reported error or failed to launch: %d",
                            WEXITSTATUS( status ) );
                    }
                }
                else
            ...
            {
                return SetError(
                    "Waiting on zenity failed: %s", strerror( errno ) );
            }
        }
    }

    inline int ShowSimple(
        const char     *message,
        const char     *caption,
        ...
        int            *buttonid )
    {
        ...
        {
            buttons[ buttonCount++ ] = {
                .flags = MessageBoxButtonFlag::EscapeKeyDefault |
                         ( hasOK ? 0 : MessageBoxButtonFlag::ReturnKeyDefault ),
                .buttonid = 0,
                .text     = "Cancel",
            };
        }
