...
static size_t utf8_size( const char *str )
{
    ...
    if ( u8 == 0 ) { return 0; }
    else if ( ( u8 & 0x80 ) == 0 ) { return 1; }
    else if ( ( u8 & 0xE0 ) == 0xC0 ) { return 2; }
    else if ( ( u8 & 0xF0 ) == 0xE0 ) { return 3; }
    else if ( ( u8 & 0xF8 ) == 0xF0 ) { return 4; }
    else
    {
        return 0;
    }
...
static std::unordered_map<
    ...
    actions = {
        ...
        { GAMESCOPE_INPUT_METHOD_ACTION_DELETE_RIGHT,
...
{
    // There's a bug in libxkbcommon where the EURO symbol doesn't map to the
    // correct keysym
    if ( ch == 0x20ac ) { return XKB_KEY_EuroSign; }
    if ( ( ch >= 0x3130 && ch <= 0x318f ) // Hangul Compatibility Jamo that CEF
                                          // does not like as keysyms
         || ( ch >= 0x11A8 && ch <= 0x11C2 ) // subset of Hangul Jamo that CEF
                                             // does not like as keysyms
         || ( ch == 0x11EB )                 // continued
         || ( ch == 0x11F0 )                 // continued
         || ( ch == 0x11F9 )                 // continued
...
static uint32_t
keycode_from_ch( struct wlserver_input_method *ime, uint32_t ch )
{
    xkb_keysym_t keysym = keysym_from_ch( ch );
    if ( keysym == XKB_KEY_NoSymbol ) { return XKB_KEYCODE_INVALID; }
    ...
    uint32_t keycode =
        allow_keycodes[ ime->next_keycode_index++ % allow_keycodes_len ];
    ime->keys.push_back(
        ( struct wlserver_input_method_key ){ keycode, keysym } );
    return keycode;
}

static bool
generate_keymap_key( FILE *f, uint32_t keycode, xkb_keysym_t keysym )
{
...
static int release_key_if_needed( void *data )
{
    ...
    if ( ime->held_keycode >= 0 )
        wlr_seat_keyboard_notify_key(
            seat, 0, ime->held_keycode, WL_KEYBOARD_KEY_STATE_RELEASED );
    ime->held_keycode = -1;
    ...
    if ( ime->held_modifier_mask )
    {
        if ( ime->held_modifier_mask & WLR_MODIFIER_ALT )
            wlr_seat_keyboard_notify_key(
                seat, 0, KEY_LEFTALT, WL_KEYBOARD_KEY_STATE_RELEASED );
        ...
        if ( ime->held_modifier_mask & WLR_MODIFIER_SHIFT )
            wlr_seat_keyboard_notify_key(
                seat, 0, KEY_LEFTSHIFT, WL_KEYBOARD_KEY_STATE_RELEASED );
...
static void press_key(
    struct wlserver_input_method  *ime,
    uint32_t                       keycode,
    struct wlr_keyboard_modifiers *pmods = nullptr )
{
    struct wlr_seat *seat = ime->manager->server->wlr.seat;
    ...
    if ( pmods )
    {
        ...
        if ( ime->held_modifier_mask & WLR_MODIFIER_SHIFT )
            wlr_seat_keyboard_notify_key(
                seat, 0, KEY_LEFTSHIFT, WL_KEYBOARD_KEY_STATE_PRESSED );
        if ( ime->held_modifier_mask & WLR_MODIFIER_CTRL )
            wlr_seat_keyboard_notify_key(
                seat, 0, KEY_LEFTCTRL, WL_KEYBOARD_KEY_STATE_PRESSED );
        if ( ime->held_modifier_mask & WLR_MODIFIER_ALT )
            wlr_seat_keyboard_notify_key(
                seat, 0, KEY_LEFTALT, WL_KEYBOARD_KEY_STATE_PRESSED );
    }

    // Note: Xwayland doesn't care about the time field of the events
    wlr_seat_keyboard_notify_key(
        seat, 0, keycode, WL_KEYBOARD_KEY_STATE_PRESSED );
    ime->held_keycode = keycode;

    wl_event_source_timer_update(
        ime->ime_release_ime_keypress_event_source, 30 /* ms */ );
}

static bool
try_type_keysym( struct wlserver_input_method *ime, xkb_keysym_t keysym )
{
    struct wlr_seat     *seat = ime->manager->server->wlr.seat;
    struct wlr_keyboard *keyboard =
        ime->manager->server->wlr.virtual_keyboard_device;
          ...
          keycode++ )
    {
        ...
        for ( xkb_layout_index_t layout = 0; layout < num_layouts; layout++ )
        {
            ...
            for ( xkb_level_index_t level = 0; level < num_levels; level++ )
            {
                ...
                if ( num_syms != 1 ) { continue; }
                if ( syms[ 0 ] != keysym ) { continue; }

                xkb_mod_mask_t mask;
                size_t         num_masks = xkb_keymap_key_get_mods_for_level(
                    keymap, keycode, layout, level, &mask, 1 );
                if ( num_masks != 1 ) { continue; }

                xkb_mod_mask_t allowed =
                    WLR_MODIFIER_SHIFT | WLR_MODIFIER_CTRL | WLR_MODIFIER_ALT;
                if ( ( mask & allowed ) != mask ) { continue; }
...
void type_text( struct wlserver_input_method *ime, const char *text )
{
    ...
    wl_event_source_timer_update(
        ime->ime_reset_ime_keyboard_event_source, 100 /* ms */ );
}
    ...
    enum gamescope_input_method_action action )
{
    ...
    if ( ime->keys.empty( ) && try_type_keysym( ime, key.keysym ) ) { return; }
...
static void ime_handle_commit(
    ...
    struct wl_resource *ime_resource,
    uint32_t            serial )
{
    struct wlserver_input_method *ime =
        ( struct wlserver_input_method * )wl_resource_get_user_data(
            ime_resource );

    if ( serial != ime->serial ) { return; }
    ...
    wl_event_source_timer_update(
        ime->manager->ime_reset_keyboard_event_source, 100 /* ms */ );
}

static void ime_handle_set_string(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    const char         *text )
{
...
static void ime_handle_set_action(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    uint32_t            action )
{
...
static void
ime_handle_destroy( struct wl_client *client, struct wl_resource *ime_resource )
{ wl_resource_destroy( ime_resource ); }

static void ime_handle_pointer_motion(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    wl_fixed_t          dx,
    wl_fixed_t          dy )
...
static void ime_handle_pointer_warp(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    wl_fixed_t          x,
    wl_fixed_t          y )
...
static void ime_handle_pointer_wheel(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    int32_t             x,
    int32_t             y )
...
static void ime_handle_pointer_button(
    struct wl_client   *client,
    struct wl_resource *ime_resource,
    uint32_t            button,
    uint32_t            state )
{
    ...
    wlserver_mousebutton(
        button,
        state == GAMESCOPE_INPUT_METHOD_BUTTON_STATE_PRESSED,
        ++ime->uFakeTimestamp );
}
...
static int reset_ime_keyboard( void *data )
{
    ...
    release_key_if_needed( ime );
    ime->keys.clear( );
    ime->next_keycode_index =
        0; // preserve old behavior; could just let this keep going

    return 0;
}

static void manager_handle_create_input_method(
    struct wl_client   *client,
...
{
    struct wlserver_input_method_manager *manager =
        ( struct wlserver_input_method_manager * )wl_resource_get_user_data(
            manager_resource );

    uint32_t            version = wl_resource_get_version( manager_resource );
    struct wl_resource *ime_resource = wl_resource_create(
        client, &gamescope_input_method_interface, version, id );
    wl_resource_set_implementation(
        ime_resource, &ime_impl, nullptr, ime_handle_resource_destroy );
    ...
    ime->ime_reset_ime_keyboard_event_source = wl_event_loop_add_timer(
        manager->server->event_loop, reset_ime_keyboard, ime );
    ime->ime_release_ime_keypress_event_source = wl_event_loop_add_timer(
        manager->server->event_loop, release_key_if_needed, ime );
}
...
struct wlserver_input_method *create_local_ime( )
{
    ...
    ime->ime_reset_ime_keyboard_event_source = wl_event_loop_add_timer(
        global_manager->server->event_loop, reset_ime_keyboard, ime );
    ime->ime_release_ime_keypress_event_source = wl_event_loop_add_timer(
        global_manager->server->event_loop, release_key_if_needed, ime );
...
{
    struct wlserver_input_method_manager *manager =
        ( struct wlserver_input_method_manager * )data;

    struct wl_resource *resource = wl_resource_create(
        client, &gamescope_input_method_manager_interface, version, id );
...
static int reset_keyboard( void *data )
{
    ...
    if ( seat->keyboard_state.keyboard == nullptr ||
         seat->keyboard_state.keyboard->data == nullptr )
    {
...
{
    struct wlserver_input_method_manager *manager =
        new wlserver_input_method_manager( );
    manager->server = wlserver;
    manager->global = wl_global_create(
        wlserver->display,
        &gamescope_input_method_manager_interface,
        IME_MANAGER_VERSION,
        manager,
        manager_bind );
    manager->ime_reset_keyboard_event_source = wl_event_loop_add_timer(
        wlserver->event_loop, reset_keyboard, wlserver );
