...
namespace gamescope
{
    ...
    const libinput_interface CLibInputHandler::s_LibInputInterface = {
        .open_restricted =
            []( const char *pszPath, int nFlags, void *pUserData ) -> int
    ...
    bool CLibInputHandler::Init( )
    {
        ...
        m_pLibInput = libinput_udev_create_context(
            &s_LibInputInterface, nullptr, m_pUdev );
        if ( !m_pLibInput )
        {
        ...
        {
            log_input_stealer.errorf(
                "Could not assign seat \"%s\"", pszSeatName );
            return false;
        }
    ...
    void CLibInputHandler::OnPollIn( )
    {
        ...
        while ( libinput_event *pEvent = libinput_get_event( m_pLibInput ) )
        {
            ...
            switch ( eEventType )
            {
                ...
                {
                    libinput_event_pointer *pPointerEvent =
                        libinput_event_get_pointer_event( pEvent );

                    double flDx =
                        libinput_event_pointer_get_dx( pPointerEvent );
                    double flDy =
                        libinput_event_pointer_get_dy( pPointerEvent );
                ...
                case LIBINPUT_EVENT_POINTER_MOTION_ABSOLUTE:
                {
                    ...
                    double flX =
                        libinput_event_pointer_get_absolute_x( pPointerEvent );
                    double flY =
                        libinput_event_pointer_get_absolute_y( pPointerEvent );
                ...
                case LIBINPUT_EVENT_POINTER_BUTTON:
                {
                    ...
                    uint32_t uButton =
                        libinput_event_pointer_get_button( pPointerEvent );
                    libinput_button_state eButtonState =
                    ...
                    wlserver_mousebutton(
                        uButton,
                        eButtonState == LIBINPUT_BUTTON_STATE_PRESSED,
                        ++s_uSequence );
                    wlserver_unlock( );
                }
                break;
                ...
                {
                    libinput_event_keyboard *pKeyboardEvent =
                        libinput_event_get_keyboard_event( pEvent );
                    uint32_t uKey =
                        libinput_event_keyboard_get_key( pKeyboardEvent );
                    libinput_key_state eState =
                        libinput_event_keyboard_get_key_state( pKeyboardEvent );

                    wlserver_lock( );
                    wlserver_key(
                        uKey,
                        eState == LIBINPUT_KEY_STATE_PRESSED,
                        ++s_uSequence );
                    wlserver_unlock( );
                }
                break;
