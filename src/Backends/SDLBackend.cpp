...
namespace gamescope
{
    ...
    public:
        ...
        virtual const char *GetName( ) const override { return "SDLWindow"; }
    ...
    private:
        ...
        std::atomic<SDLInitState> m_eSDLInit = {
            SDLInitState::SDLInit_Waiting
        };

        std::atomic<bool> m_bApplicationGrabbed = { false };
        std::atomic<bool> m_bApplicationVisible = { false };
        std::atomic<std::shared_ptr<INestedHints::CursorInfo>>
                                                  m_pApplicationCursor;
    ...
    bool CSDLConnector::Init( )
    {
        ...
        uint32_t uSDLWindowFlags = SDL_WINDOW_VULKAN | SDL_WINDOW_RESIZABLE |
        ...
        if ( !SDL_Vulkan_CreateSurface(
                 m_pWindow, vulkan_get_instance( ), &m_pVkSurface ) )
        {
            fprintf(
                stderr,
    ...
    int CSDLConnector::Present( const FrameInfo_t *pFrameInfo, bool bAsync )
    {
        ...
        if ( !oCompositeResult ) return -EINVAL;
    ...
    void CSDLConnector::SetCursorImage(
    ...
    void
    CSDLConnector::SetIcon( std::shared_ptr<std::vector<uint32_t>> uIconPixels )
    { m_pBackend->SetIcon( std::move( uIconPixels ) ); }

    void CSDLConnector::SetSelection(
    ...
    bool CSDLBackend::PostInit( ) { return true; }

    std::span<const char *const> CSDLBackend::GetInstanceExtensions( ) const
    {
        return std::span<const char *const>{ m_pszInstanceExtensions.begin( ),
                                             m_pszInstanceExtensions.end( ) };
    }

    std::span<const char *const>
    CSDLBackend::GetDeviceExtensions( VkPhysicalDevice pVkPhysicalDevice ) const
    { return std::span<const char *const>{}; }

    VkImageLayout CSDLBackend::GetPresentLayout( ) const
    { return VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; }

    void CSDLBackend::GetPreferredOutputFormat(
    ...
    bool
    CSDLBackend::ValidPhysicalDevice( VkPhysicalDevice pVkPhysicalDevice ) const
    { return true; }

    void CSDLBackend::DirtyState( bool bForce, bool bForceModeset ) {}
    bool CSDLBackend::PollState( ) { return false; }

    std::shared_ptr<BackendBlob> CSDLBackend::CreateBackendBlob(
        const std::type_info &type, std::span<const uint8_t> data )
    { return std::make_shared<BackendBlob>( data ); }

    OwningRc<IBackendFb>
    CSDLBackend::ImportDmabufToBackend( wlr_dmabuf_attributes *pDmaBuf )
    { return new CBaseBackendFb( ); }

    bool CSDLBackend::UsesModifiers( ) const { return false; }
    std::span<const uint64_t>
    CSDLBackend::GetSupportedModifiers( uint32_t uDrmFormat ) const
    { return std::span<const uint64_t>{}; }

    IBackendConnector *CSDLBackend::GetCurrentConnector( )
    { return &m_Connector; }
    IBackendConnector *
    CSDLBackend::GetConnector( GamescopeScreenType eScreenType )
    {
    ...
    bool CSDLBackend::SupportsTearing( ) const { return false; }
    bool CSDLBackend::UsesVulkanSwapchain( ) const { return true; }

    bool CSDLBackend::IsSessionBased( ) const { return false; }
    ...
    bool CSDLBackend::IsPaused( ) const { return false; }

    bool CSDLBackend::IsVisible( ) const { return true; }
    ...
    void CSDLBackend::SetCursorImage(
        std::shared_ptr<INestedHints::CursorInfo> info )
    {
    ...
    void
    CSDLBackend::SetIcon( std::shared_ptr<std::vector<uint32_t>> uIconPixels )
    {
        m_pApplicationIcon = uIconPixels;
        PushUserEvent( GAMESCOPE_SDL_EVENT_ICON );
    }

    void CSDLBackend::SetSelection(
        std::shared_ptr<std::string> szContents, GamescopeSelection eSelection )
    {
        if ( eSelection == GAMESCOPE_SELECTION_CLIPBOARD )
            SDL_SetClipboardText( szContents->c_str( ) );
    ...
    void CSDLBackend::SDLThreadFunc( )
    {
        ...
        if ( SDL_Vulkan_LoadLibrary( nullptr ) != 0 )
        {
            fprintf(
                stderr,
                "SDL_Vulkan_LoadLibrary failed: %s\n",
                SDL_GetError( ) );
            m_eSDLInit = SDLInitState::SDLInit_Failure;
            m_eSDLInit.notify_all( );
            return;
        ...
        SDL_Vulkan_GetInstanceExtensions(
            nullptr, &uExtCount, m_pszInstanceExtensions.data( ) );
        ...
        if ( !vulkan_init(
                 vulkan_get_instance( ), m_Connector.GetVulkanSurface( ) ) )
        {
            m_eSDLInit = SDLInitState::SDLInit_Failure;
            m_eSDLInit.notify_all( );
            return;
        ...
        // Update g_nOutputWidthPts.
        {
            ...
            SDL_GetWindowSizeInPixels(
                m_Connector.GetSDLWindow( ), &width, &height );
#endif
            ...
            switch ( event.type )
            {
                case SDL_CLIPBOARDUPDATE:
                {
                    ...
                    gamescope_set_selection(
                        pClipBoard, GAMESCOPE_SELECTION_CLIPBOARD );
                    gamescope_set_selection(
                        pPrimarySelection, GAMESCOPE_SELECTION_PRIMARY );
                ...
                case SDL_MOUSEBUTTONUP:
                {
                    ...
                    wlserver_mousebutton(
                        SDLButtonToLinuxButton( event.button.button ),
                ...
                case SDL_MOUSEWHEEL:
                {
                    wlserver_lock( );
                    wlserver_mousewheel(
                        -event.wheel.x, -event.wheel.y, fake_timestamp );
                    wlserver_unlock( );
                }
                break;
                ...
                case SDL_KEYDOWN:
                {
                    // If this keydown event is super + one of the shortcut
                    // keys, consume the keydown event, since the corresponding
                    // keyup event will be consumed by the next case statement
                    // when the user releases the key
                    if ( event.key.keysym.mod & KMOD_LGUI )
                    {
                        uint32_t key =
                            SDLScancodeToLinuxKey( event.key.keysym.scancode );
                        const uint32_t shortcutKeys[] = { KEY_F, KEY_N, KEY_B,
                                                          KEY_U, KEY_Y, KEY_I,
                                                          KEY_O, KEY_S, KEY_G };
                        const bool     isShortcutKey =
                            std::find(
                                std::begin( shortcutKeys ),
                                std::end( shortcutKeys ),
                                key ) != std::end( shortcutKeys );
                        if ( isShortcutKey ) { break; }
                    }
                }
                    [[fallthrough]];
                case SDL_KEYUP:
                {
                         ...
                         ( event.key.keysym.mod & KMOD_LGUI ) )
                    {
                        ...
                        switch ( key )
                        {
                            ...
                            case KEY_Y:
                                ...
                                break;
                            case KEY_I:
                                g_upscaleFilterSharpness = std::min(
                                    20, g_upscaleFilterSharpness + 1 );
                                break;
                            case KEY_O:
                                ...
                                break;
                            case KEY_S:
                                gamescope::CScreenshotManager::Get( )
                                    .TakeScreenshot( true );
                                break;
                            case KEY_G:
                                ...
                                SDL_SetWindowKeyboardGrab(
                                    m_Connector.GetSDLWindow( ),
                                    g_bGrabbed ? SDL_TRUE : SDL_FALSE );

                                SDL_Event event;
                                event.type = GetUserEventIndex(
                                    GAMESCOPE_SDL_EVENT_TITLE );
                                SDL_PushEvent( &event );
                                break;
                            default:
                    ...
                    wlserver_lock( );
                    wlserver_key(
                        key, event.type == SDL_KEYDOWN, fake_timestamp );
                    wlserver_unlock( );
                }
                break;
                ...
                case SDL_WINDOWEVENT:
                {
                    switch ( event.window.event )
                    {
                        ...
                        case SDL_WINDOWEVENT_SHOWN:
                        {
                            ...
                            display_index = SDL_GetWindowDisplayIndex(
                                m_Connector.GetSDLWindow( ) );
                            if ( SDL_GetDesktopDisplayMode(
                                     display_index, &mode ) == 0 )
                            {
                                g_nOutputRefresh =
                                    ConvertHztomHz( mode.refresh_rate );
                            }
                        }
                        break;
                ...
                {
                    if ( event.type ==
                         GetUserEventIndex( GAMESCOPE_SDL_EVENT_VISIBLE ) )
                    {
                        ...
                        // held commits, etc.
                        if ( steamMode ) bVisible |= !g_bFirstFrame;
                        ...
                        event.type ==
                        GetUserEventIndex( GAMESCOPE_SDL_EVENT_TITLE ) )
                    {
                        std::shared_ptr<std::string> pAppTitle =
                            m_pApplicationTitle;

                        std::string szTitle =
                            pAppTitle ? *pAppTitle : "gamescope";
                        if ( g_bGrabbed ) szTitle += " (grabbed)";
                        SDL_SetWindowTitle(
                            m_Connector.GetSDLWindow( ), szTitle.c_str( ) );
                        ...
                        event.type ==
                        GetUserEventIndex( GAMESCOPE_SDL_EVENT_ICON ) )
                    {
                        ...
                        SDL_SetWindowIcon(
                            m_Connector.GetSDLWindow( ), m_pIconSurface );
                    }
                    else if (
                        event.type ==
                        GetUserEventIndex( GAMESCOPE_SDL_EVENT_GRAB ) )
                    {
                        SDL_SetRelativeMouseMode(
                            m_bApplicationGrabbed ? SDL_TRUE : SDL_FALSE );
                    }
                    else if (
                        event.type ==
                        GetUserEventIndex( GAMESCOPE_SDL_EVENT_CURSOR ) )
                    {
                        ...
                        if ( pCursorInfo )
                        {
                            ...
                            m_pCursor = SDL_CreateColorCursor(
                                m_pCursorSurface,
                                pCursorInfo->uXHotspot,
                                pCursorInfo->uYHotspot );
                        }
    ...
    template<> bool IBackend::Set<CSDLBackend>( )
