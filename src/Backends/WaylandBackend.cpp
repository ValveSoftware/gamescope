...
WaylandScaleToPhysical( uint32_t pValue, uint32_t pFactor )
{ return pValue * pFactor / WL_FRACTIONAL_SCALE_DENOMINATOR; }
static inline uint32_t
WaylandScaleToLogical( uint32_t pValue, uint32_t pFactor )
...
static bool IsGamescopeToplevel( wl_surface *pSurface )
{
    // HACK: this probably should never be called with a null pointer, but it
    // was happening after a window was closed.
    return pSurface && ( wl_proxy_get_tag( ( wl_proxy * )pSurface ) ==
                         &GAMESCOPE_toplevel_tag );
}
...
// Libdecor puts its userdata ptr at the end... how fun! I shouldn't have spent
// so long writing this total atrocity to mankind.
#define LIBDECOR_USERDATA_TO_THIS( type, name )                                \
    []<typename... Args>( Args... args )                                       \
    {                                                                          \
        type *pThing = ( type * )std::get<sizeof...( Args ) - 1>(              \
            std::forward_as_tuple( args... ) );                                \
        CallWithAllButLast(                                                    \
...
namespace gamescope
{
    ...
    gamescope::ConVar<bool> cv_wayland_mouse_warp_without_keyboard_focus(
        "wayland_mouse_warp_without_keyboard_focus",
        true,
        "Should we only forward mouse warps to the app when we have keyboard "
        "focus?" );
    gamescope::ConVar<bool> cv_wayland_mouse_relmotion_without_keyboard_focus(
        "wayland_mouse_relmotion_without_keyboard_focus",
        false,
        "Should we only forward mouse relative motion to the app when we have "
        "keyboard focus?" );
    gamescope::ConVar<bool> cv_wayland_use_modifiers(
        "wayland_use_modifiers", true, "Use DMA-BUF modifiers?" );

    gamescope::ConVar<float> cv_wayland_hdr10_saturation_scale(
        "wayland_hdr10_saturation_scale",
        1.0,
        "Saturation scale for HDR10 content by gamut expansion. 1.0 - 1.2 is a "
        "good range to play with." );
    ...
    inline WaylandPlaneState ClipPlane( const WaylandPlaneState &state )
    {
        int32_t nClippedDstWidth =
            std::min<int32_t>(
                g_nOutputWidth, state.nDstWidth + state.nDestX ) -
            state.nDestX;
        int32_t nClippedDstHeight =
            std::min<int32_t>(
                g_nOutputHeight, state.nDstHeight + state.nDestY ) -
            state.nDestY;
        double flClippedSrcWidth =
            state.flSrcWidth * ( nClippedDstWidth / double( state.nDstWidth ) );
        double flClippedSrcHeight =
            state.flSrcHeight *
            ( nClippedDstHeight / double( state.nDstHeight ) );
    ...
    static int CreateShmBuffer( uint32_t uSize, void *pData )
    {
        ...
        if ( pData )
        {
            void *pMappedData = mmap(
                nullptr, uSize, PROT_READ | PROT_WRITE, MAP_SHARED, nFd, 0 );
            if ( pMappedData == MAP_FAILED ) return -1;
    ...
    private:
        ...
        void LibDecor_Frame_DismissPopup(
            libdecor_frame *pFrame, const char *pSeatName );
        ...
        void Wayland_PresentationFeedback_Presented(
            ...
            uint32_t                         uSeqLo,
            uint32_t                         uFlags );
        void Wayland_PresentationFeedback_Discarded(
            struct wp_presentation_feedback *pFeedback );
        static const wp_presentation_feedback_listener
            s_PresentationFeedbackListener;
        ...
        static const frog_color_managed_surface_listener
            s_FrogColorManagedSurfaceListener;

        void Wayland_WPColorManagementSurfaceFeedback_PreferredChanged(
            wp_color_management_surface_feedback_v1 *pColorManagementSurface,
            unsigned int                             data );
        ...
        void Wayland_WPImageDescriptionInfo_Done(
        ...
        static const wp_image_description_info_v1_listener
            s_ImageDescriptionInfoListener;
    ...
    const wl_surface_listener CWaylandPlane::s_SurfaceListener = {
        ...
        .leave =
            WAYLAND_USERDATA_TO_THIS( CWaylandPlane, Wayland_Surface_Leave ),
    ...
    libdecor_frame_interface CWaylandPlane::s_LibDecorFrameInterface = {
        .configure = LIBDECOR_USERDATA_TO_THIS(
            CWaylandPlane, LibDecor_Frame_Configure ),
        .close =
            LIBDECOR_USERDATA_TO_THIS( CWaylandPlane, LibDecor_Frame_Close ),
        .commit =
            LIBDECOR_USERDATA_TO_THIS( CWaylandPlane, LibDecor_Frame_Commit ),
        .dismiss_popup = LIBDECOR_USERDATA_TO_THIS(
            CWaylandPlane, LibDecor_Frame_DismissPopup ),
    };
    const wp_presentation_feedback_listener
        CWaylandPlane::s_PresentationFeedbackListener = {
            .sync_output = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_PresentationFeedback_SyncOutput ),
            .presented = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_PresentationFeedback_Presented ),
            .discarded = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_PresentationFeedback_Discarded ),
        };
    const frog_color_managed_surface_listener
        CWaylandPlane::s_FrogColorManagedSurfaceListener = {
            .preferred_metadata = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane,
                Wayland_FrogColorManagedSurface_PreferredMetadata ),
        };
    const wp_color_management_surface_feedback_v1_listener
        CWaylandPlane::s_WPColorManagementSurfaceListener = {
            .preferred_changed = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane,
                Wayland_WPColorManagementSurfaceFeedback_PreferredChanged ),
        };
    const wp_image_description_info_v1_listener
        CWaylandPlane::s_ImageDescriptionInfoListener = {
            .done = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_Done ),
            .icc_file = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_ICCFile ),
            .primaries = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_Primaries ),
            .primaries_named = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_PrimariesNamed ),
            .tf_power = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_TFPower ),
            .tf_named = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_TFNamed ),
            .luminances = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_Luminances ),
            .target_primaries = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_TargetPrimaries ),
            .target_luminance = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_TargetLuminance ),
            .target_max_cll = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_Target_MaxCLL ),
            .target_max_fall = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_WPImageDescriptionInfo_Target_MaxFALL ),
        };
    const wp_fractional_scale_v1_listener
        CWaylandPlane::s_FractionalScaleListener = {
            .preferred_scale = WAYLAND_USERDATA_TO_THIS(
                CWaylandPlane, Wayland_FractionalScale_PreferredScale ),
        };
    ...
    WaylandModifierToXkbModifierName( WaylandModifierIndex eIndex )
    {
        switch ( eIndex )
        {
    ...
    class CWaylandConnector final : public CBaseBackendConnector,
                                    public INestedHints
    {
    public:
        CWaylandConnector(
            CWaylandBackend *pBackend, uint64_t ulVirtualConnectorKey );
        virtual ~CWaylandConnector( );

        bool UpdateEdid( );
        bool Init( );
        void SetFullscreen( bool bFullscreen ); // Thread safe, can be called
                                                // from the input thread.
        void UpdateFullscreenState( );

        bool HostCompositorIsCurrentlyVRR( ) const
        { return m_bHostCompositorIsCurrentlyVRR; }
        void SetHostCompositorIsCurrentlyVRR( bool bActive )
        { m_bHostCompositorIsCurrentlyVRR = bActive; }
        bool CurrentDisplaySupportsVRR( ) const
        { return HostCompositorIsCurrentlyVRR( ); }
        ...
        virtual GamescopePanelOrientation
                     GetCurrentOrientation( ) const override;
        ...
        virtual std::span<const uint32_t>
        GetValidDynamicRefreshRates( ) const override;
        ...
        virtual void SetCursorImage(
            std::shared_ptr<INestedHints::CursorInfo> info ) override;
        ...
        virtual void
        SetIcon( std::shared_ptr<std::vector<uint32_t>> uIconPixels ) override;
        virtual void SetSelection(
            std::shared_ptr<std::string> szContents,
            GamescopeSelection           eSelection ) override;
    ...
    const wl_buffer_listener CWaylandFb::s_BufferListener = {
        .release =
            WAYLAND_USERDATA_TO_THIS( CWaylandFb, Wayland_Buffer_Release ),
    };
    ...
    public:
        ...
        template<typename T> std::shared_ptr<T> QueueLaunder( T *pObject );
    ...
    private:
        ...
        std::atomic<std::shared_ptr<zwp_relative_pointer_v1>>
        ...
        void
        Wayland_Seat_Capabilities( wl_seat *pSeat, uint32_t uCapabilities );
        ...
        void Wayland_Pointer_Enter(
            ...
            uint32_t    uSerial,
            wl_surface *pSurface,
            wl_fixed_t  fSurfaceX,
            wl_fixed_t  fSurfaceY );
        void Wayland_Pointer_Leave(
        ...
        void Wayland_Pointer_Button(
            ...
            uint32_t    uButton,
            uint32_t    uState );
        void Wayland_Pointer_Axis(
            wl_pointer *pPointer,
            uint32_t    uTime,
            uint32_t    uAxis,
            wl_fixed_t  fValue );
        void Wayland_Pointer_Axis_Source(
            wl_pointer *pPointer, uint32_t uAxisSource );
        void Wayland_Pointer_Axis_Stop(
            wl_pointer *pPointer, uint32_t uTime, uint32_t uAxis );
        void Wayland_Pointer_Axis_Discrete(
            wl_pointer *pPointer, uint32_t uAxis, int32_t nDiscrete );
        void Wayland_Pointer_Axis_Value120(
        ...
        void Wayland_Keyboard_Modifiers(
            wl_keyboard *pKeyboard,
            uint32_t     uSerial,
            uint32_t     uModsDepressed,
            uint32_t     uModsLatched,
            uint32_t     uModsLocked,
            uint32_t     uGroup );
        void Wayland_Keyboard_RepeatInfo(
        ...
        void Wayland_RelativePointer_RelativeMotion(
            ...
            wl_fixed_t               fDx,
            wl_fixed_t               fDy,
            wl_fixed_t               fDxUnaccel,
            wl_fixed_t               fDyUnaccel );
    ...
    const wl_registry_listener CWaylandInputThread::s_RegistryListener = {
        .global = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Registry_Global ),
        .global_remove = WAYLAND_NULL( ),
    };
    const wl_seat_listener CWaylandInputThread::s_SeatListener = {
        .capabilities = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Seat_Capabilities ),
        .name =
            WAYLAND_USERDATA_TO_THIS( CWaylandInputThread, Wayland_Seat_Name ),
    };
    const wl_pointer_listener CWaylandInputThread::s_PointerListener = {
        .enter = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Enter ),
        .leave = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Leave ),
        .motion = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Motion ),
        .button = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Button ),
        .axis = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Axis ),
        .frame = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Frame ),
        .axis_source = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Axis_Source ),
        .axis_stop = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Axis_Stop ),
        .axis_discrete = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Axis_Discrete ),
        .axis_value120 = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Pointer_Axis_Value120 ),
    };
    const wl_keyboard_listener CWaylandInputThread::s_KeyboardListener = {
        .keymap = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_Keymap ),
        .enter = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_Enter ),
        .leave = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_Leave ),
        .key = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_Key ),
        .modifiers = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_Modifiers ),
        .repeat_info = WAYLAND_USERDATA_TO_THIS(
            CWaylandInputThread, Wayland_Keyboard_RepeatInfo ),
    };
    const zwp_relative_pointer_v1_listener
        CWaylandInputThread::s_RelativePointerListener = {
    ...
    public:
        ...
        virtual std::span<const char *const> GetDeviceExtensions(
            VkPhysicalDevice pVkPhysicalDevice ) const override;
        ...
        virtual bool ValidPhysicalDevice(
            VkPhysicalDevice pVkPhysicalDevice ) const override;

        virtual void
        DirtyState( bool bForce = false, bool bForceModeset = false ) override;
    ...
    protected:
        ...
        wl_surface *CursorInfoToSurface(
            const std::shared_ptr<INestedHints::CursorInfo> &info );
        ...
        wp_color_manager_v1 *GetWPColorManager( ) const
        { return m_pWPColorManager; }
        ...
        xdg_toplevel_icon_manager_v1 *GetToplevelIconManager( ) const
        { return m_pToplevelIconManager; }
        ...
        WaylandOutputInfo *GetOutputInfo( wl_output *pOutput )
        {
            ...
            if ( iter == m_pOutputs.end( ) ) return nullptr;
    ...
    private:
        ...
        void Wayland_Modifier(
            ...
            uint32_t             uFormat,
            uint32_t             uModifierHi,
            uint32_t             uModifierLo );

        void Wayland_Output_Geometry(
            wl_output  *pOutput,
            int32_t     nX,
            int32_t     nY,
            int32_t     nPhysicalWidth,
            int32_t     nPhysicalHeight,
            int32_t     nSubpixel,
            const char *pMake,
            const char *pModel,
            int32_t     nTransform );
        ...
        void Wayland_Output_Description(
            wl_output *pOutput, const char *pDescription );
        ...
        void
        Wayland_LockedPointer_Locked( zwp_locked_pointer_v1 *pLockedPointer );
        void
        Wayland_LockedPointer_Unlocked( zwp_locked_pointer_v1 *pLockedPointer );
        ...
        void Wayland_WPColorManager_SupportedTFNamed(
            wp_color_manager_v1 *pWPColorManager, uint32_t uTF );
        void Wayland_WPColorManager_SupportedPrimariesNamed(
            wp_color_manager_v1 *pWPColorManager, uint32_t uPrimaries );
        void Wayland_WPColorManager_ColorManagerDone(
            wp_color_manager_v1 *pWPColorManager );
        ...
        void Wayland_PrimarySelectionSource_Send(
            ...
            const char                             *pMime,
            int                                     nFd );
        void Wayland_PrimarySelectionSource_Cancelled(
            struct zwp_primary_selection_source_v1 *pSource );
        static const zwp_primary_selection_source_v1_listener
            s_PrimarySelectionSourceListener;
        ...
        wp_single_pixel_buffer_manager_v1 *m_pSinglePixelBufferManager =
            nullptr;
        ...
        wp_image_description_v1
            *m_pWPImageDescriptions[ GamescopeAppTextureColorspace_Count ]{};
        ...
        zwp_primary_selection_device_manager_v1
            *m_pPrimarySelectionDeviceManager                      = nullptr;
        ...
        struct
        {
            ...
            std::vector<wp_color_manager_v1_transfer_function>
                                                           eTransferFunctions;
            std::vector<wp_color_manager_v1_render_intent> eRenderIntents;
            std::vector<wp_color_manager_v1_feature>       eFeatures;

            bool bSupportsGamescopeColorManagement =
                false; // Has everything we want and need?
    ...
    const wl_registry_listener CWaylandBackend::s_RegistryListener = {
        .global = WAYLAND_USERDATA_TO_THIS(
            CWaylandBackend, Wayland_Registry_Global ),
        .global_remove = WAYLAND_NULL( ),
    };
    const wl_output_listener CWaylandBackend::s_OutputListener = {
        .geometry = WAYLAND_USERDATA_TO_THIS(
            CWaylandBackend, Wayland_Output_Geometry ),
        .mode =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Output_Mode ),
        .done =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Output_Done ),
        .scale =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Output_Scale ),
        .name =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Output_Name ),
        .description = WAYLAND_USERDATA_TO_THIS(
            CWaylandBackend, Wayland_Output_Description ),
    };
    ...
    const wl_pointer_listener CWaylandBackend::s_PointerListener = {
        .enter =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Pointer_Enter ),
        .leave =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Pointer_Leave ),
        .motion        = WAYLAND_NULL( ),
        .button        = WAYLAND_NULL( ),
        .axis          = WAYLAND_NULL( ),
        .frame         = WAYLAND_NULL( ),
    ...
    const wl_keyboard_listener CWaylandBackend::s_KeyboardListener = {
        .keymap = WAYLAND_NULL( ),
        .enter =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Keyboard_Enter ),
        .leave =
            WAYLAND_USERDATA_TO_THIS( CWaylandBackend, Wayland_Keyboard_Leave ),
        .key         = WAYLAND_NULL( ),
        .modifiers   = WAYLAND_NULL( ),
        .repeat_info = WAYLAND_NULL( ),
    };
    const zwp_locked_pointer_v1_listener
        CWaylandBackend::s_LockedPointerListener = {
            .locked = WAYLAND_USERDATA_TO_THIS(
                CWaylandBackend, Wayland_LockedPointer_Locked ),
            .unlocked = WAYLAND_USERDATA_TO_THIS(
                CWaylandBackend, Wayland_LockedPointer_Unlocked ),
        };

    const wp_color_manager_v1_listener
                                  CWaylandBackend::s_WPColorManagerListener{
                                      .supported_intent = WAYLAND_USERDATA_TO_THIS(
                                          CWaylandBackend, Wayland_WPColorManager_SupportedIntent ),
                                      .supported_feature = WAYLAND_USERDATA_TO_THIS(
                                          CWaylandBackend, Wayland_WPColorManager_SupportedFeature ),
                                      .supported_tf_named = WAYLAND_USERDATA_TO_THIS(
                                          CWaylandBackend, Wayland_WPColorManager_SupportedTFNamed ),
                                      .supported_primaries_named = WAYLAND_USERDATA_TO_THIS(
                                          CWaylandBackend,
                                          Wayland_WPColorManager_SupportedPrimariesNamed ),
                                      .done = WAYLAND_USERDATA_TO_THIS(
                                          CWaylandBackend, Wayland_WPColorManager_ColorManagerDone ),
                                  };
    const wl_data_source_listener CWaylandBackend::s_DataSourceListener = {
        .target = WAYLAND_NULL( ),
        .send   = WAYLAND_USERDATA_TO_THIS(
            CWaylandBackend, Wayland_DataSource_Send ),
        .cancelled = WAYLAND_USERDATA_TO_THIS(
            CWaylandBackend, Wayland_DataSource_Cancelled ),
        .dnd_drop_performed = WAYLAND_NULL( ),
        .dnd_finished       = WAYLAND_NULL( ),
        .action             = WAYLAND_NULL( ),
    };
    const zwp_primary_selection_source_v1_listener
        CWaylandBackend::s_PrimarySelectionSourceListener = {
            .send = WAYLAND_USERDATA_TO_THIS(
                CWaylandBackend, Wayland_PrimarySelectionSource_Send ),
            .cancelled = WAYLAND_USERDATA_TO_THIS(
                CWaylandBackend, Wayland_PrimarySelectionSource_Cancelled ),
        };
    ...
    CWaylandFb::CWaylandFb(
        CWaylandBackend *pBackend, wl_buffer *pHostBuffer ) :
    ...
    void CWaylandFb::OnCompositorRelease( )
    {
        ...
        {
            xdg_log.errorf(
                "Compositor released us but we were not acquired. Oh no." );
        }
    }
    ...
    bool CWaylandConnector::Init( )
    {
        ...
        {
            bool bSuccess = m_Planes[ i ].Init(
                i == 0 ? nullptr : &m_Planes[ 0 ],
                i == 0 ? nullptr : &m_Planes[ i - 1 ] );
        ...
        if ( g_bForceRelativeMouse ) this->SetRelativeMouseMode( true );
    ...
    int CWaylandConnector::Present( const FrameInfo_t *pFrameInfo, bool bAsync )
    {
        ...
        {
            // TODO: Dedupe some of this composite check code between us and
            // drm.cpp
            ...
            if ( g_bOutputHDREnabled ) bNeedsFullComposite |= g_bHDRItmEnable;
            ...
            if ( !bNeedsFullComposite )
            {
                ...
                if ( bNeedsBacking )
                {
                    ...
                    pPlane->Present(
                        WaylandPlaneState{
                            .pBuffer =
                                m_pBackend->GetBlackFb( )->GetHostBuffer( ),
                            .flSrcWidth  = 1.0,
                            .flSrcHeight = 1.0,
                ...
                for ( int i = 0; i < 8 && uCurrentPlane < 8; i++ )
                    m_Planes[ uCurrentPlane++ ].Present(
                        i < pFrameInfo->layerCount ? &pFrameInfo->layers[ i ]
                                                   : nullptr );
            }
            else
            {
                std::optional oCompositeResult = vulkan_composite(
                    ( FrameInfo_t * )pFrameInfo, nullptr, false );
    ...
    {
        return std::span<const uint8_t>{ m_FakeEdid.begin( ),
                                         m_FakeEdid.end( ) };
    }
    std::span<const uint32_t>
    CWaylandConnector::GetValidDynamicRefreshRates( ) const
    ...
    void CWaylandConnector::SetCursorImage(
        std::shared_ptr<INestedHints::CursorInfo> info )
    { m_pBackend->SetCursorImage( std::move( info ) ); }
    void CWaylandConnector::SetRelativeMouseMode( bool bRelative )
    {
        // TODO: Do more tracking across multiple connectors, and activity here
        // if we ever want to use this.
        m_pBackend->SetRelativeMouseMode(
            m_Planes[ 0 ].GetSurface( ), bRelative );
    }
    ...
    void CWaylandConnector::SetTitle( std::shared_ptr<std::string> pAppTitle )
    {
        ...
        if ( g_bGrabbed ) szTitle += " (grabbed)";
        libdecor_frame_set_title( m_Planes[ 0 ].GetFrame( ), szTitle.c_str( ) );
    }
    void CWaylandConnector::SetIcon(
        std::shared_ptr<std::vector<uint32_t>> uIconPixels )
    {
        if ( !m_pBackend->GetToplevelIconManager( ) ) return;

        if ( uIconPixels && uIconPixels->size( ) >= 3 )
        {
            xdg_toplevel_icon_v1 *pIcon =
                xdg_toplevel_icon_manager_v1_create_icon(
                    m_pBackend->GetToplevelIconManager( ) );
            if ( !pIcon )
            {
            ...
            wl_shm_pool *pPool =
                wl_shm_create_pool( m_pBackend->GetShm( ), nFd, uSize );
            defer( wl_shm_pool_destroy( pPool ) );

            wl_buffer *pBuffer = wl_shm_pool_create_buffer(
                pPool, 0, uWidth, uHeight, uStride, WL_SHM_FORMAT_ARGB8888 );
        ...
        std::shared_ptr<std::string> szContents, GamescopeSelection eSelection )
    {
        ...
        if ( m_pBackend->m_pPrimarySelectionDeviceManager &&
             !m_pBackend->m_pPrimarySelectionDevice )
            m_pBackend->m_pPrimarySelectionDevice =
             ...
             m_pBackend->m_pDataDevice )
        {
            ...
            wl_data_source_add_listener(
                source, &m_pBackend->s_DataSourceListener, m_pBackend );
            ...
            wl_data_device_set_selection(
                m_pBackend->m_pDataDevice,
                source,
                m_pBackend->m_uKeyboardEnterSerial );
        }
        else if (
            eSelection == GAMESCOPE_SELECTION_PRIMARY &&
            m_pBackend->m_pPrimarySelectionDevice )
        {
            ...
            zwp_primary_selection_source_v1_add_listener(
                source,
                &m_pBackend->s_PrimarySelectionSourceListener,
                m_pBackend );
            ...
            zwp_primary_selection_device_v1_set_selection(
                m_pBackend->m_pPrimarySelectionDevice,
                source,
                m_pBackend->m_uPointerEnterSerial );
        }
    }
    ...
    CWaylandPlane::~CWaylandPlane( )
    {
        ...
        if ( m_pFrame ) libdecor_frame_unref( m_pFrame ); // Ew.

        if ( m_pSubsurface ) wl_subsurface_destroy( m_pSubsurface );
    ...
    bool
    CWaylandPlane::Init( CWaylandPlane *pParent, CWaylandPlane *pSiblingBelow )
    {
        m_pParent = pParent;
        m_pSurface =
            wl_compositor_create_surface( m_pBackend->GetCompositor( ) );
        wl_surface_set_user_data( m_pSurface, this );
        wl_surface_add_listener( m_pSurface, &s_SurfaceListener, this );

        m_pViewport = wp_viewporter_get_viewport(
            m_pBackend->GetViewporter( ), m_pSurface );

        if ( m_pBackend->GetWPColorManager( ) )
        {
            m_pWPColorManagedSurface = wp_color_manager_v1_get_surface(
                m_pBackend->GetWPColorManager( ), m_pSurface );
            m_pWPColorManagedSurfaceFeedback =
        ...
        if ( m_pFrame )
            libdecor_frame_set_visibility(
                m_pFrame, !g_bBorderlessOutputWindow );
    ...
    {
        if ( m_pParent ) return m_pParent->GetScale( );
    ...
    void CWaylandPlane::Present( std::optional<WaylandPlaneState> oState )
    {
        ...
        if ( oState )
        {
            ...
            if ( m_pFrame )
            {
                struct wp_presentation_feedback *pFeedback =
                    wp_presentation_feedback(
                        m_pBackend->GetPresentation( ), m_pSurface );
                wp_presentation_feedback_add_listener(
                    pFeedback, &s_PresentationFeedbackListener, this );
            }
                        ...
                        GAMESCOPE_APP_TEXTURE_COLORSPACE_HDR10_PQ )
                    {
                        ...
                        if ( m_ColorState->pHDRMetadata )
                        {
                            const hdr_metadata_infoframe *pInfoframe =
                                &m_ColorState->pHDRMetadata
                                     ->View<hdr_output_metadata>( )
                                     .hdmi_metadata_type1;
                            ...
                            wp_image_description_creator_params_v1_set_mastering_display_primaries(
                                ...
                                ( ( ( int32_t )pInfoframe
                                  ...
                                  1'000'000 ) /
                                    0xC350,
                                ( ( ( int32_t )pInfoframe->white_point.x ) *
                                  1'000'000 ) /
                                    0xC350,
                                ( ( ( int32_t )pInfoframe->white_point.y ) *
                                  1'000'000 ) /
                                    0xC350 );
                ...
                switch ( oState->eColorspace )
                {
                    ...
                    case GAMESCOPE_APP_TEXTURE_COLORSPACE_PASSTHRU:
                        frog_color_managed_surface_set_known_container_color_volume(
                            m_pFrogColorManagedSurface,
                            FROG_COLOR_MANAGED_SURFACE_PRIMARIES_UNDEFINED );
                        frog_color_managed_surface_set_known_container_color_volume(
                            m_pFrogColorManagedSurface,
            ...
            wl_surface_set_opaque_region(
                m_pSurface,
                oState->bOpaque ? m_pBackend->GetFullRegion( ) : nullptr );
    ...
    void CWaylandPlane::Present( const FrameInfo_t::Layer_t *pLayer )
    {
        CWaylandFb *pWaylandFb =
            pLayer && pLayer->tex != nullptr
                ? static_cast<CWaylandFb *>(
                      pLayer->tex->GetBackendFb( )->EnsureImported( ) )
                : nullptr;
        wl_buffer *pBuffer =
            pWaylandFb ? pWaylandFb->GetHostBuffer( ) : nullptr;
        ...
        if ( pBuffer )
        {
            ...
            Present( ClipPlane(
                WaylandPlaneState{
                    ...
                    .nDstWidth        = int32_t( ceil(
                        pLayer->tex->width( ) / double( pLayer->scale.x ) ) ),
                    .nDstHeight       = int32_t( ceil(
    ...
    void CWaylandPlane::UpdateVRRRefreshRate( )
    {
        ...
        if ( !m_pConnector->HostCompositorIsCurrentlyVRR( ) ) return;

        if ( m_pOutputs.empty( ) ) return;

        int32_t nLargestRefreshRateMhz = 0;
        for ( wl_output *pOutput : m_pOutputs )
        {
            WaylandOutputInfo *pOutputInfo =
                m_pBackend->GetOutputInfo( pOutput );
            if ( !pOutputInfo ) continue;

            nLargestRefreshRateMhz =
                std::max( nLargestRefreshRateMhz, pOutputInfo->nRefresh );
        }

        if ( nLargestRefreshRateMhz &&
             nLargestRefreshRateMhz != g_nOutputRefresh )
        {
            // TODO(strategy): We should pick the largest refresh rate.
            xdg_log.infof(
                "Changed refresh to: %.3fhz",
                ConvertmHzToHz( ( float )nLargestRefreshRateMhz ) );
            g_nOutputRefresh = nLargestRefreshRateMhz;
        }
    }

    void CWaylandPlane::Wayland_Surface_Enter(
        wl_surface *pSurface, wl_output *pOutput )
    {
        if ( !IsGamescopeToplevel( pSurface ) ) return;

        m_pOutputs.emplace_back( pOutput );

        UpdateVRRRefreshRate( );
    }
    void CWaylandPlane::Wayland_Surface_Leave(
        wl_surface *pSurface, wl_output *pOutput )
    {
        if ( !IsGamescopeToplevel( pSurface ) ) return;
        ...
        libdecor_frame *pFrame, libdecor_configuration *pConfiguration )
    {
        ...
        if ( !libdecor_configuration_get_content_size(
                 pConfiguration, m_pFrame, &nWidth, &nHeight ) )
        {
    ...
    void CWaylandPlane::LibDecor_Frame_DismissPopup(
        libdecor_frame *pFrame, const char *pSeatName )
    {}
    ...
    void CWaylandPlane::Wayland_PresentationFeedback_Presented(
        ...
        uint32_t                         uTVSecLo,
        uint32_t                         uTVNSec,
        uint32_t                         uRefreshCycle,
        uint32_t                         uSeqHi,
        uint32_t                         uSeqLo,
        uint32_t                         uFlags )
    {
        uint64_t ulTime = ( ( ( uint64_t( uTVSecHi ) << 32ul ) | uTVSecLo ) *
                            1'000'000'000lu ) +
                          ( uint64_t( uTVNSec ) );
        ...
        uint32_t                    uMaxFullFrameLuminance )
    {
        ...
        pDisplayColorimetry->primaries.r =
            glm::vec2{ uOutputDisplayPrimaryRedX * 0.00002f,
                       uOutputDisplayPrimaryRedY * 0.00002f };
        pDisplayColorimetry->primaries.g =
            glm::vec2{ uOutputDisplayPrimaryGreenX * 0.00002f,
                       uOutputDisplayPrimaryGreenY * 0.00002f };
        pDisplayColorimetry->primaries.b =
            glm::vec2{ uOutputDisplayPrimaryBlueX * 0.00002f,
                       uOutputDisplayPrimaryBlueY * 0.00002f };
        pDisplayColorimetry->white = glm::vec2{ uOutputWhitePointX * 0.00002f,
                                                uOutputWhitePointY * 0.00002f };

        xdg_log.infof(
            "PreferredMetadata: Red: %g %g, Green: %g %g, Blue: %g %g, White: "
            "%g %g, Max Luminance: %u nits, Min Luminance: %g nits, Max Full "
            "Frame Luminance: %u nits",
            uOutputDisplayPrimaryRedX * 0.00002,
            uOutputDisplayPrimaryRedY * 0.00002,
            uOutputDisplayPrimaryGreenX * 0.00002,
            uOutputDisplayPrimaryGreenY * 0.00002,
            uOutputDisplayPrimaryBlueX * 0.00002,
            uOutputDisplayPrimaryBlueY * 0.00002,
            uOutputWhitePointX * 0.00002,
            uOutputWhitePointY * 0.00002,
            uint32_t( uMaxLuminance ),
        ...
        wp_image_description_info_v1 *pImageDescInfo )
    {
        ...
        {
            pHDRInfo->bExposeHDRSupport =
                ( cv_hdr_enabled && m_pConnector->m_uMaxTargetLuminance >
                                        m_pConnector->m_uReferenceLuminance );
            pHDRInfo->eOutputEncodingEOTF =
                pHDRInfo->bExposeHDRSupport ? EOTF_PQ : EOTF_Gamma22;
        }

        xdg_log.infof( "HDR INFO" );
        xdg_log.infof(
            "  cv_hdr_enabled: %s", cv_hdr_enabled ? "true" : "false" );
        xdg_log.infof(
            "  uMaxLum: %u, uRefLum: %u",
            m_pConnector->m_uMaxTargetLuminance,
            m_pConnector->m_uReferenceLuminance );
        xdg_log.infof(
            "  bExposeHDRSupport: %s",
            pHDRInfo->bExposeHDRSupport ? "true" : "false" );
    }
        ...
        int32_t                       nWhiteY )
    {
        auto *pDisplayColorimetry = &m_pConnector->m_DisplayColorimetry;
        pDisplayColorimetry->primaries.r =
            glm::vec2{ nRedX / 1000000.0f, nRedY / 1000000.0f };
        pDisplayColorimetry->primaries.g =
            glm::vec2{ nGreenX / 1000000.0f, nGreenY / 1000000.0f };
        pDisplayColorimetry->primaries.b =
            glm::vec2{ nBlueX / 1000000.0f, nBlueY / 1000000.0f };
        pDisplayColorimetry->white =
            glm::vec2{ nWhiteX / 1000000.0f, nWhiteY / 1000000.0f };
    }
        ...
        wp_fractional_scale_v1 *pFractionalScale, uint32_t uScale )
    {
        ...
        if ( s_uGlobalFractionalScale != uScale )
        {
            if ( m_bHasRecievedScale )
            {
                ...
                g_nOutputHeight =
                    ( g_nOutputHeight * uScale ) / m_uFractionalScale;
            }
    ...
    static libdecor_interface s_LibDecorInterface = {
        .error = []( libdecor      *pContext,
                     libdecor_error eError,
                     const char    *pMessage )
    ...
    bool CWaylandBackend::Init( )
    {
        ...
        if ( g_nOutputRefresh == 0 ) g_nOutputRefresh = ConvertHztomHz( 60 );
        ...
        if ( !m_pCompositor || !m_pSubcompositor || !m_pXdgWmBase ||
             !m_pLinuxDmabuf || !m_pViewporter || !m_pPresentation ||
             !m_pRelativePointerManager || !m_pPointerConstraints || !m_pShm )
        {
        ...
        // Grab stuff from any extra bindings/listeners we set up, eg.
        // format/modifiers.
        wl_display_roundtrip( m_pDisplay );
        ...
        if ( m_pWPColorManager )
        {
                ...
                [ this ]( ) -> bool
            {
                ...
                // Primaries
                if ( !Algorithm::Contains(
                         m_WPColorManagerFeatures.ePrimaries,
    ...
    bool CWaylandBackend::PostInit( )
    {
        ...
        {
            wl_buffer *pBlackBuffer =
                wp_single_pixel_buffer_manager_v1_create_u32_rgba_buffer(
                    m_pSinglePixelBufferManager, 0, 0, 0, ~0u );
        ...
        else
        {
            ...
            m_BlackFb =
                static_cast<CWaylandFb *>( m_pBlackTexture->GetBackendFb( ) );
        }
    ...
    std::span<const char *const> CWaylandBackend::GetDeviceExtensions(
        VkPhysicalDevice pVkPhysicalDevice ) const
    { return std::span<const char *const>{}; }

    VkImageLayout CWaylandBackend::GetPresentLayout( ) const
    { return VK_IMAGE_LAYOUT_GENERAL; }

    void CWaylandBackend::GetPreferredOutputFormat(
        uint32_t *pPrimaryPlaneFormat, uint32_t *pOverlayPlaneFormat ) const
    {
        ...
        *pPrimaryPlaneFormat =
            u10BitFormat != DRM_FORMAT_INVALID ? u10BitFormat : u8BitFormat;
        *pOverlayPlaneFormat = u8BitFormat;
    }

    bool CWaylandBackend::ValidPhysicalDevice(
        VkPhysicalDevice pVkPhysicalDevice ) const
    { return true; }
    ...
    bool CWaylandBackend::PollState( )
    {
        ...
        if ( wl_display_prepare_read( m_pDisplay ) == 0 )
        {
            ...
            if ( nRet > 0 ) wl_display_read_events( m_pDisplay );
            else
    ...
    std::shared_ptr<BackendBlob> CWaylandBackend::CreateBackendBlob(
        const std::type_info &type, std::span<const uint8_t> data )
    { return std::make_shared<BackendBlob>( data ); }

    OwningRc<IBackendFb>
    CWaylandBackend::ImportDmabufToBackend( wlr_dmabuf_attributes *pDmaBuf )
    {
        zwp_linux_buffer_params_v1 *pBufferParams =
            zwp_linux_dmabuf_v1_create_params( m_pLinuxDmabuf );
        if ( !pBufferParams )
        {
    ...
    {
        if ( !cv_wayland_use_modifiers ) return false;

        return m_bCanUseModifiers;
    }
    std::span<const uint64_t>
    CWaylandBackend::GetSupportedModifiers( uint32_t uDrmFormat ) const
    {
        auto iter = m_FormatModifiers.find( uDrmFormat );
        if ( iter == m_FormatModifiers.end( ) )
            return std::span<const uint64_t>{};

        return std::span<const uint64_t>{ iter->second.begin( ),
                                          iter->second.end( ) };
    }
    ...
    bool CWaylandBackend::SupportsTearing( ) const { return false; }
    bool CWaylandBackend::UsesVulkanSwapchain( ) const { return false; }

    bool CWaylandBackend::IsSessionBased( ) const { return false; }

    bool CWaylandBackend::SupportsExplicitSync( ) const { return true; }

    bool CWaylandBackend::IsPaused( ) const { return false; }

    bool CWaylandBackend::IsVisible( ) const { return true; }
    ...
    void CWaylandBackend::HackUpdatePatchedEdid( )
    {
        ...
        // XXX: We should do this a better way that handles per-window and appid
        // stuff down the line
        if ( cv_hdr_enabled &&
             GetCurrentConnector( )->GetHDRInfo( ).bExposeHDRSupport )
        {
    ...
    bool CWaylandBackend::UsesVirtualConnectors( ) { return true; }
    std::shared_ptr<IBackendConnector>
    CWaylandBackend::CreateVirtualConnector( uint64_t ulVirtualConnectorKey )
    {
        std::shared_ptr<CWaylandConnector> pConnector =
            std::make_shared<CWaylandConnector>( this, ulVirtualConnectorKey );
        m_pFocusConnector = pConnector.get( );

        if ( !pConnector->Init( ) ) { return nullptr; }
    ...
    wl_surface *CWaylandBackend::CursorInfoToSurface(
        const std::shared_ptr<INestedHints::CursorInfo> &info )
    {
        ...
        wl_surface *pCursorSurface =
            wl_compositor_create_surface( m_pCompositor );
    ...
    bool CWaylandBackend::SupportsColorManagement( ) const
    {
        return m_pFrogColorMgmtFactory != nullptr ||
               ( m_pWPColorManager != nullptr &&
                 m_WPColorManagerFeatures.bSupportsGamescopeColorManagement );
    }

    void CWaylandBackend::SetCursorImage(
        std::shared_ptr<INestedHints::CursorInfo> info )
    {
        m_pCursorInfo = info;
    ...
    void CWaylandBackend::SetRelativeMouseMode(
        wl_surface *pSurface, bool bRelative )
    {
        if ( !m_pPointer ) return;

        if ( !!bRelative != !!m_pLockedPointer ||
             ( pSurface != m_pLockedSurface && bRelative ) )
        {
            if ( m_pLockedPointer )
            {
            ...
            if ( bRelative )
            {
                ...
                zwp_locked_pointer_v1_add_listener(
                    m_pLockedPointer, &s_LockedPointerListener, this );
    ...
    void CWaylandBackend::UpdateCursor( )
    {
        ...
        if ( cv_wayland_mouse_warp_without_keyboard_focus )
            bUseHostCursor = m_bPointerLocked && !m_bKeyboardEntered &&
        ...
        if ( bUseHostCursor )
        {
            wl_pointer_set_cursor(
                m_pPointer,
                m_uPointerEnterSerial,
                m_pDefaultCursorSurface,
                m_pDefaultCursorInfo->uXHotspot,
                m_pDefaultCursorInfo->uYHotspot );
        }
        else
        {
            ...
            if ( bHideCursor )
                wl_pointer_set_cursor(
                    m_pPointer, m_uPointerEnterSerial, nullptr, 0, 0 );
            else
                wl_pointer_set_cursor(
                    m_pPointer,
                    m_uPointerEnterSerial,
                    m_pCursorSurface,
                    m_pCursorInfo->uXHotspot,
                    m_pCursorInfo->uYHotspot );
        }
    }
    ...
    void CWaylandBackend::Wayland_Registry_Global(
        ...
        uint32_t     uVersion )
    {
        if ( !strcmp( pInterface, wl_compositor_interface.name ) &&
             uVersion >= 4u )
        {
            m_pCompositor = ( wl_compositor * )wl_registry_bind(
                pRegistry, uName, &wl_compositor_interface, 4u );
        }
        if ( !strcmp(
                 pInterface,
        ...
        {
            m_pSubcompositor = ( wl_subcompositor * )wl_registry_bind(
                pRegistry, uName, &wl_subcompositor_interface, 1u );
        }
        else if (
            !strcmp( pInterface, xdg_wm_base_interface.name ) &&
            uVersion >= 1u )
        {
            static constexpr xdg_wm_base_listener s_Listener = {
                .ping =
                    []( void *pData, xdg_wm_base *pXdgWmBase, uint32_t uSerial )
                { xdg_wm_base_pong( pXdgWmBase, uSerial ); }
            };
            m_pXdgWmBase = ( xdg_wm_base * )wl_registry_bind(
                pRegistry, uName, &xdg_wm_base_interface, 1u );
            xdg_wm_base_add_listener( m_pXdgWmBase, &s_Listener, this );
        }
        else if (
            !strcmp( pInterface, zwp_linux_dmabuf_v1_interface.name ) &&
            uVersion >= 3 )
        {
            m_pLinuxDmabuf = ( zwp_linux_dmabuf_v1 * )wl_registry_bind(
                pRegistry, uName, &zwp_linux_dmabuf_v1_interface, 3u );
            static constexpr zwp_linux_dmabuf_v1_listener s_Listener = {
                .format = WAYLAND_NULL( ), // Formats are also advertised by the
                                           // modifier event, ignore them here.
                .modifier = WAYLAND_USERDATA_TO_THIS(
                    CWaylandBackend, Wayland_Modifier ),
            };
            zwp_linux_dmabuf_v1_add_listener(
                m_pLinuxDmabuf, &s_Listener, this );
        }
        else if ( !strcmp( pInterface, wp_viewporter_interface.name ) )
        {
            m_pViewporter = ( wp_viewporter * )wl_registry_bind(
                pRegistry, uName, &wp_viewporter_interface, 1u );
        }
        else if (
            !strcmp( pInterface, wl_seat_interface.name ) && uVersion >= 8u )
        {
            m_pSeat = ( wl_seat * )wl_registry_bind(
                pRegistry, uName, &wl_seat_interface, 8u );
            wl_seat_add_listener( m_pSeat, &s_SeatListener, this );
        }
        else if ( !strcmp( pInterface, wp_presentation_interface.name ) )
        {
            m_pPresentation = ( wp_presentation * )wl_registry_bind(
                pRegistry, uName, &wp_presentation_interface, 1u );
        }
        else if ( !strcmp( pInterface, wl_output_interface.name ) )
        {
            wl_output *pOutput = ( wl_output * )wl_registry_bind(
                pRegistry, uName, &wl_output_interface, 4u );
            wl_output_add_listener( pOutput, &s_OutputListener, this );
            m_pOutputs.emplace(
                std::make_pair<struct wl_output *, WaylandOutputInfo>(
                    std::move( pOutput ), WaylandOutputInfo{} ) );
        }
        else if ( !strcmp(
                      pInterface,
                      frog_color_management_factory_v1_interface.name ) )
        {
            m_pFrogColorMgmtFactory =
                ( frog_color_management_factory_v1 * )wl_registry_bind(
                    pRegistry,
                    uName,
        ...
        else if ( !strcmp( pInterface, wp_color_manager_v1_interface.name ) )
        {
            m_pWPColorManager = ( wp_color_manager_v1 * )wl_registry_bind(
                pRegistry, uName, &wp_color_manager_v1_interface, 1u );
            wp_color_manager_v1_add_listener(
                m_pWPColorManager, &s_WPColorManagerListener, this );
        }
        else if ( !strcmp(
                      pInterface, zwp_pointer_constraints_v1_interface.name ) )
        {
            m_pPointerConstraints =
        ...
        {
            m_pShm = ( wl_shm * )wl_registry_bind(
                pRegistry, uName, &wl_shm_interface, 1u );
        }
        else if ( !strcmp(
                      pInterface,
                      xdg_toplevel_icon_manager_v1_interface.name ) )
        {
            m_pToplevelIconManager =
                ( xdg_toplevel_icon_manager_v1 * )wl_registry_bind(
                    pRegistry,
                    uName,
        ...
        {
            m_pDataDeviceManager = ( wl_data_device_manager * )wl_registry_bind(
                pRegistry, uName, &wl_data_device_manager_interface, 3u );
        }
        else if ( !strcmp(
                      pInterface,
                      zwp_primary_selection_device_manager_v1_interface.name ) )
        {
            m_pPrimarySelectionDeviceManager =
                ( zwp_primary_selection_device_manager_v1 * )wl_registry_bind(
                    pRegistry,
                    uName,
    ...
    void CWaylandBackend::Wayland_Modifier(
        zwp_linux_dmabuf_v1 *pDmabuf,
        uint32_t             uFormat,
        uint32_t             uModifierHi,
        uint32_t             uModifierLo )
    {
        ...
        if ( ulModifier != DRM_FORMAT_MOD_INVALID ) m_bCanUseModifiers = true;
    ...
    void CWaylandBackend::Wayland_Output_Geometry(
        wl_output  *pOutput,
        int32_t     nX,
        int32_t     nY,
        int32_t     nPhysicalWidth,
        ...
        int32_t     nTransform )
    {}
    void CWaylandBackend::Wayland_Output_Mode(
        wl_output *pOutput,
        uint32_t   uFlags,
        int32_t    nWidth,
        int32_t    nHeight,
    ...
    void
    CWaylandBackend::Wayland_Output_Scale( wl_output *pOutput, int32_t nFactor )
    { m_pOutputs[ pOutput ].nScale = nFactor; }
    void CWaylandBackend::Wayland_Output_Name(
        wl_output *pOutput, const char *pName )
    {}
    void CWaylandBackend::Wayland_Output_Description(
        wl_output *pOutput, const char *pDescription )
    {}

    // Seat

    void CWaylandBackend::Wayland_Seat_Capabilities(
        wl_seat *pSeat, uint32_t uCapabilities )
    {
        ...
        if ( !!( uCapabilities & WL_SEAT_CAPABILITY_KEYBOARD ) !=
             !!m_pKeyboard )
        {
            if ( m_pKeyboard )
            {
            ...
            else
            {
                ...
                wl_keyboard_add_listener(
                    m_pKeyboard, &s_KeyboardListener, this );
            }
        }
    }
    ...
    void CWaylandBackend::Wayland_Pointer_Enter(
        wl_pointer *pPointer,
        uint32_t    uSerial,
        wl_surface *pSurface,
        wl_fixed_t  fSurfaceX,
    ...
    void CWaylandBackend::Wayland_Pointer_Leave(
        wl_pointer *pPointer, uint32_t uSerial, wl_surface *pSurface )
    {
        if ( !IsGamescopeToplevel( pSurface ) ) return;
    ...
    void CWaylandBackend::Wayland_Keyboard_Enter(
        wl_keyboard *pKeyboard,
        uint32_t     uSerial,
        wl_surface  *pSurface,
        wl_array    *pKeys )
    ...
    void CWaylandBackend::Wayland_Keyboard_Leave(
        wl_keyboard *pKeyboard, uint32_t uSerial, wl_surface *pSurface )
    {
        if ( !IsGamescopeToplevel( pSurface ) ) return;
    ...
    void CWaylandBackend::Wayland_LockedPointer_Locked(
        zwp_locked_pointer_v1 *pLockedPointer )
    {
        m_bPointerLocked = true;
        UpdateCursor( );
    }
    void CWaylandBackend::Wayland_LockedPointer_Unlocked(
        zwp_locked_pointer_v1 *pLockedPointer )
    {
        m_bPointerLocked = false;
        UpdateCursor( );
    ...
    void CWaylandBackend::Wayland_WPColorManager_SupportedIntent(
        wp_color_manager_v1 *pWPColorManager, uint32_t uRenderIntent )
    {
        m_WPColorManagerFeatures.eRenderIntents.push_back(
            static_cast<wp_color_manager_v1_render_intent>( uRenderIntent ) );
    }
    void CWaylandBackend::Wayland_WPColorManager_SupportedFeature(
        wp_color_manager_v1 *pWPColorManager, uint32_t uFeature )
    {
        m_WPColorManagerFeatures.eFeatures.push_back(
            static_cast<wp_color_manager_v1_feature>( uFeature ) );
    }
    void CWaylandBackend::Wayland_WPColorManager_SupportedTFNamed(
        wp_color_manager_v1 *pWPColorManager, uint32_t uTF )
    {
        m_WPColorManagerFeatures.eTransferFunctions.push_back(
            static_cast<wp_color_manager_v1_transfer_function>( uTF ) );
    }
    void CWaylandBackend::Wayland_WPColorManager_SupportedPrimariesNamed(
        wp_color_manager_v1 *pWPColorManager, uint32_t uPrimaries )
    {
        m_WPColorManagerFeatures.ePrimaries.push_back(
            static_cast<wp_color_manager_v1_primaries>( uPrimaries ) );
    }
    void CWaylandBackend::Wayland_WPColorManager_ColorManagerDone(
        wp_color_manager_v1 *pWPColorManager )
    {}
    ...
    void CWaylandBackend::Wayland_DataSource_Cancelled(
        struct wl_data_source *pSource )
    ...
    bool CWaylandInputThread::Init( CWaylandBackend *pBackend )
    {
        ...
        if ( !( m_pQueue =
                    wl_display_create_queue( m_pBackend->GetDisplay( ) ) ) )
        {
            xdg_log.errorf( "Couldn't create input thread queue." );
            return false;
        }

        if ( !( m_pDisplayWrapper =
                    QueueLaunder( m_pBackend->GetDisplay( ) ) ) )
        {
        ...
        wl_registry *pRegistry;
        if ( !( pRegistry =
                    wl_display_get_registry( m_pDisplayWrapper.get( ) ) ) )
        {
    ...
    void CWaylandInputThread::ThreadFunc( )
    {
        ...
        {
            if ( ( nRet = wl_display_dispatch_queue_pending(
                       m_pBackend->GetDisplay( ), m_pQueue ) ) < 0 )
            {
                abort( );
            }

            if ( ( nRet = wl_display_prepare_read_queue(
                       m_pBackend->GetDisplay( ), m_pQueue ) ) < 0 )
            {
            ...
            if ( ( nRet = wl_display_read_events(
                       m_pBackend->GetDisplay( ) ) ) < 0 )
            {
                abort( );
            }
        }
    ...
    std::shared_ptr<T> CWaylandInputThread::QueueLaunder( T *pObject )
    {
        ...
        if ( !pObjectWrapper ) return nullptr;
        wl_proxy_set_queue( ( wl_proxy * )pObjectWrapper, m_pQueue );

        return std::shared_ptr<T>{
            pObjectWrapper,
            []( T *pThing ) { wl_proxy_wrapper_destroy( ( void * )pThing ); }
        };
    }

    void CWaylandInputThread::SetRelativePointer( bool bRelative )
    {
        if ( bRelative == !!m_pRelativePointer.load( ) ) return;
        // This constructors/destructors the display's mutex, so should be safe
        // to do across threads.
        if ( !bRelative ) { m_pRelativePointer = nullptr; }
        else
        {
            zwp_relative_pointer_v1 *pRelativePointer =
                zwp_relative_pointer_manager_v1_get_relative_pointer(
            ...
            m_pRelativePointer = std::shared_ptr<zwp_relative_pointer_v1>{
                pRelativePointer,
                []( zwp_relative_pointer_v1 *pObject )
                { zwp_relative_pointer_v1_destroy( pObject ); }
            };
            zwp_relative_pointer_v1_add_listener(
                pRelativePointer, &s_RelativePointerListener, this );
        }
    }
        ...
        if ( m_uKeyModifiers & m_uModMask[ GAMESCOPE_WAYLAND_MOD_META ] )
        {
            switch ( uKey )
            {
                case KEY_F:
                {
                    if ( !bPressed )
                    {
                        static_cast<CWaylandConnector *>(
                            m_pBackend->GetCurrentConnector( ) )
                ...
                case KEY_I:
                {
                    if ( !bPressed )
                    {
                        g_upscaleFilterSharpness =
                            std::min( 20, g_upscaleFilterSharpness + 1 );
                    }
                    return;
                }
                ...
                case KEY_O:
                {
                    ...
                    {
                        g_upscaleFilterSharpness =
                            std::max( 0, g_upscaleFilterSharpness - 1 );
                    }
                    return;
                }
    ...
    void CWaylandInputThread::Wayland_Seat_Capabilities(
        wl_seat *pSeat, uint32_t uCapabilities )
    {
        if ( !!( uCapabilities & WL_SEAT_CAPABILITY_POINTER ) != !!m_pPointer )
        {
    ...
    void
    CWaylandInputThread::Wayland_Seat_Name( wl_seat *pSeat, const char *pName )
    ...
    void CWaylandInputThread::Wayland_Pointer_Motion(
        ...
        wl_fixed_t  fSurfaceY )
    {
        if ( !m_bMouseEntered ) return;

        CWaylandPlane *pPlane = ( CWaylandPlane * )wl_surface_get_user_data(
            m_pCurrentCursorSurface );

        if ( !pPlane ) return;

        if ( m_pRelativePointer.load( ) != nullptr ) return;
        ...
        double flX = ( wl_fixed_to_double( fSurfaceX ) * uScale / 120.0 +
                       oState->nDestX ) /
                     g_nOutputWidth;
        double flY = ( wl_fixed_to_double( fSurfaceY ) * uScale / 120.0 +
                       oState->nDestY ) /
                     g_nOutputHeight;
        ...
        uint32_t    uState )
    {
        ...
        wlserver_mousebutton(
            uButton,
            uState == WL_POINTER_BUTTON_STATE_PRESSED,
            ++m_uFakeTimestamp );
        wlserver_unlock( );
    }
    void CWaylandInputThread::Wayland_Pointer_Axis(
        ...
        wl_fixed_t  fValue )
    {}
    void CWaylandInputThread::Wayland_Pointer_Axis_Source(
        wl_pointer *pPointer, uint32_t uAxisSource )
        ...
        wl_pointer *pPointer, uint32_t uAxis, int32_t nValue120 )
    {
        ...
        assert(
            ...
            uAxis == WL_POINTER_AXIS_HORIZONTAL_SCROLL );
    ...
    void CWaylandInputThread::Wayland_Pointer_Frame( wl_pointer *pPointer )
    {
        ...
        if ( m_uAxisSource != WL_POINTER_AXIS_SOURCE_WHEEL ) return;

        if ( flX == 0.0 && flY == 0.0 ) return;
        ...
        wl_keyboard *pKeyboard, uint32_t uFormat, int32_t nFd, uint32_t uSize )
    {
        ...
        // Ideally we'd use this to influence our keymap to clients, eg. x
        // server.

        defer( close( nFd ) );
        if ( uFormat != WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1 ) return;

        char *pMap =
            ( char * )mmap( nullptr, uSize, PROT_READ, MAP_PRIVATE, nFd, 0 );
        ...
        for ( uint32_t i = 0; i < GAMESCOPE_WAYLAND_MOD_COUNT; i++ )
            m_uModMask[ i ] = 1u << xkb_keymap_mod_get_index(
                                  m_pXkbKeymap,
                                  WaylandModifierToXkbModifierName(
                                      ( WaylandModifierIndex )i ) );
    }
        ...
        wl_array    *pKeys )
    {
        ...
        if ( m_ofPendingCursorX )
        {
            ...
            Wayland_Pointer_Motion(
                m_pPointer, 0, *m_ofPendingCursorX, *m_ofPendingCursorY );
        ...
        wl_fixed_t               fDyUnaccel )
    {
        ...
        if ( !m_pBackend->m_bPointerLocked ||
             ( !cv_wayland_mouse_relmotion_without_keyboard_focus &&
               !m_bKeyboardEntered ) )
            return;

        wlserver_lock( );
        wlserver_mousemotion(
            wl_fixed_to_double( fDxUnaccel ),
            wl_fixed_to_double( fDyUnaccel ),
            ++m_uFakeTimestamp );
        wlserver_unlock( );
    }
    ...
    template<> bool IBackend::Set<CWaylandBackend>( )
