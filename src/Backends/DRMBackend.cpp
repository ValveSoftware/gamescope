...
gamescope::ConVar<bool> cv_drm_single_plane_optimizations(
    "drm_single_plane_optimizations",
    true,
    "Whether or not to enable optimizations for single plane usage." );

gamescope::ConVar<bool> cv_drm_debug_disable_shaper_and_3dlut(
    "drm_debug_disable_shaper_and_3dlut",
    false,
    "Shaper + 3DLUT chicken bit. (Force disable/DEFAULT, no logic change)" );
gamescope::ConVar<bool> cv_drm_debug_disable_degamma_tf(
    "drm_debug_disable_degamma_tf",
    false,
    "Degamma chicken bit. (Forces DEGAMMA_TF to DEFAULT, does not affect other "
    "logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_regamma_tf(
    "drm_debug_disable_regamma_tf",
    false,
    "Regamma chicken bit. (Forces REGAMMA_TF to DEFAULT, does not affect other "
    "logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_output_tf(
    "drm_debug_disable_output_tf",
    false,
    "Force default (identity) output TF, affects other logic. Not a property "
    "directly." );
gamescope::ConVar<bool> cv_drm_debug_disable_blend_tf(
    "drm_debug_disable_blend_tf",
    false,
    "Blending chicken bit. (Forces BLEND_TF to DEFAULT, does not affect other "
    "logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_ctm(
    "drm_debug_disable_ctm",
    false,
    "CTM chicken bit. (Forces CTM off, does not affect other logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_color_encoding(
    "drm_debug_disable_color_encoding",
    false,
    "YUV Color Encoding chicken bit. (Forces COLOR_ENCODING to DEFAULT, does "
    "not affect other logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_color_range(
    "drm_debug_disable_color_range",
    false,
    "YUV Color Range chicken bit. (Forces COLOR_RANGE to DEFAULT, does not "
    "affect other logic)" );
gamescope::ConVar<bool> cv_drm_debug_disable_explicit_sync(
    "drm_debug_disable_explicit_sync",
    false,
    "Force disable explicit sync on the DRM backend." );
gamescope::ConVar<bool> cv_drm_debug_disable_in_fence_fd(
    "drm_debug_disable_in_fence_fd",
    false,
    "Force disable IN_FENCE_FD being set to avoid over-synchronization on the "
    "DRM backend." );

gamescope::ConVar<bool> cv_drm_allow_dynamic_modes_for_external_display(
    "drm_allow_dynamic_modes_for_external_display",
    false,
    "Allow dynamic mode/refresh rate switching for external displays." );
...
gamescope::ConVar<bool> cv_drm_ignore_internal_connectors(
    "drm_ignore_internal_connectors",
    false,
    "Disable internal displays for good, for debugging." );
...
namespace gamescope
{
    ...
    {
        utsname name;
        if ( uname( &name ) != 0 ) return std::make_tuple( 0, 0, 0 );

        std::vector<std::string_view> szVersionParts =
            Split( name.release, "." );
    ...
    {
        int32_t nRefresh =
            ( mode->clock * 1'000'000ll / mode->htotal + mode->vtotal / 2 ) /
            mode->vtotal;

        if ( mode->flags & DRM_MODE_FLAG_INTERLACE ) nRefresh *= 2;

        if ( mode->flags & DRM_MODE_FLAG_DBLSCAN ) nRefresh /= 2;

        if ( mode->vscan > 1 ) nRefresh /= mode->vscan;
    ...
    using DRMObjectRawProperties =
        std::unordered_map<std::string, DRMObjectRawProperty>;
    ...
    public:
        CDRMAtomicProperty(
            CDRMAtomicObject *pObject, DRMObjectRawProperty rawProperty );

        static std::optional<CDRMAtomicProperty> Instantiate(
            const char                   *pszName,
            CDRMAtomicObject             *pObject,
            const DRMObjectRawProperties &rawProperties );
        ...
        int      SetPendingValue(
            drmModeAtomicReq *pRequest, uint64_t ulValue, bool bForce );
    ...
    class CDRMConnector final
        : public CBaseBackendConnector,
          public CDRMAtomicTypedObject<DRM_MODE_OBJECT_CONNECTOR>
    {
    public:
        ...
        struct ConnectorProperties
        {
            ...
            std::optional<CDRMAtomicProperty>
                content_type; // "content type" with space!
            std::optional<CDRMAtomicProperty>
                panel_orientation; // "panel orientation" with space!
        ...
        uint32_t    GetPossibleCRTCMask( ) const
        { return m_Mutable.uPossibleCRTCMask; }
        std::span<const uint32_t> GetValidDynamicRefreshRates( ) const override
        { return m_Mutable.ValidDynamicRefreshRates; }
        const displaycolorimetry_t &GetDisplayColorimetry( ) const
        { return m_Mutable.DisplayColorimetry; }

        std::span<const uint8_t> GetRawEDID( ) const override
        {
            return std::span<const uint8_t>{ m_Mutable.EdidData.begin( ),
                                             m_Mutable.EdidData.end( ) };
        }
        ...
        {
            return !!GetProperties( ).Colorspace &&
                   !!GetProperties( ).HDR_OUTPUT_METADATA &&
                   GetHDRInfo( ).IsHDR10( );
        }

        bool SupportsHDRG22( ) const { return GetHDRInfo( ).IsHDRG22( ); }
        ...
        bool IsHDRActive( ) const override
        {
            ...
            else if ( SupportsHDRG22( ) ) { return true; }

            return false;
        }

        const BackendConnectorHDRInfo &GetHDRInfo( ) const override
        { return m_Mutable.HDR; }
        ...
        virtual bool IsVRRActive( ) const override
        {
            ...
            return !!g_DRM.pCRTC->GetProperties( )
                         .VRR_ENABLED->GetCurrentValue( );
        }

        virtual std::span<const BackendMode> GetModes( ) const override
        { return m_Mutable.BackendModes; }

        bool SupportsVRR( ) const override
        {
            return this->GetProperties( ).vrr_capable &&
                   !!this->GetProperties( ).vrr_capable->GetCurrentValue( );
        }
            ...
            EOTF                 *outputEncodingEOTF ) const override
        {
            ...
            *displayEOTF        = EOTF_Gamma22;
            ...
            {
                // For HDR10 output, expected content colorspace != native
                // colorspace.
        ...
        virtual int
        Present( const FrameInfo_t *pFrameInfo, bool bAsync ) override;

        using DRMModeGenerator =
            std::function<drmModeModeInfo( const drmModeModeInfo *, int )>;
    ...
    private:
        ...
        struct MutableConnectorState
        {
            ...
            const char *pszMake = ""; // Not owned, no free. This is a pointer
                                      // to pnp db or szMakePNP.
        ...
        GamescopePanelOrientation m_ChosenOrientation =
...
uint32_t g_nDRMFormatOverlay =
    DRM_FORMAT_INVALID; // for partial composition, we may have more limited
                        // formats than base planes + alpha.
...
static constexpr uint32_t s_kSteamDeckOLEDRates[] = {
    45, 47, 48, 49, 50, 51, 53, 55, 56, 59, 60, 62, 64, 65, 66,
    68, 72, 73, 76, 77, 78, 80, 81, 82, 84, 85, 86, 87, 88, 90,
};
...
static gamescope::CDRMCRTC *find_crtc_for_connector(
    struct drm_t *drm, gamescope::CDRMConnector *pConnector )
{
...
static bool get_plane_formats(
    struct drm_t              *drm,
    gamescope::CDRMPlane      *pPlane,
    struct wlr_drm_format_set *pFormatSet )
{
    ...
    {
        const uint64_t ulBlobId =
            pPlane->GetProperties( ).IN_FORMATS->GetCurrentValue( );

        drmModePropertyBlobRes *pBlob =
            drmModeGetPropertyBlob( drm->fd, ulBlobId );
        if ( !pBlob )
        {
        ...
        drm_format_modifier_blob *pModifierBlob =
            reinterpret_cast<drm_format_modifier_blob *>( pBlob->data );

        uint32_t *pFormats = reinterpret_cast<uint32_t *>(
            reinterpret_cast<uint8_t *>( pBlob->data ) +
            pModifierBlob->formats_offset );
        drm_format_modifier *pMods = reinterpret_cast<drm_format_modifier *>(
            reinterpret_cast<uint8_t *>( pBlob->data ) +
            pModifierBlob->modifiers_offset );
...
static uint32_t pick_plane_format(
...
{
    if ( !drm->pCRTC ) return nullptr;

    for ( std::unique_ptr<gamescope::CDRMPlane> &pPlane : drm->planes )
    {
        if ( pPlane->GetModePlane( )->possible_crtcs &
             drm->pCRTC->GetCRTCMask( ) )
        {
            if ( pPlane->GetProperties( ).type->GetCurrentValue( ) ==
    ...
    void        *data )
{
    ...
    if ( !g_DRM.pCRTC ) return;

    if ( g_DRM.pCRTC->GetObjectId( ) != crtc_id ) return;
    ...
    // TODO: get the fbids_queued instance from data if we ever have more than
    // one in flight

    drm_log.debugf(
        "page_flip_handler %" PRIu64 " delta: %" PRIu64,
        pCtx->ulPendingFlipCount,
        vblanktime - ulLastVBlankTime );
    gpuvis_trace_printf(
        "page_flip_handler %" PRIu64, pCtx->ulPendingFlipCount );

    ulLastVBlankTime = vblanktime;

    {
        std::scoped_lock lock{ g_DRM.m_QueuedFbIdsMutex,
                               g_DRM.m_mutVisibleFbIds };
...
static bool refresh_state( drm_t *drm )
{
    ...
    for ( int i = 0; i < pResources->count_connectors; i++ )
    {
        ...
        drmModeConnector *pConnector =
            drmModeGetConnector( drm->fd, uConnectorId );
        ...
        if ( !drm->connectors.contains( uConnectorId ) )
        {
            drm->connectors.emplace(
                ...
                std::forward_as_tuple(
                    reinterpret_cast<gamescope::CDRMBackend *>( GetBackend( ) ),
                    pConnector ) );
        }
    }

    // Remove connectors which disappeared
    for ( auto iter = drm->connectors.begin( );
          iter != drm->connectors.end( ); )
    {
        ...
        {
            drm_log.debugf(
                "Connector '%s' disappeared.", pConnector->GetName( ) );

            if ( drm->pConnector == pConnector )
            {
                drm_log.infof(
                    "Current connector '%s' disappeared.",
                    pConnector->GetName( ) );
...
{
    {
        ...
        {
            drmModeCrtc *pCRTC =
                drmModeGetCrtc( drm->fd, pResources->crtcs[ i ] );
            if ( pCRTC )
                drm->crtcs.emplace_back(
                    std::make_unique<gamescope::CDRMCRTC>( pCRTC, 1u << i ) );
        }
    }

    {
        ...
        {
            drmModePlane *pPlane =
                drmModeGetPlane( drm->fd, pPlaneResources->planes[ i ] );
            if ( pPlane )
                drm->planes.emplace_back(
                    std::make_unique<gamescope::CDRMPlane>( pPlane ) );
        }
    }
...
static const drmModeModeInfo *find_mode(
    ...
    uint32_t                vrefresh )
{
    for ( int i = 0; i < connector->count_modes; i++ )
    {
        ...
        {
            bad |=
                ( badMode.width == 0 || mode->hdisplay == badMode.width ) &&
        ...
        if ( hdisplay != 0 && hdisplay != mode->hdisplay ) continue;
        if ( vdisplay != 0 && vdisplay != mode->vdisplay ) continue;
        if ( vrefresh != 0 && vrefresh != mode->vrefresh ) continue;
...
parse_connector_priorities( const char *str )
{
    std::unordered_map<std::string, int> priorities{};
    if ( !str ) { return priorities; }
    int   i    = 0;
    char *buf  = strdup( str );
...
static bool get_saved_mode( const char *description, saved_mode &mode_info )
{
    ...
    if ( !mode_file || !*mode_file ) return false;
...
static GamescopeBroadcastRGBMode_t s_ExternalBroadcastRGBMode =
    GAMESCOPE_BROADCAST_RGB_MODE_AUTOMATIC;

static bool setup_best_connector( struct drm_t *drm, bool force, bool initial )
{
    if ( drm->pConnector && drm->pConnector->GetModeConnector( )->connection !=
                                DRM_MODE_CONNECTED )
    {
        drm_log.infof(
            "current connector '%s' disconnected",
            drm->pConnector->GetName( ) );
        drm->pConnector = nullptr;
    }
    ...
    for ( auto &iter : drm->connectors )
    {
        ...
        if ( g_bForceInternal && pConnector->GetScreenType( ) ==
                                     gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL )
            continue;
    ...
    if ( best && best == drm->pConnector )
    {
        ...
        // as we might
        if ( best->HandleEdidChange( ) ) { force = true; }
    }

    if ( !force )
    {
        if ( ( !best && drm->pConnector ) ||
             ( best && best == drm->pConnector ) )
        {
    ...
    if ( !drm_set_connector( drm, best ) ) { return false; }
    ...
    if ( drm->preferred_width != 0 || drm->preferred_height != 0 ||
         drm->preferred_refresh != 0 )
    {
        mode = find_mode(
            best->GetModeConnector( ),
            drm->preferred_width,
            drm->preferred_height,
            gamescope::ConvertmHzToHz( drm->preferred_refresh ) );
    }

    if ( !mode &&
         best->GetScreenType( ) == gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL )
    {
    ...
    if ( !mode ) { mode = find_mode( best->GetModeConnector( ), 0, 0, 0 ); }
    ...
    if ( !drm_set_mode( drm, mode ) ) { return false; }
    ...
    if ( !initial )
        WritePatchedEdid(
            best->GetRawEDID( ), best->GetHDRInfo( ), g_bRotated );
...
static void gamescope_liftoff_log_handler(
    enum liftoff_log_priority liftoff_priority, const char *fmt, va_list args )
{
...
bool init_drm( struct drm_t *drm, int width, int height, int refresh )
{
    ...
    if ( vulkan_primary_dev_id( &dev_id ) )
    {
        ...
        {
            drm_log.errorf(
                "Failed to find DRM device with device ID %" PRIu64,
                ( uint64_t )dev_id );
            return false;
        }
    ...
    uint64_t cap;
    g_bSupportsSyncObjs =
        drmGetCap( drm->fd, DRM_CAP_SYNCOBJ, &cap ) == 0 && cap != 0;
    if ( g_bSupportsSyncObjs )
    {
        int err = drmSyncobjCreate(
            drm->fd, DRM_SYNCOBJ_CREATE_SIGNALED, &g_uAlwaysSignalledSyncobj );
        if ( err < 0 )
        {
        ...
        err = drmSyncobjExportSyncFile(
            drm->fd, g_uAlwaysSignalledSyncobj, &g_nAlwaysSignalledSyncFile );
        if ( err < 0 )
        {
    ...
    g_bSupportsAsyncFlips =
        drmGetCap( drm->fd, DRM_CAP_ATOMIC_ASYNC_PAGE_FLIP, &cap ) == 0 &&
        cap != 0;
    if ( !g_bSupportsAsyncFlips )
        drm_log.errorf( "Immediate flips are not supported by the KMS driver" );

    static bool async_disabled =
        env_to_bool( getenv( "GAMESCOPE_DISABLE_ASYNC_FLIPS" ) );
    ...
    if ( !get_resources( drm ) ) { return false; }

    drm->lo_device = liftoff_device_create( drm->fd );
    if ( drm->lo_device == nullptr ) return false;
    ...
    if ( !setup_best_connector( drm, true, true ) ) { return false; }
    ...
    if ( !drm->pPrimaryPlane ) drm->pPrimaryPlane = find_primary_plane( drm );
    ...
    // Pick a 10-bit format at first for our composition buffer, for a couple of
    // reasons:
    //
    // 1. Many game engines automatically render to 10-bit formats such as UE4
    // which means that when we have to composite, we can keep the same HW
    // dithering that we would get if we just scanned them out directly.
    //
    // 2. When compositing HDR content as a fallback when we undock, it avoids
    // introducing a bunch of horrible banding when going to G2.2 curve. It
    // ensures that we can dither that.
    g_nDRMFormat = pick_plane_format(
        &drm->primary_formats, DRM_FORMAT_XRGB2101010, DRM_FORMAT_ARGB2101010 );
    if ( g_nDRMFormat == DRM_FORMAT_INVALID )
    {
        ...
        if ( g_nDRMFormat == DRM_FORMAT_INVALID )
        {
            ...
            {
                drm_log.errorf(
                    "Primary plane doesn't support any formats >= 8888" );
                return false;
            }
        }
    }
    ...
    {
        // ARGB8888 is the Xformat and AFormat here in this function as we want
        // transparent overlay
        g_nDRMFormatOverlay = pick_plane_format(
            &drm->formats, DRM_FORMAT_ARGB2101010, DRM_FORMAT_ARGB2101010 );
        if ( g_nDRMFormatOverlay == DRM_FORMAT_INVALID )
        {
            g_nDRMFormatOverlay = pick_plane_format(
                &drm->formats, DRM_FORMAT_ABGR2101010, DRM_FORMAT_ABGR2101010 );
            if ( g_nDRMFormatOverlay == DRM_FORMAT_INVALID )
            {
                g_nDRMFormatOverlay = pick_plane_format(
                    &drm->formats, DRM_FORMAT_ARGB8888, DRM_FORMAT_ARGB8888 );
                if ( g_nDRMFormatOverlay == DRM_FORMAT_INVALID )
                {
                    drm_log.errorf(
                        "Overlay plane doesn't support any formats >= 8888" );
                    return false;
                }
            }
        }
...
void OnSleepScreenChanged( gamescope::ConVar<bool> & ) { force_repaint( ); }

gamescope::ConVar<bool> cv_drm_sleep_screens[] = {
    { "drm_sleep_internal_screen",
      false,
      "Force the internal screen to be asleep",
      OnSleepScreenChanged },
    { "drm_sleep_external_screen",
      false,
      "Force the external screen to be asleep",
      OnSleepScreenChanged },
};

void drm_sleep_screen( gamescope::GamescopeScreenType eType, bool bSleep )
{
    if ( cv_drm_sleep_screens[ eType ] == bSleep ) return;
...
void finish_drm( struct drm_t *drm )
{
    ...
    for ( auto &iter : drm->connectors )
    {
        ...
        if ( pConnector->GetProperties( ).Colorspace )
            pConnector->GetProperties( ).Colorspace->SetPendingValue(
                req, 0, true );

        if ( pConnector->GetProperties( ).HDR_OUTPUT_METADATA )
        {
            if ( drm->sdr_static_metadata &&
                 pConnector->GetHDRInfo( ).IsHDR10( ) )
                pConnector->GetProperties( )
                    .HDR_OUTPUT_METADATA->SetPendingValue(
                        req, drm->sdr_static_metadata->GetBlobValue( ), true );
            else
                pConnector->GetProperties( )
                    .HDR_OUTPUT_METADATA->SetPendingValue( req, 0, true );
        }

        if ( pConnector->GetProperties( ).content_type )
            pConnector->GetProperties( ).content_type->SetPendingValue(
                req, 0, true );
    }
    ...
    for ( std::unique_ptr<gamescope::CDRMCRTC> &pCRTC : drm->crtcs )
    {
        ...
        if ( pCRTC->GetProperties( ).DEGAMMA_LUT )
            pCRTC->GetProperties( ).DEGAMMA_LUT->SetPendingValue(
                req, 0, true );
        ...
        if ( pCRTC->GetProperties( ).VRR_ENABLED )
            pCRTC->GetProperties( ).VRR_ENABLED->SetPendingValue(
                req, 0, true );

        if ( pCRTC->GetProperties( ).OUT_FENCE_PTR )
            pCRTC->GetProperties( ).OUT_FENCE_PTR->SetPendingValue(
                req, 0, true );

        if ( pCRTC->GetProperties( ).AMD_CRTC_REGAMMA_TF )
            pCRTC->GetProperties( ).AMD_CRTC_REGAMMA_TF->SetPendingValue(
                req, 0, true );
    }
    ...
    for ( std::unique_ptr<gamescope::CDRMPlane> &pPlane : drm->planes )
    {
        ...
        if ( pPlane->GetProperties( ).rotation )
            pPlane->GetProperties( ).rotation->SetPendingValue(
                req, DRM_MODE_ROTATE_0, true );

        if ( pPlane->GetProperties( ).alpha )
            pPlane->GetProperties( ).alpha->SetPendingValue(
                req, 0xFFFF, true );
        ...
        if ( pPlane->GetProperties( ).AMD_PLANE_DEGAMMA_TF )
            pPlane->GetProperties( ).AMD_PLANE_DEGAMMA_TF->SetPendingValue(
                req, AMDGPU_TRANSFER_FUNCTION_DEFAULT, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_DEGAMMA_LUT )
            pPlane->GetProperties( ).AMD_PLANE_DEGAMMA_LUT->SetPendingValue(
                req, 0, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_CTM )
            pPlane->GetProperties( ).AMD_PLANE_CTM->SetPendingValue(
                req, 0, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_HDR_MULT )
            pPlane->GetProperties( ).AMD_PLANE_HDR_MULT->SetPendingValue(
                req, 0x100000000ULL, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_SHAPER_TF )
            pPlane->GetProperties( ).AMD_PLANE_SHAPER_TF->SetPendingValue(
                req, AMDGPU_TRANSFER_FUNCTION_DEFAULT, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_SHAPER_LUT )
            pPlane->GetProperties( ).AMD_PLANE_SHAPER_LUT->SetPendingValue(
                req, 0, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_LUT3D )
            pPlane->GetProperties( ).AMD_PLANE_LUT3D->SetPendingValue(
                req, 0, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_BLEND_TF )
            pPlane->GetProperties( ).AMD_PLANE_BLEND_TF->SetPendingValue(
                req, AMDGPU_TRANSFER_FUNCTION_DEFAULT, true );

        if ( pPlane->GetProperties( ).AMD_PLANE_BLEND_LUT )
            pPlane->GetProperties( ).AMD_PLANE_BLEND_LUT->SetPendingValue(
                req, 0, true );
    }
...
drm_fbid_from_dmabuf( struct drm_t *drm, struct wlr_dmabuf_attributes *dma_buf )
{
    gamescope::OwningRc<gamescope::IBackendFb> pBackendFb;
    uint32_t                                   fb_id = 0;

    if ( !wlr_drm_format_set_has(
             &drm->formats, dma_buf->format, dma_buf->modifier ) )
    {
        drm_log.errorf(
            "Cannot import FB to DRM: format 0x%" PRIX32
            " and modifier 0x%" PRIX64 " not supported for scan-out",
            dma_buf->format,
            dma_buf->modifier );
        return nullptr;
    }
    ...
    {
        if ( drmPrimeFDToHandle( drm->fd, dma_buf->fd[ i ], &handles[ i ] ) !=
             0 )
        {
    ...
    if ( dma_buf->modifier != DRM_FORMAT_MOD_INVALID )
    {
        ...
        {
            drm_log.errorf(
                "Cannot import DMA-BUF: has a modifier (0x%" PRIX64
                "), but KMS doesn't support them",
                dma_buf->modifier );
            goto out;
        }
...
out:
    for ( int i = 0; i < dma_buf->n_planes; i++ )
    {
        ...
        for ( int j = 0; j < i; j++ )
        {
            if ( handles[ i ] == handles[ j ] ) already_closed = true;
        }
...
static void update_drm_effective_orientations(
    struct drm_t *drm, const drmModeModeInfo *pMode )
{
    gamescope::IBackendConnector *pInternalConnector =
        GetBackend( )->GetConnector(
            gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL );
    if ( pInternalConnector )
    {
        gamescope::CDRMConnector *pDRMInternalConnector =
            static_cast<gamescope::CDRMConnector *>( pInternalConnector );
        const drmModeModeInfo *pInternalMode = pMode;
        if ( pDRMInternalConnector != drm->pConnector )
            pInternalMode = find_mode(
                pDRMInternalConnector->GetModeConnector( ), 0, 0, 0 );

        pDRMInternalConnector->UpdateEffectiveOrientation( pInternalMode );
    }

    gamescope::IBackendConnector *pExternalConnector =
        GetBackend( )->GetConnector(
            gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL );
    if ( pExternalConnector )
    {
        gamescope::CDRMConnector *pDRMExternalConnector =
            static_cast<gamescope::CDRMConnector *>( pExternalConnector );
        const drmModeModeInfo *pExternalMode = pMode;
        if ( pDRMExternalConnector != drm->pConnector )
            pExternalMode = find_mode(
                pDRMExternalConnector->GetModeConnector( ), 0, 0, 0 );
...
template<typename T> void hash_combine( size_t &s, const T &v )
{
    std::hash<T> h;
...
std::unordered_set<LiftoffStateCacheEntry, LiftoffStateCacheEntryKasher>
    g_LiftoffStateCache;

static inline amdgpu_transfer_function
colorspace_to_plane_degamma_tf( GamescopeAppTextureColorspace colorspace )
{
    switch ( colorspace )
    {
        default: // Linear in this sense is SRGB. Linear = sRGB image view doing
                 // automatic sRGB -> Linear which doesn't happen on DRM side.
        ...
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_SCRGB:
            // Use LINEAR TF for scRGB float format as 80 nit = 1.0 in scRGB,
            // which matches what PQ TF decodes to/encodes from. AMD internal
            // format is FP16, and generally expected for 1.0 -> 80 nit. which
            // just so happens to match scRGB.
...
static inline amdgpu_transfer_function
colorspace_to_plane_shaper_tf( GamescopeAppTextureColorspace colorspace )
{
    switch ( colorspace )
    {
        default:
        ...
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_SCRGB: // scRGB Linear -> PQ for
                                                     // shaper + 3D LUT
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_HDR10_PQ:
...
ColorSpaceToEOTFIndex( GamescopeAppTextureColorspace colorspace )
{
    switch ( colorspace )
    {
        default:
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_LINEAR: // Not actually linear,
                                                      // just Linear vs sRGB
                                                      // image views in Vulkan.
                                                      // Still viewed as sRGB on
                                                      // the DRM side.
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_SRGB:
            // SDR sRGB content treated as native Gamma 22 curve. No need to do
            // sRGB -> 2.2 or whatever.
            return EOTF_Gamma22;
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_SCRGB:
            // Okay, so this is WEIRD right? OKAY Let me explain it to you.
            // The plan for scRGB content is to go from scRGB -> PQ in a
            // SHAPER_TF before indexing into the shaper. (input from
            // colorspace_to_plane_regamma_tf!)
            return EOTF_PQ;
        case GAMESCOPE_APP_TEXTURE_COLORSPACE_HDR10_PQ:
            return EOTF_PQ;
    }
...
LiftoffStateCacheEntry FrameInfoToLiftoffStateCacheEntry(
    struct drm_t *drm, const FrameInfo_t *frameInfo )
{
    ...
    for ( int i = 0; i < entry.nLayerCount; i++ )
    {
        ...
        if ( g_bRotated )
        {
            int64_t imageH = frameInfo->layers[ i ].tex->contentHeight( ) /
                             frameInfo->layers[ i ].scale.y;
        ...
        if ( entry.layerState[ i ].ycbcr )
        {
            entry.layerState[ i ].colorEncoding =
                drm_get_color_encoding( g_ForcedNV12ColorSpace );
            entry.layerState[ i ].colorRange =
                drm_get_color_range( g_ForcedNV12ColorSpace );
            entry.layerState[ i ].colorspace =
        ...
        entry.layerState[ i ].eAlphaBlendingMode =
            frameInfo->layers[ i ].eAlphaBlendingMode;
    }
...
static bool is_liftoff_caching_enabled( )
{
    static bool disabled =
        env_to_bool( getenv( "GAMESCOPE_LIFTOFF_CACHE_DISABLE" ) );
    return !disabled;
}
...
namespace gamescope
{
    ...
    /////////////////////////
    template<uint32_t DRMObjectType>
    CDRMAtomicTypedObject<DRMObjectType>::CDRMAtomicTypedObject(
        uint32_t ulObjectId ) :
        CDRMAtomicObject{ ulObjectId }
    {}

    template<uint32_t DRMObjectType>
    std::optional<DRMObjectRawProperties>
    CDRMAtomicTypedObject<DRMObjectType>::GetRawProperties( )
    {
        drmModeObjectProperties *pProperties =
            drmModeObjectGetProperties( g_DRM.fd, m_ulObjectId, DRMObjectType );
        if ( !pProperties )
        {
        ...
        {
            drmModePropertyRes *pProperty =
                drmModeGetProperty( g_DRM.fd, pProperties->props[ i ] );
            if ( !pProperty ) continue;
            defer( drmModeFreeProperty( pProperty ) );

            rawProperties[ pProperty->name ] =
                DRMObjectRawProperty{ pProperty->prop_id,
                                      pProperties->prop_values[ i ] };
        }
    ...
    // CDRMAtomicProperty
    /////////////////////////
    CDRMAtomicProperty::CDRMAtomicProperty(
        CDRMAtomicObject *pObject, DRMObjectRawProperty rawProperty ) :
        m_pObject{ pObject },
    ...
    /*static*/ std::optional<CDRMAtomicProperty>
        ...
        const DRMObjectRawProperties &rawProperties )
    {
        ...
        if ( iter == rawProperties.end( ) ) return std::nullopt;

        return CDRMAtomicProperty{ pObject, iter->second };
    }

    int CDRMAtomicProperty::SetPendingValue(
        drmModeAtomicReq *pRequest, uint64_t ulValue, bool bForce /*= false*/ )
    {
        // In instances where we rolled back due to -EINVAL, or we want to
        // ensure a value from an unclean state eg. from an unclean or other
        // initial state, you can force an update in the request with bForce.

        if ( ulValue == m_ulPendingValue && !bForce ) return 0;

        int ret = drmModeAtomicAddProperty(
            pRequest, m_pObject->GetObjectId( ), m_uPropertyId, ulValue );
        if ( ret < 0 ) return ret;
    ...
    CDRMPlane::CDRMPlane( drmModePlane *pPlane ) :
        ...
        m_pPlane{ pPlane,
                  []( drmModePlane *pPlane ) { drmModeFreePlane( pPlane ); } }
    { RefreshState( ); }
    ...
    void CDRMPlane::RefreshState( )
    {
        ...
        {
            m_Props.type =
                CDRMAtomicProperty::Instantiate( "type", this, *rawProperties );
            m_Props.IN_FORMATS = CDRMAtomicProperty::Instantiate(
                "IN_FORMATS", this, *rawProperties );

            m_Props.FB_ID = CDRMAtomicProperty::Instantiate(
                "FB_ID", this, *rawProperties );
            m_Props.IN_FENCE_FD = CDRMAtomicProperty::Instantiate(
                "IN_FENCE_FD", this, *rawProperties );
            m_Props.CRTC_ID = CDRMAtomicProperty::Instantiate(
                "CRTC_ID", this, *rawProperties );
            m_Props.SRC_X = CDRMAtomicProperty::Instantiate(
                "SRC_X", this, *rawProperties );
            m_Props.SRC_Y = CDRMAtomicProperty::Instantiate(
                "SRC_Y", this, *rawProperties );
            m_Props.SRC_W = CDRMAtomicProperty::Instantiate(
                "SRC_W", this, *rawProperties );
            m_Props.SRC_H = CDRMAtomicProperty::Instantiate(
                "SRC_H", this, *rawProperties );
            m_Props.CRTC_X = CDRMAtomicProperty::Instantiate(
                "CRTC_X", this, *rawProperties );
            m_Props.CRTC_Y = CDRMAtomicProperty::Instantiate(
                "CRTC_Y", this, *rawProperties );
            m_Props.CRTC_W = CDRMAtomicProperty::Instantiate(
                "CRTC_W", this, *rawProperties );
            m_Props.CRTC_H = CDRMAtomicProperty::Instantiate(
                "CRTC_H", this, *rawProperties );
            m_Props.zpos =
                CDRMAtomicProperty::Instantiate( "zpos", this, *rawProperties );
            m_Props.alpha = CDRMAtomicProperty::Instantiate(
                "alpha", this, *rawProperties );
            m_Props.rotation = CDRMAtomicProperty::Instantiate(
                "rotation", this, *rawProperties );
            m_Props.COLOR_ENCODING = CDRMAtomicProperty::Instantiate(
                "COLOR_ENCODING", this, *rawProperties );
            m_Props.COLOR_RANGE = CDRMAtomicProperty::Instantiate(
                "COLOR_RANGE", this, *rawProperties );
            m_Props.AMD_PLANE_DEGAMMA_TF = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_DEGAMMA_TF", this, *rawProperties );
            m_Props.AMD_PLANE_DEGAMMA_LUT = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_DEGAMMA_LUT", this, *rawProperties );
            m_Props.AMD_PLANE_CTM = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_CTM", this, *rawProperties );
            m_Props.AMD_PLANE_HDR_MULT = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_HDR_MULT", this, *rawProperties );
            m_Props.AMD_PLANE_SHAPER_LUT = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_SHAPER_LUT", this, *rawProperties );
            m_Props.AMD_PLANE_SHAPER_TF = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_SHAPER_TF", this, *rawProperties );
            m_Props.AMD_PLANE_LUT3D = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_LUT3D", this, *rawProperties );
            m_Props.AMD_PLANE_BLEND_TF = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_BLEND_TF", this, *rawProperties );
            m_Props.AMD_PLANE_BLEND_LUT = CDRMAtomicProperty::Instantiate(
                "AMD_PLANE_BLEND_LUT", this, *rawProperties );
        }
    }
    ...
    CDRMCRTC::CDRMCRTC( drmModeCrtc *pCRTC, uint32_t uCRTCMask ) :
        ...
        m_pCRTC{ pCRTC,
                 []( drmModeCrtc *pCRTC ) { drmModeFreeCrtc( pCRTC ); } },
    ...
    void CDRMCRTC::RefreshState( )
    {
        ...
        if ( rawProperties )
        {
            m_Props.ACTIVE = CDRMAtomicProperty::Instantiate(
                "ACTIVE", this, *rawProperties );
            m_Props.MODE_ID = CDRMAtomicProperty::Instantiate(
                "MODE_ID", this, *rawProperties );
            m_Props.GAMMA_LUT = CDRMAtomicProperty::Instantiate(
                "GAMMA_LUT", this, *rawProperties );
            m_Props.DEGAMMA_LUT = CDRMAtomicProperty::Instantiate(
                "DEGAMMA_LUT", this, *rawProperties );
            m_Props.CTM =
                CDRMAtomicProperty::Instantiate( "CTM", this, *rawProperties );
            m_Props.VRR_ENABLED = CDRMAtomicProperty::Instantiate(
                "VRR_ENABLED", this, *rawProperties );
            m_Props.OUT_FENCE_PTR = CDRMAtomicProperty::Instantiate(
                "OUT_FENCE_PTR", this, *rawProperties );
            m_Props.AMD_CRTC_REGAMMA_TF = CDRMAtomicProperty::Instantiate(
                "AMD_CRTC_REGAMMA_TF", this, *rawProperties );
        }
    }
    ...
    // CDRMConnector
    /////////////////////////
    CDRMConnector::CDRMConnector(
        CDRMBackend *pBackend, drmModeConnector *pConnector ) :
        CDRMAtomicTypedObject<DRM_MODE_OBJECT_CONNECTOR>(
            pConnector->connector_id ),
        m_pBackend{ pBackend },
    ...
    void CDRMConnector::RefreshState( )
    {
        ...
        // This isn't needed for CRTC/Planes in which the state is immutable for
        // their lifetimes. Connectors can be re-plugged.
        ...
        std::stable_sort(
            m_pConnector->modes,
            m_pConnector->modes + m_pConnector->count_modes,
            []( const drmModeModeInfo &a, const drmModeModeInfo &b )
            {
                bool bGoodRefreshA = a.vrefresh >= 60;
                bool bGoodRefreshB = b.vrefresh >= 60;
                if ( bGoodRefreshA != bGoodRefreshB ) return bGoodRefreshA;

                bool bPreferredA = a.type & DRM_MODE_TYPE_PREFERRED;
                bool bPreferredB = b.type & DRM_MODE_TYPE_PREFERRED;
                if ( bPreferredA != bPreferredB ) return bPreferredA;

                int nAreaA = a.hdisplay * a.vdisplay;
                int nAreaB = b.hdisplay * b.vdisplay;
                if ( nAreaA != nAreaB ) return nAreaA > nAreaB;
        ...
        m_Mutable.uPossibleCRTCMask =
            drmModeConnectorGetPossibleCrtcs( g_DRM.fd, GetModeConnector( ) );

        // These are string constants from libdrm, no free.
        const char *pszTypeStr =
            drmModeGetConnectorTypeName( GetModeConnector( )->connector_type );
        if ( !pszTypeStr ) pszTypeStr = "Unknown";

        snprintf(
            m_Mutable.szName,
            sizeof( m_Mutable.szName ),
            "%s-%d",
            pszTypeStr,
            GetModeConnector( )->connector_type_id );
        ...
        for ( int i = 0; i < m_pConnector->count_modes; i++ )
        {
            ...
            m_Mutable.BackendModes.emplace_back(
                BackendMode{
        ...
        if ( rawProperties )
        {
            m_Props.CRTC_ID = CDRMAtomicProperty::Instantiate(
                "CRTC_ID", this, *rawProperties );
            m_Props.Colorspace = CDRMAtomicProperty::Instantiate(
                "Colorspace", this, *rawProperties );
            m_Props.content_type = CDRMAtomicProperty::Instantiate(
                "content type", this, *rawProperties );
            m_Props.panel_orientation = CDRMAtomicProperty::Instantiate(
                "panel orientation", this, *rawProperties );
            m_Props.HDR_OUTPUT_METADATA = CDRMAtomicProperty::Instantiate(
                "HDR_OUTPUT_METADATA", this, *rawProperties );
            m_Props.vrr_capable = CDRMAtomicProperty::Instantiate(
                "vrr_capable", this, *rawProperties );
            m_Props.EDID =
                CDRMAtomicProperty::Instantiate( "EDID", this, *rawProperties );
            m_Props.Broadcast_RGB = CDRMAtomicProperty::Instantiate(
                "Broadcast RGB", this, *rawProperties );
        }

        ParseEDID( );

        if ( m_Mutable.EdidData != oldEdid ) { m_Mutable.bEdidChanged = true; }
    }
    ...
    void
    CDRMConnector::UpdateEffectiveOrientation( const drmModeModeInfo *pMode )
    {
        ...
        else
        {
            ...
            {
                switch ( this->GetProperties( )
                             .panel_orientation->GetCurrentValue( ) )
                {
    ...
    {
        if ( !GetProperties( ).EDID ) return;

        uint64_t ulBlobId = GetProperties( ).EDID->GetCurrentValue( );
        if ( !ulBlobId ) return;

        drmModePropertyBlobRes *pBlob =
            drmModeGetPropertyBlob( g_DRM.fd, ulBlobId );
        if ( !pBlob ) return;
        defer( drmModeFreePropertyBlob( pBlob ) );

        const uint8_t *pDataPointer =
            reinterpret_cast<const uint8_t *>( pBlob->data );
        m_Mutable.EdidData =
            std::vector<uint8_t>{ pDataPointer, pDataPointer + pBlob->length };

        di_info *pInfo = di_info_parse_edid(
            m_Mutable.EdidData.data( ), m_Mutable.EdidData.size( ) );
        if ( !pInfo )
        {
            drm_log.errorf(
                "Failed to parse edid for connector: %s", m_Mutable.szName );
            return;
        }
        ...
        const di_edid_vendor_product *pProduct =
            di_edid_get_vendor_product( pEdid );
        ...
        const di_edid_display_descriptor *const *pDescriptors =
            di_edid_get_display_descriptors( pEdid );
        for ( size_t i = 0; pDescriptors[ i ] != nullptr; i++ )
        {
            const di_edid_display_descriptor    *pDesc = pDescriptors[ i ];
            const di_edid_display_descriptor_tag eTag =
                di_edid_display_descriptor_get_tag( pDesc );
            if ( eTag == DI_EDID_DISPLAY_DESCRIPTOR_PRODUCT_NAME )
            {
                ...
                const char *pszModel =
                    di_edid_display_descriptor_get_string( pDesc );
                strncpy(
                    m_Mutable.szModel, pszModel, sizeof( m_Mutable.szModel ) );
            }
            else if ( eTag == DI_EDID_DISPLAY_DESCRIPTOR_DATA_STRING )
            {
                const char *pszDataString =
                    di_edid_display_descriptor_get_string( pDesc );
                strncpy(
                    m_Mutable.szDataString,
                    pszDataString,
                    sizeof( m_Mutable.szDataString ) );
            }
        }
        ...
        m_Mutable.fnDynamicModeGenerator = nullptr;
        {
            ...
            auto oKnownDisplay =
                script.Manager( ).Gamescope( ).Config.LookupDisplay(
                    script,
                    m_Mutable.szMakePNP,
                    pProduct->product,
                    m_Mutable.szModel,
            ...
            if ( oKnownDisplay )
            {
                ...
                std::string_view psvPrettyName = tTable.get_or(
                    "pretty_name", std::string_view{ "Untitled Display" } );
                drm_log.infof(
                    "Got known display: %.*s (%.*s)",
                    ( int )oKnownDisplay->first.size( ),
                    oKnownDisplay->first.data( ),
                    ( int )psvPrettyName.size( ),
                    psvPrettyName.data( ) );

                sol::optional<sol::table> otDynamicRefreshRates =
                    tTable[ "dynamic_refresh_rates" ];
                sol::optional<sol::function> ofnDynamicModegen =
                    tTable[ "dynamic_modegen" ];

                if ( otDynamicRefreshRates && ofnDynamicModegen )
                {
                    m_Mutable.ValidDynamicRefreshRates =
                        TableToVector<uint32_t>( *otDynamicRefreshRates );

                    m_Mutable.fnDynamicModeGenerator =
                        [ fnDynamicModegen = *ofnDynamicModegen ](
                            const drmModeModeInfo *pBaseMode,
                            int nRefreshHz ) -> drmModeModeInfo
                    {
                        CScriptScopedLock script;
                        ...
                        sol::function_result ret =
                            fnDynamicModegen( tInMode, nRefreshHz );
                ...
                if ( sol::optional<sol::table> otColorimetry =
                         tTable[ "colorimetry" ] )
                {
                    ...
                    if ( otR && otG && otB && otW )
                    {
                        m_Mutable.DisplayColorimetry.primaries.r =
                            TableToVec<glm::vec2>( *otR );
                        m_Mutable.DisplayColorimetry.primaries.g =
                            TableToVec<glm::vec2>( *otG );
                        m_Mutable.DisplayColorimetry.primaries.b =
                            TableToVec<glm::vec2>( *otB );
                        m_Mutable.DisplayColorimetry.white =
                            TableToVec<glm::vec2>( *otW );
                ...
                {
                    m_Mutable.HDR.bExposeHDRSupport =
                        otHDRInfo->get_or( "supported", false );
                    m_Mutable.HDR.eOutputEncodingEOTF =
                        otHDRInfo->get_or( "eotf", EOTF_Gamma22 );
                    m_Mutable.HDR.uMaxContentLightLevel =
                        nits_to_u16( otHDRInfo->get_or(
                            "max_content_light_level", 400.0f ) );
                    m_Mutable.HDR.uMaxFrameAverageLuminance =
                        nits_to_u16( otHDRInfo->get_or(
                            "max_frame_average_luminance", 400.0f ) );
                    m_Mutable.HDR.uMinContentLightLevel = nits_to_u16_dark(
                        otHDRInfo->get_or( "min_content_light_level", 0.1f ) );
            ...
            {
                // Unknown display, see if there are any other refresh rates in
                // the EDID we can get.
                if ( GetScreenType( ) == GAMESCOPE_SCREEN_TYPE_INTERNAL ||
                     cv_drm_allow_dynamic_modes_for_external_display )
                {
                    const drmModeModeInfo *pPreferredMode =
                        find_mode( m_pConnector.get( ), 0, 0, 0 );
                    ...
                    if ( pPreferredMode )
                    {
                        ...
                        for ( int i = 0; i < m_pConnector->count_modes; i++ )
                        {
                            const drmModeModeInfo *pMode =
                                &m_pConnector->modes[ i ];

                            if ( pMode->hdisplay != pPreferredMode->hdisplay ||
                                 pMode->vdisplay != pPreferredMode->vdisplay )
                                continue;
                            ...
                            if ( !Algorithm::Contains(
                                     ...
                                     pMode->vrefresh ) )
                            {
                                m_Mutable.ValidDynamicRefreshRates.push_back(
                                    pMode->vrefresh );
                            }
                        }

                        std::sort(
                            m_Mutable.ValidDynamicRefreshRates.begin( ),
                            m_Mutable.ValidDynamicRefreshRates.end( ) );
                    }
                }
            }
        }
        ...
        {
            // Steam Deck OLED has calibrated chromaticity coordinates in the
            // EDID for each unit. Other external displays probably have this
            // too.

            const di_edid_chromaticity_coords *pChroma =
                di_edid_get_chromaticity_coords( pEdid );
            if ( pChroma && pChroma->red_x != 0.0f )
            {
                drm_log.infof(
                    "[colorimetry]: EDID with colorimetry detected. Using it" );
                m_Mutable.DisplayColorimetry = displaycolorimetry_t{
                    .primaries = { { pChroma->red_x, pChroma->red_y },
                                   { pChroma->green_x, pChroma->green_y },
                                   { pChroma->blue_x, pChroma->blue_y } },
        ...
        drm_log.infof(
            "[colorimetry]: r %f %f",
            m_Mutable.DisplayColorimetry.primaries.r.x,
            m_Mutable.DisplayColorimetry.primaries.r.y );
        drm_log.infof(
            "[colorimetry]: g %f %f",
            m_Mutable.DisplayColorimetry.primaries.g.x,
            m_Mutable.DisplayColorimetry.primaries.g.y );
        drm_log.infof(
            "[colorimetry]: b %f %f",
            m_Mutable.DisplayColorimetry.primaries.b.x,
            m_Mutable.DisplayColorimetry.primaries.b.y );
        drm_log.infof(
            "[colorimetry]: w %f %f",
            m_Mutable.DisplayColorimetry.white.x,
            m_Mutable.DisplayColorimetry.white.y );
        ...
        if ( !bHasKnownHDRInfo )
        {
            const di_cta_hdr_static_metadata_block *pHDRStaticMetadata =
                nullptr;
            ...
            {
                if (( pCTA = di_edid_ext_get_cta( *ppExts ) )) break;
            }

            if ( pCTA )
            {
                const di_cta_data_block *const *ppBlocks =
                    di_edid_cta_get_data_blocks( pCTA );
                for ( ; *ppBlocks != nullptr; ppBlocks++ )
                {
                    if ( di_cta_data_block_get_tag( *ppBlocks ) ==
                    ...
                    {
                        pColorimetry =
                            di_cta_data_block_get_colorimetry( *ppBlocks );
                        continue;
                    }
                }
            }
            ...
            if ( pColorimetry && pColorimetry->bt2020_rgb &&
                 pHDRStaticMetadata && pHDRStaticMetadata->eotfs &&
                 pHDRStaticMetadata->eotfs->pq )
            {
                ...
                m_Mutable.HDR.uMaxContentLightLevel =
                    pHDRStaticMetadata->desired_content_max_luminance
                        ? nits_to_u16( pHDRStaticMetadata
        ...
        if ( m_Mutable.HDR.IsHDR10( ) )
        {
            ...
            hdr_metadata_infoframe *pInfoframe =
                &defaultHDRMetadata.hdmi_metadata_type1;

            // To be filled in by the app based on the scene, default to
            // desired_content_max_luminance
            //
            // Using display's max_fall for the default metadata max_cll to
            // avoid displays overcompensating with tonemapping for SDR content.
            uint16_t uDefaultInfoframeLuminances =
                m_Mutable.HDR.uMaxFrameAverageLuminance;

            pInfoframe->display_primaries[ 0 ].x =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.r.x );
            pInfoframe->display_primaries[ 0 ].y =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.r.y );
            pInfoframe->display_primaries[ 1 ].x =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.g.x );
            pInfoframe->display_primaries[ 1 ].y =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.g.y );
            pInfoframe->display_primaries[ 2 ].x =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.b.x );
            pInfoframe->display_primaries[ 2 ].y =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.primaries.b.y );
            pInfoframe->white_point.x =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.white.x );
            pInfoframe->white_point.y =
                color_xy_to_u16( m_Mutable.DisplayColorimetry.white.y );
            pInfoframe->max_display_mastering_luminance =
                uDefaultInfoframeLuminances;
            ...
            m_Mutable.HDR.pDefaultMetadataBlob =
                GetBackend( )->CreateBackendBlob( defaultHDRMetadata );
        }
    }
    ...
    // CDRMFb
    /////////////////////////
    CDRMFb::CDRMFb( uint32_t uFbId ) : m_uFbId{ uFbId } {}
    CDRMFb::~CDRMFb( )
    {
        // I own the fbid.
...
static int drm_prepare_liftoff(
    struct drm_t *drm, const struct FrameInfo_t *frameInfo, bool needs_modeset )
{
    ...
    if ( needs_modeset ) g_LiftoffStateCache.clear( );

    if ( is_liftoff_caching_enabled( ) )
    {
        if ( g_LiftoffStateCache.count( entry ) != 0 ) return -EINVAL;
    }

    bool bSinglePlane =
        frameInfo->layerCount < 2 && cv_drm_single_plane_optimizations;
    ...
    for ( int i = 0; i < k_nMaxLayers; i++ )
    {
        if ( i < frameInfo->layerCount )
        {
            ...
            gamescope::CDRMFb *pDrmFb = static_cast<gamescope::CDRMFb *>(
                ( pLayer->tex && pLayer->tex->GetBackendFb( ) )
                    ? pLayer->tex->GetBackendFb( )->EnsureImported( )
                    : nullptr );
            ...
            const int nFence = cv_drm_debug_disable_in_fence_fd
                                   ? -1
                                   : g_nAlwaysSignalledSyncFile;

            liftoff_layer_set_property(
                drm->lo_layers[ i ], "FB_ID", pDrmFb->GetFbId( ) );
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "IN_FENCE_FD", nFence );
            drm->m_FbIdsInRequest.emplace_back( pDrmFb );

            liftoff_layer_set_property(
                drm->lo_layers[ i ], "zpos", entry.layerState[ i ].zpos );
            liftoff_layer_set_property(
                drm->lo_layers[ i ],
                "alpha",
                frameInfo->layers[ i ].opacity * 0xffff );
            ...
            if ( entry.layerState[ i ].zpos != g_zposBase )
            {
                liftoff_layer_set_property(
                    drm->lo_layers[ i ],
                    "pixel blend mode",
                    ( uint64_t )frameInfo->layers[ i ].eAlphaBlendingMode );
            }
            else
            {
                liftoff_layer_unset_property(
                    drm->lo_layers[ i ], "pixel blend mode" );
            }

            liftoff_layer_set_property( drm->lo_layers[ i ], "SRC_X", 0 );
            liftoff_layer_set_property( drm->lo_layers[ i ], "SRC_Y", 0 );
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "SRC_W", entry.layerState[ i ].srcW );
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "SRC_H", entry.layerState[ i ].srcH );
            ...
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "rotation", ulOrientation );

            liftoff_layer_set_property(
                drm->lo_layers[ i ], "CRTC_X", entry.layerState[ i ].crtcX );
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "CRTC_Y", entry.layerState[ i ].crtcY );

            liftoff_layer_set_property(
                drm->lo_layers[ i ], "CRTC_W", entry.layerState[ i ].crtcW );
            liftoff_layer_set_property(
                drm->lo_layers[ i ], "CRTC_H", entry.layerState[ i ].crtcH );
            ...
            if ( frameInfo->layers[ i ].applyColorMgmt )
            {
                ...
                {
                    liftoff_layer_unset_property(
                        drm->lo_layers[ i ], "COLOR_ENCODING" );
                }
                ...
                {
                    liftoff_layer_unset_property(
                        drm->lo_layers[ i ], "COLOR_RANGE" );
                }

                if ( drm_supports_color_mgmt( drm ) )
                {
                    amdgpu_transfer_function degamma_tf =
                        colorspace_to_plane_degamma_tf(
                            entry.layerState[ i ].colorspace );
                    amdgpu_transfer_function shaper_tf =
                        colorspace_to_plane_shaper_tf(
                            entry.layerState[ i ].colorspace );
                    ...
                    if ( bYCbCr )
                    {
                        ...
                        //
                        // Matching BT709 for degamma -> regamma on shaper TF
                        // here is identity and works on YUV NV12 planes to
                        // preserve this.
                        //
                        // Doing LINEAR/DEFAULT here introduces banding so...
                        // this is the best way. (sRGB DEGAMMA does NOT work on
                        // YUV planes!)
                    ...
                    if ( bUseDegamma )
                        ...
                        liftoff_layer_set_property(
                            drm->lo_layers[ i ], "AMD_PLANE_DEGAMMA_TF", 0 );

                    bool bUseShaperAnd3DLUT =
                        !cv_drm_debug_disable_shaper_and_3dlut;
                    if ( bUseShaperAnd3DLUT )
                    {
                        liftoff_layer_set_property(
                            ...
                            drm->pending
                                .shaperlut_id[ ColorSpaceToEOTFIndex(
                                    entry.layerState[ i ].colorspace ) ]
                                ->GetBlobValue( ) );
                        liftoff_layer_set_property(
                            drm->lo_layers[ i ],
                        ...
                        liftoff_layer_set_property(
                            ...
                            "AMD_PLANE_LUT3D",
                            drm->pending
                                .lut3d_id[ ColorSpaceToEOTFIndex(
                                    entry.layerState[ i ].colorspace ) ]
                                ->GetBlobValue( ) );
                        // Josh: See shaders/colorimetry.h colorspace_blend_tf
                        // if you have questions as to why we start doing sRGB
                        // for BLEND_TF despite potentially working in Gamma 2.2
                        // space prior.
                    }
                    else
                    {
                        liftoff_layer_set_property(
                            drm->lo_layers[ i ], "AMD_PLANE_SHAPER_LUT", 0 );
                        liftoff_layer_set_property(
                            drm->lo_layers[ i ], "AMD_PLANE_SHAPER_TF", 0 );
                        liftoff_layer_set_property(
                            drm->lo_layers[ i ], "AMD_PLANE_LUT3D", 0 );
                    }
                }
            }
            else
            {
                if ( drm_supports_color_mgmt( drm ) )
                {
                    ...
                    liftoff_layer_set_property(
                        drm->lo_layers[ i ], "AMD_PLANE_LUT3D", 0 );
                    liftoff_layer_set_property(
                        drm->lo_layers[ i ], "AMD_PLANE_CTM", 0 );
                }
            }
            ...
            if ( drm_supports_color_mgmt( drm ) )
            {
                ...
                if ( !cv_drm_debug_disable_ctm &&
                     frameInfo->layers[ i ].ctm != nullptr )
                    liftoff_layer_set_property(
                        drm->lo_layers[ i ],
                        "AMD_PLANE_CTM",
                        frameInfo->layers[ i ].ctm->GetBlobValue( ) );
                else
                    liftoff_layer_set_property(
                        drm->lo_layers[ i ], "AMD_PLANE_CTM", 0 );
            }
    ...
    int ret = liftoff_output_apply(
        drm->lo_output, drm->req, drm->flags, &lo_options );
    ...
    if ( ret == -EPERM && !attempted_in_fence_fallback &&
         !cv_drm_debug_disable_in_fence_fd )
    {
    ...
    if ( ret == 0 )
    {
        ...
        if ( liftoff_output_needs_composition( drm->lo_output ) ) ret = -EINVAL;
    }
    ...
    if ( !needs_modeset )
    {
        if ( ret == -EINVAL ) g_LiftoffStateCache.insert( entry );
    }
    ...
    if ( ret == 0 )
        ...
        drm_log.debugf(
            "can NOT drm present %i layers", frameInfo->layerCount );
...
void drm_rollback( struct drm_t *drm )
{
    ...
    for ( std::unique_ptr<gamescope::CDRMCRTC> &pCRTC : drm->crtcs )
    {
        for ( std::optional<gamescope::CDRMAtomicProperty> &oProperty :
              pCRTC->GetProperties( ) )
        {
            if ( oProperty ) oProperty->Rollback( );
        }
    }
 ...
 */
int drm_prepare(
    struct drm_t *drm, bool async, const struct FrameInfo_t *frameInfo )
{
    if ( !drm->pConnector ) return -EACCES;

    drm_update_color_mgmt( drm );

    const bool bIsVRRCapable =
        drm->pConnector && drm->pConnector->GetProperties( ).vrr_capable &&
        !!drm->pConnector->GetProperties( ).vrr_capable->GetCurrentValue( );
    const bool bHasVRREnable =
        drm->pCRTC && drm->pCRTC->GetProperties( ).VRR_ENABLED;

    const bool bVRREnabled =
        bIsVRRCapable && bHasVRREnable && frameInfo->allowVRR;
    if ( bIsVRRCapable )
    {
        if ( bVRREnabled !=
             !!drm->pCRTC->GetProperties( ).VRR_ENABLED->GetCurrentValue( ) )
            drm->needs_modeset = true;
    }

    drm_colorspace uColorimetry = DRM_MODE_COLORIMETRY_DEFAULT;

    const bool bWantsHDR10 =
        g_bOutputHDREnabled && frameInfo->outputEncodingEOTF == EOTF_PQ;
    ...
    if ( drm->pConnector && drm->pConnector->SupportsHDR10( ) )
    {
        if ( bWantsHDR10 )
        {
            ...
            wlserver_vk_swapchain_feedback *pFeedback =
        ...
        if ( uColorimetry !=
             drm->pConnector->GetProperties( ).Colorspace->GetCurrentValue( ) )
            drm->needs_modeset = true;
    }
    ...
    if ( drm->pConnector )
    {
        ...
        bool bCurrentlyAsleep =
            drm->pConnector->GetProperties( ).CRTC_ID->GetCurrentValue( ) == 0;

        if ( bCurrentlyAsleep != bSleep ) needs_modeset = true;
    }

    if ( !bSleep )
    {
        if ( drm->pCRTC != nullptr ) flags |= DRM_MODE_PAGE_FLIP_EVENT;

        if ( async || g_bForceAsyncFlips ) flags |= DRM_MODE_PAGE_FLIP_ASYNC;
    }
    ...
    if ( needs_modeset )
    {
        ...
        for ( auto &iter : drm->connectors )
        {
            ...
            pConnector->GetProperties( ).CRTC_ID->SetPendingValue(
            ...
            if ( pConnector->GetProperties( ).Broadcast_RGB )
                pConnector->GetProperties( ).Broadcast_RGB->SetPendingValue(
                    drm->req, 0, bForceInRequest );
        }
        ...
        for ( std::unique_ptr<gamescope::CDRMCRTC> &pCRTC : drm->crtcs )
        {
            ...
            pCRTC->GetProperties( ).ACTIVE->SetPendingValue(
                drm->req, 0, bForceInRequest );
            pCRTC->GetProperties( ).MODE_ID->SetPendingValue(
                drm->req, 0, bForceInRequest );

            if ( pCRTC->GetProperties( ).GAMMA_LUT )
                pCRTC->GetProperties( ).GAMMA_LUT->SetPendingValue(
                    drm->req, 0, bForceInRequest );
            ...
            if ( pCRTC->GetProperties( ).CTM )
                pCRTC->GetProperties( ).CTM->SetPendingValue(
                    drm->req, 0, bForceInRequest );
        ...
        if ( drm->pConnector && !bSleep )
        {
            ...
            drm->pConnector->GetProperties( ).CRTC_ID->SetPendingValue(
                drm->req, drm->pCRTC->GetObjectId( ), bForceInRequest );

            if ( drm->pConnector->GetProperties( ).Colorspace )
                drm->pConnector->GetProperties( ).Colorspace->SetPendingValue(
                    drm->req, uColorimetry, bForceInRequest );
        }

        if ( drm->pCRTC && !bSleep )
        {
            drm->pCRTC->GetProperties( ).ACTIVE->SetPendingValue(
                drm->req, 1u, true );
            drm->pCRTC->GetProperties( ).MODE_ID->SetPendingValue(
                drm->req,
                drm->pending.mode_id ? drm->pending.mode_id->GetBlobValue( )
                                     : 0lu,
                true );

            if ( drm->pCRTC->GetProperties( ).VRR_ENABLED )
                drm->pCRTC->GetProperties( ).VRR_ENABLED->SetPendingValue(
                    drm->req, bVRREnabled, true );
        }
    }
    ...
    if ( drm->pConnector && !bSleep )
    {
        if ( drm->pConnector->GetProperties( ).HDR_OUTPUT_METADATA )
            drm->pConnector->GetProperties( )
                .HDR_OUTPUT_METADATA->SetPendingValue(
                    drm->req,
                    pHDRMetadata ? pHDRMetadata->GetBlobValue( ) : 0lu,
                    bForceInRequest );

        if ( drm->pConnector->GetProperties( ).content_type )
            drm->pConnector->GetProperties( ).content_type->SetPendingValue(
                drm->req, DRM_MODE_CONTENT_TYPE_GAME, bForceInRequest );

        GamescopeBroadcastRGBMode_t eBroadcastRGB =
            drm->pConnector->GetScreenType( ) ==
                    gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL
        ...
        if ( drm->pConnector->GetProperties( ).Broadcast_RGB )
            drm->pConnector->GetProperties( ).Broadcast_RGB->SetPendingValue(
                drm->req, eBroadcastRGB, bForceInRequest );
    }
    ...
    if ( drm->pCRTC && !bSleep )
    {
        if ( drm->pCRTC->GetProperties( ).AMD_CRTC_REGAMMA_TF )
        {
            if ( !cv_drm_debug_disable_regamma_tf )
                drm->pCRTC->GetProperties( )
                    .AMD_CRTC_REGAMMA_TF->SetPendingValue(
                        drm->req,
                        inverse_tf( drm->pending.output_tf ),
                        bForceInRequest );
            else
                drm->pCRTC->GetProperties( )
                    .AMD_CRTC_REGAMMA_TF->SetPendingValue(
    ...
    int ret;
    if ( drm->pCRTC == nullptr || bSleep ) { ret = 0; }
    ...
    if ( ret != 0 )
    {
        ...
        if ( needs_modeset ) drm->needs_modeset = true;
    }
...
static bool drm_set_crtc( struct drm_t *drm, gamescope::CDRMCRTC *pCRTC )
{
    ...
    struct liftoff_output *lo_output =
        liftoff_output_create( drm->lo_device, pCRTC->GetObjectId( ) );
    if ( lo_output == nullptr ) return false;

    for ( int i = 0; i < k_nMaxLayers; i++ )
    {
        liftoff_layer_destroy( drm->lo_layers[ i ] );
        drm->lo_layers[ i ] = liftoff_layer_create( lo_output );
        if ( drm->lo_layers[ i ] == nullptr ) return false;
    }
...
bool drm_set_connector( struct drm_t *drm, gamescope::CDRMConnector *conn )
{
    ...
    if ( !drm_set_crtc( drm, pCRTC ) ) { return false; }

    // If we are changing connector, zero out the current and pending mode IDs.
    // So we don't try to use one mode from the old connector on the new one if
    // we roll back.
...
{
    if ( !drm->pConnector ) return gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL;
...
bool drm_update_color_mgmt( struct drm_t *drm )
{
    if ( !drm_supports_color_mgmt( drm ) ) return true;

    if ( g_ColorMgmt.serial == drm->current.color_mgmt_serial ) return true;
    ...
    for ( uint32_t i = 0; i < EOTF_Count; i++ )
    {
        if ( !g_ColorMgmtLuts[ i ].HasLuts( ) ) continue;

        drm->pending.shaperlut_id[ i ] =
            GetBackend( )->CreateBackendBlob( g_ColorMgmtLuts[ i ].lut1d );
        drm->pending.lut3d_id[ i ] =
            GetBackend( )->CreateBackendBlob( g_ColorMgmtLuts[ i ].lut3d );
    }
...
static void drm_unset_mode( struct drm_t *drm )
{
    ...
    if ( g_nOutputHeight == 0 ) g_nOutputHeight = 720;
    if ( g_nOutputWidth == 0 ) g_nOutputWidth = g_nOutputHeight * 16 / 9;
...
bool drm_set_refresh( struct drm_t *drm, int refresh )
{
    ...
    const drmModeModeInfo *existing_mode =
        find_mode( connector, width, height, refresh );
    drmModeModeInfo mode = { 0 };
    if ( existing_mode ) { mode = *existing_mode; }
    else
    {
        if ( g_DRM.pConnector && g_DRM.pConnector->GetModeGenerator( ) )
        {
            const drmModeModeInfo *preferred_mode =
                find_mode( connector, 0, 0, 0 );
            mode = g_DRM.pConnector->GetModeGenerator( )(
        ...
        else
        {
            ...
            switch ( g_eGamescopeModeGeneration )
            {
                case gamescope::GAMESCOPE_MODE_GENERATE_CVT:
                    generate_cvt_mode(
                        &mode, width, height, refresh, true, false );
                    break;
...
{
    if ( drm->pConnector ) return drm->pConnector->SupportsVRR( );
...
{
    if ( !drm->pConnector ) return nullptr;
...
{
    if ( !drm->pConnector ) return { 0u, 0u };

    return std::make_pair(
        drm->pConnector->GetModeConnector( )->connector_type,
        drm->pConnector->GetModeConnector( )->connector_type_id );
}

bool drm_supports_color_mgmt( struct drm_t *drm )
{
    if ( g_bForceDisableColorMgmt ) return false;

    if ( !drm->pPrimaryPlane ) return false;

    return drm->pPrimaryPlane->GetProperties( ).AMD_PLANE_CTM.has_value( ) &&
           drm->pPrimaryPlane->GetProperties( ).AMD_PLANE_BLEND_TF.has_value( );
}
...
namespace gamescope
{
    ...
    public:
        ...
        virtual ~CDRMBackend( )
        {
            if ( g_DRM.fd != -1 ) finish_drm( &g_DRM );
        }
        ...
        virtual bool Init( ) override
        {
            ...
            return init_drm(
                &g_DRM,
                g_nPreferredOutputWidth,
                g_nPreferredOutputHeight,
                g_nNestedRefresh );
        }
        ...
        virtual bool PostInit( ) override
        {
            if ( g_DRM.pConnector )
                WritePatchedEdid(
                    g_DRM.pConnector->GetRawEDID( ),
                    g_DRM.pConnector->GetHDRInfo( ),
                    g_bRotated );
            return true;
        }

        virtual std::span<const char *const>
        GetInstanceExtensions( ) const override
        ...
        virtual void GetPreferredOutputFormat(
            uint32_t *pPrimaryPlaneFormat,
            uint32_t *pOverlayPlaneFormat ) const override
        {
            *pPrimaryPlaneFormat = g_nDRMFormat;
            *pOverlayPlaneFormat = g_nDRMFormatOverlay;
        }
        virtual bool
        ValidPhysicalDevice( VkPhysicalDevice pVkPhysicalDevice ) const override
        ...
        int Present( const FrameInfo_t *pFrameInfo, bool bAsync )
        {
            ...
            drm_log.debugf(
                "CDRMBackend::Present Begin: %lu -> delta: %lu",
                ulNow,
                ulNow - s_ulLastTime );
            s_ulLastTime = ulNow;

            bool bWantsPartialComposite =
                pFrameInfo->layerCount >= 3 && !kDisablePartialComposition;
            ...
            bool bLayer0ScreenSize =
                close_enough( pFrameInfo->layers[ 0 ].scale.x, 1.0f ) &&
                close_enough( pFrameInfo->layers[ 0 ].scale.y, 1.0f );

            bool bNeedsCompositeFromFilter =
                ...
                if ( !SupportsColorManagement( ) )
                    bNeedsFullComposite |=
                        ( pFrameInfo->layerCount > 1 ||
                          pFrameInfo->layers[ 0 ].colorspace !=
            ...
            else
            {
                if ( !SupportsColorManagement( ) )
                    bNeedsFullComposite |=
                        ColorspaceIsHDR( pFrameInfo->layers[ 0 ].colorspace );
            }
            ...
            if ( !bNeedsFullComposite && !bWantsPartialComposite )
            {
                ...
                if ( ret == 0 ) bDoComposite = false;
                else if ( ret == -EACCES )
                    return 0;
            }

            // Update to let the vblank manager know we are currently
            // compositing.
            ...
            // Composition Path
            if ( kDisablePartialComposition ) bNeedsFullComposite = true;
            ...
            if ( !bNeedsFullComposite )
            {
                ...
                // This is 2, and we do i-1 so 1...layerCount. So AFTER we have
                // removed baseplane. Overlays only.
                //
                // Josh:
                // We could handle mismatching colorspaces for partial
                // composition but I want to keep overlay -> partial composition
                // promotion as simple as possible, using the same 3D + SHAPER
                // LUTs + BLEND in DRM as changing them is incredibly
                // expensive!! It takes forever. We can't just point it to
                // random BDA or whatever, it has to be uploaded slowly thru
                // registers which is SUPER SLOW. This avoids stutter.
                for ( int i = 2; i < compositeFrameInfo.layerCount; i++ )
                {
                    if ( pFrameInfo->layers[ i - 1 ].colorspace !=
                         pFrameInfo->layers[ i ].colorspace )
                    {
                        bNeedsFullComposite = true;
                        break;
                    }
            ...
            // We were already stalling for the full composition before, so it's
            // not an issue for latency, we just need to make sure we get 1
            // partial frame that isn't deferred in time so we don't lose
            // layers.
            bool bDefer = !bNeedsFullComposite &&
            ...
            if ( !bNeedsFullComposite )
            {
                for ( int i = 1; i < compositeFrameInfo.layerCount; i++ )
                    compositeFrameInfo.layers[ i - 1 ] =
                        compositeFrameInfo.layers[ i ];
                compositeFrameInfo.layerCount -= 1;

                // When doing partial composition, apply the shaper + 3D LUT
                // stuff at scanout.
            ...
            // If using composite debug markers, make sure we mark them as
            // partial so we know!
            if ( bDefer &&
                 !!( g_uCompositeDebug & CompositeDebugFlag::Markers ) )
                g_uCompositeDebug |= CompositeDebugFlag::Markers_Partial;

            std::optional oCompositeResult = vulkan_composite(
                &compositeFrameInfo, nullptr, !bNeedsFullComposite );
            ...
            presentCompFrameInfo.outputEncodingEOTF =
            ...
            if ( bNeedsFullComposite )
            {
                ...
                FrameInfo_t::Layer_t *baseLayer =
                    &presentCompFrameInfo.layers[ 0 ];
                ...
                baseLayer->colorspace =
                    pFrameInfo->outputEncodingEOTF == EOTF_PQ
                        ? GAMESCOPE_APP_TEXTURE_COLORSPACE_HDR10_PQ
                        : GAMESCOPE_APP_TEXTURE_COLORSPACE_SRGB;
            ...
            else
            {
                if ( m_bWasPartialCompositing || !bDefer )
                {
                    ...
                    FrameInfo_t::Layer_t *overlayLayer =
                        &presentCompFrameInfo.layers[ 1 ];
                    ...
                    overlayLayer->tex =
                        vulkan_get_last_output_image( true, bDefer );
                    ...
                    overlayLayer->colorspace =
                        compositeFrameInfo.layers[ 0 ].colorspace;
                }
                else
                {
                    ...
                    const FrameInfo_t::Layer_t *lastPresentedOverlayLayer =
                        nullptr;
                    for ( int i = 0; i < pFrameInfo->layerCount; i++ )
                    {
                        if ( pFrameInfo->layers[ i ].zpos ==
                             m_nLastSingleOverlayZPos )
                        {
                            lastPresentedOverlayLayer =
                                &pFrameInfo->layers[ i ];
                            break;
                        }
                    }
            ...
            if ( ret != 0 )
            {
                ...
                {
                    xwm_log.errorf(
                        "We failed our modeset and have no mode to fall back "
                        "to! (Initial modeset failed?): %s",
                        strerror( -ret ) );
                    return 0;
                }

                xwm_log.errorf(
                    "Failed to prepare 1-layer flip (%s), trying again with "
                    "previous mode if modeset needed",
                    strerror( -ret ) );

                // Try once again to in case we need to fall back to another
                // mode.
                ...
                {
                    xwm_log.errorf(
                        "Failed to prepare 1-layer flip entirely: %s",
                        strerror( -ret ) );
                    // We should always handle a 1-layer flip, this used to
                    // abort, but lets be more friendly and just avoid a commit
                    // and try again later. Let's re-poll our state, and force
                    // grab the best connector again.
                    //
                    // Some intense connector hotplugging could be occuring and
                    // the connector could become destroyed before we had a
                    // chance to use it as we hadn't reffed it in a commit yet.
        ...
        {
            if ( bForceModeset ) g_DRM.needs_modeset = true;
            g_DRM.out_of_date =
                std::max<int>( g_DRM.out_of_date, bForce ? 2 : 1 );
            g_DRM.paused = !wlsession_active( );
        }

        virtual bool PollState( ) override { return drm_poll_state( &g_DRM ); }

        virtual std::shared_ptr<BackendBlob> CreateBackendBlob(
            const std::type_info &type, std::span<const uint8_t> data ) override
        {
            uint32_t uBlob = 0;
            if ( type == typeid( glm::mat3x4 ) )
            {
                ...
                if ( drmModeCreatePropertyBlob(
                         g_DRM.fd,
                         reinterpret_cast<const void *>( &ctm2 ),
                         sizeof( ctm2 ),
                         &uBlob ) != 0 )
                    return nullptr;
            }
            ...
            {
                if ( drmModeCreatePropertyBlob(
                         g_DRM.fd, data.data( ), data.size( ), &uBlob ) != 0 )
                    return nullptr;
            }
        ...
        virtual OwningRc<IBackendFb>
        ImportDmabufToBackend( wlr_dmabuf_attributes *pDmaBuf ) override
        { return drm_fbid_from_dmabuf( &g_DRM, pDmaBuf ); }

        virtual bool UsesModifiers( ) const override
        { return g_DRM.allow_modifiers; }
        virtual std::span<const uint64_t>
        GetSupportedModifiers( uint32_t uDrmFormat ) const override
        {
            const wlr_drm_format *pFormat =
                wlr_drm_format_set_get( &g_DRM.formats, uDrmFormat );
            if ( !pFormat ) return std::span<const uint64_t>{};

            return std::span<const uint64_t>{
                pFormat->modifiers, pFormat->modifiers + pFormat->len
            };
        }
        ...
        GetConnector( GamescopeScreenType eScreenType ) override
        {
            if ( GetCurrentConnector( ) &&
                 GetCurrentConnector( )->GetScreenType( ) == eScreenType )
        ...
        virtual bool UsesVulkanSwapchain( ) const override { return false; }

        virtual bool IsSessionBased( ) const override { return true; }
        ...
        virtual bool SupportsExplicitSync( ) const override
        {
            ...
            if ( nMajor < 6 ) return false;

            if ( nMajor == 6 && nMinor < 8 ) return false;
#else
        ...
        virtual bool IsPaused( ) const override { return g_DRM.paused; }

        virtual bool IsVisible( ) const override { return !this->IsPaused( ); }

        virtual glm::uvec2
        CursorSurfaceSize( glm::uvec2 uvecSize ) const override
        {
            if ( !k_bUseCursorPlane ) return uvecSize;
        ...
        {
            if ( !GetCurrentConnector( ) ) return;

            WritePatchedEdid(
                GetCurrentConnector( )->GetRawEDID( ),
                GetCurrentConnector( )->GetHDRInfo( ),
                g_bRotated );
        }
    ...
    private:
        ...
        int Commit( const FrameInfo_t *pFrameInfo )
        {
            ...
            if ( isPageFlip )
            {
                ...
                // Do it before the commit, as otherwise the pageflip handler
                // could potentially beat us to the refcount checks.
            ...
            m_PresentCtxs[ uCurrentPresentCtx ].ulPendingFlipCount =
                GetCurrentConnector( )
                    ->PresentationFeedback( )
                    .m_uQueuedPresents;
            ...
            ret = drmModeAtomicCommit(
                ...
                drm->req,
                drm->flags,
                &m_PresentCtxs[ uCurrentPresentCtx ] );
            if ( ret != 0 )
            {
                ...
                if ( ret != -EBUSY && ret != -EACCES )
                {
                    ...
                    if ( isPageFlip ) drm->uPendingFlipCount--;
                    abort( );
                }

                drm_rollback( drm );

                // Swap back over to what was previously queued (probably
                // nothing) if this commit failed.
                {
                    std::unique_lock lock( drm->m_QueuedFbIdsMutex );
                    drm->m_QueuedFbIds.swap( drm->m_FbIdsInRequest );
                }
            ...
            else
            {
                ...
                for ( std::unique_ptr<gamescope::CDRMCRTC> &pCRTC : drm->crtcs )
                {
                    for ( std::optional<gamescope::CDRMAtomicProperty>
                              &oProperty : pCRTC->GetProperties( ) )
                    {
                        if ( oProperty ) oProperty->OnCommit( );
                    }
                }

                for ( std::unique_ptr<gamescope::CDRMPlane> &pPlane :
                      drm->planes )
                {
                    for ( std::optional<gamescope::CDRMAtomicProperty>
                              &oProperty : pPlane->GetProperties( ) )
                    {
                        if ( oProperty ) oProperty->OnCommit( );
                    }
                }
                ...
                for ( auto &iter : drm->connectors )
                {
                    gamescope::CDRMConnector *pConnector = &iter.second;
                    for ( std::optional<gamescope::CDRMAtomicProperty>
                              &oProperty : pConnector->GetProperties( ) )
                    {
                        if ( oProperty ) oProperty->OnCommit( );
                    }
                }
            ...
            // Ideally this would be updated by something right before the page
            // flip is queued and would end up being the new page flip, rather
            // than here. However, the page flip handler is called when the page
            // flip occurs, not when it is successfully queued.
    ...
    template<> bool IBackend::Set<CDRMBackend>( )
    { return Set( new CDRMBackend{} ); }
} // namespace gamescope

int HackyDRMPresent( const FrameInfo_t *pFrameInfo, bool bAsync )
{
    return static_cast<gamescope::CDRMBackend *>( GetBackend( ) )
        ->Present( pFrameInfo, bAsync );
}
