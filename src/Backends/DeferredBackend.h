...
namespace gamescope
{
    ...
    {
    public:
        CDeferredBackend( IBackend *pChild ) : m_pChild{ pChild } {}
        ...
        {
            {
                ...
                if ( m_bInittedChild ) return m_pChild->PostInit( );
            }
        ...
        virtual VkImageLayout GetPresentLayout( ) const override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) return m_pChild->GetPresentLayout( );
            }
        ...
        virtual bool PollState( ) override
        {
            TryInittingChild( );
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild )
                    return m_pChild->PollState( ) ||
                           m_bJustInittedPoll.exchange( false );
            }
            return false;
        }
        ...
        virtual bool UsesModifiers( ) const override { return true; }
        ...
        virtual IBackendConnector *GetCurrentConnector( ) override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) return m_pChild->GetCurrentConnector( );
            }
        ...
        virtual bool SupportsTearing( ) const override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) return m_pChild->SupportsTearing( );
            }
        ...
        virtual bool IsPaused( ) const override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) return m_pChild->IsPaused( );
            }
        ...
        virtual bool IsVisible( ) const override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) return m_pChild->IsVisible( );
            }
        ...
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild )
                    return m_pChild->CreateVirtualConnector(
                        ulVirtualConnectorKey );
            }
        ...
        {
            {
                ...
                if ( m_bInittedChild )
                    return m_pChild->ForwardFramebuffer(
                        pPlane, pFramebuffer, pData );
            }
        }

        bool IsChildInitted( ) { return m_bInittedChild; }

        IBackend *GetChild( ) { return m_pChild; }
        ...
        void OnEndFrame( ) override
        {
            {
                std::shared_lock lock{ m_mutInit };
                if ( m_bInittedChild ) m_pChild->OnEndFrame( );
            }
        }
