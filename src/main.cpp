...
const char usage[] =
    ...
    "  -S, --scaler                   upscaler type (auto, integer, fit, fill, "
    "stretch)\n"
    "  -F, --filter                   upscaler filter (linear, nearest, fsr, "
    "nis, pixel)\n"
    "                                     fsr => AMD FidelityFX™ Super "
    "Resolution 1.0\n"
    "                                     nis => NVIDIA Image Scaling v1.0.3\n"
    "  --sharpness, --fsr-sharpness   upscaler sharpness from 0 (max) to 20 "
    "(min)\n"
    "  --expose-wayland               support wayland clients using xdg-shell\n"
    "  -s, --mouse-sensitivity        multiply mouse movement by given decimal "
    "number\n"
    "  --backend                      select rendering backend\n"
    "                                     auto => autodetect (default)\n"
#if HAVE_DRM
    "                                     drm => use DRM backend (standalone "
    "display session)\n"
#endif
#if HAVE_SDL2
    "                                     sdl => use SDL backend\n"
#endif
#if HAVE_OPENVR
    "                                     openvr => use OpenVR backend "
    "(outputs as a VR overlay)\n"
#endif
    "                                     headless => use headless backend (no "
    "window, no DRM output)\n"
    ...
    "  --prefer-vk-device             prefer Vulkan device for compositing "
    "(ex: 1002:7300)\n"
    "  --force-orientation            rotate the internal display (left, "
    "right, normal, upsidedown)\n"
    "  --force-windows-fullscreen     force windows inside of gamescope to be "
    "the size of the nested display (fullscreen)\n"
    "  --cursor-scale-height          if specified, sets a base output height "
    "to linearly scale the cursor against.\n"
    "  --virtual-connector-strategy   Specifies how we should make virtual "
    "connectors.\n"
    "  --hdr-enabled                  enable HDR output (needs Gamescope WSI "
    "layer enabled for support from clients)\n"
    "                                 If this is not set, and there is a HDR "
    "client, it will be tonemapped SDR.\n"
    "  --sdr-gamut-wideness           Set the 'wideness' of the gamut for SDR "
    "comment. 0 - 1.\n"
    "  --hdr-sdr-content-nits         set the luminance of SDR content in "
    "nits. Default: 400 nits.\n"
    "  --hdr-itm-enabled              enable SDR->HDR inverse tone mapping. "
    "only works for SDR input.\n"
    "  --hdr-itm-sdr-nits             set the luminance of SDR content in nits "
    "used as the input for the inverse tone mapping process.\n"
    "                                 Default: 100 nits, Max: 1000 nits\n"
    "  --hdr-itm-target-nits          set the target luminace of the inverse "
    "tone mapping process.\n"
    "                                 Default: 1000 nits, Max: 10000 nits\n"
    "  --framerate-limit              Set a simple framerate limit. Used as a "
    "divisor of the refresh rate, rounds down eg 60 / 59 -> 60fps, 60 / 25 -> "
    "30fps. Default: 0, disabled.\n"
    "  --mangoapp                     Launch with the mangoapp (mangohud) "
    "performance overlay enabled. You should use this instead of using "
    "mangohud on the game or gamescope.\n"
    ...
    "  --force-grab-cursor            always use relative mouse mode instead "
    "of flipping dependent on cursor visibility.\n"
    "  --display-index                forces gamescope to use a specific "
    "display in nested mode."
    "\n"
    "Embedded mode options:\n"
    "  -O, --prefer-output            list of connectors in order of "
    "preference (ex: DP-1,DP-2,DP-3,HDMI-A-1)\n"
    "  --default-touch-mode           0: hover, 1: left, 2: right, 3: middle, "
    "4: passthrough\n"
    "  --generate-drm-mode            DRM mode generation algorithm (cvt, "
    "fixed)\n"
    "  --immediate-flips              Enable immediate flips, may result in "
    "tearing\n"
    "\n"
#if HAVE_OPENVR
    "VR mode options:\n"
    ...
    "to this string\n"
    "  --vr-app-overlay-key						Sets the SteamVR overlay key "
    "to use for child apps\n"
    "  --vr-overlay-explicit-name               Force the SteamVR overlay name "
    "to always be this string\n"
    "  --vr-overlay-default-name                Sets the fallback SteamVR "
    "overlay name when there is no window title\n"
    "  --vr-overlay-icon                        Sets the SteamVR overlay icon "
    "to this file\n"
    "  --vr-overlay-show-immediately            Makes our VR overlay take "
    "focus immediately\n"
    "  --vr-overlay-enable-control-bar          Enables the SteamVR control "
    "bar\n"
    "  --vr-overlay-enable-control-bar-keyboard Enables the SteamVR keyboard "
    "button on the control bar\n"
    "  --vr-overlay-enable-control-bar-close    Enables the SteamVR close "
    "button on the control bar\n"
    "  --vr-overlay-enable-click-stabilization  Enables the SteamVR click "
    "stabilization\n"
    "  --vr-overlay-modal                       Makes our VR overlay appear as "
    "a modal\n"
    "  --vr-overlay-physical-width              Sets the physical width of our "
    "VR overlay in metres\n"
    "  --vr-overlay-physical-curvature          Sets the curvature of our VR "
    "overlay\n"
    "  --vr-overlay-physical-pre-curve-pitch    Sets the pre-curve pitch of "
    "our VR overlay\n"
    "  --vr-scrolls-speed                       Mouse scrolling speed of "
    "trackpad scroll in VR. Default: 8.0\n"
    "\n"
#endif
    "Debug options:\n"
    ...
    "  --composite-debug              draw frame markers on alternating "
    "corners of the screen when compositing\n"
    "  --disable-color-management     disable color management\n"
    "  --disable-xres                 disable XRes for PID lookup\n"
    "  --hdr-debug-force-support      forces support for HDR, etc even if the "
    "display doesn't support it. HDR clients will be outputted as SDR still in "
    "that case.\n"
    "  --hdr-debug-force-output       forces support and output to HDR10 PQ "
    "even if the output does not support it (will look very wrong if it "
    "doesn't)\n"
    "  --hdr-debug-heatmap            displays a heatmap-style debug view of "
    "HDR luminence across the scene in nits."
    "\n"
    "Reshade shader options:\n"
    "  --reshade-effect               sets the name of a reshade shader to use "
    "in either /usr/share/gamescope/reshade/Shaders or "
    "~/.local/share/gamescope/reshade/Shaders\n"
    "  --reshade-technique-idx        sets technique idx to use from the "
    "reshade effect\n"
    "\n"
    "Steam Deck options:\n"
    "  --mura-map                     Set the mura compensation map to use for "
    "the display. Takes in a path to the mura map.\n"
    "\n"
    "Platform options:\n"
    "  --allow-deferred-backend       Allows initting the backend in a "
    "deferred way, if it doesn't work immediately. (Note: This has some very "
    "minor correctness compromises that you should consider wrt. your platform "
    "with modifiers, etc).\n"
    "  --keep-alive                   Keep Gamescope alive even when the "
...
gamescope::GamescopeModeGeneration g_eGamescopeModeGeneration =
    gamescope::GAMESCOPE_MODE_GENERATE_CVT;
...
static std::string build_optstring( const struct option *options )
{
    ...
    {
        if ( !options[ i ].name || !options[ i ].val ) continue;

        assert(
            optstring.find( ( char )options[ i ].val ) == std::string::npos );

        char str[] = { ( char )options[ i ].val, '\0' };
        optstring.append( str );

        if ( options[ i ].has_arg ) optstring.append( ":" );
    }
    return optstring;
}

static gamescope::GamescopeModeGeneration
parse_gamescope_mode_generation( const char *str )
{
    if ( str == "cvt"sv ) { return gamescope::GAMESCOPE_MODE_GENERATE_CVT; }
...
GamescopePanelOrientation g_DesiredInternalOrientation =
...
{
    if ( strcmp( str, "auto" ) == 0 ) { return GamescopeUpscaleScaler::AUTO; }
...
static int parse_integer( const char *str, const char *optionName )
{
    ...
    else
    {
        fprintf(
            stderr,
            "gamescope: invalid value for --%s, \"%s\" is either not an "
...
static float parse_float( const char *str, const char *optionName )
{
    ...
    else
    {
        fprintf(
            stderr,
            "gamescope: invalid value for --%s, \"%s\" could not be "
...
static gamescope::ConCommand cc_shutdown(
    "shutdown",
...
static bool CheckWaylandPresentationTime( )
{
    ...
    if ( !display )
    {
        fprintf(
            stderr,
            "Failed to connect to wayland socket: %s.\n",
...
static void UpdateCompatEnvVars( )
{
    ...
    if ( steamMode )
    {
        ...
        // We no longer need to set GAMESCOPE_EXTERNAL_OVERLAY from steam,
        ...
        // These are mostly SteamOS specific, and are set by our Gamescope
        // session, but might be useful for you.
        ...
        // STEAM_USE_DYNAMIC_VRS is RADV specific, so don't expose this right
        // now.
    ...
    if ( ( g_bLaunchMangoapp && steamMode ) &&
         ( !pszMangoConfigPath || !*pszMangoConfigPath ) )
    {
        char  szMangoConfigPath[ PATH_MAX ];
        FILE *pMangoConfigFile = gamescope::MakeTempFile(
            szMangoConfigPath,
            gamescope::k_szGamescopeTempMangoappTemplate,
            "w",
            true );
        if ( pMangoConfigFile )
        {
    ...
    if ( !pszLimiterFile || !*pszLimiterFile )
    {
        ...
        int  nLimiterFd = gamescope::MakeTempFile(
            szLimiterPath, gamescope::k_szGamescopeTempLimiterTemplate, true );
        if ( nLimiterFd >= 0 )
        {
...
int main( int argc, char **argv )
{
    ...
    gamescope::GamescopeBackend eCurrentBackend =
                  ...
                  &opt_index ) ) != -1 )
    {
        ...
        switch ( o )
        {
            ...
            case 'h':
                ...
                break;
            case 'r':
                g_nNestedRefresh = gamescope::ConvertHztomHz(
                    parse_integer( optarg, "nested-refresh" ) );
                break;
            case 'W':
                g_nPreferredOutputWidth =
                    parse_integer( optarg, "output-width" );
                break;
            case 'H':
                g_nPreferredOutputHeight =
                    parse_integer( optarg, "output-height" );
                break;
            case 'o':
                g_nNestedUnfocusedRefresh = gamescope::ConvertHztomHz(
                    parse_integer( optarg, "nested-unfocused-refresh" ) );
                break;
            case 'm':
            ...
            case 0: // long options without a short option
                ...
                {
                    gamescope::cv_touch_click_mode =
                        ( gamescope::TouchClickMode )parse_integer(
                            optarg, opt_name );
                }
                else if ( strcmp( opt_name, "generate-drm-mode" ) == 0 )
                {
                    g_eGamescopeModeGeneration =
                        parse_gamescope_mode_generation( optarg );
                }
                else if ( strcmp( opt_name, "force-orientation" ) == 0 )
                {
                    ...
                    strcmp( opt_name, "sharpness" ) == 0 ||
                    strcmp( opt_name, "fsr-sharpness" ) == 0 )
                {
                    g_upscaleFilterSharpness =
                        parse_integer( optarg, opt_name );
                }
                else if ( strcmp( opt_name, "rt" ) == 0 ) { g_bRt = true; }
                ...
                else if ( strcmp( opt_name, "keep-alive" ) == 0 )
                {
                    ...
                    strcmp( opt_name, "virtual-connector-strategy" ) == 0 )
                {
                    for ( uint32_t i = 0;
                          i < gamescope::VirtualConnectorStrategies::Count;
                          i++ )
                    {
                        gamescope::VirtualConnectorStrategy eStrategy =
                            static_cast<gamescope::VirtualConnectorStrategy>(
                                i );
                        if ( optarg ==
                             gamescope::VirtualConnectorStrategyToString(
                                 eStrategy ) )
                        {
                            gamescope::cv_backend_virtual_connector_strategy =
                                eStrategy;
    ...
    if ( gamescope::Process::HasCapSysNice( ) )
    {
        ...
        if ( g_bRt ) gamescope::Process::SetRealtime( );
    }
    else
    {
        fprintf(
...

    {
        ...
        // Allow overriding the value of any ConVar with a suitably named
        // environment variable (gamescope_ConVar_name=override_value). This
        // takes precedence over the ConVar's default value and over values
        // assigned by default scripts.
        char      **s          = environ;
            ...
            if ( strncmp( envvar, prefix, prefix_len ) == 0 )
            {
                ...
                if ( !name.empty( ) )
                {
                    ...
                    if ( script.Manager( )
                             ...
                             .valid( ) )
                    {
                        auto override_script = std::format(
                            "gamescope.convars.{}.value = {}", name, value );
                        console_log.infof(
                            "Overriding from environment variable: %s",
                            override_script.c_str( ) );
    ...
    auto backendOverride =
        parse_backend_name( gamescope::cv_backend.Get( ).c_str( ) );
    ...
    if ( g_pOriginalWaylandDisplay != NULL )
    {
        ...
        {
            // Default to SDL_VIDEODRIVER wayland under Wayland and force enable
            // vk_khr_present_wait (not enabled by default in Mesa because
            // instance does not know if Wayland
            //  compositor supports wp_presentation, but we can check that
            //  ourselves.)
        ...
        else
        {
            fprintf(
                stderr,
                "Your Wayland compositor does NOT support "
                "wp_presentation/presentation-time which is required for "
                "VK_KHR_present_wait and VK_KHR_present_id.\n"
                "Please complain to your compositor vendor for support. "
                "Falling back to X11 window with less accurate present "
                "wait.\n" );
            setenv( "SDL_VIDEODRIVER", "x11", 1 );
        }
    }

    g_ForcedNV12ColorSpace =
        parse_colorspace_string( getenv( "GAMESCOPE_NV12_COLORSPACE" ) );
    ...
    if ( !vulkan_supports_modifiers( ) )
    {
        ...
        if ( pchR600Debug == nullptr ) { setenv( "R600_DEBUG", "nodcc", 1 ); }
    ...
    if ( g_nNestedWidth == 0 ) g_nNestedWidth = g_nNestedHeight * 16 / 9;
    ...
    gamescope_xwayland_server_t *base_server =
        wlserver_get_xwayland_server( 0 );

    setenv( "DISPLAY", base_server->get_nested_display_name( ), 1 );
    if ( g_bExposeWayland ) setenv( "XDG_SESSION_TYPE", "wayland", 1 );
    else
    ...
    if ( g_nXWaylandCount > 1 )
    {
        for ( int i = 1; i < g_nXWaylandCount; i++ )
        {
            char env_name[ 64 ];
            snprintf(
                env_name, sizeof( env_name ), "STEAM_GAME_DISPLAY_%d", i - 1 );
            gamescope_xwayland_server_t *server =
                wlserver_get_xwayland_server( i );
    ...
    else
    {
        setenv(
            ...
            base_server->get_nested_display_name( ),
            1 );
    }
    ...
    if ( !init_pipewire( ) )
    {
        fprintf(
            stderr,
            "Warning: failed to setup PipeWire, screen capture won't be "
            "available\n" );
    }
#endif
