...
namespace gamescope
{
    ...
    static constexpr uint32_t EDID_MAX_DESCRIPTOR_ESTABLISHED_TIMING_III_COUNT =
        44;
    ...
    std::optional<std::vector<uint8_t>> PatchEdid(
        std::span<const uint8_t>       pEdid,
        const BackendConnectorHDRInfo &hdrInfo,
        bool                           bRotate )
    {
        // A zero length indicates that the edid parsing failed.
        if ( pEdid.empty( ) ) return std::nullopt;
        ...
        if ( bRotate )
        {
            ...
            {
                uint8_t *byte_desc_data =
                    &edid[ 0x36 + i * EDID_BYTE_DESCRIPTOR_SIZE ];
                if ( byte_desc_data[ 0 ] || byte_desc_data[ 1 ] )
                {
                    uint32_t horiz =
                        ( get_bit_range( byte_desc_data[ 4 ], 7, 4 ) << 8 ) |
                        byte_desc_data[ 2 ];
                    uint32_t vert =
                        ( get_bit_range( byte_desc_data[ 7 ], 7, 4 ) << 8 ) |
                        byte_desc_data[ 5 ];
                    edid_log.infof(
                        "Patching res %ux%u -> %ux%u",
                        horiz,
                        vert,
                        vert,
                        horiz );
        ...
        // just hotpatch the edid for the game so we get values we want as if we
        // had an external display attached. (Allows for debugging undocked
        // fallback without undocking/redocking)
...
#endif
        // TODO(JoshA): Need to resolve flInternalDisplayBrightness vs new
        // connector hdrinfo mechanism.

        edid_log.infof(
            "[edid] Patching HDR static metadata:\n"
        ...
        const uint8_t new_hdr_static_metadata_block[]{
            ( 1 << HDMI_EOTF_SDR ) | ( 1 << HDMI_EOTF_TRADITIONAL_HDR ) |
                ( 1 << HDMI_EOTF_ST2084 ), /* supported eotfs */
            1,                             /* type 1 */
            encode_max_luminance(
                float( hdrInfo.uMaxContentLightLevel ) ), /* desired content max
                                                             peak luminance */
            encode_max_luminance(
                float( hdrInfo.uMaxFrameAverageLuminance ) ), /* desired content
                                                                 max frame avg
                                                                 luminance */
        ...
        for ( int i = 0; i < ext_count; i++ )
        {
            ...
            if ( tag == DI_EDID_EXT_CEA )
            {
                ...
                {
                    edid_log.infof(
                        "Hmmmm.... dtd start is 0. Interesting... Not going "
                        "further! :-(" );
                    continue;
                }
                if ( flags != 0 )
                {
                    edid_log.infof(
                        "Hmmmm.... non-zero CTA flags. Interesting... Not "
                        "going further! :-(" );
                    continue;
                }
                ...
                while ( j < dtd_start )
                {
                    ...
                    uint8_t data_block_tag =
                        get_bit_range( data_block_header, 7, 5 );
                    uint8_t data_block_size =
                        get_bit_range( data_block_header, 4, 0 );

                    if ( j + 1 + data_block_size > dtd_start )
                    {
                        edid_log.infof(
                            "Hmmmm.... CTA malformatted. Interesting... Not "
                            "going further! :-(" );
                        break;
                    }
                    ...
                    if ( data_block_tag == 7 ) // extended
                    {
                        ...
                        {
                            if ( extended_block_size >=
                                 sizeof( new_hdr_static_metadata_block ) )
                            {
                                edid_log.infof(
                                    "Patching existing HDR Metadata with our "
                                    "own!" );
                                memcpy(
                                    extended_block,
                                    new_hdr_static_metadata_block,
                                    sizeof( new_hdr_static_metadata_block ) );
                ...
                {
                    const int hdr_metadata_block_size_plus_headers =
                        sizeof( new_hdr_static_metadata_block ) +
                        2; // +1 for header, +1 for extended header -> +2
                    edid_log.infof(
                        "No HDR metadata block to patch... Trying to insert "
                        "one." );
                    ...
                    {
                        edid_log.infof(
                            "dtd_start != end of blocks. Giving up patching. "
                            "I'm too scared to attempt it." );
                    }

                    // Move back the dtd to make way for our block at the end.
                    uint8_t *dtd = &ext_data[ dtd_start ];
                    memmove(
                        dtd + hdr_metadata_block_size_plus_headers,
                        dtd,
                        hdr_metadata_block_size_plus_headers );
                    ...
                    set_bit_range(
                        ...
                        0,
                        sizeof( new_hdr_static_metadata_block ) +
                            1 ); // size (+1 for extended header, does not
                ...
                edid_log.infof(
                    "CTA Checksum valid? %s", sum_valid ? "Y" : "N" );
            }
        }
        ...
        {
            edid_log.infof(
                "Couldn't patch for HDR metadata as we had no CTA block! Womp "
                "womp =c" );
        }
    ...
    {
        const char *pszPatchedEdidPath =
            getenv( "GAMESCOPE_PATCHED_EDID_FILE" );
        if ( !pszPatchedEdidPath || !*pszPatchedEdidPath ) return nullptr;

        return pszPatchedEdidPath;
    }

    void WritePatchedEdid(
        std::span<const uint8_t>       pEdid,
        const BackendConnectorHDRInfo &hdrInfo,
        bool                           bRotate )
    {
        const char *pszPatchedEdidPath = GetPatchedEdidPath( );
        if ( !pszPatchedEdidPath ) return;

        std::span<const uint8_t> pEdidToWrite = pEdid;

        auto oPatchedEdid = PatchEdid( pEdid, hdrInfo, bRotate );
        if ( oPatchedEdid )
            pEdidToWrite = std::span<const uint8_t>{ oPatchedEdid->begin( ),
                                                     oPatchedEdid->end( ) };

        char szTmpFilename[ PATH_MAX ];
        snprintf(
            szTmpFilename,
            sizeof( szTmpFilename ),
            "%s.tmp",
            pszPatchedEdidPath );
    ...
    std::vector<uint8_t> GenerateSimpleEdid( uint32_t uWidth, uint32_t uHeight )
    {
        ...
        std::vector<uint8_t> edid(
            s_GamescopeBaseEdid,
            s_GamescopeBaseEdid + std::size( s_GamescopeBaseEdid ) );
        ...
        for ( uint32_t i = 0; i < EDID_BYTE_DESCRIPTOR_COUNT; i++ )
        {
            ...
            if ( byte_desc_data[ 0 ] || byte_desc_data[ 1 ] )
            {
                ...
                set_bit_range(
                    &byte_desc_data[ 4 ], 7, 4, ( uWidth >> 8 ) & 0xff );
                byte_desc_data[ 2 ] = uWidth & 0xff;

                set_bit_range(
                    &byte_desc_data[ 7 ], 7, 4, ( uHeight >> 8 ) & 0xff );
