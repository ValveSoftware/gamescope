...
namespace gamescope
{
    ...
    template<typename T> inline std::string ToString( const T &thing )
    { return std::to_string( thing ); }

    template<> inline std::string ToString( const std::string &sThing )
    { return sThing; }

    template<> inline std::string ToString( const std::string_view &svThing )
    { return std::string( svThing ); }

    template<typename T> inline std::optional<T> Parse( std::string_view chars )
    {
        T    obj;
        auto result = std::from_chars( chars.begin( ), chars.end( ), obj );
        if ( result.ec == std::errc{} ) return obj;
        else
            return std::nullopt;
    }

    template<> inline std::optional<bool> Parse( std::string_view chars )
    {
        std::optional<uint32_t> oNumber = Parse<uint32_t>( chars );
        if ( oNumber ) return !!*oNumber;

        if ( chars == "true" ) return true;
        else
            return false;
    }
        ...
        std::string_view               delims = " " )
    {
        size_t end = 0;
        for ( size_t start = 0;
              start < string.size( ) && end != std::string_view::npos;
              start = end + 1 )
        {
    ...
    Split( std::string_view string, std::string_view delims = " " )
    {
        std::vector<std::string_view> tokens;
    ...
    class ConCommand
    {
        ...
        using ConCommandFunc =
            std::function<void( std::span<std::string_view> )>;

    public:
        ConCommand(
            std::string_view pszName,
            std::string_view pszDescription,
            ConCommandFunc   func,
            bool             bRegisterScript = true );
        ~ConCommand( );
    ...
    template<typename T> class ConVar : public ConCommand
    {
        friend struct detail::ConVarScriptRegistrar;
    ...
    public:
        ConVar(
            ...
            T                  defaultValue          = T{},
            std::string_view   pszDescription        = "",
            ConVarCallbackFunc func                  = nullptr,
            bool               bRunCallbackAtStartup = false,
            bool               bRegisterScript       = true ) :
            ConCommand(
                pszName,
                pszDescription,
                [ this ]( std::span<std::string_view> pArgs )
                { this->InvokeFunc( pArgs ); },
                false ),
            m_Value{ defaultValue },
            m_Callback{ func }
        {
            if ( bRunCallbackAtStartup ) { RunCallback( ); }

#if HAVE_SCRIPTING
            if ( bRegisterScript ) RegisterScript( pszName, this );
#endif
        }
        ...
        template<typename J> void SetValue( const J &newValue )
        {
            m_Value = T{ newValue };
        ...
        template<typename J> ConVar<T> &operator=( const J &newValue )
        {
            SetValue<J>( newValue );
            return *this;
        }
        ...
        template<typename J> bool operator==( const J &other ) const
        { return m_Value == other; }
        template<typename J> bool operator!=( const J &other ) const
        { return m_Value != other; }
        template<typename J> auto operator<=>( const J &other ) const
        { return m_Value <=> other; }

        template<typename J> bool operator==( const ConVar<J> &other ) const
        { return *this == other.Get( ); }
        template<typename J> bool operator!=( const ConVar<J> &other ) const
        { return *this != other.Get( ); }
        template<typename J> auto operator<=>( const ConVar<J> &other ) const
        { return *this <=> other.Get( ); }
        ...
        void InvokeFunc( std::span<std::string_view> pArgs )
        {
            if ( pArgs.size( ) == 1 )
            {
                ...
                console_log.infof(
                    "%.*s: %.*s\n%.*s",
                    ( int )m_pszName.length( ),
                    m_pszName.data( ),
                    ( int )sValue.length( ),
                    sValue.data( ),
                    ( int )m_pszDescription.length( ),
                    m_pszDescription.data( ) );

                return;
            }

            if ( pArgs.size( ) != 2 ) return;

            if constexpr ( std::is_enum<T>::value )
            {
                using Underlying = std::underlying_type<T>::type;
                std::optional<Underlying> oResult =
                    Parse<Underlying>( pArgs[ 1 ] );
