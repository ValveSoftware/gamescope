...
struct wlserver_t wlserver = { .touch_down_ids = {} };
...
static void wlserver_x11_surface_info_set_wlr(
    struct wlserver_x11_surface_info *surf,
...
static void
wlserver_constrain_cursor( struct wlr_pointer_constraint_v1 *pNewConstraint );
struct wlr_surface *
wlserver_surface_to_main_surface( struct wlr_surface *pSurface );
bool wlserver_process_hotkeys(
    wlr_keyboard *keyboard, uint32_t key, bool press );
...
gamescope::ConVar<bool> cv_drm_debug_syncobj_force_wait_on_commit(
    "drm_debug_syncobj_force_wait_on_commit",
    false,
    "Force a wait on DRM sync objects before committing buffers" );

std::optional<ResListEntry_t>
PrepareCommit( struct wlr_surface *surf, struct wlr_buffer *buf )
{
    auto wl_surf = get_wl_surface_info( surf );

    const auto &pFeedback = wlserver_surface_swapchain_feedback( surf );

    bool bExplicitSync = wl_surf->pSyncobjSurface &&
                         wl_surf->pSyncobjSurface->HasExplicitSync( );
    ...
    {
        pAcquirePoint =
            wl_surf->pSyncobjSurface->ExtractAcquireTimelinePoint( );
        pReleasePoint =
            wl_surf->pSyncobjSurface->ExtractReleaseTimelinePoint( );
    ...
    struct wlr_surface *pConstraintSurface =
        wlserver_surface_to_main_surface( surf );
...
void gamescope_xwayland_server_t::wayland_commit(
    struct wlr_surface *surf, struct wlr_buffer *buf )
{
...
void xwayland_surface_commit( struct wlr_surface *wlr_surface )
{
    ...
    wlserver_x11_surface_info *wlserver_x11_surface_info =
        get_wl_surface_info( wlr_surface )->x11_surface;
    wlserver_xdg_surface_info *wlserver_xdg_surface_info =
        get_wl_surface_info( wlr_surface )->xdg_surface;
        ...
        if ( !wlserver_xdg_surface_info->bDoneConfigure )
        {
            if ( wlserver_xdg_surface_info->xdg_surface )
                wlr_xdg_surface_schedule_configure(
                    wlserver_xdg_surface_info->xdg_surface );

            if ( wlserver_xdg_surface_info->layer_surface )
                wlr_layer_surface_v1_configure(
                    wlserver_xdg_surface_info->layer_surface,
                    g_nNestedWidth,
                    g_nNestedHeight );
    ...
    // Committing without buffer state is valid and commits the same buffer
    // again. Mutter and Weston have forward progress on the frame callback in
    // this situation, so let the commit go through. It will be
    // duplication-eliminated later.

    VulkanWlrTexture_t *tex =
        ( VulkanWlrTexture_t * )wlr_surface_get_texture( wlr_surface );
    if ( tex == NULL ) { return; }

    struct wlr_buffer *buf = wlr_buffer_lock( tex->buf );

    gpuvis_trace_printf(
        "xwayland_surface_commit wlr_surface %p", wlr_surface );

    if ( wlserver_x11_surface_info )
    {
        assert( wlserver_x11_surface_info->xwayland_server );
        wlserver_x11_surface_info->xwayland_server->wayland_commit(
            wlr_surface, buf );
    }
...
void gamescope_xwayland_server_t::on_xwayland_ready( void *data )
{
    ...
    if ( !xwayland_server->options.no_touch_pointer_emulation )
        wl_log.infof(
            "Xwayland doesn't support -noTouchPointerEmulation, touch events "
            "might get duplicated" );

    dpy = XOpenDisplay( get_nested_display_name( ) );
}

void gamescope_xwayland_server_t::xwayland_ready_callback(
    struct wl_listener *listener, void *data )
{
    gamescope_xwayland_server_t *server =
        wl_container_of( listener, server, xwayland_ready_listener );
...
static void wlserver_handle_key( struct wl_listener *listener, void *data )
{
    ...
    struct wlr_keyboard_key_event *event =
        ( struct wlr_keyboard_key_event * )data;
    ...
    bool forbidden_key = keysym == XKB_KEY_XF86AudioLowerVolume ||
    ...
    if ( ( event->state == WL_KEYBOARD_KEY_STATE_PRESSED ||
         ...
         forbidden_key )
    {
        // Always send volume+/- to root server only, to avoid it reaching the
        // game.
        struct wlr_surface *old_kb_surf = wlserver.kb_focus_surface;
        struct wlr_surface *new_kb_surf =
            steamcompmgr_get_server_input_surface( 0 );
        if ( new_kb_surf )
        {
             ...
             event->state == WL_KEYBOARD_KEY_STATE_PRESSED ) )
    {
        wlr_seat_set_keyboard( wlserver.wlr.seat, keyboard );
        wlr_seat_keyboard_notify_key(
            wlserver.wlr.seat, event->time_msec, event->keycode, event->state );
    }
...
static void
wlserver_perform_rel_pointer_motion( double unaccel_dx, double unaccel_dy )
{
    assert( wlserver_is_lock_held( ) );

    wlr_relative_pointer_manager_v1_send_relative_motion(
        wlserver.relative_pointer_manager,
        wlserver.wlr.seat,
        0,
        unaccel_dx,
        unaccel_dy,
...
static void
wlserver_handle_pointer_motion( struct wl_listener *listener, void *data )
{
    struct wlr_pointer_motion_event *event =
        ( struct wlr_pointer_motion_event * )data;
...
void wlserver_open_steam_menu( bool qam )
{
    ...
    XTestFakeKeyEvent(
        server->get_xdisplay( ),
        XKeysymToKeycode( server->get_xdisplay( ), XK_Control_L ),
        True,
        CurrentTime );
    XTestFakeKeyEvent(
        server->get_xdisplay( ),
        XKeysymToKeycode( server->get_xdisplay( ), keycode ),
        True,
        CurrentTime );
...
static void
wlserver_handle_pointer_button( struct wl_listener *listener, void *data )
{
    struct wlserver_pointer *pointer =
        wl_container_of( listener, pointer, button );
    struct wlr_pointer_button_event *event =
        ( struct wlr_pointer_button_event * )data;

    wlr_seat_pointer_notify_button(
        wlserver.wlr.seat, event->time_msec, event->button, event->state );
}

static void
wlserver_handle_pointer_axis( struct wl_listener *listener, void *data )
{
    struct wlserver_pointer *pointer =
        wl_container_of( listener, pointer, axis );
    struct wlr_pointer_axis_event *event =
        ( struct wlr_pointer_axis_event * )data;
...
static void
wlserver_handle_pointer_frame( struct wl_listener *listener, void *data )
{
...
TouchClickModeToLinuxButton( gamescope::TouchClickMode eTouchClickMode )
{
...
static void wlserver_touch_associate_connector( struct wlserver_touch *touch )
{
    ...
    //  - if a touchscreen's bus is I²C, it can very likely be associated to an
    //  internal monitor.
    //  - if its bus is USB, it can be associated to an external monitor.
    // This isn't perfect, but we can't rely on the physical sizes reported by
    // both devices, because it's not uncommon for touchscreens to report wildly
    // incorrect sizes.
    gamescope::IBackendConnector *connector = nullptr;
    struct libinput_device       *lidev =
        wlr_libinput_get_device_handle( &touch->wlr->base );
    ...
    while ( parent )
    {
        ...
        if ( subsystem )
        {
            ...
            {
                connector = GetBackend( )->GetConnector(
    ...
    if ( connector != nullptr )
    {
        ...
        wl_log.infof(
            "associating connector %s (%s %s) with touch input device %s",
            connector->GetName( ),
            connector->GetMake( ),
            connector->GetModel( ),
...
static void
wlserver_handle_touch_down( struct wl_listener *listener, void *data )
{
...
static void
wlserver_handle_touch_motion( struct wl_listener *listener, void *data )
{
    struct wlserver_touch *touch = wl_container_of( listener, touch, motion );
    struct wlr_touch_motion_event *event =
        ( struct wlr_touch_motion_event * )data;
    ...
    switch ( device->type )
    {
        case WLR_INPUT_DEVICE_KEYBOARD:
        {
            ...
            wlr_keyboard_set_keymap(
                keyboard, wlserver.keyboard_group->keyboard.keymap );
            if ( !wlr_keyboard_group_add_keyboard(
                     wlserver.keyboard_group, keyboard ) )
            {
                wl_log.errorf(
                    "failed to add physical keyboard %s", device->name );
                break;
            }
        ...
        {
            struct wlserver_pointer *pointer =
                ( struct wlserver_pointer * )calloc(
                    1, sizeof( struct wlserver_pointer ) );
        ...
        {
            struct wlserver_touch *touch = ( struct wlserver_touch * )calloc(
                1, sizeof( struct wlserver_touch ) );
...
static void handle_wl_surface_destroy( struct wl_listener *l, void *data )
{
    ...
    if ( surf->x11_surface )
    {
        ...
        wlserver_x11_surface_info_init(
            x11_surface, x11_surface->xwayland_server, x11_surface->x11_id );
    }
...
static struct wl_listener new_surface_listener = { .notify =
                                                       wlserver_new_surface };

void gamescope_xwayland_server_t::destroy_content_override(
    struct wlserver_content_override *co )
{
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
    wl_log.infof(
        "destroy_content_override REAL: co: %p co->surface: %p co->x11_window: "
        "0x%x co->gamescope_swapchain: %p",
        co,
        co->surface,
        co->x11_window,
        co->gamescope_swapchain );
#endif
    if ( co->surface )
    {
        wlserver_wl_surface_info *wl_surface_info =
            get_wl_surface_info( co->surface );
        if ( wl_surface_info ) wl_surface_info->x11_surface = nullptr;
    }
...
void gamescope_xwayland_server_t::destroy_content_override(
    struct wlserver_x11_surface_info *x11_surface, struct wlr_surface *surf )
{
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
    wl_log.infof(
        "destroy_content_override LOOKUP: x11_surface: %p x11_window: 0x%x "
        "surf: %p",
        x11_surface,
        x11_surface->x11_id,
        surf );
    ...
    if ( iter == content_overrides.end( ) ) return;
...
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
    wl_log.infof(
        "destroy_content_override LOOKUP FOUND: x11_surface: %p x11_window: "
        "0x%x surf: %p co: %p co->surface: %p",
        x11_surface,
        x11_surface->x11_id,
        surf,
        co,
    ...
    if ( co->surface == surf ) destroy_content_override( iter->second );
}

static void content_override_handle_surface_destroy(
    struct wl_listener *listener, void *data )
{
    struct wlserver_content_override *co =
        wl_container_of( listener, co, surface_destroy_listener );
...
void gamescope_xwayland_server_t::handle_override_window_content(
    struct wl_client   *client,
    struct wl_resource *gamescope_swapchain_resource,
    struct wlr_surface *surface,
    uint32_t            x11_window )
{
    wlserver_x11_surface_info *x11_surface =
        lookup_x11_surface_info_from_xid( this, x11_window );
    // If we found an x11_surface, go back up to our parent.
    if ( x11_surface ) x11_window = x11_surface->x11_id;

#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
    wl_log.infof(
        "handle_override_window_content: (1) x11_window: 0x%x "
        "swapchain_resource: %p surface: %p",
        x11_window,
    ...
    {
        if ( content_overrides[ x11_window ]->gamescope_swapchain ==
             gamescope_swapchain_resource )
            return;
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
        wl_log.infof(
            "handle_override_window_content: (2) DESTROYING x11_window: 0x%x "
            "old_swapchain: %p new_swapchain: %p",
            x11_window,
            content_overrides[ x11_window ]->gamescope_swapchain,
            gamescope_swapchain_resource );
#endif
    ...
    if ( x11_surface )
    {
        for ( auto it = g_PendingCommits.begin( );
              it != g_PendingCommits.end( ); )
        {
            if ( it->surf == surface )
            {
                ...
                x11_surface->xwayland_server->wayland_commit(
                    pending.surf, pending.buf );
...
struct wlr_output *gamescope_xwayland_server_t::get_output( ) { return output; }

struct wlr_output_state *gamescope_xwayland_server_t::get_output_state( )
{ return output_state; }

static void gamescope_xwayland_handle_override_window_content(
    struct wl_client   *client,
    ...
    uint32_t            x11_window )
{
    ...
    server->handle_override_window_content(
        client, nullptr, surface, x11_window );
}

static void gamescope_xwayland_handle_destroy(
...
static const struct gamescope_xwayland_interface gamescope_xwayland_impl = {
    ...
    .override_window_content =
        gamescope_xwayland_handle_override_window_content,
};

static void gamescope_xwayland_bind(
    struct wl_client *client, void *data, uint32_t version, uint32_t id )
{
    struct wl_resource *resource = wl_resource_create(
        client, &gamescope_xwayland_interface, version, id );
    wl_resource_set_implementation(
        resource, &gamescope_xwayland_impl, NULL, NULL );
}
...
static void create_gamescope_xwayland( void )
{
    ...
    wl_global_create(
        wlserver.display,
        &gamescope_xwayland_interface,
        version,
        NULL,
        gamescope_xwayland_bind );
}
...
#endif
    wlserver_wl_surface_info *wl_surface_info =
        ( wlserver_wl_surface_info * )wl_resource_get_user_data( resource );
    if ( wl_surface_info )
    {
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
        wl_log.infof(
            "gamescope_swapchain_destroy_co swapchain: %p GOT WAYLAND SURFACE",
            resource );
#endif
...
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
            wl_log.infof(
                "gamescope_swapchain_destroy_co swapchain: %p GOT X11 SURFACE",
                resource );
#endif
            x11_surface->xwayland_server->destroy_content_override(
                x11_surface, wl_surface_info->wlr );
        }
    }
}

static void
gamescope_swapchain_handle_resource_destroy( struct wl_resource *resource )
{
#ifdef GAMESCOPE_SWAPCHAIN_DEBUG
    wl_log.infof(
        "gamescope_swapchain_handle_resource_destroy swapchain: %p", resource );
#endif
    wlserver_wl_surface_info *wl_surface_info =
        ( wlserver_wl_surface_info * )wl_resource_get_user_data( resource );
    if ( wl_surface_info )
...
static void gamescope_swapchain_override_window_content(
    struct wl_client   *client,
    struct wl_resource *resource,
    uint32_t            server_id,
    uint32_t            x11_window )
{
    ...
    assert( server );
    server->handle_override_window_content(
        client, resource, wl_surface_info->wlr, x11_window );
}

static void gamescope_swapchain_swapchain_feedback(
    struct wl_client   *client,
    struct wl_resource *resource,
...
{
    wlserver_wl_surface_info *wl_info =
        ( wlserver_wl_surface_info * )wl_resource_get_user_data( resource );
    if ( wl_info )
    {
        wl_info->swapchain_feedback =
            std::make_unique<wlserver_vk_swapchain_feedback>(
                wlserver_vk_swapchain_feedback{
                    ...
                    .vk_engine_name =
                        std::make_shared<std::string>( vk_engine_name ),
                    .hdr_metadata_blob = nullptr,
                } );
    }
}

static void gamescope_swapchain_set_hdr_metadata(
    struct wl_client   *client,
    struct wl_resource *resource,
    ...
    uint32_t            max_fall )
{
    ...
    if ( wl_info )
    {
        ...
        wl_info->swapchain_feedback->hdr_metadata_blob =
            GetBackend( )->CreateBackendBlob( metadata );
    }
}

static void gamescope_swapchain_set_present_time(
    struct wl_client   *client,
    struct wl_resource *resource,
    uint32_t            present_id,
    ...
    uint32_t            desired_present_time_lo )
{
    ...
    if ( wl_info )
    {
        ...
        wl_info->desired_present_time =
            ( uint64_t( desired_present_time_hi ) << 32 ) |
            desired_present_time_lo;
    }
}

static void gamescope_swapchain_set_present_mode(
    struct wl_client   *client,
    struct wl_resource *resource,
...
static void gamescope_swapchain_factory_v2_destroy(
    struct wl_client *client, struct wl_resource *resource )
{ wl_resource_destroy( resource ); }

static void gamescope_swapchain_factory_v2_create_swapchain(
    struct wl_client   *client,
    struct wl_resource *resource,
    struct wl_resource *surface_resource,
    uint32_t            id )
{
    ...
    struct wl_resource *gamescope_swapchain_resource = wl_resource_create(
        client,
    ...
    wl_resource_set_implementation(
        gamescope_swapchain_resource,
        &gamescope_swapchain_impl,
        wl_surface_info,
        gamescope_swapchain_handle_resource_destroy );

    if ( wl_surface_info->gamescope_swapchains.size( ) )
        wl_log.errorf(
            "create_swapchain: Surface already had a gamescope_swapchain! "
            "Warning!" );

    wl_surface_info->gamescope_swapchains.emplace_back(
        gamescope_swapchain_resource );
}

static const struct gamescope_swapchain_factory_v2_interface
    gamescope_swapchain_factory_v2_impl = {
        .destroy          = gamescope_swapchain_factory_v2_destroy,
        .create_swapchain = gamescope_swapchain_factory_v2_create_swapchain,
    };

static void gamescope_swapchain_factory_v2_bind(
    struct wl_client *client, void *data, uint32_t version, uint32_t id )
{
    struct wl_resource *resource = wl_resource_create(
        client, &gamescope_swapchain_factory_v2_interface, version, id );
    wl_resource_set_implementation(
        resource, &gamescope_swapchain_factory_v2_impl, NULL, NULL );
}
...
static void create_gamescope_swapchain_factory_v2( void )
{
    ...
    wl_global_create(
        wlserver.display,
        &gamescope_swapchain_factory_v2_interface,
        version,
        NULL,
        gamescope_swapchain_factory_v2_bind );
}

#if HAVE_PIPEWIRE
static void gamescope_pipewire_handle_destroy(
    struct wl_client *client, struct wl_resource *resource )
{ wl_resource_destroy( resource ); }
...
static void gamescope_pipewire_bind(
    struct wl_client *client, void *data, uint32_t version, uint32_t id )
{
    struct wl_resource *resource = wl_resource_create(
        client, &gamescope_pipewire_interface, version, id );
    wl_resource_set_implementation(
        resource, &gamescope_pipewire_impl, NULL, NULL );
...
static void create_gamescope_pipewire( void )
{
    ...
    wl_global_create(
        wlserver.display,
        &gamescope_pipewire_interface,
        version,
        NULL,
        gamescope_pipewire_bind );
}
#endif

//

static void gamescope_control_set_app_target_refresh_cycle(
    struct wl_client   *client,
    struct wl_resource *resource,
    uint32_t            fps,
    uint32_t            flags )
{
    ...
    steamcompmgr_set_app_refresh_cycle_override(
        ...
        !( flags &
           GAMESCOPE_CONTROL_TARGET_REFRESH_CYCLE_FLAG_ONLY_CHANGE_REFRESH_RATE ) );
}

static void gamescope_control_take_screenshot(
    struct wl_client   *client,
    struct wl_resource *resource,
    const char         *path,
    uint32_t            type,
...
static void gamescope_control_display_sleep(
    struct wl_client   *client,
    struct wl_resource *resource,
    uint32_t            display_type_flags,
    uint32_t            flags )
{
    if ( flags & ( GAMESCOPE_CONTROL_DISPLAY_SLEEP_FLAGS_SLEEP |
                   GAMESCOPE_CONTROL_DISPLAY_SLEEP_FLAGS_WAKE ) )
    {
        const bool sleep =
            !!( flags & GAMESCOPE_CONTROL_DISPLAY_SLEEP_FLAGS_SLEEP );
        if ( display_type_flags &
             GAMESCOPE_CONTROL_DISPLAY_TYPE_FLAGS_EXTERNAL_DISPLAY )
            drm_sleep_screen(
                gamescope::GAMESCOPE_SCREEN_TYPE_EXTERNAL, sleep );

        if ( display_type_flags &
             GAMESCOPE_CONTROL_DISPLAY_TYPE_FLAGS_INTERNAL_DISPLAY )
            drm_sleep_screen(
                gamescope::GAMESCOPE_SCREEN_TYPE_INTERNAL, sleep );
    }
}
...
static gamescope::ConCommand cc_set_look(
    "set_look",
    "Set a look for a specific EOTF. Eg. set_look mylook.cube (g22 only), "
    "set_look pq mylook.cube, set_look mylook_g22.cube mylook_pq.cube",
    []( std::span<std::string_view> args )
    {
        if ( args.size( ) == 2 )
        {
            ...
            g_ColorMgmtLooks[ EOTF_Gamma22 ] =
                LoadCubeLut( arg1.c_str( ), bRaisesBlackLevelFloor );
        ...
        else if ( args.size( ) == 3 )
        {
            ...
            {
                g_ColorMgmtLooks[ EOTF_Gamma22 ] =
                    LoadCubeLut( arg2.c_str( ), bRaisesBlackLevelFloor );
            }
            else if ( args[ 1 ] == "pq" || args[ 1 ] == "PQ" )
            {
                g_ColorMgmtLooks[ EOTF_PQ ] =
                    LoadCubeLut( arg2.c_str( ), bRaisesBlackLevelFloor );
            }
            else
            {
...
static void gamescope_control_set_look(
    struct wl_client   *client,
    struct wl_resource *resource,
    int                 g22_fd,
    int                 pq_fd,
    uint32_t            flags )
{
    ...
    if ( g22_fd >= 0 )
    {
        ...
        {
            wl_log.errorf_errno(
                "gamescope_control_set_look error opening g22 lut" );
            close( g22_fd );
        }
        g22_fd = -1;
    }
    ...
    if ( pq_fd >= 0 )
    {
        ...
        {
            wl_log.errorf_errno(
                "gamescope_control_set_look error opening pq lut" );
            close( pq_fd );
        }
        pq_fd = -1;
    }
...
static void gamescope_control_unset_look(
    struct wl_client *client, struct wl_resource *resource )
{
    cv_overlay_unmultiplied_alpha    = false;
    g_ColorMgmtLooks[ EOTF_Gamma22 ] = nullptr;
...
static void gamescope_control_handle_destroy(
    struct wl_client *client, struct wl_resource *resource )
{ wl_resource_destroy( resource ); }

static void gamescope_control_request_app_performance_stats(
    struct wl_client *client, struct wl_resource *resource, uint32_t app_id )
{
    assert( wlserver_is_lock_held( ) );
...
void wlserver_app_presented( uint32_t app_id, uint64_t frametime_ns )
{
    ...
    if ( it == wlserver.app_perf_requests.end( ) ) return;
    ...
    for ( wl_resource *resource : it->second )
    {
        ...
        gamescope_control_send_app_performance_stats(
            resource, app_id, frametime_lo, frametime_hi );
    }
...
void wlserver_send_gamescope_control( wl_resource *control )
{
    ...
    if ( pConn->GetValidDynamicRefreshRates( ).size( ) )
    {
        ...
        {
            uint32_t *ptr = ( uint32_t * )wl_array_add(
                &display_rates, sizeof( uint32_t ) );
            *ptr = uRateHz;
        }
    }
    ...
    {
        uint32_t *ptr =
            ( uint32_t * )wl_array_add( &display_rates, sizeof( uint32_t ) );
        *ptr = ( uint32_t )gamescope::ConvertmHzToHz( g_nOutputRefresh );
    }
    gamescope_control_send_active_display_info(
        control,
        pConn->GetName( ),
        pConn->GetMake( ),
        pConn->GetModel( ),
...
{
    struct wl_resource *resource =
        wl_resource_create( client, &gamescope_control_interface, version, id );
    wl_resource_set_implementation(
        resource,
        &gamescope_control_impl,
        NULL,
        []( struct wl_resource *resource )
        {
            std::erase_if(
                wlserver.gamescope_controls,
                [ = ]( struct wl_resource *control )
    ...
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_RESHADE_SHADERS, 1, 0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_DISPLAY_INFO, 1, 0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_PIXEL_FILTER, 1, 0 );
    gamescope_control_send_feature_support(
        resource,
        ...
        0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_MURA_CORRECTION, 1, 0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_LOOK, 1, 0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_PERF_QUERY, 1, 0 );
    gamescope_control_send_feature_support(
        resource, GAMESCOPE_CONTROL_FEATURE_DONE, 0, 0 );
...
static void gamescope_private_execute(
    struct wl_client   *client,
    struct wl_resource *resource,
...
static void gamescope_private_handle_destroy(
    struct wl_client *client, struct wl_resource *resource )
{ wl_resource_destroy( resource ); }
...
static void gamescope_private_bind(
    struct wl_client *client, void *data, uint32_t version, uint32_t id )
{
    struct wl_resource *resource =
        wl_resource_create( client, &gamescope_private_interface, version, id );
    console_log.m_LoggingListeners[ ( uintptr_t )resource ] =
        [ resource ](
            LogPriority      ePriority,
            std::string_view psvScope,
            std::string_view psvText )
    {
        if ( !wlserver_is_lock_held( ) ) return;
...
static void create_gamescope_private( void )
{
    ...
    wl_global_create(
        wlserver.display,
        &gamescope_private_interface,
        version,
        NULL,
        gamescope_private_bind );
}
...
static void presentation_time_destroy(
    struct wl_client *client, struct wl_resource *resource )
{ wl_resource_destroy( resource ); }

static void presentation_time_feedback(
    struct wl_client   *client,
    struct wl_resource *resource,
    struct wl_resource *surface_resource,
    uint32_t            id )
{
    ...
    struct wl_resource *presentation_feedback_resource = wl_resource_create(
        ...
        id );
    wl_resource_set_implementation(
        presentation_feedback_resource, NULL, wl_surface_info, NULL );

    wl_surface_info->pending_presentation_feedbacks.emplace_back(
        presentation_feedback_resource );
}
...
static void presentation_time_bind(
    struct wl_client *client, void *data, uint32_t version, uint32_t id )
{
    struct wl_resource *resource =
        wl_resource_create( client, &wp_presentation_interface, version, id );
    wl_resource_set_implementation(
        resource, &presentation_time_impl, NULL, NULL );
...
static void create_presentation_time( void )
{
    ...
    wl_global_create(
        ...
        version,
        NULL,
        presentation_time_bind );
}

void wlserver_presentation_feedback_presented(
    struct wlr_surface                *surface,
    std::vector<struct wl_resource *> &presentation_feedbacks,
    uint64_t                           last_refresh_nsec,
    uint64_t                           refresh_cycle )
{
    wlserver_wl_surface_info *wl_surface_info = get_wl_surface_info( surface );

    if ( !wl_surface_info ) return;
    ...
    // We don't set HW_COMPLETION because we actually kinda are using a timer
    // here. We want to signal this event at latest latch time, but with the
    // info of the refresh cadence we want to track.
...
void wlserver_presentation_feedback_discard(
    struct wlr_surface                *surface,
    std::vector<struct wl_resource *> &presentation_feedbacks )
{
    wlserver_wl_surface_info *wl_surface_info = get_wl_surface_info( surface );
...
void wlserver_past_present_timing(
    struct wlr_surface *surface,
    uint32_t            present_id,
...
void wlserver_refresh_cycle(
    struct wlr_surface *surface, uint64_t refresh_cycle )
{
    wlserver_wl_surface_info *wl_info = get_wl_surface_info( surface );
    if ( !wl_info ) return;
    ...
    for ( auto &swapchain : wl_info->gamescope_swapchains )
    {
        gamescope_swapchain_send_refresh_cycle(
            swapchain, refresh_cycle >> 32, refresh_cycle & 0xffffffff );
    }
}
...
#if HAVE_SESSION
bool wlsession_active( ) { return wlserver.wlr.session->active; }

static void handle_session_active( struct wl_listener *listener, void *data )
{
    GetBackend( )->DirtyState(
        wlserver.wlr.session->active, wlserver.wlr.session->active );
    wl_log.infof(
        "Session %s", wlserver.wlr.session->active ? "resumed" : "paused" );
}
#endif
...
void wlserver_set_output_info( const wlserver_output_info *info )
{
    ...
    if ( wlserver.wlr.xwayland_servers.empty( ) ) return;

    gamescope_xwayland_server_t *server =
        wlserver.wlr.xwayland_servers[ 0 ].get( );
    server->update_output_info( );
}

static bool filter_global(
    const struct wl_client *client, const struct wl_global *global, void *data )
{
    ...
    if ( cv_drm_debug_disable_explicit_sync &&
         iface->name == "wp_linux_drm_syncobj_manager_v1"sv )
        return false;

    if ( strcmp( iface->name, wl_output_interface.name ) != 0 ) return true;

    struct wlr_output *output =
        ( struct wlr_output * )wl_global_get_user_data( global );
    if ( !output )
    {
    ...
    if ( g_bShutdownWLServer ) return false;
    ...
    {
        gamescope_xwayland_server_t *server =
            wlserver.wlr.xwayland_servers[ i ].get( );
        if ( server && server->get_client( ) == client )
            return server->get_output( ) == output;
    }

    if ( wlserver.wlr.xwayland_servers.empty( ) ) return false;
...
bool wlsession_init( void )
{
    ...
    wl_signal_add(
        &wlserver.wlr.session->events.active, &wlserver.session_active );
#endif
...
int wlsession_open_kms( const char *device_name )
{
    ...
    {
        wlserver.wlr.device =
            wlr_session_open_file( wlserver.wlr.session, device_name );
        if ( wlserver.wlr.device == nullptr ) return -1;
    }
    else
    {
        ssize_t n = wlr_session_find_gpus(
            wlserver.wlr.session, 1, &wlserver.wlr.device );
        if ( n < 0 )
        {
    ...
    wl_display *display, int nIndex )
{
    ...
    output =
        wlr_headless_add_output( wlserver.wlr.headless_backend, 1280, 720 );
    ...
    if ( refresh == 0 ) { refresh = g_nOutputRefresh; }
...
static void waylandy_surface_destroy( struct wl_listener *listener, void *data )
{
    ...
    wlserver_wl_surface_info *wlserver_surface =
        get_wl_surface_info( info->main_surface );
    if ( !wlserver_surface )
    {
...
wlserver_xdg_surface_info *waylandy_type_surface_new(
    struct wl_client *client, struct wlr_surface *surface )
{
...
void xdg_surface_new( struct wl_listener *listener, void *data )
{
    ...
    wlserver_xdg_surface_info *surface_info = waylandy_type_surface_new(
        xdg_surface->client->client, xdg_surface->surface );
...
void layer_shell_surface_new( struct wl_listener *listener, void *data )
{
    ...
    wlserver_xdg_surface_info *surface_info =
        waylandy_type_surface_new( nullptr, layer_surface->surface );
    surface_info->destroy.notify = waylandy_surface_destroy;
...
bool wlserver_init( void )
{
    ...
    wlserver.wlr.multi_backend =
        wlr_multi_backend_create( wlserver.event_loop );
    wlr_multi_backend_add(
        wlserver.wlr.multi_backend, wlserver.wlr.headless_backend );

    assert( wlserver.event_loop && wlserver.wlr.multi_backend );

    wl_signal_add(
        &wlserver.wlr.multi_backend->events.new_input, &new_input_listener );

    if ( GetBackend( )->IsSessionBased( ) )
    {
#if HAVE_DRM
        wlserver.wlr.libinput_backend =
            wlr_libinput_backend_create( wlserver.wlr.session );
        if ( wlserver.wlr.libinput_backend == NULL ) { return false; }
        wlr_multi_backend_add(
            wlserver.wlr.multi_backend, wlserver.wlr.libinput_backend );
#endif
    }
    ...
    struct wlr_keyboard *kbd =
        ( struct wlr_keyboard * )calloc( 1, sizeof( *kbd ) );
    ...
    wl_signal_add(
        &keyboard->events.modifiers, &wlserver.keyboard_group_modifiers );
    ...
    wlserver.wlr.compositor =
        wlr_compositor_create( wlserver.display, 5, wlserver.wlr.renderer );

    wl_signal_add(
        &wlserver.wlr.compositor->events.new_surface, &new_surface_listener );

    create_ime_manager( &wlserver );

    create_reshade( );

    new gamescope::WaylandServer::CGamescopeActionBindingProtocol(
        wlserver.display );
    ...
    wlserver.relative_pointer_manager =
        wlr_relative_pointer_manager_v1_create( wlserver.display );
    if ( !wlserver.relative_pointer_manager )
    {
        wl_log.errorf( "Failed to create relative pointer manager" );
        return false;
    }
    wlserver.constraints =
        wlr_pointer_constraints_v1_create( wlserver.display );
    ...
    wl_signal_add(
        &wlserver.constraints->events.new_constraint,
        &wlserver.new_pointer_constraint );
    ...
    wl_signal_add(
        &wlserver.xdg_shell->events.new_surface, &wlserver.new_xdg_surface );
    wl_signal_add(
        &wlserver.xdg_shell->events.new_toplevel, &wlserver.new_xdg_toplevel );
    ...
    wl_signal_add(
        &wlserver.layer_shell_v1->events.new_surface,
        &wlserver.new_layer_shell_surface );
    ...
    while ( result != 0 && display_slot < 128 )
    {
        ...
        result =
            wl_display_add_socket( wlserver.display, wlserver.wl_display_name );
        display_slot++;
    }
...
#if HAVE_LIBEIS
    {
        ...
        std::unique_ptr<gamescope::GamescopeInputServer> pInputServer =
            std::make_unique<gamescope::GamescopeInputServer>( );
...
#else
    wl_log.errorf(
        "Gamescope built without libei, XTEST will not be available!" );
#endif

    for ( int i = 0; i < g_nXWaylandCount; i++ )
    {
        auto server = std::make_unique<gamescope_xwayland_server_t>(
            wlserver.display, i );
...
void wlserver_lock( void ) { pthread_mutex_lock( &waylock ); }

void wlserver_unlock( bool flush )
{
    if ( flush ) wl_display_flush_clients( wlserver.display );
    pthread_mutex_unlock( &waylock );
}
...
void wlserver_run( void )
{
    ...
    while ( !g_bShutdownWLServer )
    {
        ...
        if ( pollfds[ 0 ].revents & ( POLLHUP | POLLERR ) )
        {
            wl_log.errorf(
                "socket %s",
                ( pollfds[ 0 ].revents & POLLERR ) ? "error" : "closed" );
            break;
        }
...
void wlserver_keyboardfocus( struct wlr_surface *surface, bool bConstrain )
{
    ...
    if ( wlserver.kb_focus_surface != surface )
    {
        ...
        if ( old_wl_surf && old_wl_surf->xdg_surface &&
             old_wl_surf->xdg_surface->xdg_surface &&
             old_wl_surf->xdg_surface->xdg_surface->toplevel )
            wlr_xdg_toplevel_set_activated(
                old_wl_surf->xdg_surface->xdg_surface->toplevel, false );

        auto new_wl_surf = get_wl_surface_info( surface );
        if ( new_wl_surf && new_wl_surf->xdg_surface &&
             new_wl_surf->xdg_surface->xdg_surface &&
             new_wl_surf->xdg_surface->xdg_surface->toplevel )
            wlr_xdg_toplevel_set_activated(
                new_wl_surf->xdg_surface->xdg_surface->toplevel, true );
    }

    assert( wlserver.wlr.virtual_keyboard_device != nullptr );
    wlr_seat_set_keyboard(
        wlserver.wlr.seat, wlserver.wlr.virtual_keyboard_device );

    struct wlr_keyboard *keyboard = wlr_seat_get_keyboard( wlserver.wlr.seat );
    if ( keyboard == nullptr )
        wlr_seat_keyboard_notify_enter(
            wlserver.wlr.seat, surface, nullptr, 0, nullptr );
    else
        wlr_seat_keyboard_notify_enter(
            wlserver.wlr.seat,
            surface,
    ...
    if ( bConstrain )
    {
        struct wlr_pointer_constraint_v1 *constraint =
            wlr_pointer_constraints_v1_constraint_for_surface(
                wlserver.constraints, surface, wlserver.wlr.seat );
    ...
    wlr_keyboard *keyboard, uint32_t key, bool press )
{
    ...
    if ( press ) { s_setPressedKeySyms.emplace( keysym ); }
    else
    {
    ...
    {
        std::string sPressedKeySymsDebugName =
            ComputeDebugName( s_setPressedKeySyms );
        log_binding.debugf(
            "Looking for: [%s].", sPressedKeySymsDebugName.c_str( ) );
    }

    {
        using namespace gamescope::WaylandServer;

        std::span<CGamescopeActionBinding *> ppBindings =
            CGamescopeActionBinding::GetBindings( );

        for ( CGamescopeActionBinding *pBinding : ppBindings )
        {
            if ( !pBinding->IsArmed( ) ) continue;

            std::span<Keybind_t> pKeybinds = pBinding->GetKeyboardTriggers( );
            for ( const Keybind_t &keybind : pKeybinds )
            {
                if ( !pBinding->IsArmed( ) ) break;

                if ( s_setPressedKeySyms != keybind.setKeySyms ) continue;

                if ( pBinding->Execute( ) ) return true;
            }
        }
    }
...
struct wlr_surface *
wlserver_surface_to_override_surface( struct wlr_surface *pSurface )
{
    if ( !pSurface ) return nullptr;
    ...
    if ( pInfo->x11_surface )
    {
        ...
        if ( pOverride ) pSurface = pOverride;
    }
...
std::pair<int, int> wlserver_get_surface_extent( struct wlr_surface *pSurface )
{
    ...
    if ( !pSurface ) return std::make_pair( g_nNestedWidth, g_nNestedHeight );
...
{
    auto [ nWidth, nHeight ] =
        wlserver_get_surface_extent( wlserver.mouse_focus_surface );
    for ( auto iter : wlserver.current_dropdown_surfaces )
    {
        auto [ nDropdownX, nDropdownY ] = iter.second;
        auto [ nDropdownWidth, nDropdownHeight ] =
            wlserver_get_surface_extent( iter.first );
...
static void wlserver_clampcursor( )
{
    ...
    wlserver.mouse_surface_cursorx = std::clamp(
        wlserver.mouse_surface_cursorx,
        0.0,
        double( std::max( nWidth - 1, 0 ) ) );
    wlserver.mouse_surface_cursory = std::clamp(
        wlserver.mouse_surface_cursory,
        0.0,
        double( std::max( nHeight - 1, 0 ) ) );
}

void wlserver_mousefocus(
    struct wlr_surface *wlrsurface, int x /* = 0 */, int y /* = 0 */ )
{
    assert( wlserver_is_lock_held( ) );
...
static void wlserver_warp_to_constraint_hint( )
{
    ...
    if ( pConstraint->current.cursor_hint.enabled )
    {
        ...
        if ( wlserver.mouse_surface_cursorx == sx &&
        ...
        {
            if ( s_unSyntheticMoveCount++ >= 3 ) { bSynthetic = false; }
        }
        else
        {
        ...
        if ( !bSynthetic ) wlserver.bCursorHidden = !wlserver.bCursorHasImage;
    }
}
...
static void wlserver_update_cursor_constraint( )
{
    ...
    if ( wlserver.mouse_constraint_requires_warp && pConstraint->surface )
    {
        ...
        if ( !pixman_region32_contains_point(
                 pRegion,
                 floor( wlserver.mouse_surface_cursorx ),
                 floor( wlserver.mouse_surface_cursory ),
                 NULL ) )
        {
            int             nboxes;
            pixman_box32_t *boxes =
                pixman_region32_rectangles( pRegion, &nboxes );
            if ( nboxes )
            {
                wlserver.mouse_surface_cursorx = std::clamp<double>(
                    wlserver.mouse_surface_cursorx,
                    boxes[ 0 ].x1,
                    boxes[ 0 ].x2 );
                wlserver.mouse_surface_cursory = std::clamp<double>(
                    wlserver.mouse_surface_cursory,
                    boxes[ 0 ].y1,
                    boxes[ 0 ].y2 );
...
wlserver_constrain_cursor( struct wlr_pointer_constraint_v1 *pNewConstraint )
{
    struct wlr_pointer_constraint_v1 *pOldConstraint =
        wlserver.GetCursorConstraint( );

    if ( pOldConstraint == pNewConstraint ) return;

    if ( pOldConstraint )
    {
        if ( !pNewConstraint ) wlserver_warp_to_constraint_hint( );
...
{
    GamescopePointerConstraint *pGamescopeConstraint =
        wl_container_of( listener, pGamescopeConstraint, set_region );
...
{
    GamescopePointerConstraint *pGamescopeConstraint =
        wl_container_of( listener, pGamescopeConstraint, destroy );

    wl_list_remove( &pGamescopeConstraint->set_region.link );
    wl_list_remove( &pGamescopeConstraint->destroy.link );

    struct wlr_pointer_constraint_v1 *pCurrentConstraint =
        wlserver.GetCursorConstraint( );
...
{
    struct wlr_pointer_constraint_v1 *pConstraint =
        ( struct wlr_pointer_constraint_v1 * )data;

    GamescopePointerConstraint *pGamescopeConstraint =
        new GamescopePointerConstraint;
    pGamescopeConstraint->pConstraint = pConstraint;

    pGamescopeConstraint->set_region.notify =
        handle_pointer_constraint_set_region;
    wl_signal_add(
        &pConstraint->events.set_region, &pGamescopeConstraint->set_region );

    pGamescopeConstraint->destroy.notify = handle_constraint_destroy;
    wl_signal_add(
        &pConstraint->events.destroy, &pGamescopeConstraint->destroy );

    if ( wlserver.kb_focus_surface &&
         wlserver.kb_focus_surface == pConstraint->surface )
...
void wlserver_mousewheel( double flX, double flY, uint32_t time )
{
    ...
    wlr_seat_pointer_notify_axis(
        ...
        flX,
        flX * WLR_POINTER_AXIS_DISCRETE_STEP,
    ...
    wlr_seat_pointer_notify_axis(
        ...
        flY,
        flY * WLR_POINTER_AXIS_DISCRETE_STEP,
        WL_POINTER_AXIS_SOURCE_WHEEL,
        WL_POINTER_AXIS_RELATIVE_DIRECTION_IDENTICAL );
    wlr_seat_pointer_notify_frame( wlserver.wlr.seat );
}

void wlserver_send_frame_done(
    struct wlr_surface *surf, const struct timespec *when )
{
    assert( wlserver_is_lock_held( ) );
...
bool wlserver_surface_is_async( struct wlr_surface *surf )
{
    ...
    auto wl_surf = get_wl_surface_info( surf );
    if ( !wl_surf ) return false;

    // If we are using the Gamescope WSI layer, treat both immediate and mailbox
    // as "async", this is because we have a global tearing override for games.
...
const std::shared_ptr<wlserver_vk_swapchain_feedback> &
wlserver_surface_swapchain_feedback( struct wlr_surface *surf )
{
    assert( wlserver_is_lock_held( ) );

    auto wl_surf = get_wl_surface_info( surf );
    if ( !wl_surf ) return s_NullFeedback;

    return wl_surf->swapchain_feedback;
}

/* Handle the orientation of the touch inputs */
static void apply_touchscreen_orientation(
    GamescopePanelOrientation orientation, double *x, double *y )
{
    double tx = 0;
    double ty = 0;
...
void wlserver_touchmotion(
    double                        x,
    double                        y,
    int                           touch_id,
    uint32_t                      time,
    bool                          bAlwaysWarpCursor,
    gamescope::IBackendConnector *connector )
{
    ...
    if ( wlserver.mouse_focus_surface != NULL )
    {
        ...
        if ( connector )
        {
            apply_touchscreen_orientation(
                connector->GetCurrentOrientation( ), &tx, &ty );
        }
        ...
        if ( eMode == gamescope::TouchClickModes::Passthrough )
        {
            wlr_seat_touch_notify_motion(
                wlserver.wlr.seat, time, touch_id, tx, ty );

            if ( bAlwaysWarpCursor ) wlserver_mousewarp( tx, ty, time, false );
        }
        else if ( eMode == gamescope::TouchClickModes::Disabled ) { return; }
...
void wlserver_touchup( int touch_id, uint32_t time )
{
    ...
    if ( wlserver.mouse_focus_surface != NULL )
    {
        ...
        for ( int i = 0; i < WLSERVER_BUTTON_COUNT; i++ )
        {
            ...
            {
                uint32_t button = TouchClickModeToLinuxButton(
                    ( gamescope::TouchClickMode )i );
...
{
    if ( index >= wlserver.wlr.xwayland_servers.size( ) ) return NULL;
    ...
    bool                              override )
{
    ...
    if ( override )
    {
        ...
        {
            wlserver_wl_surface_info *wl_info =
                get_wl_surface_info( surf->override_surface );
            if ( wl_info ) wl_info->x11_surface = nullptr;
        }
    ...
    else
    {
        ...
        {
            wlserver_wl_surface_info *wl_info =
                get_wl_surface_info( surf->main_surface );
            if ( wl_info ) wl_info->x11_surface = nullptr;
        }
    ...
    for ( auto it = g_PendingCommits.begin( ); it != g_PendingCommits.end( ); )
    {
        if ( it->surf == wlr_surf )
        {
            ...
            // Still have the buffer lock from before...
            wlserver_x11_surface_info *wlserver_x11_surface_info =
                get_wl_surface_info( wlr_surf )->x11_surface;
...
void wlserver_x11_surface_info_init(
    struct wlserver_x11_surface_info *surf,
    gamescope_xwayland_server_t      *server,
    uint32_t                          x11_id )
{
...
void gamescope_xwayland_server_t::set_wl_id(
    struct wlserver_x11_surface_info *surf, uint32_t id )
{
    if ( surf->wl_id )
    {
        if ( surf->main_surface )
        {
            struct wl_resource *old_resource =
                wl_client_get_object( xwayland_server->client, surf->wl_id );
            if ( !old_resource )
            {
                wl_log.errorf(
                    "wlserver_x11_surface_info had bad wl_id? Oh no! %d",
                    surf->wl_id );
                return;
            }
            wlr_surface *old_wlr_surf =
                wlr_surface_from_resource( old_resource );
            if ( !old_wlr_surf )
            {
                wl_log.errorf(
                    "wlserver_x11_surface_info wl_id's was not a wlr_surf?! Oh "
                    "no! %d",
                    surf->wl_id );
                return;
            }

            wlserver_wl_surface_info *old_surface_info =
                get_wl_surface_info( old_wlr_surf );
    ...
    struct wl_resource *resource =
        wl_client_get_object( xwayland_server->client, id );
    if ( resource != nullptr ) wlr_surf = wlr_surface_from_resource( resource );
...
_XDisplay *gamescope_xwayland_server_t::get_xdisplay( ) { return dpy; }
...
void wlserver_x11_surface_info_finish( struct wlserver_x11_surface_info *surf )
{
    ...
    if ( surf->main_surface )
    {
        surf->xwayland_server->destroy_content_override(
...
void wlserver_set_xwayland_server_mode(
    size_t idx, int w, int h, int nRefreshmHz )
{
    assert( wlserver_is_lock_held( ) );
    ...
    wl_log.infof(
        "Updating mode for xwayland server #%zu: %dx%d@%d",
        idx,
        w,
        h,
        gamescope::ConvertmHzToHz( nRefreshmHz ) );
}
...
std::vector<std::shared_ptr<steamcompmgr_win_t>>
wlserver_get_xdg_shell_windows( )
{
    std::unique_lock lock( g_wlserver_xdg_shell_windows_lock );
    return wlserver.xdg_wins;
}

bool wlserver_xdg_dirty( ) { return wlserver.xdg_dirty.exchange( false ); }
...
uint32_t wlserver_make_new_xwayland_server( )
{
    ...
    auto &server = wlserver.wlr.xwayland_servers.emplace_back(
        std::make_unique<gamescope_xwayland_server_t>(
            wlserver.display, ( int )wlserver.wlr.xwayland_servers.size( ) ) );
...
void wlserver_destroy_xwayland_server( gamescope_xwayland_server_t *server )
{
    ...
    std::erase_if(
        wlserver.wlr.xwayland_servers,
        [ = ]( const auto &other ) { return other.get( ) == server; } );
}
