...
ComputeDebugName( const std::unordered_set<xkb_keysym_t> &syms )
{
    ...
    for ( xkb_keysym_t uKeySym : syms )
    {
        ...
        {
            snprintf(
                szName, sizeof( szName ), "xkb_keysym_t( 0x%x )", uKeySym );
        }
...
static inline xkb_keysym_t NormalizeKeysymForHotkey( xkb_keysym_t uKeySym )
{
    ...
    {
        if ( uKeySym == uBadKey ) uKeySym = uGoodKey;
    }
...
namespace gamescope::WaylandServer
{
    ...
    public:
        ...
        ~CGamescopeActionBinding( )
        {
            std::erase_if(
                s_Bindings,
                [ this ]( CGamescopeActionBinding *pBinding )
                { return pBinding == this; } );
        }
        ...
        void AddKeyboardTrigger( wl_array *pKeysymsArray )
        {
            ...
            std::span<const xkb_keysym_t> pKeysyms =
                std::span<const xkb_keysym_t>{
                    reinterpret_cast<const xkb_keysym_t *>(
                        pKeysymsArray->data ),
                    zKeysymCount
                };
            ...
            log_binding.infof(
                "(%s) -> Adding new trigger [%s].",
                m_sDescription.c_str( ),
                sTriggerDebugName.c_str( ) );
            m_KeyboardTriggers.emplace_back(
                std::move( setKeySyms ), std::move( sTriggerDebugName ) );
        }

        void ClearTriggers( )
        {
            log_binding.infof(
                "(%s) -> Cleared triggers.", m_sDescription.c_str( ) );
        ...
        {
            log_binding.debugf(
                "(%s) -> Arming: %x.", m_sDescription.c_str( ), uArmFlags );
        ...
        std::span<Keybind_t> GetKeyboardTriggers( )
        { return m_KeyboardTriggers; }
        ...
        bool Execute( )
        {
            ...
            bool     bBlockInput =
                !!( uArmFlags & GAMESCOPE_ACTION_BINDING_ARM_FLAG_NO_BLOCK );

            uint32_t uTriggerFlags =
                GAMESCOPE_ACTION_BINDING_TRIGGER_FLAG_KEYBOARD;
            ...
            gamescope_action_binding_send_triggered(
                GetResource( ),
                s_uSequence++,
                uTriggerFlags,
                uTimeLo,
    ...
    const struct gamescope_action_binding_interface
        CGamescopeActionBinding::Implementation = {
            .destroy = WL_PROTO_DESTROY( ),
            .set_description =
                WL_PROTO( CGamescopeActionBinding, SetDescription ),
            .add_keyboard_trigger =
                WL_PROTO( CGamescopeActionBinding, AddKeyboardTrigger ),
            .clear_triggers =
                WL_PROTO( CGamescopeActionBinding, ClearTriggers ),
    ...
    public:
        ...
        {
            CWaylandResource::Create<CGamescopeActionBinding>(
                m_pClient, m_uVersion, uId );
        }
    };

    const struct gamescope_action_binding_manager_interface
        CGamescopeActionBindingManager::Implementation = {
            .destroy = WL_PROTO_DESTROY( ),
            .create_action_binding =
                WL_PROTO( CGamescopeActionBindingManager, CreateActionBinding ),
        };
