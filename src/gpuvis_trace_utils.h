...
GPUVIS_EXTERN int gpuvis_trace_printf( const char *fmt, ... )
    GPUVIS_ATTR_PRINTF( 1, 2 );
GPUVIS_EXTERN int gpuvis_trace_vprintf( const char *fmt, va_list ap )
    GPUVIS_ATTR_PRINTF( 1, 0 );

// Write user event (with duration=XXms) to tracefs trace_marker.
GPUVIS_EXTERN int
gpuvis_trace_duration_printf( float duration, const char *fmt, ... )
    GPUVIS_ATTR_PRINTF( 2, 3 );
GPUVIS_EXTERN int
gpuvis_trace_duration_vprintf( float duration, const char *fmt, va_list ap )
    GPUVIS_ATTR_PRINTF( 2, 0 );

// Write user event (with begin_ctx=XX) to tracefs trace_marker.
GPUVIS_EXTERN int
gpuvis_trace_begin_ctx_printf( unsigned int ctx, const char *fmt, ... )
    GPUVIS_ATTR_PRINTF( 2, 3 );
GPUVIS_EXTERN int
gpuvis_trace_begin_ctx_vprintf( unsigned int ctx, const char *fmt, va_list ap )
    GPUVIS_ATTR_PRINTF( 2, 0 );

// Write user event (with end_ctx=XX) to tracefs trace_marker.
GPUVIS_EXTERN int
gpuvis_trace_end_ctx_printf( unsigned int ctx, const char *fmt, ... )
    GPUVIS_ATTR_PRINTF( 2, 3 );
GPUVIS_EXTERN int
gpuvis_trace_end_ctx_vprintf( unsigned int ctx, const char *fmt, va_list ap )
    GPUVIS_ATTR_PRINTF( 2, 0 );
...
GPUVIS_EXTERN int
gpuvis_trigger_capture_and_keep_tracing( char *filename, size_t size );
...
// Get tracefs file path in buf. Ie: /sys/kernel/tracing/trace_marker. Returns
// NULL on error.
GPUVIS_EXTERN const char *
gpuvis_get_tracefs_filename( char *buf, size_t buflen, const char *file );
...
static inline void
gpuvis_trace_block_begin( struct GpuvisTraceBlock *block, const char *str );
static inline void gpuvis_trace_block_end( struct GpuvisTraceBlock *block );

struct GpuvisTraceBlockf;
static inline void gpuvis_trace_blockf_vbegin(
    struct GpuvisTraceBlockf *block, const char *fmt, va_list ap );
static inline void gpuvis_trace_blockf_begin(
    struct GpuvisTraceBlockf *block, const char *fmt, ... )
    GPUVIS_ATTR_PRINTF( 2, 3 );
...
struct GpuvisTraceBlock
{
    ...
    ~GpuvisTraceBlock( ) { gpuvis_trace_block_end( this ); }
    #endif
};
...
struct GpuvisTraceBlockf
{
    ...
    ~GpuvisTraceBlockf( ) { gpuvis_trace_blockf_end( this ); }
    #endif
};
        ...
        #define GPUVIS_TRACE_BLOCK( _conststr )                                \
            GpuvisTraceBlock LNAME( gpuvistimeblock )( _conststr )
        #define GPUVIS_TRACE_BLOCKF( _fmt, ... )                               \
            GpuvisTraceBlockf LNAME( gpuvistimeblock )( _fmt, __VA_ARGS__ )
            ...
            #define GPUVIS_TRACE_BLOCKF( _fmt, ... )                           \
                GPUVIS_CLEANUP_FUNC( gpuvis_trace_blockf_end )                 \
                GPUVIS_MAY_BE_UNUSED struct GpuvisTraceBlockf LNAME(           \
                    gpuvistimeblock ) =                                        \
                    GPUVIS_TRACE_BLOCKF_INIT(                                  \
                        LNAME( gpuvistimeblock_init ), _fmt, __VA_ARGS__ )

            #define GPUVIS_TRACE_BLOCK( _conststr )                            \
                GPUVIS_CLEANUP_FUNC( gpuvis_trace_block_end )                  \
                GPUVIS_MAY_BE_UNUSED struct GpuvisTraceBlock LNAME(            \
                    gpuvistimeblock ) = { .m_t0  = gpuvis_gettime_u64( ),      \
                                          .m_str = _conststr }
...
{ gpuvis_trace_block_finalize( block->m_t0, block->m_buf ); }
    ...
    #define GPUVIS_COUNT_HOT_FUNC_CALLS( )                                     \
        gpuvis_count_hot_func_calls_internal_( __func__ );
...
static inline int gpuvis_trace_vprintf( const char *fmt, va_list ap )
{ return 0; }
...
#if defined( GPUVIS_TRACE_IMPLEMENTATION ) &&                                  \
    !defined( GPUVIS_TRACE_UTILS_DISABLE )
...
static std::unordered_map<pid_t, std::unordered_map<const char *, funcinfo_t>>
    g_hotfuncs;
    #endif // __cplusplus

static pid_t gpuvis_gettid( ) { return ( pid_t )syscall( SYS_gettid ); }
...
GPUVIS_EXTERN int gpuvis_trace_init( )
{
    if ( g_trace_fd == -2 )
    {
        ...
        // The "trace_marker" file allows userspace to write into the ftrace
        // buffer.
        if ( !gpuvis_get_tracefs_filename(
                 filename, sizeof( filename ), "trace_marker" ) )
            g_trace_fd = -1;
        else
    ...
    for ( auto &x : g_hotfuncs )
    {
        for ( auto &y : x.second )
        {
            if ( y.second.count )
            {
                ...
                gpuvis_trace_printf(
                    "%s calls:%u (lduration=%lu tid=%d offset=-%lu)\n",
                    func,
                    y.second.count,
                    duration,
                    tid,
...
GPUVIS_EXTERN void gpuvis_count_hot_func_calls_internal_( const char *func )
{
    ...
    {
        gpuvis_trace_printf(
            "%s calls:%u (lduration=%lu offset=-%lu)\n",
            func,
            y.count,
            y.tlast - y.tfirst,
            t0 - y.tfirst );
...
gpuvis_trace_duration_vprintf( float duration, const char *fmt, va_list ap )
{
    ...
    snprintf(
        ...
        sizeof( keystr ),
        " (duration=%f)",
        duration ); //$ TODO: Try this with more precision?
...
GPUVIS_EXTERN int gpuvis_start_tracing( unsigned int kbuffersize )
{
    static const char fmt[] =
        ...
        " -e i915:i915_gem_request_submit" // Require
    ...
    if ( !kbuffersize ) kbuffersize = 16 * 1024;
...
gpuvis_trigger_capture_and_keep_tracing( char *filename, size_t size )
{
    ...
    if ( gpuvis_tracing_on( ) )
    {
        ...
        // Save the trace data to something like
        // "glxgears_2017-10-13_17-52-56.dat"
        snprintf(
            cmd,
            sizeof( cmd ),
...
GPUVIS_EXTERN int gpuvis_tracing_on( )
{
    ...
    if ( gpuvis_get_tracefs_filename( filename, PATH_MAX, "tracing_on" ) )
    {
        ...
        if ( fd >= 0 )
        {
            if ( read( fd, buf, sizeof( buf ) ) > 0 ) ret = atoi( buf );
...
GPUVIS_EXTERN const char *gpuvis_get_tracefs_dir( )
{
    if ( !g_tracefs_dir_inited )
    {
        ...
        for ( i = 0; i < sizeof( tracefs_dirs ) / sizeof( tracefs_dirs[ 0 ] );
              i++ )
        {
