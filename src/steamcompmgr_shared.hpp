...
struct wlserver_x11_surface_info
{
    ...
    {
        if ( override_surface ) return override_surface;
        return main_surface;
    }
...
struct wlserver_xdg_surface_info
{
    ...
    struct wlr_surface *current_surface( ) { return main_surface; }
...
struct steamcompmgr_win_t
{
    ...
    bool IsAnyOverlay( ) const { return isOverlay || isExternalOverlay; }
    ...
    steamcompmgr_xwayland_win_t &xwayland( )
    { return std::get<steamcompmgr_xwayland_win_t>( _window_types ); }
    const steamcompmgr_xwayland_win_t &xwayland( ) const
    { return std::get<steamcompmgr_xwayland_win_t>( _window_types ); }

    steamcompmgr_xdg_win_t &xdg( )
    { return std::get<steamcompmgr_xdg_win_t>( _window_types ); }
    const steamcompmgr_xdg_win_t &xdg( ) const
    { return std::get<steamcompmgr_xdg_win_t>( _window_types ); }
    ...
    {
        if ( type == steamcompmgr_win_type_t::XWAYLAND )
            return Rect{ xwayland( ).a.x,
                         xwayland( ).a.y,
                         xwayland( ).a.width,
                         xwayland( ).a.height };
        else if ( type == steamcompmgr_win_type_t::XDG )
            return Rect{ xdg( ).geometry.x,
                         xdg( ).geometry.y,
                         xdg( ).geometry.width,
                         xdg( ).geometry.height };
        else
    ...
    {
        if ( title ) return title->c_str( );

        return pid_name.c_str( );
    }

    gamescope::VirtualConnectorKey_t
    GetVirtualConnectorKey( gamescope::VirtualConnectorStrategy eStrategy )
    {
        switch ( eStrategy )
        {
            default:
            ...
            case gamescope::VirtualConnectorStrategies::PerAppId:
                ...
                {
                    return static_cast<gamescope::VirtualConnectorKey_t>(
                        this->appID );
                }
                else
                {
                    return static_cast<gamescope::VirtualConnectorKey_t>(
                        gamescope::k_ulNonSteamWindowBit | this->seq );
                }
...
namespace gamescope
{
    ...
    public:
        void TakeScreenshot(
            GamescopeScreenshotInfo info = GamescopeScreenshotInfo{} )
        {
        ...
        void TakeScreenshot( bool bAVIF )
        {
            ...
            strftime(
                szTimeBuffer,
                sizeof( szTimeBuffer ),
                bAVIF ? "/tmp/gamescope_%Y-%m-%d_%H-%M-%S.avif"
                      : "/tmp/gamescope_%Y-%m-%d_%H-%M-%S.png",
                pLocalTime );
