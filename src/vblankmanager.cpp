// Try to figure out when vblank is and notify steamcompmgr to render some time
// before it
...
namespace gamescope
{
    ConVar<bool> vblank_debug(
        "vblank_debug", false, "Enable vblank debug spew to stderr." );
    ...
    CVBlankTimer::CVBlankTimer( )
    {
        ...
        else
        {
            ...
            {
                g_VBlankLog.errorf_errno(
                    "Failed to create VBlankTimer pipe." );
                abort( );
            }
    ...
    uint64_t CVBlankTimer::GetLastVBlank( ) const { return m_ulLastVBlank; }
    ...
    VBlankScheduleTime CVBlankTimer::CalcNextWakeupTime( bool bPreemptive )
    {
        ...
        bool     bVRR = GetBackend( )->GetCurrentConnector( ) &&
        ...
        if ( !bVRR )
        {
            ...
            //
            // TODO(Josh): Is this fudging still needed with our SteamOS kernel
            // patches to not account for vertical front porch when dealing with
            // the vblank drm_commit is going to target? Need to re-test that.
            const uint64_t ulRedZone =
                eScreenType == GAMESCOPE_SCREEN_TYPE_INTERNAL
                    ? m_ulVBlankDrawBufferRedZone
                    : std::min<uint64_t>(
                          m_ulVBlankDrawBufferRedZone,
                          ( m_ulVBlankDrawBufferRedZone * 60'000 *
                            nRefreshRate ) /
                              60'000 );

            const uint64_t ulDecayAlpha =
                m_ulVBlankRateOfDecayPercentage; // eg. 980 = 98%

            uint64_t ulDrawTime = m_ulLastDrawTime;
            /// See comment of m_ulVBlankDrawTimeMinCompositing.
            if ( m_bCurrentlyCompositing )
                ulDrawTime =
                    std::max( ulDrawTime, m_ulVBlankDrawTimeMinCompositing );

            uint64_t ulNewRollingDrawTime;
            // This is a rolling average when ulDrawTime <
            // m_ulRollingMaxDrawTime, and a maximum when ulDrawTime >
            // m_ulRollingMaxDrawTime.
            //
            // This allows us to deal with spikes in the draw buffer time very
            // easily. eg. if we suddenly spike up (eg. because of test commits
            // taking a stupid long time), we will then be able to deal with
            // spikes in the long term, even if several commits after we get
            // back into a good state and then regress again.

            // If we go over half of our deadzone, be more defensive about
            // things and spike up back to our current drawtime (sawtooth).
            if ( int64_t( ulDrawTime ) - int64_t( ulRedZone / 2 ) >
                 int64_t( m_ulRollingMaxDrawTime ) )
                ulNewRollingDrawTime = ulDrawTime;
            else
                ulNewRollingDrawTime =
                    ( ( ulDecayAlpha * m_ulRollingMaxDrawTime ) +
                      ( kVBlankRateOfDecayMax - ulDecayAlpha ) * ulDrawTime ) /
                    kVBlankRateOfDecayMax;

            // If we need to offset for our draw more than half of our vblank,
            // something is very wrong. Clamp our max time to half of the vblank
            // if we can.
            ulNewRollingDrawTime =
                std::min( ulNewRollingDrawTime, ulRefreshInterval - ulRedZone );

            // If this is not a pre-emptive re-arming, then update
            // the rolling internal max draw time for next time.
            if ( !bPreemptive ) m_ulRollingMaxDrawTime = ulNewRollingDrawTime;
    ...
    void CVBlankTimer::ArmNextVBlank( bool bPreemptive )
    {
        ...
        if ( bPreemptive && m_bArmed ) return;
        ...
        if ( UsingTimerFD( ) )
        {
            ...
            ITimerWaitable::ArmTimer(
                m_TimerFDSchedule.ulScheduledWakeupPoint );
        }
    }

    bool CVBlankTimer::UsingTimerFD( ) const { return m_nNudgePipe[ 0 ] < 0; }
    ...
    void CVBlankTimer::OnPollIn( )
    {
        if ( UsingTimerFD( ) )
        {
            ...
            if ( !m_bArmed.exchange( false ) ) return;

            m_PendingVBlank = VBlankTime{
                .schedule = m_TimerFDSchedule,
                // One might think this should just be 'now', however consider
                // the fact that the effective draw-time should also include the
                // scheduling quantums and any work before we reached this poll.
                // The old path used to be be on its own thread, simply awaking
                // from sleep then writing to a pipe and going back to sleep,
                // the wakeup time was before we did the write, so we included
                // the quantum of pipe nudge -> wakeup. Doing this aims to
                // include that, like we were before, but with timerfd.
        ...
        else
        {
            ...
            for ( ;; )
            {
                ...
                if ( ret < 0 )
                {
                    ...
                    g_VBlankLog.errorf_errno(
                        "Failed to read nudge pipe. Pre-emptively re-arming." );
                    ArmNextVBlank( true );
                    return;
                }
                ...
                {
                    g_VBlankLog.errorf(
                        "Nudge pipe had less data than sizeof( VBlankTime ). "
                        "Pre-emptively re-arming." );
                    ArmNextVBlank( true );
                    return;
                }
                else
    ...
    void CVBlankTimer::VBlankDebugSpew(
        uint64_t ulOffset, uint64_t ulDrawTime, uint64_t ulRedZone )
    {
        ...
        if ( s_ulVBlankID++ % 300 == 0 || ulDrawTime > s_ulLastOffset )
        {
            ...
            g_VBlankLog.infof(
                "redZone: %.2fms decayRate: %lu%% - rollingMaxDrawTime: %.2fms "
                "lastDrawTime: %.2fms lastOffset: %.2fms - drawTime: %.2fms "
                "offset: %.2fms",
    ...
    void CVBlankTimer::NudgeThread( )
    {
        ...
        for ( ;; )
        {
            ...
            {
                // If we don't currently have a connector, make up some dummy
                // refresh cycle.
                sleep_for_nanos( mHzToRefreshCycle(
            ...
            const uint64_t ulWakeupTime = get_time_in_nanos( );
            {
                ...
                ssize_t ret =
                    write( m_nNudgePipe[ 1 ], &timeInfo, sizeof( timeInfo ) );
                if ( ret <= 0 )
                {
