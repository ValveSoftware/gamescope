...
commit_t::~commit_t( )
{
    ...
    if ( vulkanTex != nullptr ) vulkanTex = nullptr;
...
GamescopeAppTextureColorspace commit_t::colorspace( ) const
{
    ...
    return VkColorSpaceToGamescopeAppTextureColorSpace(
        vulkanTex->format( ), colorspace );
}

int commit_t::GetFD( ) { return m_nCommitFence; }
...
void commit_t::Signal( )
{
    ...
    // When we get the new IWaitable stuff in there.
    {
        std::unique_lock<std::mutex> lock(
            m_pDoneCommits->listCommitsDoneLock );
        m_pDoneCommits->listCommitsDone.push_back(
            CommitDoneEntry_t{
                .winSeq             = win_seq,
                .commitID           = commitID,
    ...
    if ( m_bMangoNudge )
        mangoapp_update(
            IsPerfOverlayFIFO( ) ? uint64_t( ~0ull ) : frametime,
            frametime,
            uint64_t( ~0ull ) );
}
...
bool commit_t::IsPerfOverlayFIFO( ) { return fifo || is_steam; }

// Returns true if we had a fence that was closed.
bool commit_t::CloseFenceInternal( )
{
    if ( m_nCommitFence < 0 ) return false;
...
void commit_t::SetFence(
    int nFence, bool bMangoNudge, CommitDoneList_t *pDoneCommits )
{
    std::unique_lock lock( m_WaitableCommitStateMutex );
    CloseFenceInternal( );
...
bool commit_t::ShouldPreemptivelyUpscale( )
{
    ...
    if ( DoesHardwareSupportUpscaleFilter( g_upscaleFilter ) ) return false;

    if ( !vulkanTex ) return false;

    float flScaleX = 1.0f;
    float flScaleY = 1.0f;
    // I wish this function was more programatic with its inputs, but it does do
    // exactly what we want right now... It should also return a std::pair or a
    // glm uvec
    calc_scale_factor(
        flScaleX, flScaleY, vulkanTex->width( ), vulkanTex->height( ) );
