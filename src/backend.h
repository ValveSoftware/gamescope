...
namespace gamescope
{
    ...
    using VirtualConnectorStrategy =
        VirtualConnectorStrategies::VirtualConnectorStrategy;
    using VirtualConnectorKey_t = uint64_t;
    extern ConVar<VirtualConnectorStrategy>
        cv_backend_virtual_connector_strategy;

    static constexpr bool
    VirtualConnectorStrategyIsSingleOutput( VirtualConnectorStrategy eStrategy )
    {
        return eStrategy == VirtualConnectorStrategies::SingleApplication ||
               eStrategy == VirtualConnectorStrategies::SteamControlled;
    }

    static inline bool VirtualConnectorIsSingleOutput( )
    {
        return VirtualConnectorStrategyIsSingleOutput(
            cv_backend_virtual_connector_strategy );
    }

    static inline bool VirtualConnectorInSteamPerAppState( )
    {
        return steamMode && cv_backend_virtual_connector_strategy ==
                                gamescope::VirtualConnectorStrategies::PerAppId;
    }
    ...
    static constexpr gamescope::VirtualConnectorKey_t k_ulSteamBootstrapperKey =
        ( uint64_t( 1 ) | k_ulReservedBit );

    static inline bool
    VirtualConnectorKeyIsNonSteamWindow( VirtualConnectorKey_t ulKey )
    {
        return VirtualConnectorInSteamPerAppState( ) &&
               ( ulKey & k_ulNonSteamWindowBit ) == k_ulNonSteamWindowBit;
    }

    static inline std::string_view
    VirtualConnectorStrategyToString( VirtualConnectorStrategy eStrategy )
    {
        switch ( eStrategy )
        {
            case VirtualConnectorStrategies::SingleApplication:
                return "SingleApplication";
            case VirtualConnectorStrategies::SteamControlled:
                return "SteamControlled";
            case VirtualConnectorStrategies::PerAppId:
                return "PerAppId";
            case VirtualConnectorStrategies::PerWindow:
                return "PerWindow";
            default:
                return "Unknown";
        }
    ...
    struct BackendConnectorHDRInfo
    {
        ...
        // target/mapping values for the display brightness for undocking from a
        // HDR display, but don't want to expose HDR there as it is not good.
        ...
        // For displays doing "traditional HDR" such as Steam Deck OLED, this is
        // Gamma 2.2.
    ...
    {
    public:
        uint64_t CurrentPresentsInFlight( ) const
        { return TotalPresentsQueued( ) - TotalPresentsCompleted( ); }

        // Across the lifetime of the backend.
        uint64_t TotalPresentsQueued( ) const
        { return m_uQueuedPresents.load( ); }
        uint64_t TotalPresentsCompleted( ) const
        { return m_uCompletedPresents.load( ); }
    ...
    public:
        ...
        GetValidDynamicRefreshRates( ) const = 0;
        ...
        virtual void GetNativeColorimetry(
            ...
            EOTF                 *outputEncodingEOTF ) const = 0;
        ...
        virtual void
        SetProperty( ConnectorProperty eProperty, std::any value ) = 0;
    };

    class CBaseBackendConnector : public IBackendConnector
    {
    public:
        CBaseBackendConnector( ) { AssignConnectorId( ); }
        ...
        { return m_ulConnectorId; }
        virtual VBlankScheduleTime      FrameSync( ) override;
        virtual BackendPresentFeedback &PresentationFeedback( ) override
        { return m_PresentFeedback; }
        virtual uint64_t GetVirtualConnectorKey( ) const override
        { return m_ulVirtualConnectorKey; }
    ...
    public:
        ...
        virtual void
        SetIcon( std::shared_ptr<std::vector<uint32_t>> uIconPixels ) = 0;
        virtual void SetSelection(
            std::shared_ptr<std::string> szContents,
            GamescopeSelection           eSelection ) = 0;
    ...
    public:
        ...
        virtual void SetReleasePoint(
            std::shared_ptr<CReleaseTimelinePoint> pReleasePoint ) = 0;
    ...
    public:
        ...
        void SetReleasePoint(
            std::shared_ptr<CReleaseTimelinePoint> pReleasePoint ) override;
    ...
    public:
        ...
        GetDeviceExtensions( VkPhysicalDevice pVkPhysicalDevice ) const = 0;
        virtual VkImageLayout GetPresentLayout( ) const                 = 0;
        virtual void          GetPreferredOutputFormat(
            uint32_t *pPrimaryPlaneFormat,
            uint32_t *pOverlayPlaneFormat ) const = 0;
        virtual bool
        ValidPhysicalDevice( VkPhysicalDevice pVkPhysicalDevice ) const = 0;

        virtual void
        DirtyState( bool bForce = false, bool bForceModeset = false ) = 0;
        virtual bool PollState( )                                     = 0;

        virtual std::shared_ptr<BackendBlob> CreateBackendBlob(
            const std::type_info &type, std::span<const uint8_t> data ) = 0;
        template<typename T>
        std::shared_ptr<BackendBlob> CreateBackendBlob( const T &thing )
        {
            ...
            return CreateBackendBlob(
                typeid( T ), std::span<const uint8_t>( pBegin, pEnd ) );
        }
        ...
        ImportDmabufToBackend( wlr_dmabuf_attributes *pDmaBuf ) = 0;

        virtual bool UsesModifiers( ) const = 0;
        virtual std::span<const uint64_t>
                    GetSupportedModifiers( uint32_t uDrmFormat ) const = 0;
        ...
        inline bool SupportsInvalidModifier( uint32_t uDrmFormat ) const
        {
            return Algorithm::Contains(
                this->GetSupportedModifiers( uDrmFormat ),
        ...
        virtual IBackendConnector *
        GetConnector( GamescopeScreenType eScreenType ) = 0;
        ...
        static IBackend                 *Get( );
        template<typename T> static bool Set( );
    ...
    public:
        ...
        BackendBlob(
            std::span<const uint8_t> data, uint32_t uBlob, bool bOwned ) :
            m_Data( data.begin( ), data.end( ) ),
            m_uBlob( uBlob ),
            m_bOwned( bOwned )
        {}
        ...
        ~BackendBlob( )
        {
            if ( m_bOwned )
            {
                ...
                if ( pBackend ) pBackend->OnBackendBlobDestroyed( this );
            }
        }
        ...
        // No move constructor, because we use shared_ptr anyway, but can be
        // added if necessary.
        BackendBlob( BackendBlob && )            = delete;
        BackendBlob &operator=( BackendBlob && ) = delete;

        std::span<const uint8_t> GetData( ) const
        { return std::span<const uint8_t>( m_Data.begin( ), m_Data.end( ) ); }
        template<typename T> const T &View( ) const
        {
