...
{
    return ( fVal < 0.04045f ) ? fVal / 12.92f
                               : std::pow( ( fVal + 0.055f ) / 1.055f, 2.4f );
}

inline float linear_to_srgb( float fVal )
{
    return ( fVal < 0.0031308f )
               ? fVal * 12.92f
               : std::pow( fVal, 1.0f / 2.4f ) * 1.055f - 0.055f;
}

template<typename T> inline T pq_to_nits( const T &pq )
{
...
template<typename T> inline T nits_to_pq( const T &nits )
{
...
struct tonemap_info_t
{
    ...
    bool BIsValid( ) const { return ( flWhitePointNits > flBlackPointNits ); }
...
// Apply an HDR tonemapping according to eetf 2390
// (R-REP-BT.2390-8-2020-PDF-E.pdf) sourceXXX == "Mastering Display" == Lw, Lb
// (in the paper) targetXXX == "Target Display" == Lmin, Lmax (in the paper) Be
// warned... PQ in, PQ out, for ALL params [0,1] This does not imply this
// function has anything to do with PQ (it's quite sensible to apply it to
// linear values created in other ways... you just have to PQ all params first,
// and undo the output) Values outside of 0-1 are not defined
...
struct eetf_2390_t
{
        ...
        float targetWhitePQ )
    {
        ...
        m_invSourcePQScale =
            m_sourcePQScale > 0.f ? 1.f / m_sourcePQScale : 0.f;
    ...
    // "Max RGB" approach, as defined in "Color Volume and Hue-Preservation in
    // HDR Tone Mapping" Digital Object Identifier 10.5594/JMI.2020.2984046 Date
    // of publication: 4 May 2020
    inline glm::vec3 apply_max_rgb( const glm::vec3 &inputNits ) const
    {
        float input_scalar_nits =
            std::max( inputNits.r, std::max( inputNits.g, inputNits.b ) );
        float output_scalar_nits =
            pq_to_nits( apply_pq( nits_to_pq( input_scalar_nits ) ) );
        float gain = input_scalar_nits > 0.f
                         ? output_scalar_nits / input_scalar_nits
                         : 0.f;
    ...
    {
        float input_scalar_nits = 0.2627f * inputNits.r +
                                  0.6780f * inputNits.g + 0.0593f * inputNits.b;
        float output_scalar_nits =
            pq_to_nits( apply_pq( nits_to_pq( input_scalar_nits ) ) );
        float gain = input_scalar_nits > 0.f
                         ? output_scalar_nits / input_scalar_nits
    ...
    inline glm::vec3 apply_independent_rgb( const glm::vec3 &inputNits ) const
    {
        ...
        glm::vec3 outputPQ = { apply_pq( inputPQ.r ),
                               apply_pq( inputPQ.g ),
                               apply_pq( inputPQ.b ) };
...
private:
    ...
    {
        float t =
            ( value - ks ) / ( 1.f - ks ); // TODO : guard against ks == 1.f?
        float t_sq  = t * t;
        float t_cub = t_sq * t;
...
colormapping_t
lerp( const colormapping_t &a, const colormapping_t &b, float t );
...
struct tonemapping_t
{
    ...
    float g22_luminance =
        1.f; // what luminance should be applied for g22 EOTF conversions?
...
// Generate a color transform from the source colorspace, to the dest
// colorspace, nLutSize1d is the number of color entries in the shaper lut I.e.,
// for a shaper lut with 256 input colors  nLutSize1d = 256,
// countof(pRgbxData1d) = 1024 nLutEdgeSize3d is the number of color entries,
// per edge, in the 3d lut I.e., for a 17x17x17 lut nLutEdgeSize3d = 17,
// countof(pRgbxData3d) = 19652
//
...
#define REGISTER_LUT_EDGE_SIZE( size )                                         \
    template void calcColorTransform<( size )>(                                \
        lut1d_t * pShaper,                                                     \
        int                         nLutSize1d,                                \
        lut3d_t                    *pLut3d,                                    \
        const displaycolorimetry_t &source,                                    \
...
// Josh: I changed the asserts to clamps here (going to 0, rather than 1) to
// deal better with bad EDIDs (that have 0'ed out metadata) and naughty clients.

static inline uint16_t color_xy_to_u16( float v )
{
    // assert(v >= 0.0f);
    // assert(v <= 1.0f);
...
static inline float color_xy_from_u16( uint16_t v ) { return v / 50000.0f; }

static inline uint16_t nits_to_u16( float nits )
{
    // assert(nits >= 1.0f);
...
static inline float nits_from_u16( uint16_t v ) { return float( v ); }

static inline uint16_t nits_to_u16_dark( float nits )
{
    // assert(nits >= 0.0001f);
    // assert(nits <= 6.5535f);
...
static inline float nits_from_u16_dark( uint16_t v ) { return v / 10000.0f; }
